...
ISSCC는 3대 반도체 학회 중 회로·집적 설계 비중이 가장 크고, 거의 모든 발표에 회로도와 실측 데이터가 함께 제시됩니다. 예년에는 산업 영향력이 들쭉날쭉했지만, 올해는 시장 흐름과 직결된 논문이 유독 많았습니다.
...
삼성은 코어 다이 TSV(층간 관통전극) 수를 4배로 늘리고 적층 편차를 보정하는 적응형 바디 바이어스(ABB) 제어를 더해 최대 13Gb/s의 핀 속도를 달성했습니다. 또 다른 난제는 tCCDR(서로 다른 스택ID 간 연속 읽기 명령 사이 최소 간격)로, 층수·채널 수가 늘수록 이 값이 나빠집니다.
...
로직 베이스 다이 전환의 또 다른 성과는 PMBIST(프로그래머블 메모리 자체 테스트)입니다. HBM3E는 소수의 정해진 패턴만 테스트했지만, HBM4는 실제 시스템과 동일한 JEDEC 명령 전체를 풀 스피드로 실행해 양산 수율 개선에 직접 도움이 됩니다.
...
삼성 HBM4는 JEDEC 공식 표준(JESD270-4, 핀당 최대 6.4Gb/s, 약 2TB/s)을 2배 이상 웃도는 핀당 13Gb/s, 3.3TB/s 대역폭을 시연했습니다. VDDC/VDDQ를 1.05V/0.75V로 낮춰도 11.8Gb/s를 유지합니다.

다만 1c 전공정 자체가 까다로워 2025년 한 해 수율이 약 50%에 머물렀습니다(1b 노드를 건너뛰고 1a에서 바로 1c로 전환한 영향). 삼성 HBM은 역사적으로 SK하이닉스보다 마진이 낮았던 만큼, 이 수율 리스크는 HBM4 마진에도 부담으로 작용할 수 있습니다.
...
LPDDR6은 다이 하나를 2개의 서브채널로 나누고, 각 서브채널에 16개 뱅크를 배치하는 구조입니다.
...
삼성은 저전력 상태(3.2Gb/s 이하 대기 상태가 대부분)의 전압 도메인을 세밀하게 나눠 읽기 전력 27%, 쓰기 전력 22%를 줄였습니다. 배선을 물리적으로 가깝게 재배치하는 RDL(재배선층) 기법도 적용해 타이밍 여유를 확보했습니다.

이 시제품은 0.97V에서 12.8Gb/s, 1.025V에서 최대 14.4Gb/s를 기록했습니다.
...
D램 셀 면적 스케일링은 이미 한계에 다다랐습니다. VLSI 2025에서 SK하이닉스가 4F² PUC(Peri-Under-Cell) 구조를 처음 공개했고, 이번 ISSCC에서 삼성이 같은 개념을 COP(Cell-on-Peripheral)라는 이름으로 공개했습니다.
...
주변회로를 셀 배열 바로 아래 배치하는 "샌드위치" 구조로 주변회로 면적 비중을 17.0%에서 2.7%로 줄였습니다. 기존 D램은 비트라인당 셀이 늘수록 면적이 크게 늘지만, 이 구조는 주변회로가 셀 아래에 있어 면적 증가가 거의 없습니다.

낸드는 이미 하이브리드 본딩을 쓰지만 삼성은 낸드용조차 대량 양산 전이며, D램 COP는 부유체 효과라는 별도 과제가 남아있어 2020년대 후반 1d 이후 세대에나 본격 도입될 전망입니다.
...
낸드는 밀도를 높이려면 다이를 많이 쌓아야 하는데, 쌓을수록 대기 중인 다이의 누설전류가 동작 중인 다이의 전류에 근접할 만큼 커지는 문제가 있습니다. 샌디스크는 선택 안 된 다이의 데이터 경로를 차단하는 게이팅으로 대기전류를 100분의 1 수준까지 낮췄습니다.
...
로직 공정이 미세화될수록 SRAM 셀은 상대적으로 덜 작아지는 현상이 심해지고 있습니다. N5에서 N2로 넘어가며 로직 면적은 40% 줄었지만 8T 고전류 SRAM 셀은 18%, 6T 셀은 단 2%만 줄었습니다.
...
TSMC는 2023년 ISSCC에서 처음 선보인 STT-MRAM(스핀 방향으로 데이터를 저장하는 자기저항 메모리)을 N16 공정에서 개선해 재공개했습니다. 핵심은 읽기·쓰기 동시 처리 듀얼포트 구조로, 자동차 무선 펌웨어 업데이트(OTA) 중에도 읽기를 멈출 수 없는 상황에 필수적입니다.
...
인터리빙 읽기(여러 모듈을 독립된 클럭으로 번갈아 접근)를 적용해 200MHz에서 51.2Gb/s 처리량을 냈고, 84Mb 매크로 기준 0.8V·영하 40도\~150도 전 구간에서 7.5ns 읽기 접근 시간을 기록했습니다.

읽기 방해(read disturb) 오류율은 150도에서도 사실상 무시할 수 있는 수준(10⁻²²ppm 미만)이었고, 168Mb 테스트 칩은 리플로우 공정을 통과하고 150도에서 20년 데이터 보존 조건도 만족해 까다로운 자동차 신뢰성 기준을 충족했습니다.

삼성도 8LPP 공정 기반 eMRAM을 발표했지만, TSMC가 더 저렴한 공정에서 필요 기능과 성능을 모두 갖춰 유리합니다. TSMC는 비트셀을 25% 더 줄이고 내구성을 100배 높인 차세대 "Flash-Plus" 버전도 준비 중입니다.
...
광신호 방식의 선택은 스케일업 CPO가 언제 상용화되느냐를 좌우합니다. 엔비디아는 스케일업(GPU-GPU 초근접 연결)에 쓸 32Gb/s 파장 8개를 묶는 DWDM(파장분할다중화) 방식을 제안했습니다. 9번째 파장은 클럭 전용(16Gb/s)입니다.
...
클럭포워딩은 별도 파장으로 클럭 신호를 함께 보내, 수신단의 클럭·데이터 복원(CDR) 회로를 없애 SerDes(신호 변환 회로)를 단순화하는 기법입니다.
...
완전 코히런트는 저손실 C밴드 파장을 쓰지만 장거리에서는 분산(신호가 퍼지는 현상)이 커서 무거운 DSP 처리가 필요해, 캠퍼스 정도 거리에는 과한 성능입니다. 코히런트-라이트는 짧은 거리에서 분산이 거의 없는 O밴드 파장을 써 DSP 부담을 최소화합니다.
...
브로드컴은 64개 레인(레인당 약 100G, PAM4)으로 구성된 6.4T 광학엔진을 Tomahawk 5 51.2T CPO 스위치 시스템에서 시연했습니다. 각 엔진은 광신호를 만드는 PIC(광집적회로)와 이를 제어하는 EIC(전기집적회로)를 TSMC N7 공정으로 구성합니다.
...
브로드컴도 향후 COUPE로 전환할 예정이지만, 이번처럼 이전 세대 제품은 기존 공급망 파트너를 그대로 활용합니다.

이 시연은 51.2T급 스위치가 상용 CPO 시스템에서 실제로 작동함을 보여준 사례로, COUPE 전환 이전에도 과도기적 공급망으로 초대형 스위치 대역폭을 구현할 수 있음을 입증했습니다.
...
인텔이 공개한 UCIe-S 호환 다이간(D2D) 인터페이스는 16개 레인에서 표준 기준 레인당 최대 48Gb/s, 자체 프로토콜로는 56Gb/s까지 도달합니다. 일반 유기 기판에서 최대 30mm 거리를 지원하며, 흥미롭게도 22나노라는 오래된 공정으로 제작됐습니다.
...
이 인터커넥트는 차세대 인텔 다이아몬드 래피즈 제온 서버 CPU의 프로토타입으로 추정됩니다. 다이아몬드 래피즈는 IMH 다이 2개와 CBB 다이 4개로 구성되는데, CBB 다이에서 양쪽 IMH 다이까지 배선이 길게 이어져야 합니다.

이 인터커넥트가 표준 패키지 기판만으로 다이들을 연결해 기존 EMIB(첨단 패키징 기술) 없이도 장거리 배선을 대체할 후보로 거론됩니다. 22나노 시험칩보다 실제 인텔3 공정으로 옮기면 효율이 훨씬 좋아질 것으로 예상됩니다.
...
TSMC의 액티브 LSI(aLSI)는 기존 CoWoS-L·EMIB 방식보다 신호 품질(시그널 인티그리티)을 높여, 상위 다이(SoC)의 PHY·SerDes 회로 부담을 줄이는 것이 핵심입니다. 실증 칩은 32Gb/s UCIe 유사 방식 트랜시버를 사용했습니다.

신호 품질이 좋아진 덕에 범프(연결 접점) 간격을 45㎛에서 38.8㎛로 줄이고, 배선 격자를 맨해튼 그리드로 바꿔 PHY 깊이를 1043㎛에서 850㎛로 축소했습니다. 이렇게 아낀 면적은 연산·메모리·입출력에 재배분하거나 다이 크기를 줄이는 데 쓸 수 있습니다.
...
'액티브'라는 이름의 유래인 ETT(엣지 트리거 트랜시버)는 드라이버, AC 커플링 커패시터, 양·음 피드백 증폭기로 구성됩니다. 신호가 커패시터를 통과할 때 전환 지점(엣지)에서 피크가 생기고, 듀얼루프 증폭기가 이를 감지해 전압을 안정시킵니다.

이 능동 회로를 추가해도 에너지 비용은 비트당 0.07pJ만 늘어나, 적층된 다이에서 발열 부담을 최소화합니다.
...
또한 브릿지 다이 전면부에 임베디드 딥트렌치 커패시터(eDTC)를 통합해 PHY·D2D 컨트롤러로 가는 전력 공급을 개선했습니다.

테스트는 다이 자체 검증(KGD), 스택 기능 검증(KGS), 전체 조립 후 종합 검증(KGP) 3단계로 나눠 진행하며, KGD·KGP 단계 모두 0.75V·32Gb/s, 0.95V·38.4Gb/s를 기록해 다른 방식보다 배선당 대역폭·에너지 효율이 우수했습니다.
...
마이크로소프트가 공개한 D2D(다이간) 인터커넥트 시험용 칩은 다이 2개와 D2D 노드 2쌍으로 구성되며, 클럭게이팅·크로스토크까지 실제 환경과 동일하게 재현했습니다. TSMC N3P 공정, 배선 폭 532㎛·PHY 깊이 1350㎛ 규모입니다.
...
TSMC의 N3E·N3P 공정은 컨택트 게이트 피치(CGP, 트랜지스터 사이 최소 간격) 옵션을 48nm·54nm 두 가지로 제공합니다. 대부분의 칩은 다이를 작게 만들 수 있는 좁은 48nm를 쓰지만, 누설전류·배선·제조 난이도 문제가 따라옵니다.
...
미디어텍은 사용하지 않는 노화 여유분(에이징 버짓, 노후화 대비 남겨둔 성능 마진)을 활용하고 열 오버슛(순간적 과열)을 줄이는 동적 최적화로 부스트 클럭을 4.21GHz에서 4.4GHz로 끌어올렸습니다.

최신 공정이라도 항상 좁은 트랜지스터 간격이 정답은 아니며, 발열·전력이 중요한 고성능 코어에는 넓은 CGP가 오히려 유리할 수 있음을 보여준 사례입니다.
...
인텔이 공개한 M3DProc는 인텔의 첫 하이브리드 본딩(범프 없이 구리 면끼리 직접 접합) 칩입니다. 인텔3 하단 다이와 18A 상단 다이를 9㎛ 간격의 Foveros Direct로 접합했으며, 각 다이에는 코어·DNN 가속기 타일 56개가 배치됩니다.
...
타일들은 14×4×2 형태의 3D 메시로 배치돼 지연시간이 줄고 처리량이 거의 40% 늘었습니다. 2D(하단 다이 내부 이동)와 3D(양쪽 다이 간 이동)의 에너지 효율을 비교한 결과, 하이브리드 본딩 인터커넥트(HBI)로 인한 효율 손실은 무시할 수 있는 수준이었습니다.
...
패키징 방식은 클리어워터 포레스트(CWF)와 유사합니다. CWF는 인텔3 베이스 다이 위에 18A 컴퓨트 다이를 9㎛ Foveros Direct로 접합하지만, CPU 코어 클러스터의 L2 캐시만 L3에서 분리하는 데 그칩니다.

반면 M3DProc는 코어·가속기 타일 전체를 수직 통합해, 하이브리드 본딩이 캐시 분리를 넘어 연산 코어 자체의 3차원 적층으로 확장될 수 있음을 보여줍니다.
...
FP16·FP8·MXFP4 등 다양한 데이터 형식을 계산할 때, 회로를 공유하면 형식별 최적화가 부족해 전력 손해를 보고 형식마다 별도 회로를 두면 면적이 크게 늘어나는 트레이드오프가 있습니다.

AMD는 이 중간 지점을 찾는 데 공을 들였고, N3P 공정 자체의 성능 향상 외에도 순수 설계 개선만으로 동일 전력 기준 주파수를 5% 높였습니다.
...
리벨리온스는 한국의 AI 가속기 스타트업으로, 이번 학회에서 신제품 Rebel100의 아키텍처를 처음 공개했습니다. 대부분의 AI 가속기가 TSMC에서 생산되는 것과 달리, 리벨리온스는 삼성 파운드리의 SF4X 공정을 선택했습니다.

엔비디아·AMD·브로드컴 등이 TSMC 첨단 공정 물량 대부분을 가져가는 상황에서, 삼성 파운드리는 상대적으로 유연한 공급 옵션을 제공합니다.
...
## 19. 마이크로소프트 마이아 200 (Paper 17.4)

**📌 핵심:**
- 마이크로소프트 **마이아 200**은 HBM 탑재 가속기 중 사실상 마지막 **레티클 한계(칩 최대 제조 크기) 단일 다이** 설계 — 경쟁 가속기는 이미 2\~8개 컴퓨트 다이로 쪼개는 멀티칩 구조로 전환
- TSMC N3P 공정에서 다이 전체를 추론 용도에 극한까지 최적화, FP4 기준 **10페타플롭스 이상**·**HBM3E 스택 6개**·**28개의 400Gb/s 양방향 D2D 링크** 탑재
- 패키지 레벨은 H100과 유사하게 CoWoS-S 인터포저에 메인 다이 1개 + HBM3E 6개 구성, 칩 중앙에는 SRAM 272MB(L1 TSRAM 80MB + L2 CSRAM 192MB) 배치
- 결론: 마이아 100이 GPT 이전 시대 설계였다면, 마이아 200은 현재의 추론 중심 시대에 맞춰 처음부터 다시 설계된 칩

---

마이아 200은 HBM 탑재 AI 가속기 중 마지막까지 남은 레티클 한계 단일 다이 설계입니다. 레티클 한계란 노광 장비가 한 번에 찍어낼 수 있는 최대 크기를 뜻합니다. 대부분의 경쟁 가속기는 이미 컴퓨트 다이 2\~8개로 쪼개는 멀티칩 구조로 넘어갔습니다.

```mermaid
flowchart TD
    Trend["HBM 탑재 AI 가속기<br/>설계 트렌드"] --> Multi["대부분: 멀티칩 구조<br/>(컴퓨트 다이 2~8개로 분할)"]
    Trend --> Maia["마이아 200: 단일 다이 고수<br/>(레티클 한계까지 확장)"]
    Maia --> Spec["FP4 10페타플롭스+<br/>HBM3E 스택 6개<br/>D2D 링크 28개(400Gb/s)"]

    classDef default fill:#eff6ff,stroke:#3b82f6,stroke-width:1px;
    classDef highlight fill:#fff7ed,stroke:#ea580c,stroke-width:2px;
    class Maia highlight;
```

다이 전체가 오직 추론이라는 하나의 용도에 최적화되어, 엔비디아·AMD GPU에 남아있는 미디어·벡터 연산 같은 범용 회로가 없습니다. 패키지 레벨에서는 H100과 유사하게 CoWoS-S 인터포저 위에 메인 다이 1개와 HBM3E 스택 6개를 배치했습니다.

칩의 긴 변에는 HBM3E용 PHY가 각 3개씩, 짧은 변에는 400Gb/s D2D 링크가 각 14개씩(총 28개) 배치되며, 중앙에는 SRAM 272MB(L1 TSRAM 80MB + L2 CSRAM 192MB)가 자리합니다.

```mermaid
flowchart TD
    Link["마이아 200 D2D 링크<br/>총 28개"] --> Fixed["고정 링크 21개<br/>(다른 노드 칩 7개와<br/>각 3개씩 직결)"]
    Link --> Switch["스위치드 링크 7개<br/>(랙 내 스위치 4대 중<br/>하나에 연결)"]

    classDef default fill:#eff6ff,stroke:#3b82f6,stroke-width:1px;
```

마이아 100이 GPT 이전 시대에 설계된 칩이라면, 마이아 200은 지금의 추론 중심 시대에 맞춰 처음부터 다시 설계된 칩입니다. 2026년 초 애저(Azure)에 정식 공급되기 시작했습니다.

---

## 20. 삼성 SF2 온도 센서 (Paper 21.5)

**📌 핵심:**
- 화려하진 않지만 모든 칩 동작에 필수적인 부품인 온도 센서를 삼성이 SF2 공정에서 재설계 — 기존 방식인 **바이폴라 접합 트랜지스터(BJT)** 대신 **BEOL(후공정) 금속 저항체**를 사용해 면적을 극적으로 줄임
- 이 금속 저항체는 일반 배선용 금속보다 시트저항이 **518배 높아**, 같은 저항값을 만드는 데 필요한 면적이 약 **1%**로 줄어들고, 상부 배선층에 위치해 하부 회로 공간을 침범하지 않음
- 저항-커패시터 완전 적층 구조로 센서 하나의 총면적을 단 **625㎛²**로 줄였으며, 저속 충전 경로(0.1R)로 먼저 빠르게 채운 뒤 본 저항으로 전환하는 시간오프셋 압축 기법으로 낮은 온도계수(TCR, 일반 배선 금속의 0.2배)로 인한 느린 감지 속도 문제를 보완
- 결론: 시간-디지털 변환 회로도 기존의 큰 선형 지연 발생기 대신 컴팩트한 링 오실레이터 기반으로 교체해 면적을 **99.1% 절감** — 칩 하나에 수천 개씩 들어가는 부품인 만큼 이런 소형화가 전체 칩 설계 여유 공간에 실질적인 영향을 줌

---

온도 센서는 화려한 발표거리는 아니지만 모든 칩이 안정적으로 동작하는 데 필수적인 부품입니다. 삼성은 SF2 공정에서 기존의 바이폴라 접합 트랜지스터(BJT) 방식 대신 BEOL(후공정 배선층) 금속 저항체를 활용한 컴팩트 온도 센서를 공개했습니다.

```mermaid
flowchart TD
    Old["기존 온도 센서<br/>(BJT 방식)"] --> Problem["FEOL(전공정) 영역을<br/>차지해 하부 회로와 공간 경쟁"]
    New["삼성 SF2: BEOL 금속<br/>저항체 방식"] --> Benefit["시트저항 518배↑<br/>→ 필요 면적 약 1%로 축소<br/>상부 배선층 위치로 FEOL 여유 확보"]
...
이 센서는 저항-커패시터 완전 적층 구조로 총면적이 단 625㎛²에 불과합니다. 다만 금속 저항체는 온도계수(TCR)가 일반 배선 금속의 0.2배에 그쳐 감지 해상도가 낮고, 이를 보완해 기준 저항값을 높이면 RC 시정수가 커져 감지 속도가 느려지는 문제가 생깁니다.

```mermaid
flowchart TD
    Issue["낮은 TCR(0.2배) 보완<br/>→ 기준저항 상향<br/>→ 감지 속도 저하 부작용"] --> Fix["시간오프셋 압축 기법<br/>(저저항 0.1R 경로로 먼저 빠르게 충전<br/>→ 온도 민감 구간만 본 저항으로 전환)"]
    Fix --> TDC["시간-디지털 변환도<br/>링 오실레이터 기반으로 교체<br/>(지연 발생기 면적 99.1% 절감)"]
...
이 센서는 파운드리가 검증한 PDK(공정설계키트) 표준 부품으로 제공돼 양산 환경에 적합하며, 칩 하나에 발열 지점(핫스팟)마다 수천 개씩 배치될 수 있습니다. 이런 부품 하나하나의 소형화가 칩 설계 여유 공간에 누적적으로 큰 영향을 줍니다.

---

*작성 진행률: 100% 완료*
*업데이트: 마이크로소프트 마이아 200, 삼성 SF2 온도 센서 섹션 작성 — 원문 20개 발표 전체 커버 완료*
