...
예를 들어 매출 목표 관련 자료가 필요할 때, 에이전트가 UI나 API에서 정보를 직접 추출해 보고서까지 대신 만들어주는 방식입니다.

완벽하지는 않지만 대부분의 사람보다 더 빠르게 정보를 처리·종합·정리할 수 있고, 환각(hallucination) 위험은 있으나 사람이 이끄는 기존 프로세스에도 이미 오류가 존재한다는 점은 동일합니다. 정보가 쓸 만한 수준의 정확도로 처리돼 다음 단계로 넘어가기만 해도, 일할 수 있는 노동의 총량 자체가 크게 늘어납니다.
...
예를 들어 에이전트가 Postgres 데이터베이스를 직접 조회해 차트를 만들고 담당자에게 이메일로 보내면, 기존 CRM SaaS가 하던 워크플로우를 UI 학습이나 소프트웨어 업데이트 없이 대체할 수 있습니다.

BI·데이터 입력, ITSM 1\~2차 티켓 처리, 백오피스 정산 업무가 이미 자동화 대상에 오르고 있으며, 이는 소프트웨어 업계에서 가장 신성했던 해자들의 문을 두드리는 중입니다.
...
Claude for Excel은 사실상 마이크로소프트의 Copilot for Excel이 해야 했을 역할을 외부 업체가 자사 첫 제품에 먼저 구현한 사례입니다.

마이크로소프트의 현금은 여전히 대부분 Office에서 나오지만, 기업가치(terminal value)의 핵심은 애저 매출 성장에 있습니다. 애저를 키울수록 자사 생산성 소프트웨어를 위협하는 상대에게 문을 더 열어주는 역설에 놓여 있는 셈입니다.
...
챗GPT는 컨텍스트 윈도우가 훨씬 크지만, 작업당 필요한 토큰 수가 한 자릿수 더 많아 장기 계획을 오히려 무너뜨립니다.

앤트로픽은 토큰을 최대한 많이 쓰는 대신 신호 대 잡음비가 가장 깨끗한 방향을 의도적으로 택했다는 게 저자의 해석입니다. 이는 여러 컨텍스트 윈도우에 걸친 계획에서 특히 중요한데, 누적 오차가 조금만 있어도 장기 과업은 결국 실패하기 때문입니다.
...
## 10. 세미애널리시스의 바이브 코딩 사례와 결론

**📌 핵심:**
- SemiAnalysis 애널리스트들도 API 지연시간 비교 도구, 학습용 웹사이트, 토큰 효율성 비교 CLI 벤치마크, 비디오 게임, TSMC-엔테그리스 매출총이익률 비교 차트까지 모두 바이브 코딩으로 직접 제작
- "이것이 소프트웨어 기업에 좋은 소식"이라는 시각은 경계 대상 — 라디오가 인터넷 위에서 그저 하나의 기능으로 얹힌 게 아니라 완전히 대체됐듯, 소프트웨어도 단순히 AI 기능이 추가되는 게 아니라 근본적으로 재편되는 중
- 1970\~80년대 TCP/IP가 OSI·IBM SNA·Novell IPX/SPX와 경쟁했지만 결국 프로토콜 자체는 잊혀지고 그 위에 쌓인 정보·서비스가 가치의 전부가 됐듯, 오늘날 모델(토큰)도 같은 위치에 있으며 Claude Code는 이 시대 최초의 "웹사이트"
- 결론: GitHub 커밋의 4%, 그리고 계속 늘어나는 비중이 이미 Claude Code 혼자서 매일 만들어지고 있으며, 정보 노동의 가속과 경쟁 심화는 이제 막 시작된 국면

---

세미애널리시스 애널리스트들도 실제로 바이브 코딩(사람이 직접 코드를 짜지 않고 AI에게 원하는 결과를 설명해 대신 작성시키는 방식)을 다양한 업무에 적용하고 있습니다.

- API 지연시간(latency) 비교 도구 — 24시간 동안 ISO/OSI 스택 구간별 지연을 측정
- 학습용 웹사이트(learntheworld.ai) — 세계 각지를 학습하는 웹사이트
- CLI 벤치마크 도구 — 모델별 토큰 효율성 비교(Kimi 2.5는 성능은 좋지만 토큰을 많이 써, 초당 토큰 수가 적어도 실제 체감 속도는 더 빠를 수 있음을 확인)
- 비디오 게임 — 미니게임으로 돈을 버는 기능까지 구현
- TSMC vs 엔테그리스 매출총이익률 비교 차트(인수 관련 프로포마 조정 반영)

이 모든 사례는 "소프트웨어 기업이 AI 기능을 더 팔게 될 것"이라는 낙관적 시각과는 거리가 멉니다. 인터넷이 등장했다고 라디오가 인터넷 위의 한 기능으로 살아남은 게 아니라 근본적으로 대체됐듯, 정보 노동의 세계도 근본적인 재편을 겪고 있습니다.

1970\~80년대에는 TCP/IP 외에도 OSI, IBM SNA, Novell IPX/SPX 등 여러 통신 프로토콜이 경쟁했지만, 결국 중요했던 것은 프로토콜 자체가 아니라 그 위에 쌓인 정보와 서비스였습니다. 오늘날 AI 모델도 같은 위치에 있다는 것이 결론입니다.

```mermaid
flowchart TD
    A["1970~80년대<br/>TCP/IP vs OSI·IBM SNA·Novell IPX/SPX<br/>프로토콜 경쟁"] --> B["결과: 프로토콜 자체는 잊혀지고<br/>그 위에 쌓은 정보·서비스가 가치의 전부가 됨"]
    B --> C["오늘날 모델(토큰)도 같은 위치<br/>= 새로운 TCP/IP"]
    C --> D["Claude Code = 이 시대 첫 '웹사이트'<br/>(에이전틱 미래의 첫 증거)"]
...
GitHub 커밋의 4%, 그리고 계속 늘어나는 비중이 이미 Claude Code 혼자서 매일 만들어지고 있습니다. 미래는 에이전트가 방대한 양의 토큰을 조율해 더 크고 나은 프로젝트를 만드는 방향으로 향하며, 정보 노동의 가속과 경쟁 심화는 이제 막 시작된 국면이라는 것이 저자의 결론입니다.

---

*작성 진행률: 100% 완료*
*업데이트: 10장(바이브 코딩 사례와 결론)까지 원문 전 섹션 번역 완료*
