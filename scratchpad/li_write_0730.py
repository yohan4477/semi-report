...
d0728 = day('2026-07-28', [
    ...
    li_row('7487612975202185216',
           'AMD가 OpenAI·Meta에 준 워런트는 보통 "지분 인센티브"로 설명되지만, 계산해 보면 컴퓨트 가격 자체를 되돌려주는 리베이트에 가깝고 OpenAI 기준 실효 할인율이 최대 105%까지 나온다. AMD 8-K상 워런트는 주가 구간별로 베스팅되며 상단 문턱이 $600, 행사가는 사실상 0인 $0.01이라 AMD 주가가 오를수록 되돌려주는 가치가 커진다. 전량 베스팅·행사 가정 시 실효 할인 85~105% — 고객에게 돈을 얹어주며 자사 주식으로 그 값을 내는 구조다. 물량 약정이 주가를 밀어 올리면(이미 $520+) 괜찮은 거래지만, 아니면 순환 거래가 된다.',
