...
CAPTION = {
    ...
    'GOODPUT': ('식 셋을 항별로 세우면 다른 자리가 세 칸뿐이다', mf.FIG_GOODPUT,
                '원문 L131 의 식 셋을 시간 항별로 편 것입니다. 열은 그 식에 드는 시간 '
                '항이고, 칸에 적힌 것은 그 시간에 곱해지는 장수입니다. 머리 줄의 + 는 항끼리 '
                '더한다는 뜻입니다. 가운뎃점은 그 식에 '
                '그 항이 없다는 뜻이고, 테두리를 두른 칸은 윗줄과 달라진 자리입니다. '
                '① 전환이 수리를 대신하는 자리입니다. ② 수리에 작업 전체가 아니라 폭발 '
                '반경만 곱해지는 자리입니다.'),
    'EQTCO': ('월 총비용은 어떤 항으로 갈라지나', et.FIG_EQ_TCO,
              '왼쪽이 결과이고 오른쪽이 그것을 이루는 항입니다. 동그라미는 두 항을 '
              '묶는 셈이고, 수치는 안 그렸습니다. ① 할증이 앞 네 항에만 붙고 뒤 세 '
              '항에는 안 붙습니다. ② 복원 방식에 따라 식이 셋으로 갈립니다. '
