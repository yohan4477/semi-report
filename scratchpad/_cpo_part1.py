...
CAPTION = {
    ...
    'SHARE': ('광 부품이 차지하는 비중 — 클러스터에서 모듈까지', cf.FIG_SHARE,
              '기둥 하나가 100% 이고 아래에서 채운 조각이 그 줄의 대상입니다. 클러스터 총비용에서 네트워킹은 15% 이고 '
              '그 안에서 트랜시버가 60%, 클러스터 총전력에서 네트워킹은 9% 이고 그 안에서 트랜시버가 45% 입니다'
              '(짙은 조각. 클러스터 대비 몇 % 인지는 원문이 안 적었습니다). 트랜시버 한 개 안에서는 DSP 가 '
              '전력의 약 절반, 자재비의 20~30% 입니다(위 끝 높이). 전부 SemiAnalysis 추정'
              '(2026-01, GB300 NVL72 3층망 기준)입니다.'),
