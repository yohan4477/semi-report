...
HEAD_CPO = (
    ...
    '원문 기간 <b>2025-03 ~ 2026-08</b> · 유료 뉴스레터 원문<br>'
...
CAPTION = {
    ...
    'SHARE': ('광 부품이 차지하는 비중 — 클러스터에서 모듈까지', cf.FIG_SHARE,
              '기둥 하나가 100% 입니다. 클러스터 총비용에서 네트워킹이 15% 이고, 그 조각을 옆 기둥으로 펼치면(점선) '
              '그 안에서 트랜시버가 60% 입니다. 전력도 같은 짝입니다 — 네트워킹 9%, 그 안에서 트랜시버 45%. '
              '클러스터 대비 트랜시버가 몇 % 인지는 원문이 안 적어서 곱하지 않았습니다. 오른쪽 둘은 트랜시버 '
              '한 개 안에서 DSP 가 차지하는 것으로, 전력의 약 절반과 자재비의 20~30%(위 끝 높이)입니다. 전부 '
              'SemiAnalysis 추정(2026-01, GB300 NVL72 3층망 기준)입니다.'),
