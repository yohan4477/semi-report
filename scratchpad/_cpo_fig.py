...
좌표는 _row 가 계산하고 손으로 찍지 않는다(yohan-figure 규칙 2).
"""
...
FIG_REACH = _svg(W, 230, '신호 증폭 없는 직결 구리가 닿는 거리', ''.join([
    ...
    # 막대 오른쪽 글자는 2미터 점선(x=530)을 넘으면 안 된다 — 원래 글은 532 까지
    # 가서 점선을 가로질렀다(2026-09-09). 온전한 말은 캡션이 받는다
    _bar(1, '200G/레인', 1.0, '약 1m · 랙 가운데로'),
