...
CAPTION = {
    ...
    'SPLIT': ('같은 10년물을 두 사람이 다르게 쪼갠다', rf.FIG_SPLIT,
              '위는 미국주식 사관학교가 둘로 쪼갠 것이고(2026-01) 아래는 메르가 셋으로 쪼갠 '
              '것입니다(2026-07). 위쪽 화살표는 방향만 그렸습니다 — 미주사가 「내려왔다·커졌다」로만 '
              '적고 값을 안 대서 없는 값을 그리지 않았습니다. 아래쪽은 값이 있어 적었습니다. '
              '나누는 방식은 달라도 겹치는 조각이 기간 프리미엄이고, 둘 다 그것이 커졌다고 봅니다. '
              '10년물이 기준금리를 안 따라 내려온 이유가 여기 있습니다.'),
    ...
    'TIPS': ('유가가 올랐는데 튄 것은 기대인플레이션이 아니라 실질금리였다', rf.FIG_TIPS,
             '메르가 2026년 7월에 잰 값입니다. 명목금리 총합은 '
             '원문에 없어 그리지 않았습니다.'),
    'GAUGE': ('잣대를 바꾸면 같은 물가가 낮아 보인다', rf.FIG_GAUGE,
              '메르의 사슬이 인과로 이어 놓은 네 걸음입니다. 짙은 칸이 지금 자리입니다.'),
    'DISCOUNT': ('먼 미래일수록 같은 1%포인트가 크게 깎는다', rf.FIG_DISCOUNT,
                 '메르가 예금으로 푼 계산입니다. 먼 이익을 약속하는 '
