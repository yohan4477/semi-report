...
JAL = {
    ...
    'gain': ('오픈AI 첫 추론 칩의 설계 기준이 마지막 토큰까지의 '
             ...
             '프리필 전담·디코드 전담으로 왜 안 나눴는지, 첫 회로 설계부터 테이프아웃까지가 '
             '왜 9개월이었는지.'),
    ...
    'report': [
        ...
        ('toc', [
            ...
            ('구조', '부분 나눔', [('②', '메모리를 어떻게 붙였나'),
                               ('③', '왜 프리필 전담·디코드 전담으로 안 나눴나'),
                               ('④', '칩 몇 개가 한 덩어리로 도나')]),
            ('성과', '대비', [('⑤', '재 보니 무엇이 나왔나'),
                            ('⑥', '무엇이 아직 안 재졌나')]),
            ('공정', '시간 흐름', [('⑦', '첫 RTL 부터 테이프아웃까지 왜 9개월이었나')]),
            ('한계', '', [('⑧', '이 편이 밝히지 않은 것')]),
        ]),
        ...
        ('p', '오픈AI 설명은 둘이다. 연산에 쓸 데이터가 필요한 때에 레지스터에 도착하지 '
              '않는다는 것(operands arrive late), '
        ...
        ('h', '③ 왜 프리필 전담·디코드 전담으로 안 나눴나'),
        ('p', '칩 하나가 프리필(질문을 한꺼번에 읽는 단계)과 디코드(토큰을 하나씩 내는 단계)를 '
              '둘 다 하는 것이 기본이다. 갈래는 칩 설계를 달리 하는 쪽이 아니라 <b>같은 칩을 '
              '역할별로 나눠 세우는 배치</b>다 — 이 가속기는 프리필만, 저 가속기는 디코드만 '
              '맡긴다. 오픈AI 는 그 배치를 안 골랐다. 워크로드마다 프리필과 '
              '초안·검증(작은 모델이 토큰 여러 개를 미리 내고 큰 모델이 맞는 것만 고르는 방식)의 '
              '비율이 계속 바뀌어서, 비율이 바뀌면 전담으로 세워 둔 가속기가 논다는 이유다.'),
        ...
        ('p', '반례가 원문에 없다. 프리필·디코드를 역할별로 나눠 세우는 배치를 원문은 '
              '「GPU 처럼」이라고만 하고, 실제로 그렇게 돌리는 곳이 어디인지는 대지 않는다. '
              '전담으로 세웠을 때 얼마나 노는지도 오픈AI 주장일 뿐 잰 수가 없다.'),
...
CSS = '''
/* 목차 — 단원은 「1.」, 그 아래 절은 ①. 여태 이 상자에 CSS 가 없어서
   번호도 마디도 꼴도 한 줄에 붙어 흘렀다(2026-08-29) */
.uc-rep .uc-toc { margin:2px 0 16px; padding:12px 14px; border:1px solid var(--line);
  border-radius:8px; background:var(--surface); }
.uc-rep .uc-toc .uc-label { margin:0 0 8px; font-size:.72rem; font-weight:800;
  letter-spacing:.04em; color:var(--ink-3); }
.uc-rep .uc-toc .tg { display:flex; flex-wrap:wrap; align-items:baseline; gap:6px;
  padding:4px 0; }
.uc-rep .uc-toc .tg-n { flex:0 0 1.6em; font-size:.8rem; font-weight:800;
  color:var(--ink-3); }
.uc-rep .uc-toc .tg-k { font-size:.82rem; font-weight:800; color:var(--ink); }
.uc-rep .uc-toc .tg-f { padding:0 5px; border:1px solid var(--line); border-radius:3px;
  font-size:.62rem; font-weight:700; line-height:1.7; color:var(--ink-3); }
.uc-rep .uc-toc .tg ul { flex:1 0 100%; margin:2px 0 0; padding:0 0 0 1.6em;
  list-style:none; }
.uc-rep .uc-toc .tg li { font-size:.76rem; line-height:1.7; color:var(--ink-2); }
.uc-rep .uc-toc .tg li b { margin-right:5px; color:var(--ink-3); }
