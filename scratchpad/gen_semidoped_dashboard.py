...
GROK = {
    ...
    'verdict': ('에이전트를 항상 켜 두는 쓰임이 퍼지자 수요가 GPU 옆의 CPU 로 번졌고, '
                ...
                '갈리기 시작했다.'),
    'report': [
        # 인과 사슬 — 쓰임이 바뀌었다, 그래서 CPU 가 갈린다, 그래서 이만큼 산다,
        # 그런데 이것이 없다, 그리고 이건 못 믿는다
        ('toc', ('인과 사슬', [
            ('달라진 쓰임', '시간 흐름', [
                ('①', '전 → 후', '무엇이 CPU 쪽으로 번졌나')]),
            ('갈린 CPU', '부분 나눔', [
            ...
            ('사는 방식', '대비', [
                ('①', '코어 수', '몇 개를 두어야 하나')]),
            ('아직 없는 것', '문제와 처방', [
                ('①', '오케스트레이션', '무엇이 이 그림을 못 굴리나')]),
            ('못 믿을 것', '부분 나눔', [('①', '', '이 편이 밝히지 않은 것')]),
        ])),

        ('h', '<span class="h-node">달라진 쓰임</span> ① 무엇이 CPU 쪽으로 번졌나'),
        ('fig', ('일이 어디로 가나', figs.AGENT_FLOW,
                 '상자 이름은 원문의 비유를 그대로 쓴 것이다 — GPU 가 천재, 그 옆 CPU 가 비서.')),
        ('p', '<b>전.</b> 에이전트는 사람이 앉아 있을 때만 돌았다. 노트북에 하니스를 깔면 '
              '그 컴퓨터 전체에 접근권을 주게 되는 것이 꺼려져, 사람들이 <b>에이전트 전용 '
              '기기</b>를 따로 마련했다 — 맥 미니가 팔린 이유가 그것이라는 게 진행자 설명이다. '
              '집에 두면 정전과 인터넷 장애를 스스로 감당해야 한다.'),
        ('p', '<b>후.</b> 그 주 나온 <b>Grok bot</b> 은 그 기기를 클라우드로 옮겼다. 설치하면 '
              '대화로 우두머리 에이전트를 정하고 그 아래 하위 에이전트를 붙이는 식이고, '
              '도구와 로그인 정보가 전용 가상머신 안에 있어 한 번 로그인하면 에이전트들이 '
              '나눠 쓴다. <b>컴퓨터를 꺼도 계속 돈다.</b> 값은 월 200달러다.'),
        ('p', '수요가 먼저 왔다는 증거를 진행자가 하나 든다. 에이전트가 처음 퍼진 것이 '
              '2025년 12월이고, 인텔이 2026년 2월 실적발표에서 <b>CPU 수요가 이렇게 몰릴 줄 '
              '몰랐다</b>고 말했다는 것이다.'),

        ('h', '<span class="h-node">갈린 CPU</span> ① GPU 옆 CPU 는 무엇을 하나'),
        ('p', '진행자들이 세운 비유가 이 편을 관통한다. <b>GPU 는 모든 것에 박사학위를 가진 '
              '천재</b>이고, <b>CPU 는 지시를 받아 실제 일을 처리하는 비서</b>다. HBM 은 '
              '천재 책상 옆에 쌓인 서류더미라 자리에서 일어나지 않고 바로 집어 쓴다.'),
        ('p', '호스트 노드 CPU 가 하는 일은 하나다. <b>다음 일감을 끊임없이 준비해 GPU 가 쉬지 '
              '않게 만든다.</b> 그래서 이 자리는 코어를 많이 두는 것보다 <b>코어 하나가 빠른 것</b>이 '
              '중요하다. 메모리 코히런시가 있으면 비서가 천재의 메모를 직접 볼 수 있다 — '
              '진행자가 든 예가 그레이스 블랙웰의 칩투칩 연결이다.'),

        ('h', '<span class="h-node">갈린 CPU</span> ② 여분 일은 왜 따로 서나'),
        ('p', '문제는 에이전트가 시키는 일이 GPU 연산만이 아니라는 것이다. 코드를 컴파일하고 '
              '웹을 뒤지고 공시를 열어 본다. 이 일을 호스트 CPU 에 얹으면 <b>천재를 먹이는 일이 '
              '밀린다.</b> 그래서 여분 일을 맡는 CPU 랙을 따로 세운다는 논의가 나오고, 진행자들이 '
              '이것을 <b>에이전틱 CPU</b> 라 부른다.'),
        ('p', '이 이름이 이 편에서 처음 나온 말은 아니다. 진행자는 앞서 AMD 와 한 회차에서도 '
              '같은 프레이밍에 상대가 공감했다고 전한다. 인텔이 지난여름 에이전틱 AI 용으로 '
              '<b>P-랙</b>과 <b>E-랙</b>을 따로 낸 것도 같은 갈림이다 — 성능에 맞춘 판과 '
              '코어 수에 맞춘 판이다.'),

        ('h', '<span class="h-node">사는 방식</span> ① 몇 개를 두어야 하나'),
        ...
        ('p', '자리마다 답이 다르다. 호스트 자리는 <b>88코어급이라도 코어가 빠른 것</b>이 낫고, '
              '같은 칩을 에이전틱 용도로 돌려 쓸 때는 <b>128·256·288·512</b> 로 늘리는 편이 '
              '나을 수 있다는 것이 진행자 어림이다. 그래서 셈이 이렇게 바뀐다 — '
              '<b>이제는 직원 한 명당 비용의 문제다.</b> 코어를 얼마나 확보하고 그 직원이 '
              '얼마나 비싼가.'),

        ('h', '<span class="h-node">아직 없는 것</span> ① 무엇이 이 그림을 못 굴리나'),
        ('p', '천재도 비서도 있는데 <b>둘을 함께 부리는 층이 없다.</b> 진행자는 엔비디아 '
              'Dynamo 가 GPU 성능을 끌어올리는 물건이지 그 위에서 CPU 와 GPU 에 일을 나눠 '
              '주는 층은 아니라고 짚는다. Modular 의 Mojo 는 같은 코드를 CPU 와 GPU 양쪽에서 '
              '돌릴 수 있게 하지만, 일을 나눠 세우는 데까지 가는지는 본인도 모른다고 밝힌다.'),
        ('p', '천재도 한 종류가 아니다. 진행자는 셋으로 나눈다 — 아주 빠른데 기억이 짧은 쪽, '
              '기억이 크고 두루 쓰이는 대형 HBM 가속기, 답이 늦어도 되는 과학·의료 문제용으로 '
              '느리게 오래 생각하는 쪽. Cerebras 가 녹음 전날 <b>랙 단위</b>를 새 연산 단위로 '
              '내놓은 것도 같은 자리에서 읽는다.'),

        ('h', '<span class="h-node">못 믿을 것</span> ① 이 편이 밝히지 않은 것'),
        ('p', '가장 큰 수가 사고실험이다. 진행자는 <b>1,000만 명</b>이 이런 도구를 쓰고 '
              '가상머신마다 에이전트를 <b>100개</b> 돌린다고 가정해 <b>10억 코어</b> 수요를 '
              '말하는데, 두 가정을 곱한 어림이고 실측도 업계 전망도 아니다.'),
        ('p', 'Grok bot 의 세부도 확인된 것이 아니다. 어떤 CPU 위에서 어느 지역에서 도는지는 '
              '두 사람이 써 본 경험으로 미룬 것이다. 코어 수 다섯도 제품 사양표가 아니라 '
              '예로 든 수다.'),
        ('p', '<b>가장 큰 빈자리는 오픈AI·앤트로픽 쪽 실제 배분이다.</b> 진행자가 마지막에 '
              '그 회사들이 에이전틱 CPU 와 범용 CPU 에 일을 얼마나 나눠 쓰는지 알려 달라고 '
              '청한다 — 이 편은 그 답 없이 끝난다.'),
    ],
}
