...
JAL = {
    ...
    'report': [
        ...
        ('h', '① 잣대를 무엇으로 바꿨나'),
        ('p', '지표 둘이다. ① <b>마지막 토큰까지 걸리는 엔드투엔드 지연</b> ② <b>요청당 '
              '전기에너지</b>. 둘은 서로 밀어내는 값이라 오픈AI 는 수치 하나를 내지 않고 늘 '
              '파레토 곡선으로 보이겠다고 했다. 엔비디아·AMD 는 총소유비용부터 본다. 칩을 '
              '사서 굴리는 데이터센터 운영자가 치르는 돈이다. 오픈AI 는 챗을 쓰는 사람이 '
              '겪는 것을 잣대로 삼았다.'),
        ('fig', ('잣대가 어디로 옮겼나', figs.CRITERIA,
                 '왼쪽은 데이터센터 운영자의 셈이고 오른쪽은 사용자 경험이다. 오픈AI 는 '
                 '오른쪽 둘을 골랐고, 두 값이 상충하므로 곡선으로 낸다.')),
        ('p', '아래 절 셋은 그 잣대가 실리콘에서 어떻게 박혔는지다. 메모리·연산·연결이 '
              '각각 다른 답을 내놓는데, 세 답이 전부 「사람이 겪는 시간을 줄인다」는 한 '
              '잣대에서 나온다.'),

        ('h', '② 대역폭을 깔고도 왜 못 쓰나'),
        ('p', 'HBM4 가 <b>초당 읽어 내는 양</b>을 칩 128개치 합치면 약 1페타바이트다. '
              '진행자 셈으로 1조 파라미터 모델을 FP4(값 하나를 4비트로 줄여 담는 방식)로 '
              '담으면 약 0.5테라바이트니, 읽어 내는 속도만 보면 <b>초당 약 2,000토큰</b>이 '
              '나와야 한다. 실제로는 거기 못 미친다.'),
        ('p', '오픈AI 설명은 둘이다. ① 연산에 쓸 값이 필요한 때에 레지스터로 안 온다 '
              '② HBM 을 이웃 가속기와 나눠 쓰면서 다툰다. 그래서 가속기마다 HBM 을 조각내 '
              '하나씩 전담시키고 그 조각으로 가는 <b>전용 버스</b>를 따로 깔았다. '
              'NUMA(비균일 메모리 접근 — 멀티코어 CPU 에서 코어마다 메모리를 전담시키던 '
              '방식)를 그대로 가져온 것이다. 대역폭을 더 늘린 게 아니라 <b>기다리는 시간</b>을 '
              '잡았다 — 잣대가 지연이니 그쪽을 잡는다.'),

        ('h', '③ 연산을 왜 한 칩에 몰았나'),
        ('p', 'GPU 에서는 프리필(질문을 한꺼번에 읽는 단계)과 디코드(토큰을 하나씩 내는 '
              '단계)를 서로 다른 칩에 맡기는 방법이 있다. 오픈AI 는 안 나눴다. 워크로드마다 '
              '프리필과 초안·검증(작은 모델이 토큰 여러 개를 미리 내고 큰 모델이 맞는 것만 '
              '고르는 방식)의 비율이 계속 바뀌기 때문이다. 비율이 바뀌면 전담 칩이 논다.'),
        ('p', '그래서 칩 하나에 둘 다 넣고 어느 쪽에도 치우치지 않게 맞췄다. 그 순간 안 쓰는 '
              '회로는 전원을 끊는다. 슬라이드 문구가 그 판단을 그대로 말한다 — '
              '<b>dark silicon is cheaper than idle accelerators</b>. 노는 실리콘이 노는 '
              '가속기보다 싸다는 것도 요청당 전기에너지를 잣대로 놓아야 나오는 셈이다.'),

        ('h', '④ 칩을 어떻게 묶었나'),
        ('p', '칩을 묶는 단위가 두 단계다. ① <b>작은 묶음</b>은 칩 128개이고 그 안에서 칩 '
              '하나가 초당 600기가비트로 붙는다 ② <b>큰 묶음</b>은 작은 묶음을 여럿 모은 '
              '것으로 칩이 최대 2,048개까지 가고 칩 하나가 초당 200기가비트로 붙는다. 붙이는 '
              '장치는 브로드컴 Tomahawk 6 스위치이고, 칩끼리 말을 맞추는 규약은 '
              'ESUN(브로드컴이 미는 스케일업 연결 규격)이다. AMD 쪽 UALink 와는 다른 '
              '진영이다. 칩 128개가 한 랙이면 2,048개는 약 열여섯 랙이라는 것이 진행자가 '
              '나눠 본 값이다.'),
        ('fig', ('칩을 묶는 두 단계', figs.DOMAIN,
                 '작은 묶음 안에서는 칩 하나가 초당 600기가비트로 붙고, 큰 묶음에서는 초당 '
                 '200기가비트로 붙는다. 랙 수는 원문에 없고 128 과 2,048 을 나눈 진행자 '
                 '어림이다.')),

        ('h', '⑤ 성능이 얼마나 나왔나'),
        ('p', '잰 도구는 SemiAnalysis 의 <b>Inference X</b>다. 파워를 정규화해 공개 비교했고, '
              '작은 모델부터 큰 모델까지 여러 오픈 모델을 돌린다. 칩 하나가 아니라 시스템 '
              '전체가 사용자에게 어떤 속도를 주는지를 보려는 것이다. 나온 것은 셋이다. '
              '① DeepSeek R1(6,710억 파라미터)에서 파레토 프론티어를 새로 그었다 '
              '② GPT-OSS(1,200억)에서 <b>사용자당 초당 1,000토큰 이상</b>을 냈다 '
              '③ 700W 로 GB200 약 1,200W 보다 훨씬 적게 먹었다.'),
        ('tbl', ('발표에 나온 값',
                 ['재는 것', '나온 값', '언제 것 · 성격'],
                 [['DeepSeek R1 6,710억', '파레토 프론티어를 새로 그었다',
                   '2026-08 · 발표 슬라이드'],
        ...
        ('p', '첫 RTL(회로를 글로 적은 설계) 작성부터 테이프아웃까지 <b>9개월</b>이다. 통상 '
              ...
              '더 나은 것으로 바꿨다는 것이 진행자 전언이다. 진행자들은 회사의 첫 칩을 보통 '
              '버리는 셈 친다. 오픈AI 는 1호부터 완성도가 높았다고 본다.'),

        ('h', '⑦ 이 편이 밝히지 않은 것'),
        ('p', '못 잰 범위가 셋이다. ① 벤치마크의 입출력이 8K·1K 로 짧아 100만 토큰급 긴 '
              '문맥은 아직 안 나왔다 ② agentic 워크로드는 테스트하지 않아 별도 AgentX 로 '
              '재겠다고 했다 ③ 견준 상대의 세대가 다르다 — 할라페뇨는 HBM4 를 쓰고 '
              '블랙웰·MI355 는 HBM3E 다. 진행자들이 스스로 Vera Rubin·Helios 와 견줘야 '
              '공평하다고 말한다.'),
        ('p', '스펙 상당수도 진행자 추측이다. CPU 가 x86 계열 Turin 급인지, 공정이 TSMC N3 '
              ...
              '양산에 약 1억 달러가 든다는 말도 확정치가 아닌 어림이다. 랙 열여섯도 128 과 '
              '2,048 을 나눈 값이다.'),
        ('p', '이 카드가 절로 세우지 않은 마디도 남는다 — 발표자 셋이 누구인지, 무엇을 뺄지가 '
              '가장 어려운 결정이었다는 설계 태도, 남의 모델까지 돌려 보인 이유, 다음 세대 '
              '로드맵. 잣대의 사슬 밖이라 뺐다. 각도 상자를 열면 그 마디들이 흐리게 서 있다.'),
    ],
}
