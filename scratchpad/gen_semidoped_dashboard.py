...
JAL = {
    ...
    'gain': ('오픈AI 첫 추론 칩의 설계 기준이 마지막 토큰까지의 '
             '지연과 요청당 전기에너지라는 것 — 총소유비용은 그다음이다. 대역폭을 깔고도 왜 다 못 쓰는지, '
    ...
    'report': [
        ...
        # 이 편은 슬라이드를 순서대로 읽는 전사본이라 마디가 차례를 이룬다. 그 차례를
        # 목차에 밝히면 맨 위에 사슬로 걸린다. 차례가 없는 글은 축을 비우고 대상의
        # 마디로 간다 — 마디 이름은 글마다 다르다
        ('toc', ('글이 굴러가는 차례', [
                ('목표', '대비', [('①', '기준', '무엇을 재기로 했나')]),
                ('시도', '부분 나눔', [
                    ('설계', [
                ...
                ('성과', '대비', [
                    ('잰 것', [('⑥', '결과', '재 보니 무엇이 나왔나')]),
                    ('못 잰 것', [('⑦', '공백', '무엇이 아직 안 재졌나')])]),
                ('한계', '', [('⑧', '', '이 편이 밝히지 않은 것')]),
        ])),

        ('h', '① 무엇을 재기로 했나'),
        ('p', '지표 둘이다. ① <b>마지막 토큰까지 걸리는 엔드투엔드 지연</b> ② <b>요청당 '
              '전기에너지</b>. 둘은 서로 밀어내는 값이라 오픈AI 는 수치 하나를 내지 않고 늘 파레토 곡선으로 '
              '보이겠다고 했다. 엔비디아·AMD 는 총소유비용부터 본다. 칩을 사서 굴리는 '
              '데이터센터 운영자가 치르는 돈이다. 오픈AI 는 챗을 쓰는 사람이 겪는 것을 '
              '잣대로 삼았다.'),
        ...
        ('p', '오픈AI 설명은 둘이다. ① 연산에 쓸 값이 필요한 때에 레지스터로 안 온다 '
              '② HBM 을 이웃 가속기와 나눠 쓰면서 다툰다. 그래서 가속기마다 '
              'HBM 을 조각내 하나씩 전담시키고 그 조각으로 가는 <b>전용 버스</b>를 따로 '
              '깔았다. NUMA(비균일 메모리 접근 — 멀티코어 CPU 에서 코어마다 메모리를 '
              '전담시키던 방식)를 그대로 가져온 것이다.'),

        ('h', '③ 연산을 어떻게 한 칩에 몰았나'),
        ('p', 'GPU 에서는 프리필(질문을 한꺼번에 읽는 단계)과 디코드(토큰을 하나씩 내는 단계)를 '
              '서로 다른 칩에 맡기는 방법이 있다. 오픈AI 는 안 나눴다. 워크로드마다 프리필과 '
              '초안·검증(작은 모델이 토큰 여러 개를 미리 내고 큰 모델이 맞는 것만 고르는 방식)의 '
              '비율이 계속 바뀌기 때문이다. 비율이 바뀌면 전담 칩이 논다.'),
        ('p', '그래서 칩 하나에 둘 다 넣고 어느 쪽에도 치우치지 않게 맞췄다. 그 순간 '
              '안 쓰는 회로는 전원을 끊는다. 슬라이드 문구가 '
        ...
        ('p', '칩을 묶는 단위가 두 단계다. ① <b>작은 묶음</b>은 칩 128개이고 그 안에서 칩 '
              ...
              '칩이 최대 2,048개까지 가고 칩 하나가 초당 200기가비트로 붙는다. 붙이는 장치는 '
              '브로드컴 Tomahawk 6 스위치이고, 칩끼리 말을 맞추는 규약은 ESUN(브로드컴이 미는 '
              '스케일업 연결 규격)이다. AMD 쪽 UALink 와는 다른 진영이다. 칩 128개가 한 랙이면 '
              '2,048개는 약 열여섯 랙이라는 것이 진행자가 나눠 본 값이다.'),
        ...
        ('p', '잰 도구는 SemiAnalysis 의 <b>Inference X</b>다. 파워를 정규화해 공개 비교했고, '
              '작은 모델부터 큰 모델까지 여러 오픈 모델을 돌린다. 칩 하나가 아니라 '
              '시스템 전체가 사용자에게 어떤 속도를 주는지를 보려는 것이다. 나온 것은 셋이다. ① DeepSeek R1(6,710억 파라미터)에서 파레토 '
              '프론티어를 새로 그었다 ② GPT-OSS(1,200억)에서 <b>사용자당 초당 1,000토큰 이상</b>을 '
              '냈다 ③ 700W 로 GB200 약 1,200W 보다 훨씬 적게 먹었다.'),

        ('h', '⑦ 무엇이 아직 안 재졌나'),
        ('p', '셋 남는다. ① 벤치마크의 입출력이 8K·1K 로 짧아 100만 토큰급 긴 문맥은 아직 '
              '안 나왔다 ② agentic 워크로드는 테스트하지 않아 별도 AgentX 로 재겠다고 했다 '
              '③ 견준 상대의 세대가 다르다 — 할라페뇨는 HBM4 를 쓰고 블랙웰·MI355 는 HBM3E 다. '
