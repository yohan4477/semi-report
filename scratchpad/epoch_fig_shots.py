...
def notes(y, lines):
    return [ef.lab(SX, y + 16 * i, line_fit(s, 9.5, 624 - SX), fs=9.5)
...
def cn_shot_dc():
    ...
    o = [ef.lab(SX, 22, '딥시크가 채용 사이트에 올린 공고 화면. 글자는 한국어로 옮겼다', fs=9.5)]
...
def cn_shot_bd():
    ...
    o = [ef.lab(SX, 22, 'Z.ai(지푸)가 올린 미국 시장 개척 직무 공고 화면', fs=9.5)]
...
def cn_shot_law():
    ...
    o = [ef.lab(SX, 22, '미국 고용평등위원회(EEOC)가 낸 안내문 화면', fs=9.5)]
...
def clarke_sat():
    ...
    o.append(ef.lab(294, 22, '위성끼리 신호를 넘긴다', fs=9.5))
