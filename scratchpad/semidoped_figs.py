...
def _mt_cycle():
    return _chain_down([(['하이퍼스케일러가 부품값 인상분을 설비투자에 얹어', '메모리·저장장치 회사로 넘긴다'], 'fig-box'),
                       ...
                       '세 흐름을 이으면 돈이 도는 고리다. 부품값이 메모리 회사로, 부족이 기업을 클라우드로, 자체 가속기 절감이 다시 메모리 값으로')
...
FIGS = {
    (MT, 'strategy'): [
        ...
        ('1.|진행자A는 연산을 더', '돈이 도는 고리 — 닫히는 쪽과 안 닫히는 쪽', _mt_loop(),
