...
def _op_stack():
    """조립 층이 빠지면 위 고부가 광원 층도 줄어든다 — 위아래 층, 화살표는 아래에서 위로."""
    h = 2 * LH + 22
    top = ['고부가 광원 — Lumentum · Coherent', '200기가 EML · 초고출력 레이저']
    bot = ['조립 — 중국 공급망 약 50%', 'Innolight 한 곳 27~34%']
    tw, bw = w_of(top), w_of(bot)
...
FIGS = {
    (TD, 'strategy'): [
        ...
        ('6.|이 회차에서 가장', '자기 손으로 하는 것과 맡긴 것', _td_outsource(),
         ...
         '엔지니어 60% 이상이 소프트웨어다(L341 근처). 스타트업이 시스템 인증까지 지면 출시가 밀린다는 것이 이유다(L231).'),
    ],
    (OP, 'strategy'): [
