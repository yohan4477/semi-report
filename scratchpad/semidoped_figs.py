...
FIGS = {
    (GN, 'strategy'): [
        ('1.|TPU는 원래', 'TPU 는 세대마다 칩을 나눴다 붙였다 했다', _gn_gens(),
         'v1 은 추론 전용이었고 v2~v4 는 훈련과 서빙을 한 칩에 담았다. v5 에서 효율칩과 성능칩으로 나뉘었다가 v6·v7 은 다시 한 칩이었고, '
         'v8 에서 훈련용 8T 와 추론용 8I 로 갈렸다(L18·L43). 색이 다른 것이 나뉜 세대다.'),
        ...
        ('3.|구글의 이전 망', '계층이 셋에서 둘로 줄었다', _gn_layers(),
         '이전 Jupiter 망은 리프에서 스파인, 슈퍼스파인까지 올라갔다 내려와야 했고 대역폭이 초당 13.1페타비트였다(L85·L89·L99·L101). '
         'Virgo 는 포트가 많은 광회선 스위치로 계층을 둘로 눌러 초당 47페타비트를 냈고 TPU 134,000개를 한 덩이로 묶었다(L129·L137).'),
        ('5.|TPU v7 구', '같은 거리를 몇 홉에 가나', _gn_hops(),
         'TPU v7 구성인 4x4x8 토러스는 2+2+4 해서 8홉, 구글 블로그가 든 8x8x16 은 4+4+8 해서 16홉이다(L185·L187). '
         'Boardfly 는 보드에 TPU 넷, 랙에 보드 여덟, 파드에 그룹 서른여섯을 두고 그룹 간을 광회선 스위치로 이어 같은 거리를 일곱 홉에 간다(L229·L243·L245).'),
    ],
    (HW, 'strategy'): [
        ('1.|Austin은 ', '지연을 무엇으로 나누겠다는 셈인가', _hw_alpha(),
         '논문의 수식은 다음 세대의 지연이 이번 세대 지연을 알파로 나눈 값이라는 것이다. 알파는 모바일이 연 1.3배, 자동차용이 1.5배, AI 워크로드가 최대 10배로 제시됐다(L55). '
        ...
        ('3.|Austin은 ', '통제가 걸린 자리와 안 걸린 자리', _hw_control(),
         'EUV 노광 장비는 수출통제 대상이고 중국은 애초에 사 본 적이 없다(L105). 하이브리드 본딩 장비에는 통제가 없다(L91). '
         '다만 BESI 는 ASML 과 같은 네덜란드 회사이고 직전 분기 매출의 35%가 중국향이다(L91). EV Group 은 오스트리아 회사라 통제 축이 다르다(L99).'),
    ],
    (MC, 'strategy'): [
        ('5.|월스트리트저널 ', '넉 분기 만에 45%에서 85%로', _mc_margin(),
         '마이크론 매출총이익률이 45%, 56%, 75%를 거쳐 85%에 닿았고 다음 분기도 85~86%를 전망한다(L93). '
         '같은 물건을 10달러에 만들어 50달러에 판다는 것이 Austin 의 말이고, Vik 은 손님을 물리는 회사가 없는 입찰 경쟁으로 읽었다(L91·L95·L97).'),
        ('3.|HBM만의 문제', '같은 용량에 웨이퍼가 세 배 든다', _mc_wafer(),
         'HBM 은 D램 칩을 쌓아 만드는데 쌓으려면 실리콘관통전극을 뚫어야 하고 그 구멍 둘레에는 셀을 못 넣는다. '
         '그래서 같은 비트 용량에 일반 D램보다 웨이퍼가 세 배 든다(L109). 장 수가 값이다.'),
    ...
    (LI, 'strategy'): [
        ('6.', 'EUV 와 X선 — 파장을 바꾸면 다 바뀐다', _li_xray(),
         'X선은 물체를 통과해 버려 반사가 안 되니 EUV 가 쓰는 거울 광학을 못 쓴다(L278). 그러면 마스크를 5분의 1로 줄이는 축소가 사라져 마스크를 웨이퍼에 찍을 치수와 같게 만들어 가까이 대는 근접 인쇄로 가야 하고, 마스크 제작이 훨씬 어려워진다(L280). '
         '주석 방울 광원을 1나노미터 X선으로 갈아 끼우면 되는 일이 아니라고 진행자V 는 못을 박았다(L282).'),
    ...
    (WK, 'strategy'): [
        ('6.', '소프트웨어 원가가 무엇으로 바뀌나', _wk_cost(),
         '클라우드 인스턴스와 데이터베이스, 가상머신과 컨테이너는 사용자 여럿에 나눠 태울 수 있었는데 토큰 원가는 그렇게 안 된다(L215). '
         '그래서 가격 모델을 새로 짜도 한계비용이 남는다는 것이 Val 의 논지다. 출구로 그가 든 것은 추론 사업자에게 계속 맡기거나 뉴클라우드·토큰 공장과 합쳐 생성 단계를 안으로 들이는 둘이다(L215).'),
