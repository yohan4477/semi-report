...
def _jal_ladder():
    ...
    rows = [('CPU', '대체재가 널렸다 — Venice · 인텔', 'fig-stage'),
            ...
            ('메모리', '새로 열린다 — 세대 차가 비교를 뒤집는다', 'fig-agent'),
            ('스케일업 망', '새로 열린다 — 규격이 갈라져 있다', 'fig-agent'),
    ...
    return svg(y + 2, parts,
               'CPU 는 대체재가 널렸고 파운드리는 대안이 없어 그대로 가고, 새로 열리는 곳은 메모리와 스케일업 망 둘이다. 시스템 조립은 Celestica 로 추정된다')
...
FIGS = {
    (JAL, 'strategy'): [
        ...
        ('5.', '밸류체인에서 어디가 열리나', _jal_ladder(),
         '모델랩이 실리콘까지 내려와도 밸류체인 아래쪽 전부가 열리는 게 아니다. CPU 는 대체재가 널렸고(L117) 파운드리는 대안이 없어 그대로 간다. '
         '새로 열리는 곳은 메모리와 스케일업 망 둘이다. 시스템 조립은 SemiAnalysis 의 추정이다(L119).'),
