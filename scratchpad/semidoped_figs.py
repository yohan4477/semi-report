...
FIGS = {
    ...
    (CB, 'strategy'): [
        ...
        ('2.|문제는 완벽한 웨이퍼가 없다는', '죽은 코어를 건너뛰는 순서', _cb_yield(),
         '한 장에 코어가 97만쯤 들어가 그중 90만이 돌아간다(L83) — 상자로는 안 세고 순서만 그렸다.'),
    ...
    (DC, 'strategy'): [
        ...
        ('4.|속도 세대는 겹쳐서', '속도 세대 — 겹쳐서 간다', _dc_gens(),
         '1.6테라는 합산 대역폭이라 레인당 200기가 여덟 레인일 수 있고(L155), 다음이 3.2 가 아니라 레인당 300기가짜리 2.4테라일 가능성이 '
    ...
    (OP, 'strategy'): [
        ...
        ('1.|주가가 어긋난', '광트랜시버 밸류체인 — 광원과 조립', _op_chain(),
         '끝에서 끝까지다. 부품(DSP·리타이머·드라이버·TIA)은 미국(L61), 고부가 광원(200기가 EML·초고출력 레이저)은 '
         'Lumentum·Coherent(L81), 조립은 중국 공급망이 세계의 약 50% 이고 Innolight 한 곳이 27~34% 다(L79), 구축은 '
         'Azure·OCI 같은 주체다(L91).'),
