...
def _jal_ladder():
    ...
    rows = [('CPU', '대체재가 널렸다 — Venice · 인텔', 'fig-stage'),
            ...
            ('메모리', '값이 움직인다 — 세대 차가 비교를 뒤집는다', 'fig-agent'),
            ('스케일업 망', '값이 움직인다 — 규격이 갈라져 있다', 'fig-agent'),
            ('시스템 조립', 'Celestica 로 추정', 'fig-stage')]
    lw = w_of([r[0] for r in rows])
    rw = w_of([r[1] for r in rows])
    gap = 12
    x0 = (W - (lw + gap + rw)) / 2
    parts = head(x0, 26, lw, '단') + head(x0 + lw + gap, 26, rw, '이 회차가 본 것')
    y, h = 36, 44
    ...
    return svg(y + 2, parts,
               'CPU 는 대체재가 널렸고 파운드리는 대안이 없어 그대로 가고, 값이 움직이는 단은 메모리와 스케일업 망 둘이다. 시스템 조립은 Celestica 로 추정된다')
...
FIGS = {
    (JAL, 'strategy'): [
        ...
        ('5.', '단마다 무엇이 열리나', _jal_ladder(),
         '모델랩이 실리콘까지 내려와도 아래 단 전부가 열리는 게 아니다. CPU 는 대체재가 널렸고(L117) 파운드리는 대안이 없어 그대로 간다. '
         '값이 움직이는 단은 메모리와 스케일업 망 둘이다. 시스템 조립은 SemiAnalysis 의 추정이다(L119).'),
