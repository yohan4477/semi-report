...
보는 것 넷이다.
  ...
  선이 비스듬하다 x·y 축에 안 평행하다. 곧장 못 가면 꺾어서 간다 (같은 도해만)
...
def loose_ends(svg, tol=2.0):
    ...
    for m in PATH.finditer(svg):
        ...
        # F6 선은 x·y 축에 평행하게만 간다. 비스듬한 선은 어느 상자에서 나와
        # 어디로 들어가는지가 눈으로 안 잡히고, 글자 위를 지나도 가로·세로만 보는
        # 「선에 깔림」에 안 걸린다. 곧장 못 가면 꺾어서 간다.
        for (ax, ay), (bx, by) in zip(pts, pts[1:]):
            if abs(ax - bx) > 0.5 and abs(ay - by) > 0.5:
                bad.append('선이 비스듬하다 (%.0f,%.0f)-(%.0f,%.0f) — 꺾어서 간다'
                           % (ax, ay, bx, by))
...
# 글자 폭을 글자 종류로 가른 날(2026-09-09) 열셋이 걸렸고 그날 다 갚았다. 목록을 비워
# 두는 것이 정상이다 — 여기에 제목을 넣는 것은 그날 안에 못 갚을 때뿐이고, 넣었으면
# 왜 미뤘는지와 언제 갚을지를 같이 적는다.
WIDE_DEBT = set()
