...
// 사파리는 자리를 되살리는 시점이 load 뒤라 한 번만 올려서는 다시 끌려 내려간다.
// 그래서 DOM 이 서는 자리, load, 뒤로 가기(pageshow)까지 붙이고 짧게 두 번 더 민다
...
window.addEventListener('load',function(){toTop();
  setTimeout(toTop,0);setTimeout(toTop,150);});
window.addEventListener('pageshow',function(){toTop();setTimeout(toTop,0);});
})();
(function(){
