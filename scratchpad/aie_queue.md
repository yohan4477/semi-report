...
code 여섯과 train 셋이 찼다. 남은 여섯은 **사실표가 전부 없으므로
한 편마다 위임이 먼저다.** 섹션 하나를 닫으려면 product 둘이 가장 가깝다.
...
2026-09-08 에 code 넷과 train 셋을 옮겼다. 그때 걸린 것을 적어 둔다.

- **`_chain` 은 칸이 다섯이면 라벨을 못 얹는다.** `rf-link em` 과 `rf-loop em` 이 둘 다
  nowrap 이라 520·776px 에서 458px 까지 넘쳤다. 막힌 것은 칸 안 뒷문장으로 내리고
  캡션이 읽는 법을 댄다. `loop` 라벨은 스무 자 안쪽으로 쓴다.
- **용어 별표를 본문에 안 붙이면 생성기가 경고만 내고 지나간다.** FAIL 이 아니라
  `^  !` 줄이라 놓치기 쉽다 — 생성 뒤 그 줄을 반드시 본다. 표 칸에 붙여도 잡힌다.
- **카드를 고치면 색인이 낡는다.** `insights/gen_index.py` 를 돌리고 이어서
  `insights/gen_times.py` 까지 돌린다(X1·X4·X7·X8).
- **자막에 이름이 없으면 본문에 이름을 안 쓴다.** 커서 편(WE_Gnowy3uw)과
  일하면서 배우는 에이전트 편(k35LeKZEhiE)이 그렇다. 회사명도 마찬가지다.
- **자막이 회사 이름을 뭉갠 자리는 하는 일로 적는다.** 에이전트 RFT 편(p1CmPZ2j6Lk)의
  파트너 넷 중 둘이 그렇다 — 표에 「코드 리뷰 회사」·「GPU 커널 회사」로 세웠다.
- `check_deps` 의 D1 FAIL(`map_packaging.py`)은 다른 세션 작업이다. 이쪽에서 안 건드린다.
