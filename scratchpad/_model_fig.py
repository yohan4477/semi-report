...
# 판 위에 세 갈래를 얹는다. 곱해지는 대상이 다르다는 것이 이 그림의 전부다.
_MX, _MW, _MH = 20, 176, 62
_MY = 96
_MODE = [('차가운 예비', '작업 크기 × (인지+체크포인트/2', '+초기화+수리)'),
         ('뜨거운 예비', '작업 크기 × (인지+체크포인트/2', '+초기화) + 폭발 반경 × 수리'),
         ('고장 견딤', '작업 크기 × (인지+전환)', '+ 폭발 반경 × 수리')]
_MULT = [('4,096장이 통째로 멎는다', 4096),
         ('4,096장이 멎고 수리는 64장만', 4096),
         ('64장만 수리를 기다린다', 64)]
...
def _mode_col(i):
    ...
    name, l1, l2 = _MODE[i]
    note, n = _MULT[i]
    st, sw = (INK, 2.0) if i == 2 else (INK3, 1.5)
    return ''.join([
        _box(x, _MY, _MW, _MH, [l1, l2], st, sw),
        _t(x + _MW // 2, _MY - 12, name, 't-lab'),
        _t(x + _MW // 2, _MY + _MH + 22, note),
        '<rect x="%d" y="%d" width="%.1f" height="16" rx="2" fill="%s"/>'
        % (x, _MY + _MH + 34, _MW * (n / 4096.0) ** 0.5, INK if i == 2 else INK3),
    ])


FIG_GOODPUT = _svg(W, 300, '같은 고장인데 곱해지는 장수가 예순네 배 다르다', ''.join(
    [_lt(20, 30, '고장 한 번 → 잃는 GPU-시간'),
     _lt(20, 52, '클러스터 5,184장 · 평균 작업 4,096장 · 폭발 반경 64장 (그림 019)',
         't-sm', False)]
    + [_mode_col(i) for i in range(3)]
    + [_legend(238, ['왼쪽 둘은 작업 전체가 멎는다 — 체크포인트 이후 계산이 날아간다',
                     '오른쪽은 작업이 계속 돈다 — 고장 난 랙 하나만 수리를 기다린다',
