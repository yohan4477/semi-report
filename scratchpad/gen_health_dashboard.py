...
SEC_INFLAM = ('sec-inflam', '01', '염증과 면역',
              '염증이 뭔지, 과하게 먹으면 왜 만성이 되는지, 그리고 면역에 걸린 브레이크를 암이 어떻게 쓰는지')
...
SEC_BRAIN = ('sec-brain', '06', '뇌 · 마음 · 수면',
...
# 해칭 대신 면색으로만 구분한다.
FIG_TREG = '''<svg viewBox="0 0 640 320" role="img" aria-label="종양이 조절 T세포를 포섭해 킬러 T세포를 막는 상태와 항체로 관문을 덮은 상태">
...
CARDS = [{
    ...
    'section': SEC_INFLAM,
    'topic': ('market', '면역 · 암'),
    'title': '면역에는 브레이크가 있다. 암은 그 브레이크를 포섭한다',
    ...
    'links': LINKS_NOBEL,
    'slim_oneliner': ('면역 연구는 오랫동안 액셀만 봤다. 다 죽인 뒤에도 멈추지 않으면 몸이 남아나지 않으니 '
                      '브레이크가 있어야 하는데, 그 브레이크가 <b>조절 T세포</b>다. 암은 이 세포를 자기편으로 '
                      '만들어 공격을 막는다.'),
    'slim_points': [
        '<b>브레이크가 하는 일은 셋이다.</b> 균을 다 죽인 뒤에도 계속되는 선천 면역을 멈추고, 자기 몸을 공격하는 '
        ...
        '<b>암은 이 브레이크를 포섭한다.</b> 종양이 내는 물질로 주변 조절 T세포를 활성화시켜 "여기는 정상 조직" '
        '이라고 알리게 만든다. 그렇게 만들어진 구역이 종양 미세환경이다.',

        '<b>포섭된 세포는 표지를 내민다.</b> CTLA-4와 PD-1이다. 세포를 없애면 몸 전체의 염증이 폭주하니, '
    ...
    'figs': [
        (5, '그림 4 · 브레이크를 포섭한 종양',
         FIG_TREG,
         '왼쪽은 포섭된 조절 T세포가 표지를 내밀어 킬러 T세포를 막는 상태다. 오른쪽처럼 항체로 그 표지만 덮으면 '
         '킬러 T세포가 다시 들어간다.'),
    ],
    'slim_stats': [('브레이크', '조절 T세포가 면역에서 맡은 자리'),
                   ('CTLA-4 · PD-1', '포섭된 세포가 내미는 표지. 항체가 덮는 지점'),
    ...
    'section': SEC_WEIGHT,
    'topic': ('market', '비만약 · 중독'),
    'title': '비만약이 술·담배까지 줄인다. 같은 보상 회로에 걸리기 때문이다',
    'gain': 'GLP-1 계열 약이 갈망 자체를 줄인다는 근거가 어디까지 나왔는지, 그리고 어디부터 가설인지.',
    'meta': META_GLP1,
    'quote': '"중독된 뇌는 좋아함은 그대로인데 원함만 비정상적으로 커진다."',
    'note': ('처방이나 복용을 권하는 내용이 아니다. GLP-1 계열은 전문 진료 영역이고, 영상에서 비인가 조재약의 '
             '위험을 명확히 경고했다. 화자가 바뀌었다 — 내분비대사내과 전문의다.'),
    'links': LINKS_GLP1,
    'slim_oneliner': ('위고비 같은 GLP-1 계열 약은 원래 당뇨약이었고, 덜 먹게 되는 것이 부작용으로 발견됐다. '
                      '그런데 이 호르몬이 붙는 자리가 장과 췌장만이 아니다. 뇌에서 <b>원함</b>을 만드는 보상 '
                      '회로에도 수용체가 있어서, 줄어드는 것이 식욕만이 아니다.'),
    'slim_points': [
        '<b>중독이라는 말부터 갈린다.</b> 식중독처럼 독이 들어온 것과, 끊지 못하는 상태는 영어로 다른 단어다. '
        '뒤쪽을 학술 용어로 물질사용장애라 부르고, 이 카드가 다루는 것도 그쪽이다.',

        '<b>중독된 뇌는 좋아함은 그대로인데 원함만 커진다.</b> 예전엔 조금만 해도 좋았는데 점점 더 많이, 더 '
        '강하게 원해야 같은 만족이 온다. 돈도 같은 구조라는 것이 그의 예시다.',

        '<b>그 원함을 만드는 회로에 수용체가 있다.</b> 중뇌 복측피개영역에서 측좌핵으로 가는 길인데, 측좌핵에도 '
        'GLP-1 수용체가 있다는 것이 확인됐다. 고농도에서는 쾌락 관련 회로를 오히려 억누른다.',

        '<b>푸드 노이즈가 사라진다는 말이 여기서 나왔다.</b> 배고프지 않은데도 하루 종일 음식 생각이 떠도는 '
        '상태를 가리킨다. 주사를 맞은 환자들이 그 생각이 사라졌다고 말하면서 미국에서 퍼진 용어다.',

        '<b>재향군인 60만 명 관찰 연구에서 다섯 가지가 동시에 줄었다.</b> 오피오이드 25%, 코카인 20%, 니코틴 '
        '20%다. 알코올은 18%, 대마는 14% 줄었고 물질 관련 사망도 50% 감소했다. 기전이 서로 다른 물질이 함께 줄었다는 점이 '
        '공통 회로를 가리킨다는 해석이다.',

        '<b>무작위 임상도 하나 나왔다.</b> 비만하면서 술을 많이 마셔 온 108명을 절반으로 나눠 위고비와 위약을 '
        '줬더니, 과음 일수가 각각 41%와 26% 줄었다. 위약 쪽이 큰 것은 참가자들이 애초에 끊고 싶어 자원한 '
        '사람들이었기 때문이라고 그는 본다.',

        '<b>체중이 줄어서 생기는 효과도 섞여 있다.</b> 100kg에서 80kg이 되면 알코올이 퍼질 몸속 수분이 20리터쯤 '
        '줄어 같은 술에도 더 빨리 취한다. 그래서 덜 마시게 되는 몫이 따로 있다.',

        '<b>저혈당은 생기지 않는다.</b> 이 호르몬은 혈당이 이미 높을 때만 인슐린 분비를 밀어 준다. 엔진이 '
        '돌아갈 때만 출력을 올리는 터보차저와 같아서, 엔진이 꺼져 있으면 아무 일도 하지 않는다.',

        '<b>부작용은 분명하다.</b> 구역과 구토로 인한 탈수가 응급실을 찾는 흔한 이유이고, 이미 말랐는데 계속 '
        '맞으며 굶는 경우에는 영양 결핍과 비타민 B1 부족으로 신경 증상까지 온다. 탈수가 심하면 급성 신손상이 '
        '온다.',

        '<b>췌장염 우려는 최근 데이터로 가라앉았다.</b> 임상시험에서 통계적으로 유의하지 않았고, 오히려 비만 '
        '자체가 급성 췌장염의 큰 위험 요인이라는 쪽이다.',

        '<b>암 전이가 줄었다는 관찰 결과도 나왔다.</b> 올해 미국임상종양학회에서 대형 병원 세 곳이 발표했고, '
        'GLP-1 수용체를 많이 만드는 암일수록 전이가 줄어드는 정도가 컸다. 연관성까지만 나왔고 기전은 아직 '
        '가설이다.',

        '<b>짝퉁 약이 실제 위험이다.</b> 정식 시설이 아닌 곳에서 합성한 약이 돌고, 영국에서는 GLP-1이라며 판 '
        '약에서 인슐린이 나온 사례가 보도됐다.',
    ],
    'figs': [
        (3, '그림 11 · 포만 신호가 보상 회로에도 닿는다',
         FIG_GLP1,
         '장에서 나온 호르몬이 인슐린과 포만감만 건드리는 것이 아니다. 뇌에서 원함을 만드는 자리에도 수용체가 '
         '있어서 갈망 자체가 줄어든다.'),
    ],
    'slim_stats': [('25 · 20 · 20 · 18 · 14%', '오피오이드·코카인·니코틴·알코올·대마 감소율(관찰 연구)'),
                   ('50%', '같은 연구에서 줄어든 물질 관련 사망'),
                   ('41% vs 26%', '임상시험에서 위고비군과 위약군의 과음 일수 감소'),
                   ('108명', '그 임상시험의 규모. 아직 작다')],
    'clash': [
        ('관찰 연구', '<b>60만 명 자료는 관찰 연구다.</b> 인과관계까지 단정할 수 없고, 그래서 무작위 임상이 '
                   '뒤따랐다.'),
        ('대상 한정', '<b>임상 참가자는 비만하면서 술을 많이 마시고 스스로 끊고 싶어 온 유럽인 108명이다.</b> '
                   '비만하지 않은 사람이나 끊을 생각이 없는 사람, 한국인에게도 같을지는 확인되지 않았다고 그가 '
                   '직접 밝힌다.'),
        ('가설', '<b>도박이나 투자 같은 행위 중독에서는 근거가 없다.</b> 대화에서 추정을 나눈 것이고 '
                 '임상 자료는 제시되지 않았다.'),
    ],
}, {
    'section': SEC_WEIGHT,
    ...
    'slim_points': [
        ...
        '않고 그대로 통과한다. 빈속에 마시면 10분 안에 뇌에 닿는다.',
        ...
        '<b>그래서 단백질을 통째로 주사하는 쪽이 열렸다.</b> 인슐린이 첫 사례다. 먹으면 아미노산으로 분해되니 '
        '주사밖에 길이 없었다.',
