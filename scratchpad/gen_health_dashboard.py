...
# 북언더스탠딩 「뇌가 멈추기 전에」 두 편. 1부가 급사의 정체, 2부가 단계와 예방이다.
SRC_SUD1 = 'https://youtu.be/ecXOA-CKG9M'
SUM_SUD1 = blob(SUM + '언더스탠딩/[250720] 급사는 원인이 세 개뿐이다 - 이승훈 (뇌가 멈추기 전에 1부).md')
SRC_SUD2 = 'https://youtu.be/0uQkIHe51pE'
SUM_SUD2 = blob(SUM + '언더스탠딩/[250720] 장전과 방아쇠, 급사를 막는 네 단계 - 이승훈 (뇌가 멈추기 전에 2부).md')
...
META_STAGE = [SPK, '지식인사이드 EP.107 · 155 · 언더스탠딩 2편', '2024-09 · 2026', '출처 4편']
...
LINKS_STAGE = [('영상 — EP.107', SRC_EP107, ''), ('영상 — 언더스탠딩 모음집', SRC_LEGEND, ''),
               ('영상 — 뇌가 멈추기 전에 2부', SRC_SUD2, ''),
               ('요약본 — EP.107', SUM_EP107, 'secondary'), ('요약본 — 모음집', SUM_LEGEND, 'secondary'),
               ('요약본 — 2부', SUM_SUD2, 'secondary')]
LINKS_WEIGHT = [('영상 — EP.108', SRC_EP108, ''), ('영상 — EP.107', SRC_EP107, ''),
                ('요약본 — EP.108', SUM_EP108, 'secondary'), ('요약본 — EP.107', SUM_EP107, 'secondary')]

META_SUDDEN = [SPK, '언더스탠딩 · 뇌가 멈추기 전에 1부', '2025-07-20', '출처 1편']
META_FOURMIN = [SPK, '언더스탠딩 · 뇌가 멈추기 전에 1부', '2025-07-20', '출처 1편']
META_TRIGGER = [SPK, '언더스탠딩 · 뇌가 멈추기 전에 1 · 2부', '2025-07-20', '출처 2편']
LINKS_SUD1 = [('영상 — 뇌가 멈추기 전에 1부', SRC_SUD1, ''), ('요약본 전문', SUM_SUD1, 'secondary')]
LINKS_SUD12 = [('영상 — 1부', SRC_SUD1, ''), ('영상 — 2부', SRC_SUD2, ''),
               ('요약본 — 1부', SUM_SUD1, 'secondary'), ('요약본 — 2부', SUM_SUD2, 'secondary')]

META_BLEED = [SPK, '언더스탠딩 레전드 모음집 · 뇌가 멈추기 전에 2부', '2024-09 · 2025-07', '출처 2편']
META_EMERG = [SPK, '지식인사이드 EP.107 · 언더스탠딩', '2026-03 · 2024-09', '출처 2편']
LINKS_BLEED = [('영상 — 언더스탠딩 모음집', SRC_LEGEND, ''), ('영상 — 뇌가 멈추기 전에 2부', SRC_SUD2, ''),
               ('요약본 — 모음집', SUM_LEGEND, 'secondary'), ('요약본 — 2부', SUM_SUD2, 'secondary')]
...
SEC_SUDDEN = ('sec-sudden', '06', '갑자기 죽는 일',
              '멀쩡하던 사람이 한 시간 안에 죽는 병 셋, 뇌가 버티는 4분, 그리고 그날 방아쇠를 당기는 것')
SEC_GLP1 = ('sec-glp1', '07', '비만약과 중독',
            '덜 먹게 하는 약이 왜 갈망까지 줄이는지, 근거는 어디까지 왔고 무엇이 위험한지')
SEC_BRAIN = ('sec-brain', '08', '뇌 · 마음 · 수면',
...
# ── 그림 5. 과당의 외길 ─────────────────────────────────────────────────────
# 포도당은 온몸이 나눠 쓰고 과당은 간 하나만 받는다. 이 차이가 결과를 나눈다.
...
# ── 그림 11. 심장 겉면과 관상동맥 ──────────────────────────────────────────
# 왼쪽은 심장 겉면에 얹힌 관상동맥, 오른쪽은 막힌 자리의 횡단면이다. 심근경색은
# 심장 근육의 병이 아니라 그 근육에 피를 대는 혈관의 병이라는 것이 이 그림의 논지다.
FIG_HEART = '''<svg viewBox="0 0 640 320" role="img" aria-label="심장 겉면을 지나는 관상동맥과 막힌 자리의 횡단면">
  <text class="t-lab" x="18" y="22">심장 겉면 — 근육에 피를 대는 혈관</text>
  <text class="t-lab" x="392" y="22">막힌 자리 횡단면</text>

  <!-- 심장 몸통 -->
  <path class="organ" d="M126 92 C 104 116, 100 156, 112 192 C 124 228, 152 254, 186 262
                         C 210 268, 228 256, 236 232 C 250 194, 254 150, 244 118
                         C 234 90, 208 76, 178 76 C 156 76, 138 80, 126 92 Z"/>
  <!-- 위로 나가는 큰 혈관 둘 -->
  <path class="vessel" d="M170 78 C 168 58, 176 46, 192 42"/>
  <path class="vein" d="M206 80 C 214 60, 228 52, 244 52"/>
  <text class="t-sm" x="198" y="38">대동맥</text>
  <text class="t-sm" x="250" y="50">폐동맥</text>

  <!-- 관상동맥 — 심장 겉면을 타고 내려간다 -->
  <path class="vessel" d="M172 84 C 166 122, 160 168, 156 214"/>
  <path class="vessel" d="M188 86 C 214 100, 232 128, 230 162"/>
  <path class="vessel" d="M164 140 C 148 152, 138 168, 134 186"/>

  <!-- 막힌 자리 -->
  <circle class="bad" cx="162" cy="158" r="7"/>
  <circle cx="162" cy="158" r="13" fill="none" stroke="var(--fig-bad,#c2504a)" stroke-width="1.2"/>
  <path class="lead-line" d="M175 158 L 232 158"/>
  <text class="t-bad" x="236" y="150">여기가 막힌다</text>
  <text class="t-sm" x="236" y="166">심장 근육은 멀쩡하다</text>

  <text class="t-sm" x="120" y="292">관상동맥은 심장 겉면에 얹혀 있다. 심장도 피를 받아야 살기 때문이다.</text>

  <!-- 오른쪽 — 막힌 자리 횡단면 -->
  <circle cx="470" cy="150" r="62" class="organ2 cut"/>
  <circle cx="470" cy="150" r="62" class="hatch"/>
  <circle cx="470" cy="150" r="42" class="body"/>
  <path fill="var(--fig-bad,#c2504a)" fill-opacity=".26" stroke="var(--fig-bad,#c2504a)" stroke-width="1.2"
        d="M436 128 C 456 112, 490 112, 506 132 C 492 150, 456 154, 438 146 Z"/>
  <circle class="fat" cx="458" cy="130" r="4"/>
  <circle class="fat" cx="482" cy="128" r="4"/>
  <circle class="bad" cx="470" cy="166" r="20"/>
  <path class="lead-line" d="M470 186 L 470 206"/>
  <text class="t-bad" x="470" y="220" text-anchor="middle">터진 자리에 생긴 혈전</text>
  <text class="t-sm" x="470" y="236" text-anchor="middle">노란 알갱이가 쌓인 지방</text>
  <text class="t-sm" x="470" y="252" text-anchor="middle">뇌경색도 이 그림과 같다</text>
</svg>'''


# ── 그림 12. 뇌가 버티는 시간 ──────────────────────────────────────────────
# 뇌 시상단면 위에 시간을 얹는다. 4분이라는 값이 심장이 아니라 뇌의 사정에서
# 나왔다는 것을 한 장으로 보이는 것이 목적이다.
FIG_BRAINCLOCK = '''<svg viewBox="0 0 640 300" role="img" aria-label="뇌 시상단면에서 해마 위치와 혈류가 끊긴 뒤 시간">
  <text class="t-lab" x="18" y="22">뇌 시상단면 — 피가 끊긴 뒤</text>

  <!-- 대뇌 -->
  <path class="organ cut" d="M74 132 C 78 84, 124 52, 184 50 C 246 48, 290 76, 300 118
                             C 308 152, 296 182, 268 196 C 232 214, 160 216, 120 200
                             C 88 188, 70 162, 74 132 Z"/>
  <path class="organ cut" d="M74 132 C 78 84, 124 52, 184 50 C 246 48, 290 76, 300 118
                             C 308 152, 296 182, 268 196 C 232 214, 160 216, 120 200
                             C 88 188, 70 162, 74 132 Z" fill="url(#fig-hatch-wide)" stroke="none"/>
  <!-- 소뇌 -->
  <path class="organ2 cut" d="M268 196 C 296 196, 314 214, 310 234 C 306 252, 284 260, 262 254
                              C 246 250, 240 232, 246 214 Z"/>
  <!-- 뇌간 — 아래로 내려가는 줄기 -->
  <path class="organ2 cut" d="M212 200 C 216 224, 222 248, 228 268 L 250 268
                              C 244 246, 240 220, 238 198 Z"/>
  <!-- 해마 — 안쪽에 웅크린 자리 -->
  <path fill="var(--fig-bad,#c2504a)" fill-opacity=".3" stroke="var(--fig-bad,#c2504a)" stroke-width="1.4"
        d="M156 150 C 172 138, 200 138, 214 150 C 202 166, 172 170, 156 162 Z"/>
  <text class="t-sm" x="120" y="238">해마</text>
  <path class="lead-line" d="M146 232 L 176 164"/>

  <!-- 올라오는 혈류 — 심장에서 -->
  <path class="vessel" d="M228 268 C 226 282, 226 288, 226 292"/>
  <text class="t-sm" x="236" y="290">심장에서 올라오는 피</text>

  <!-- 오른쪽 — 시간 -->
  <path class="lead-line" d="M356 60 L 356 236"/>
  <circle class="bad" cx="356" cy="72" r="5"/>
  <text class="t-lab" x="372" y="76">0초 — 심장이 멈춘다</text>
  <text class="t-sm" x="372" y="92">뇌로 가는 산소와 포도당이 함께 끊긴다</text>

  <circle class="bad" cx="356" cy="128" r="5"/>
  <text class="t-lab" x="372" y="132">30초~1분 — 해마가 상한다</text>
  <text class="t-sm" x="372" y="148">기억을 담는 자리가 가장 먼저 약하다</text>

  <circle class="bad" cx="356" cy="184" r="5"/>
  <text class="t-lab" x="372" y="188">4분 — 여기까지가 정상 회복</text>
  <text class="t-sm" x="372" y="204">이 뒤에 돌아오면 식물인간이 될 수 있다</text>

  <circle class="bad" cx="356" cy="232" r="5"/>
  <text class="t-lab" x="372" y="236">5.4분 — 서울에서 가장 빠른 구급차</text>
  <text class="t-sm" x="372" y="252">그래서 4분 안에 손을 쓸 사람은 목격자뿐이다</text>
</svg>'''
...
CARDS = [{
    ...
    'note': ('EP.107의 단계 모델에 EP.155의 검사 지표를 합치고, 「뇌가 멈추기 전에」 2부에서 단계 판정과 약 '
             '이야기를 더했다. "90%가 안 생긴다"는 그의 표현이고 근거 논문은 영상에서 제시되지 않았다. 검사 '
             '비용은 지역과 기관에 따라 다르다. 단계 구분은 그가 직접 만든 모델이다.'),
    'links': LINKS_STAGE,
    ...
    'slim_points': [
        ...
        '<b>나이 하나만으로 1단계가 된다.</b> 50세를 넘으면 위험 요인이 그것뿐이어도 1단계로 본다. 아주 건강한 '
        '사람도 나이가 들면 뇌졸중 가능성이 올라가기 때문이다. 담배는 끊고 10년이 지나야 뺀다.',

        '<b>2단계에서는 정상치의 기준이 바뀐다.</b> 동맥경화가 확인되면 아스피린을 시작하고 LDL 목표를 160이 '
        '아니라 70 이하로 내린다. 일반인의 정상 범위가 100~130이므로, 이 순간 정상인의 값이 전부 비정상이 '
        '된다.',

        '<b>약을 미루면 약이 늘어난다.</b> 혈압약을 한 번 먹으면 평생 먹어야 한다며 미루는 선택이 나중에 '
        '서너 알로 돌아온다. '
        '초반에 조절해 유지한 사람은 약이 늘지 않을 가능성이 높고, 혈관 변성이 시작된 뒤에 버티면 인슐린 저항성이 '
        '심해지고 신장 기능이 떨어져 여러 약을 써도 안 잡힌다.',

        '<b>소금이 삼투압으로 혈액량을 늘린다는 설명은 그가 부정한다.</b> 소금을 먹어도 혈액의 나트륨 수치는 '
        '몸이 조절해 올라가지 않기 때문이다. 소금을 짜게 먹으면 교감신경이 항진돼 각성 상태로 지내는 기간이 '
        '길어지고, 그 상태에서 혈압이 올라간다는 것이 요즘 정설이다.',
    ],
    'figs': [
        (2, '그림 8 · 혈관이 밟는 네 단계',
         FIG_STAGE,
    ...
    'note': ('언더스탠딩 모음집에서 뇌출혈 대목을 추리고, 「뇌가 멈추기 전에」 2부의 예방 대목을 뒤에 붙였다. '
             '성공률과 비율과 검사 비용은 그가 말한 값이고 출처 논문은 영상에서 제시되지 않았다.'),
    'links': LINKS_BLEED,
    ...
    'slim_points': [
        ...
        '<b>터지기 전에 찾으면 이야기가 달라진다.</b> 혈관으로 관을 넣어 코일로 동맥류 안을 채우면 그 자리는 '
        '끝난다. 터진 뒤에도 같은 시술로 막지만, 그때는 이미 퍼진 피를 어쩌지 못한다.',

        '<b>단서는 경고성 두통 하나다.</b> 10~30%에서 동맥류가 피를 조금 흘리며 두통을 만든다. 가볍게 오지 '
        '않고 세게 온다. 두통이 없던 사람이 갑자기 심하게 아프면 찾아볼 이유가 되는데, 두통 환자가 워낙 많아 '
        '이것으로 걸러 내기는 어렵다고 그는 말한다.',

        '<b>동맥류는 타고나기보다 자란다.</b> 어릴 때는 없다가 나이가 들며 혈관의 약한 부분이 부푼다. 그래서 '
        '50세에 찍어서 없으면 남은 생에 생길 가능성이 높지 않다는 것이 그의 정리다.',

        '<b>한국은 자기 뜻대로 MRI를 찍을 수 있는 드문 나라다.</b> 보험 수가가 낮게 묶인 대신 건강검진을 민간에 '
        '풀어 준 결과 검진센터가 늘고 장비 경쟁이 붙었다. 대학병원 비급여로 200만 원쯤 드는 검사를 검진센터에서 '
        '40만~50만 원에 찍는다. 혈관을 보는 MRI를 MRA라 부른다.',
    ],
    'figs': [
        (1, '그림 10 · 같은 혈관, 다른 병',
         FIG_ANEURYSM,
    ...
    'section': SEC_SUDDEN,
    'topic': ('market', '급사 · 심장'),
    'title': '멀쩡하던 사람이 갑자기 죽는 일은 병 세 개가 전부다',
    'gain': '급사가 통계에서 사라지는 이유, 원인 셋의 비중, 그리고 부정맥을 손목에서 잡는 법.',
    'meta': META_SUDDEN,
    'quote': '"급사라는 용어 자체가 사망진단서에 안 나오거든요."',
    'note': ('"90%를 막고 있다"는 그의 표현이고 근거 논문은 영상에서 제시되지 않았다. 부정맥 비중은 그가 10%로 '
             '말했다가 뒤에서 10~30%로도 말했다. 진단도 처방도 아니다.'),
    'links': LINKS_SUD1,
    'slim_oneliner': ('급사는 외상이 아닌 원인으로 예상하지 못한 상태에서 <b>한 시간 안에</b> 죽는 것이다. 전체 '
                      '사망의 9~10%인데 사망진단서에는 원인 질환만 적어서 통계에 이름이 없다. 원인을 따지면 '
                      '병이 셋뿐이다.'),
    'slim_points': [
        '<b>따로 세면 사망 원인 2위 자리다.</b> 인구 10만 명당 64.7건이다. 사망진단서에 적는 것은 심근경색이나 '
        '뇌출혈 같은 병 이름이고, 그 사람이 직전까지 멀쩡했는지는 적는 칸이 없다.',

        '<b>허혈성 심장질환이 70%다.</b> 심장 근육에 피를 대는 관상동맥에 이미 동맥경화가 있던 사람에게서 그 '
        '동맥경화가 터지고, 그 자리에 혈전이 생겨 겨우 흐르던 피가 끊긴다. 심장 근육 자체의 문제가 10%, '
        '부정맥이 10%다.',

        '<b>심근경색은 심장 근육이 상해서 생기는 병이 아니다.</b> 그 근육에 피를 대는 혈관이 막히는 병이다. '
        '그래서 뇌경색과 원리가 같다. 같은 동맥경화가 뇌를 죽이느냐 심장을 죽이느냐의 차이다.',

        '<b>그런데 사망률이 갈린다.</b> 뇌경색은 1년을 기다려도 5% 정도인데 심근경색은 초반에 수십 퍼센트다. '
        '거꾸로 급사한 사람을 부검하면 60~70%가 심근경색이다.',

        '<b>살릴 여지는 심장 쪽이 넓다.</b> 뇌세포는 빠르게 죽지만 심장 근육은 저항력이 강해서, 막힌 곳을 '
        '뚫으면 거의 정상으로 돌아오기도 한다.',

        '<b>마지막 모습은 어떤 원인이든 같다.</b> 심실세동이나 심실빈맥이 생겨 심실이 떨기만 하고 피를 밀어내지 '
        '못한다. 그래서 이 부정맥을 미리 찾는 것은 의미가 없다. 죽을 때 벌어지는 형태이지 가지고 다니는 병이 '
        '아니다.',

        '<b>미리 찾아야 하는 부정맥은 심방세동 쪽이다.</b> 심방이 심실로 전기 신호를 제때 넘기지 못해 심실이 '
        '아무 때나 뛴다. 혈전이 잘 생겨 심장에서 시작하는 뇌경색의 으뜸 원인이기도 하다.',

        '<b>부정맥으로 죽으면 부검에서 아무것도 안 나온다.</b> 심장이 기능적으로 잠깐 이상했던 흔적은 남지 '
        '않는다. 원인이 안 나오면 부정맥이었겠다고 추정한다.',

        '<b>스마트워치가 그 10%를 겨눈다.</b> 며칠치 맥박 기록이 사실상 매일 하는 선별 검사다. 며칠을 봐서 '
        '규칙적이면 부정맥 빈도가 낮은 사람이고, 이상이 잡히면 그 기록을 들고 정밀 검사를 받는다. 정밀 검사의 '
        '기준은 24시간 심전도를 붙여 두는 홀터 검사다.',

        '<b>술은 부정맥에 특히 나쁘다.</b> 혈관 질환에서는 혈소판 기능을 낮춰 혈전을 덜 만드는 면이 있지만, '
        '심장 박동에는 압도적으로 나쁘다. 폭음한 날 자다가 사망하는 경우는 부정맥 발작일 가능성이 있다는 것이 '
        '그의 설명이다.',

        '<b>베타 차단제는 심장에 진정하라고 말하는 약이다.</b> 박동수와 박출력을 낮춰 혈압도 완화한다. 심장 '
        '질환이 있는 사람에게 이 약을 먹은 집단과 먹지 않은 집단의 사망률이 갈리고 차이가 커서, 순환기내과에서 '
        '고혈압이 없어도 처방한다.',

        '<b>흉통은 알아채기 어렵다.</b> 사람은 위장관 통증과 식도 통증과 흉통을 잘 구별하지 못해 "소화가 안 '
        '돼서 왔다"며 응급실에 온다. 그리고 어디가 아프냐는 물음에 말을 잘하는 사람보다 말을 못 하는 사람이 '
        '심각하다.',
    ],
    'figs': [
        (2, '그림 11 · 심장 겉면과 관상동맥',
         FIG_HEART,
         '왼쪽 심장 겉면에 얹힌 붉은 줄이 관상동맥이다. 오른쪽은 막힌 자리의 횡단면으로, 쌓인 지방 위로 혈전이 '
         '길을 덮었다. 같은 그림이 뇌에서 벌어지면 뇌경색이다.'),
    ],
    'slim_stats': [('9~10%', '전체 사망에서 급사가 차지하는 비중'),
                   ('64.7건', '인구 10만 명당 급사 — 넣으면 사망 원인 2위'),
                   ('70%', '급사 원인 중 허혈성 심장질환'),
                   ('5% vs 수십%', '뇌경색 1년 사망률 vs 심근경색 초기')],
    'clash': [
        ('오해 방지', '<b>심장마비는 진단명이 아니다.</b> 죽으면 심장은 어차피 멈추므로 아무 정보가 없는 말이고, '
                   '의과대학에서 쓰지 말라고 가르친다고 그는 말한다.'),
        ('본인 유보', '<b>부정맥 비중을 그는 10%로 말했다가 "크게 보면 10~30%"로도 말했다.</b> 원인 미상 '
                   '사망의 상당수가 여기 들어가기 때문이다.'),
        ('오해 방지', '<b>베타 차단제는 처방 약이다.</b> 심장 질환이 있는 사람 이야기이고, 건강한 사람이 예방 '
                   '삼아 먹는 약으로 제시된 것이 아니다.'),
    ],
}, {
    'section': SEC_SUDDEN,
    'topic': ('market', '골든타임 · 뇌'),
    'title': '골든타임 4분은 뇌가 버티는 시간이다',
    'gain': '골든타임 4분이 어디서 나온 값인지, 그리고 쓰러진 사람 옆에서 무엇을 하고 무엇을 하지 말지.',
    'meta': META_FOURMIN,
    'quote': '"30초만 돼도 이미 해마에 있는 기억 세포들은 죽어 가기 시작을 하고."',
    'note': ('심폐소생술 절차는 교육 과정에서 배우는 것이 정확하다. 이 카드는 그가 설명한 이유 쪽이다. '
             '구급대 도착 시간은 서울 기준으로 그가 든 값이다.'),
    'links': LINKS_SUD1,
    'slim_oneliner': ('심장이 멈추면 뇌로 가는 산소와 포도당이 함께 끊긴다. 골든타임 4분은 <b>뇌세포가 버티는 '
                      '시간</b>이고 심장이 버티는 한계와는 무관하다. 그래서 4분 안에 돌아오면 정상으로 '
                      '회복하고 그 뒤에 돌아오면 식물인간이 될 가능성이 커진다.'),
    'slim_points': [
        '<b>심장은 규칙 하나로 움직인다.</b> 어떤 상황에서도 혈액의 30%를 뇌로 보내라는 것이다. 눕든 뛰든 '
        '사우나에 들어가든 다른 곳의 혈류는 오가도 뇌로 가는 몫은 유지한다.',

        '<b>신경 세포는 태어났을 때가 가장 많고 이후로 줄기만 한다.</b> 재생하지 않는다. 스물네 살쯤 능력이 '
        '가장 좋은 이유는 세포 수가 늘어서가 아니다. 세포끼리 연결한 시냅스가 늘어서다.',

        '<b>기억을 담는 해마가 가장 먼저 상한다.</b> 30초에서 1분이면 상하기 시작하고 1~2분이면 상당 부분이 '
        '상한다. 다만 상하기 시작하는 것이지 다 죽는 것은 아니어서, 피가 다시 돌면 대개 회복한다.',

        '<b>서울에서 구급대가 가장 빨리 오는 시간이 5.4분이다.</b> 의사를 만나는 데는 한두 시간이 걸린다. '
        '4분 안에 손을 쓸 수 있는 사람은 옆에 있는 목격자뿐이라는 결론이 여기서 나온다.',

        '<b>멈춘 심장은 대개 고장 난 게 아니라 스텝이 꼬인 것이다.</b> 스트레스나 과부하로 잠깐 쉬어 버린 '
        '상태이고, 가슴을 강하게 누르는 물리적 자극이 그 심장을 돌아오게 만든다. 심폐소생술을 받은 사람의 '
        '상당수가 갈비뼈가 부러지는 것은 그만큼 세게 눌러야 하기 때문이다.',

        '<b>지금 지침은 세 가지로 줄었다.</b> 의식 없이 쓰러졌으면 심정지로 의심하고, 119에 전화하고, 바로 '
        '압박한다. 근처에 자동 심장충격기가 있으면 가져온다. 맥박이 뛰는지 확인하라는 단계는 빠졌다.',

        '<b>뇌사와 심장사가 갈리는 자리도 여기다.</b> 심장은 자율 기능이 있어 뇌가 죽어도 뛰고 다른 장기도 '
        '한동안 산다. 법적 사망은 심장 기준이라 뇌사 판정 위원회를 따로 연다.',

        '<b>실신은 원인이 하나다.</b> 순간적으로 뇌로 올라가는 혈액이 끊기는 것이다. 뇌경색은 한 혈관이 막히는 '
        '것이고 실신은 전체가 한꺼번에 안 올라오는 것이라, 같은 증상처럼 보여도 다른 사건이다.',

        '<b>가장 흔한 것은 미주신경성 실신이다.</b> 심장과 혈관을 다루는 자율신경이 판단을 잘못해 심장에는 '
        '뛰지 말라고 하고 혈관은 확장시킨다. 공연장에서 소리를 지르다 쓰러지는 경우가 여기 든다.',

        '<b>쓰러지는 것 자체가 치료다.</b> 누우면 중력으로 피가 뇌로 올라가 몇 분 안에 대부분 일어난다. '
        '붙잡아 세우면 안 되고 다치지 않게 눕힌다.',

        '<b>등산이나 달리기 중에 눈앞이 캄캄해지는 것은 같은 신호다.</b> 심장이 뇌로 피를 제대로 못 보내고 '
        '있다는 뜻이고, 그 상태가 이어졌으면 의식을 잃었을 것이라고 그는 말한다.',
    ],
    'figs': [
        (3, '그림 12 · 뇌가 버티는 시간',
         FIG_BRAINCLOCK,
         '왼쪽 붉은 자리가 해마다. 오른쪽 눈금은 심장이 멈춘 뒤의 시간이고, 4분과 5.4분 사이의 간격이 '
         '목격자가 채워야 하는 자리다.'),
    ],
    'slim_stats': [('4분', '심정지 뒤 뇌가 버티는 시간'),
                   ('30초~1분', '해마가 상하기 시작하는 시간'),
                   ('30%', '심장이 어떤 상황에서도 뇌로 보내는 혈액'),
                   ('5.4분', '서울에서 구급대가 가장 빨리 오는 시간')],
    'clash': [
        ('오해 방지', '<b>쓰러진 사람을 일으켜 세우면 안 된다.</b> 실신은 누워야 뇌로 피가 올라간다. '
                   '손을 따거나 약을 먹이는 것은 도움이 되지 않는다.'),
        ('본인 유보', '<b>4분은 일반적인 기준이다.</b> 그도 "일반적으로"라는 말을 붙였고, 체온이나 상황에 '
                   '따라 달라지는 값이다.'),
    ],
}, {
    'section': SEC_SUDDEN,
    'topic': ('market', '방아쇠 · 운동'),
    'title': '장전되지 않은 몸은 방아쇠를 당겨도 터지지 않는다',
    'gain': '무리한 운동과 온도차가 왜 방아쇠가 되는지, 그리고 내가 무리하는지 알 방법이 없는 이유.',
    'meta': META_TRIGGER,
    'quote': '"방아쇠를 아무리 당긴다고 사건이 생기진 않는다. 장전하지 않았다면."',
    'note': ('운동을 하지 말라는 이야기가 아니다. 강도를 올리는 속도를 말한다. 일본의 목욕 중 사망자 수는 '
             '영상에서 2만 명과 1만 9천 명이 함께 언급됐고 출처는 제시되지 않았다.'),
    'links': LINKS_SUD12,
    'slim_oneliner': ('급사에는 두 가지가 함께 있어야 한다. <b>장전</b>은 5~20년에 걸쳐 만들어진 동맥경화나 '
                      '동맥류이고, <b>방아쇠</b>는 그날의 무리다. 아주 건강한 사람이 스트레스를 심하게 받는다고 '
                      '뇌졸중이 오지는 않는다.'),
    'slim_points': [
        '<b>운동의 원리는 근육이 죽지 않을 정도의 스트레스를 주는 것이다.</b> 그 자극을 견디면 대응력이 올라간다. '
        '문제는 그 대비가 없는 사람이 같은 강도를 갑자기 받을 때다. 그때는 훈련이 되지 않고 죽을 수 있는 '
        '자극이 된다.',

        '<b>근육의 전성기는 스물네 살이고 재생 능력은 나이가 들며 떨어진다.</b> 그런데 한국 남성의 상당수는 '
        '20~30대를 회식과 야근으로 보내고 40대에 시간이 생기면서 운동을 시작한다. 근육이 나빠지기 시작할 때 '
        '갑자기, 그것도 과하게 한다.',

        '<b>운동을 하면 도파민과 엔돌핀이 나와 강도가 계속 올라간다.</b> 그래서 무리가 습관이 된다. 뇌졸중이 '
        '두 번 온 뒤에도 산에 오르는 환자를 그는 예로 든다.',

        '<b>내가 무리하고 있는지 알 방법은 없다.</b> 몸의 각 시스템은 자기 일만 열심히 하도록 만들어져 있고, '
        '전체를 보고 이 정도만 하라고 알려 주는 장치가 없다. 균이 들어오면 주변까지 다 죽이는 염증과 같은 '
        '구조다.',

        '<b>온도차도 같은 방아쇠다.</b> 일본은 집이 춥고 탕 목욕 문화가 있어서, 목욕하다 사망하는 사람이 한 해 '
        '2만 명 가까이 된다. 교통사고 사망보다 많다. 사우나 자체가 나쁘지는 않다. 갑작스러운 상태 변화가 '
        '문제다.',

        '<b>그래서 순서가 정해진다.</b> 건강할 때부터 조금씩 올려 견디는 폭을 넓혀 두고, 그 뒤로는 견딜 수 '
        '있는 강도보다 약하게 오래 하는 것이 그가 권한 방법이다.',

        '<b>운동을 안 하던 사람과 심하게 하던 사람의 처방이 반대다.</b> 안 하던 사람은 규칙적으로 조금 하고, '
        '심하게 하던 사람은 강도를 줄여 규칙적으로 한다.',

        '<b>혈관은 나이가 들며 나빠지는데 욕심은 반대로 커진다.</b> 두 선이 엇갈리는 지점에서 사건이 난다는 것이 '
        '그의 설명이다. 50대가 달리다가 러너스 하이를 느낄 때까지 가면 이미 그 지점을 넘은 것이다.',

        '<b>장전 여부는 검사로만 안다.</b> 목의 큰 동맥을 보는 경동맥 초음파가 가장 싸고, 관상동맥은 심장 CT로, '
        '뇌동맥류는 혈관을 보는 MRI인 MRA로 본다. 2단계에는 증상이 없다.',
    ],
    'slim_stats': [('5~20년', '위험 요인이 동맥경화를 만드는 기간'),
                   ('24세', '근육의 전성기'),
                   ('약 2만 명', '일본에서 한 해 목욕 중 사망'),
                   ('50대 / 70대', '한국의 심근경색 / 뇌졸중 호발 연령')],
    'clash': [
        ('오해 방지', '<b>운동을 하지 말라는 이야기가 아니다.</b> 그가 겨눈 것은 강도를 올리는 속도이고, '
                   '안 하던 사람에게는 규칙적으로 조금 하라고 말한다.'),
        ('가설', '<b>「장전과 방아쇠」는 그가 만든 설명 모델이다.</b> 영상에서 본인이 그렇게 밝혔다. 학회가 '
               '정한 분류가 아니다.'),
    ],
}, {
    'section': SEC_GLP1,
    ...
    'figs': [
        (3, '그림 13 · 포만 신호가 보상 회로에도 닿는다',
         FIG_GLP1,
         '장에서 나온 호르몬이 인슐린과 포만감만 건드리는 것으로 끝나지 않는다.'),
    ],
    ...
    'slim_points': [
        ...
        '<b>약과 독을 나누는 것은 용량이다.</b> 약효가 나기 시작하는 지점과 독성이 나타나는 지점 사이의 좁은 '
...
VID = {
    ...
    SRC_SUD1: ('언더스탠딩 뇌가 멈추기 전에 1부', '2025-07-20'),
    SRC_SUD2: ('언더스탠딩 뇌가 멈추기 전에 2부', '2025-07-20'),
}
...
COURSE = [
    ...
    ('갑자기 죽는 일',
     '앞 단계를 그냥 두면 어디로 가는지다. 병 셋을 먼저 보고, 뇌가 버티는 시간과 그날의 방아쇠를 읽는다.',
     ['멀쩡하던 사람이 갑자기 죽는 일은 병 세 개가 전부다',
      '골든타임 4분은 뇌가 버티는 시간이다',
      '장전되지 않은 몸은 방아쇠를 당겨도 터지지 않는다']),
    ('약은 어디까지 왔나',
...
# 합류도 — 섹션 타일이 카드를 주제별로 나누는 동안, 이 지도는 그 주제들이 한 곳으로
# 모인다는 것을 먼저 보인다. 이 페이지의 카드 스물은 먹는 것·염증·당뇨·단계·급사로
# 흩어져 있는데, 이승훈 교수의 설명에서 그것들은 전부 혈관 하나로 도착한다.
#
# 합류점을 「오래 사느냐」로 잡으면 아무 말도 아닌 칸이 된다. 그가 실제로 잰 것은
# 혈관이 굳었느냐와 그 혈관에서 사건이 났느냐다. 그래서 합류 칸을 그것으로 둔다.
# 배정은 카드 제목으로만 가리킨다 — 본문은 한 글자도 안 고친다.
MERGES = [{
    'id': 'vessel',
    'title': '과잉 칼로리에서 혈관 사건까지',
    'lede': ('많이 먹어 남은 에너지가 염증과 지방산을 거쳐 혈관을 굳히고, 굳은 혈관에서 '
             '뇌졸중·심근경색·급사가 난다. 카드 열한 장이 그 한 줄의 앞뒤다.'),
    'outer': [
        ('먹는 양과 종류',
         ['밥·빵 종류가 문제가 아니라 얼마나 많이 먹느냐가 문제다',
          '같은 열량이면 과일이 밀가루보다 나쁘다. 과당은 간만 받는다']),
        ('몸무게와 잠',
         ['이미 가진 살은 빼려 애쓰지 않는다. 영양제는 대부분 필요 없다',
          '뇌는 깊은 잠에서만 씻긴다']),
        # 그날의 무리는 바깥 조건이다. 장전이 없으면 이것만으로는 사건이 안 난다.
        ('그날의 방아쇠',
         ['장전되지 않은 몸은 방아쇠를 당겨도 터지지 않는다']),
    ],
    'price': [
...
if __name__ == '__main__':
    attach_kin(CARDS)
    # 합류도는 읽는 순서 안내 아래·섹션 타일 위에 접힌 채로 선다(UI 규약 1을 깨지 않는다).
