...
경로: `C:\Users\y\.claude\uploads\9417c49f-7730-4847-97af-ae9effa47479\1e8d6102-AI_DC_Capex_Big4_HBM_Scn.xlsx`

openpyxl 로 직접 읽었다. 수식은 `data_only=False`, 계산값은 `data_only=True`로 각각 불러와 나란히 적는다. 값이 없는 셀은 적지 않는다(빈 행·빈 열은 생략). 채우기 색이 있는 셀은 뒤에 표시했다 — 노란 채우기(`FFFFFF00`)가 입력 셀, 남색(`FF1F3864`)은 열 헤더 행, 회색(`FFF2F2F2`)은 섹션 구분 행, 주황(`FFFCE4D6`)은 조달 갭·출력 강조, 연두(`FFC6E0B4`)는 새로 나타난 색으로 시나리오 선택 스위치 셀(Scenario!B4)에 쓰였다.

## 시트 목록

- `README` — 범위 A1:A31 (31행 × 1열)
- `Scenario` — 범위 A1:H90 (90행 × 8열)
- `Consolidated` — 범위 A1:H42 (42행 × 8열)
- `Sheet` — 범위 A1:A1 (1행 × 1열)
- `MSFT` — 범위 A1:H44 (44행 × 8열)
- `GOOGL` — 범위 A1:H44 (44행 × 8열)
- `AMZN` — 범위 A1:H44 (44행 × 8열)
- `META` — 범위 A1:H44 (44행 × 8열)
- `HBM_Inputs` — 범위 A1:L77 (77행 × 12열)
- `HBM_Demand` — 범위 A1:L166 (166행 × 12열)

## 앞 판과 달라진 것

- 새로 생긴 시트: Scenario — `Scenario`는 이번 판에만 있다(범위 A1:H90)
- 없어진 시트: 없음
- `README`: 범위 동일(A1:A31)
- `Consolidated`: 범위 동일(A1:H42)
- `MSFT`: 범위 A1:H42 → A1:H44 (달라짐)
- `GOOGL`: 범위 A1:H42 → A1:H44 (달라짐)
- `AMZN`: 범위 A1:H42 → A1:H44 (달라짐)
- `META`: 범위 A1:H42 → A1:H44 (달라짐)
- `HBM_Inputs`: 범위 동일(A1:L77)
- `HBM_Demand`: 범위 동일(A1:L166)

## 시나리오 관련 셀

- 케이스 선택 스위치: `Scenario!B4` — 값 1=Bear, 2=Base, 3=Bull (연두 채우기, 노란 채우기가 아니라 다른 색으로 구분됨)
- `Scenario!C4` — 수식 `=CHOOSE(B4,"Bear","Base","Bull")` — B4가 가리키는 케이스 이름을 문자열로 반환
- 각 파라미터 블록은 Bear/Base/Bull 세 행(노란 입력 셀) 다음에 `INDEX(범위,$B$4)` 수식으로 현재 케이스 값을 "적용값" 행에 뽑아낸다. 발견된 블록(행 번호는 라벨·적용값 행):
  - A7 Capex 증감률 조정(2027E~, %p) → 적용값 행 11 `=INDEX(C8:C10,$B$4)` 형태
  - A13 AI 가속기 비중 조정(%p) → 적용값 행 17
  - A19 HBM $/GB 배율 → 적용값 행 23
  - A25 HBM GB/칩 배율 → 적용값 행 29
  - A31 가속기 ASP 배율 → 적용값 행 35
  - A37 빅4 글로벌 비중 조정(%p) → 적용값 행 41
  - A43 삼성 점유율 조정(%p, SK hynix에서 이전) → 적용값 행 47
- 이 적용값들은 다른 시트 수식에서 `Scenario!C11`(MSFT/GOOGL/AMZN/META의 D13:G13, capex 성장률에 가산), `Scenario!C17/C23/C29/C35/C41/C47`(HBM_Demand의 가속기 비중·ASP·GB/칩·$/GB·빅4 비중·삼성 점유율 조정)로 참조된다.
- `Scenario!A50` 아래(행 50~90)는 "시나리오 비교 (스냅샷)"— Bear/Base/Bull 세 케이스의 계산 결과를 표로 박아 둔 값(수식 아님, 리터럴 숫자). README에 언급된 갱신 방법은 없음 — 해당 블록 제목(A50)에 "입력값 변경 시 자동 갱신되지 않음, 2026-09-10 기준"이라 적혀 있다.
...
## 시트 `Scenario` (90행 × 8열, 범위 A1:H90)

- 연도 헤더: 행 6: C6=2026E, D6=2027E, E6=2028E, F6=2029E, G6=2030E / 행 51: C51=2026E, D51=2027E, E51=2028E, F51=2029E, G51=2030E / 행 55: C55=2026E, D55=2027E, E55=2028E, F55=2029E, G55=2030E / 행 59: C59=2026E, D59=2027E, E59=2028E, F59=2029E, G59=2030E / 행 63: C63=2026E, D63=2027E, E63=2028E, F63=2029E, G63=2030E / 행 67: C67=2026E, D67=2027E, E67=2028E, F67=2029E, G67=2030E / 행 71: C71=2026E, D71=2027E, E71=2028E, F71=2029E, G71=2030E / 행 75: C75=2026E, D75=2027E, E75=2028E, F75=2029E, G75=2030E / 행 79: C79=2026E, D79=2027E, E79=2028E, F79=2029E, G79=2030E / 행 83: C83=2026E, D83=2027E, E83=2028E, F83=2029E, G83=2030E / 행 87: C87=2026E, D87=2027E, E87=2028E, F87=2029E, G87=2030E

- 노란 입력 셀(총 105개): C8, D8, E8, F8, G8, C9, D9, E9, F9, G9, C10, D10, E10, F10, G10, C14, D14, E14, F14, G14, C15, D15, E15, F15, G15, C16, D16, E16, F16, G16, C20, D20, E20, F20, G20, C21, D21, E21, F21, G21, C22, D22, E22, F22, G22, C26, D26, E26, F26, G26, C27, D27, E27, F27, G27, C28, D28, E28, F28, G28, C32, D32, E32, F32, G32, C33, D33, E33, F33, G33, C34, D34, E34, F34, G34, C38, D38, E38, F38, G38, C39, D39, E39, F39, G39, C40, D40, E40, F40, G40, C44, D44, E44, F44, G44, C45, D45, E45, F45, G45, C46, D46, E46, F46, G46

- 값이 든 셀 개수: 463


- Scenario!A1 | 값: 시나리오 (Bear / Base / Bull)
- Scenario!A2 | 값: B4 셀에 1·2·3 입력 → 회사 capex, HBM 가격·용량·믹스, 공급사 점유율이 일괄 전환. 각 레버는 Base 입력값 대비 조정(배수 또는 %p 가산).
- Scenario!A4 | 값: 선택 (1=Bear, 2=Base, 3=Bull)
- Scenario!B4 | 값: 2 (시나리오 선택 스위치 — 연두 채우기)
- Scenario!C4 | 수식: `=CHOOSE(B4,"Bear","Base","Bull")` | 계산값: Base
- Scenario!A6 | 값: 레버 (헤더 행 — 남색 채우기)
- Scenario!B6 | 값: 케이스 (헤더 행 — 남색 채우기)
- Scenario!C6 | 값: 2026E (헤더 행 — 남색 채우기)
- Scenario!D6 | 값: 2027E (헤더 행 — 남색 채우기)
- Scenario!E6 | 값: 2028E (헤더 행 — 남색 채우기)
- Scenario!F6 | 값: 2029E (헤더 행 — 남색 채우기)
- Scenario!G6 | 값: 2030E (헤더 행 — 남색 채우기)
- Scenario!H6 | 값: 메모 (헤더 행 — 남색 채우기)
- Scenario!A7 | 값: Capex 성장률 조정 (2027E~, %p 가산)
- Scenario!H7 | 값: Bear: AI 수요 둔화·FCF 압박으로 2027부터 증가율 둔화 / Bull: 2027 큰 폭 증가 코멘트(GOOGL) 현실화
- Scenario!B8 | 값: Bear
- Scenario!C8 | 값: 0 (입력 셀 — 노란 채우기)
- Scenario!D8 | 값: -0.1 (입력 셀 — 노란 채우기)
- Scenario!E8 | 값: -0.1 (입력 셀 — 노란 채우기)
- Scenario!F8 | 값: -0.08 (입력 셀 — 노란 채우기)
- Scenario!G8 | 값: -0.05 (입력 셀 — 노란 채우기)
- Scenario!B9 | 값: Base
- Scenario!C9 | 값: 0 (입력 셀 — 노란 채우기)
- Scenario!D9 | 값: 0 (입력 셀 — 노란 채우기)
- Scenario!E9 | 값: 0 (입력 셀 — 노란 채우기)
- Scenario!F9 | 값: 0 (입력 셀 — 노란 채우기)
- Scenario!G9 | 값: 0 (입력 셀 — 노란 채우기)
- Scenario!B10 | 값: Bull
- Scenario!C10 | 값: 0 (입력 셀 — 노란 채우기)
- Scenario!D10 | 값: 0.1 (입력 셀 — 노란 채우기)
- Scenario!E10 | 값: 0.1 (입력 셀 — 노란 채우기)
- Scenario!F10 | 값: 0.08 (입력 셀 — 노란 채우기)
- Scenario!G10 | 값: 0.05 (입력 셀 — 노란 채우기)
- Scenario!A11 | 값:   → 적용값
- Scenario!B11 | 수식: `=$C$4` | 계산값: Base
- Scenario!C11 | 수식: `=INDEX(C8:C10,$B$4)` | 계산값: 0
- Scenario!D11 | 수식: `=INDEX(D8:D10,$B$4)` | 계산값: 0
- Scenario!E11 | 수식: `=INDEX(E8:E10,$B$4)` | 계산값: 0
- Scenario!F11 | 수식: `=INDEX(F8:F10,$B$4)` | 계산값: 0
- Scenario!G11 | 수식: `=INDEX(G8:G10,$B$4)` | 계산값: 0
- Scenario!A13 | 값: AI 가속기 비중 조정 (%p 가산)
- Scenario!H13 | 값: 서버 capex 중 GPU/ASIC 시스템 비중
- Scenario!B14 | 값: Bear
- Scenario!C14 | 값: -0.05 (입력 셀 — 노란 채우기)
- Scenario!D14 | 값: -0.05 (입력 셀 — 노란 채우기)
- Scenario!E14 | 값: -0.05 (입력 셀 — 노란 채우기)
- Scenario!F14 | 값: -0.05 (입력 셀 — 노란 채우기)
- Scenario!G14 | 값: -0.05 (입력 셀 — 노란 채우기)
- Scenario!B15 | 값: Base
- Scenario!C15 | 값: 0 (입력 셀 — 노란 채우기)
- Scenario!D15 | 값: 0 (입력 셀 — 노란 채우기)
- Scenario!E15 | 값: 0 (입력 셀 — 노란 채우기)
- Scenario!F15 | 값: 0 (입력 셀 — 노란 채우기)
- Scenario!G15 | 값: 0 (입력 셀 — 노란 채우기)
- Scenario!B16 | 값: Bull
- Scenario!C16 | 값: 0.05 (입력 셀 — 노란 채우기)
- Scenario!D16 | 값: 0.05 (입력 셀 — 노란 채우기)
- Scenario!E16 | 값: 0.05 (입력 셀 — 노란 채우기)
- Scenario!F16 | 값: 0.05 (입력 셀 — 노란 채우기)
- Scenario!G16 | 값: 0.05 (입력 셀 — 노란 채우기)
- Scenario!A17 | 값:   → 적용값
- Scenario!B17 | 수식: `=$C$4` | 계산값: Base
- Scenario!C17 | 수식: `=INDEX(C14:C16,$B$4)` | 계산값: 0
- Scenario!D17 | 수식: `=INDEX(D14:D16,$B$4)` | 계산값: 0
- Scenario!E17 | 수식: `=INDEX(E14:E16,$B$4)` | 계산값: 0
- Scenario!F17 | 수식: `=INDEX(F14:F16,$B$4)` | 계산값: 0
- Scenario!G17 | 수식: `=INDEX(G14:G16,$B$4)` | 계산값: 0
- Scenario!A19 | 값: HBM $/GB 배수
- Scenario!H19 | 값: Bull: TrendForce 2027 계약가 '수 배' 상승 반영 / Bear: 공급 증설(삼성·마이크론 capa)과 CXMT 진입으로 가격 정상화
- Scenario!B20 | 값: Bear
- Scenario!C20 | 값: 0.9 (입력 셀 — 노란 채우기)
- Scenario!D20 | 값: 0.75 (입력 셀 — 노란 채우기)
- Scenario!E20 | 값: 0.7 (입력 셀 — 노란 채우기)
- Scenario!F20 | 값: 0.7 (입력 셀 — 노란 채우기)
- Scenario!G20 | 값: 0.7 (입력 셀 — 노란 채우기)
- Scenario!B21 | 값: Base
- Scenario!C21 | 값: 1 (입력 셀 — 노란 채우기)
- Scenario!D21 | 값: 1 (입력 셀 — 노란 채우기)
- Scenario!E21 | 값: 1 (입력 셀 — 노란 채우기)
- Scenario!F21 | 값: 1 (입력 셀 — 노란 채우기)
- Scenario!G21 | 값: 1 (입력 셀 — 노란 채우기)
- Scenario!B22 | 값: Bull
- Scenario!C22 | 값: 1.1 (입력 셀 — 노란 채우기)
- Scenario!D22 | 값: 1.5 (입력 셀 — 노란 채우기)
- Scenario!E22 | 값: 1.4 (입력 셀 — 노란 채우기)
- Scenario!F22 | 값: 1.3 (입력 셀 — 노란 채우기)
- Scenario!G22 | 값: 1.2 (입력 셀 — 노란 채우기)
- Scenario!A23 | 값:   → 적용값
- Scenario!B23 | 수식: `=$C$4` | 계산값: Base
- Scenario!C23 | 수식: `=INDEX(C20:C22,$B$4)` | 계산값: 1
- Scenario!D23 | 수식: `=INDEX(D20:D22,$B$4)` | 계산값: 1
- Scenario!E23 | 수식: `=INDEX(E20:E22,$B$4)` | 계산값: 1
- Scenario!F23 | 수식: `=INDEX(F20:F22,$B$4)` | 계산값: 1
- Scenario!G23 | 수식: `=INDEX(G20:G22,$B$4)` | 계산값: 1
- Scenario!A25 | 값: HBM GB/칩 배수 (용량 업그레이드 속도)
- Scenario!H25 | 값: Bear: Rubin Ultra 384GB 지연·ASIC 용량 상향 지연 / Bull: 추론용 KV캐시 수요로 칩당 HBM 추가 상향
- Scenario!B26 | 값: Bear
- Scenario!C26 | 값: 0.95 (입력 셀 — 노란 채우기)
- Scenario!D26 | 값: 0.9 (입력 셀 — 노란 채우기)
- Scenario!E26 | 값: 0.9 (입력 셀 — 노란 채우기)
- Scenario!F26 | 값: 0.9 (입력 셀 — 노란 채우기)
- Scenario!G26 | 값: 0.9 (입력 셀 — 노란 채우기)
- Scenario!B27 | 값: Base
- Scenario!C27 | 값: 1 (입력 셀 — 노란 채우기)
- Scenario!D27 | 값: 1 (입력 셀 — 노란 채우기)
- Scenario!E27 | 값: 1 (입력 셀 — 노란 채우기)
- Scenario!F27 | 값: 1 (입력 셀 — 노란 채우기)
- Scenario!G27 | 값: 1 (입력 셀 — 노란 채우기)
- Scenario!B28 | 값: Bull
- Scenario!C28 | 값: 1.05 (입력 셀 — 노란 채우기)
- Scenario!D28 | 값: 1.1 (입력 셀 — 노란 채우기)
- Scenario!E28 | 값: 1.15 (입력 셀 — 노란 채우기)
- Scenario!F28 | 값: 1.15 (입력 셀 — 노란 채우기)
- Scenario!G28 | 값: 1.15 (입력 셀 — 노란 채우기)
- Scenario!A29 | 값:   → 적용값
- Scenario!B29 | 수식: `=$C$4` | 계산값: Base
- Scenario!C29 | 수식: `=INDEX(C26:C28,$B$4)` | 계산값: 1
- Scenario!D29 | 수식: `=INDEX(D26:D28,$B$4)` | 계산값: 1
- Scenario!E29 | 수식: `=INDEX(E26:E28,$B$4)` | 계산값: 1
- Scenario!F29 | 수식: `=INDEX(F26:F28,$B$4)` | 계산값: 1
- Scenario!G29 | 수식: `=INDEX(G26:G28,$B$4)` | 계산값: 1
- Scenario!A31 | 값: 가속기 ASP 배수
- Scenario!H31 | 값: ASP 높으면 같은 capex로 대수 감소 → HBM 감소 (Bear: 메모리 원가 전가로 ASP 상승)
- Scenario!B32 | 값: Bear
- Scenario!C32 | 값: 1.05 (입력 셀 — 노란 채우기)
- Scenario!D32 | 값: 1.05 (입력 셀 — 노란 채우기)
- Scenario!E32 | 값: 1.05 (입력 셀 — 노란 채우기)
- Scenario!F32 | 값: 1.05 (입력 셀 — 노란 채우기)
- Scenario!G32 | 값: 1.05 (입력 셀 — 노란 채우기)
- Scenario!B33 | 값: Base
- Scenario!C33 | 값: 1 (입력 셀 — 노란 채우기)
- Scenario!D33 | 값: 1 (입력 셀 — 노란 채우기)
- Scenario!E33 | 값: 1 (입력 셀 — 노란 채우기)
- Scenario!F33 | 값: 1 (입력 셀 — 노란 채우기)
- Scenario!G33 | 값: 1 (입력 셀 — 노란 채우기)
- Scenario!B34 | 값: Bull
- Scenario!C34 | 값: 0.95 (입력 셀 — 노란 채우기)
- Scenario!D34 | 값: 0.95 (입력 셀 — 노란 채우기)
- Scenario!E34 | 값: 0.95 (입력 셀 — 노란 채우기)
- Scenario!F34 | 값: 0.95 (입력 셀 — 노란 채우기)
- Scenario!G34 | 값: 0.95 (입력 셀 — 노란 채우기)
- Scenario!A35 | 값:   → 적용값
- Scenario!B35 | 수식: `=$C$4` | 계산값: Base
- Scenario!C35 | 수식: `=INDEX(C32:C34,$B$4)` | 계산값: 1
- Scenario!D35 | 수식: `=INDEX(D32:D34,$B$4)` | 계산값: 1
- Scenario!E35 | 수식: `=INDEX(E32:E34,$B$4)` | 계산값: 1
- Scenario!F35 | 수식: `=INDEX(F32:F34,$B$4)` | 계산값: 1
- Scenario!G35 | 수식: `=INDEX(G32:G34,$B$4)` | 계산값: 1
- Scenario!A37 | 값: 빅4 글로벌 비중 조정 (%p 가산)
- Scenario!H37 | 값: Bull: 네오클라우드·소버린·OpenAI 자체 인프라 등 빅4 외 수요 확대 → 빅4 비중 하락 = 글로벌 시장 확대
- Scenario!B38 | 값: Bear
- Scenario!C38 | 값: 0.05 (입력 셀 — 노란 채우기)
- Scenario!D38 | 값: 0.05 (입력 셀 — 노란 채우기)
- Scenario!E38 | 값: 0.05 (입력 셀 — 노란 채우기)
- Scenario!F38 | 값: 0.05 (입력 셀 — 노란 채우기)
- Scenario!G38 | 값: 0.05 (입력 셀 — 노란 채우기)
- Scenario!B39 | 값: Base
- Scenario!C39 | 값: 0 (입력 셀 — 노란 채우기)
- Scenario!D39 | 값: 0 (입력 셀 — 노란 채우기)
- Scenario!E39 | 값: 0 (입력 셀 — 노란 채우기)
- Scenario!F39 | 값: 0 (입력 셀 — 노란 채우기)
- Scenario!G39 | 값: 0 (입력 셀 — 노란 채우기)
- Scenario!B40 | 값: Bull
- Scenario!C40 | 값: -0.05 (입력 셀 — 노란 채우기)
- Scenario!D40 | 값: -0.05 (입력 셀 — 노란 채우기)
- Scenario!E40 | 값: -0.05 (입력 셀 — 노란 채우기)
- Scenario!F40 | 값: -0.05 (입력 셀 — 노란 채우기)
- Scenario!G40 | 값: -0.05 (입력 셀 — 노란 채우기)
- Scenario!A41 | 값:   → 적용값
- Scenario!B41 | 수식: `=$C$4` | 계산값: Base
- Scenario!C41 | 수식: `=INDEX(C38:C40,$B$4)` | 계산값: 0
- Scenario!D41 | 수식: `=INDEX(D38:D40,$B$4)` | 계산값: 0
- Scenario!E41 | 수식: `=INDEX(E38:E40,$B$4)` | 계산값: 0
- Scenario!F41 | 수식: `=INDEX(F38:F40,$B$4)` | 계산값: 0
- Scenario!G41 | 수식: `=INDEX(G38:G40,$B$4)` | 계산값: 0
- Scenario!A43 | 값: 삼성 점유율 조정 (%p 가산, SK hynix에서 이전)
- Scenario!H43 | 값: 삼성 HBM4 램프 성공 여부. 마이크론은 고정
- Scenario!B44 | 값: Bear
- Scenario!C44 | 값: -0.05 (입력 셀 — 노란 채우기)
- Scenario!D44 | 값: -0.08 (입력 셀 — 노란 채우기)
- Scenario!E44 | 값: -0.08 (입력 셀 — 노란 채우기)
- Scenario!F44 | 값: -0.08 (입력 셀 — 노란 채우기)
- Scenario!G44 | 값: -0.08 (입력 셀 — 노란 채우기)
- Scenario!B45 | 값: Base
- Scenario!C45 | 값: 0 (입력 셀 — 노란 채우기)
- Scenario!D45 | 값: 0 (입력 셀 — 노란 채우기)
- Scenario!E45 | 값: 0 (입력 셀 — 노란 채우기)
- Scenario!F45 | 값: 0 (입력 셀 — 노란 채우기)
- Scenario!G45 | 값: 0 (입력 셀 — 노란 채우기)
- Scenario!B46 | 값: Bull
- Scenario!C46 | 값: 0.05 (입력 셀 — 노란 채우기)
- Scenario!D46 | 값: 0.08 (입력 셀 — 노란 채우기)
- Scenario!E46 | 값: 0.1 (입력 셀 — 노란 채우기)
- Scenario!F46 | 값: 0.1 (입력 셀 — 노란 채우기)
- Scenario!G46 | 값: 0.1 (입력 셀 — 노란 채우기)
- Scenario!A47 | 값:   → 적용값
- Scenario!B47 | 수식: `=$C$4` | 계산값: Base
- Scenario!C47 | 수식: `=INDEX(C44:C46,$B$4)` | 계산값: 0
- Scenario!D47 | 수식: `=INDEX(D44:D46,$B$4)` | 계산값: 0
- Scenario!E47 | 수식: `=INDEX(E44:E46,$B$4)` | 계산값: 0
- Scenario!F47 | 수식: `=INDEX(F44:F46,$B$4)` | 계산값: 0
- Scenario!G47 | 수식: `=INDEX(G44:G46,$B$4)` | 계산값: 0
- Scenario!A50 | 값: 시나리오 비교 (스냅샷 — 입력값 변경 시 자동 갱신되지 않음, 2026-09-10 기준)
- Scenario!A51 | 값: 지표 (헤더 행 — 남색 채우기)
- Scenario!B51 | 값: 케이스 (헤더 행 — 남색 채우기)
- Scenario!C51 | 값: 2026E (헤더 행 — 남색 채우기)
- Scenario!D51 | 값: 2027E (헤더 행 — 남색 채우기)
- Scenario!E51 | 값: 2028E (헤더 행 — 남색 채우기)
- Scenario!F51 | 값: 2029E (헤더 행 — 남색 채우기)
- Scenario!G51 | 값: 2030E (헤더 행 — 남색 채우기)
- Scenario!A52 | 값: 빅4 총 Capex ($B)
- Scenario!B52 | 값: Bear
- Scenario!C52 | 값: 732.5
- Scenario!D52 | 값: 834.5
- Scenario!E52 | 값: 864.43
- Scenario!F52 | 값: 873.69
- Scenario!G52 | 값: 873.69
- Scenario!B53 | 값: Base
- Scenario!C53 | 값: 732.5
- Scenario!D53 | 값: 907.75
- Scenario!E53 | 값: 1031.04
- Scenario!F53 | 값: 1124.54
- Scenario!G53 | 값: 1180.76
- Scenario!B54 | 값: Bull
- Scenario!C54 | 값: 732.5
- Scenario!D54 | 값: 981
- Scenario!E54 | 값: 1212.31
- Scenario!F54 | 값: 1419.18
- Scenario!G54 | 값: 1561.1
- Scenario!A55 | 값: 지표 (헤더 행 — 남색 채우기)
- Scenario!B55 | 값: 케이스 (헤더 행 — 남색 채우기)
- Scenario!C55 | 값: 2026E (헤더 행 — 남색 채우기)
- Scenario!D55 | 값: 2027E (헤더 행 — 남색 채우기)
- Scenario!E55 | 값: 2028E (헤더 행 — 남색 채우기)
- Scenario!F55 | 값: 2029E (헤더 행 — 남색 채우기)
- Scenario!G55 | 값: 2030E (헤더 행 — 남색 채우기)
- Scenario!A56 | 값: 빅4 FCF ($B)
- Scenario!B56 | 값: Bear
- Scenario!C56 | 값: -13.3
- Scenario!D56 | 값: 11.8
- Scenario!E56 | 값: 80.44
- Scenario!F56 | 값: 162.88
- Scenario!G56 | 값: 254.01
- Scenario!B57 | 값: Base
- Scenario!C57 | 값: -13.3
- Scenario!D57 | 값: -53.28
- Scenario!E57 | 값: -67.39
- Scenario!F57 | 값: -59.75
- Scenario!G57 | 값: -18.54
- Scenario!B58 | 값: Bull
- Scenario!C58 | 값: -13.3
- Scenario!D58 | 값: -118.37
- Scenario!E58 | 값: -228.22
- Scenario!F58 | 값: -321.23
- Scenario!G58 | 값: -356.09
- Scenario!A59 | 값: 지표 (헤더 행 — 남색 채우기)
- Scenario!B59 | 값: 케이스 (헤더 행 — 남색 채우기)
- Scenario!C59 | 값: 2026E (헤더 행 — 남색 채우기)
- Scenario!D59 | 값: 2027E (헤더 행 — 남색 채우기)
- Scenario!E59 | 값: 2028E (헤더 행 — 남색 채우기)
- Scenario!F59 | 값: 2029E (헤더 행 — 남색 채우기)
- Scenario!G59 | 값: 2030E (헤더 행 — 남색 채우기)
- Scenario!A60 | 값: 빅4 추가 조달 필요액 ($B)
- Scenario!B60 | 값: Bear
- Scenario!C60 | 값: 0
- Scenario!D60 | 값: 0
- Scenario!E60 | 값: 0
...
- Scenario!C61 | 값: 0
- Scenario!D61 | 값: 0.86
- Scenario!E61 | 값: 29.65
- Scenario!F61 | 값: 28.28
- Scenario!G61 | 값: 18.16
- Scenario!B62 | 값: Bull
- Scenario!C62 | 값: 0
- Scenario!D62 | 값: 14.32
- Scenario!E62 | 값: 62.68
- Scenario!F62 | 값: 82.45
- Scenario!G62 | 값: 219.14
- Scenario!A63 | 값: 지표 (헤더 행 — 남색 채우기)
- Scenario!B63 | 값: 케이스 (헤더 행 — 남색 채우기)
- Scenario!C63 | 값: 2026E (헤더 행 — 남색 채우기)
- Scenario!D63 | 값: 2027E (헤더 행 — 남색 채우기)
- Scenario!E63 | 값: 2028E (헤더 행 — 남색 채우기)
- Scenario!F63 | 값: 2029E (헤더 행 — 남색 채우기)
- Scenario!G63 | 값: 2030E (헤더 행 — 남색 채우기)
- Scenario!A64 | 값: AI 가속기 대수 (백만)
- Scenario!B64 | 값: Bear
- Scenario!C64 | 값: 12.69
- Scenario!D64 | 값: 14.2
- Scenario!E64 | 값: 15.63
- Scenario!F64 | 값: 16.62
- Scenario!G64 | 값: 17.88
- Scenario!B65 | 값: Base
- Scenario!C65 | 값: 14.38
- Scenario!D65 | 값: 17.49
- Scenario!E65 | 값: 21.11
- Scenario!F65 | 값: 24.22
- Scenario!G65 | 값: 27.35
- Scenario!B66 | 값: Bull
- Scenario!C66 | 값: 16.24
- Scenario!D66 | 값: 21.34
- Scenario!E66 | 값: 28.02
- Scenario!F66 | 값: 34.5
- Scenario!G66 | 값: 40.8
- Scenario!A67 | 값: 지표 (헤더 행 — 남색 채우기)
- Scenario!B67 | 값: 케이스 (헤더 행 — 남색 채우기)
- Scenario!C67 | 값: 2026E (헤더 행 — 남색 채우기)
- Scenario!D67 | 값: 2027E (헤더 행 — 남색 채우기)
- Scenario!E67 | 값: 2028E (헤더 행 — 남색 채우기)
- Scenario!F67 | 값: 2029E (헤더 행 — 남색 채우기)
- Scenario!G67 | 값: 2030E (헤더 행 — 남색 채우기)
- Scenario!A68 | 값: 빅4 HBM 수요 (EB)
- Scenario!B68 | 값: Bear
- Scenario!C68 | 값: 2.5
- Scenario!D68 | 값: 2.78
- Scenario!E68 | 값: 3.13
...
- Scenario!E70 | 값: 7.15
- Scenario!F70 | 값: 8.91
- Scenario!G70 | 값: 10.62
- Scenario!A71 | 값: 지표 (헤더 행 — 남색 채우기)
- Scenario!B71 | 값: 케이스 (헤더 행 — 남색 채우기)
- Scenario!C71 | 값: 2026E (헤더 행 — 남색 채우기)
- Scenario!D71 | 값: 2027E (헤더 행 — 남색 채우기)
- Scenario!E71 | 값: 2028E (헤더 행 — 남색 채우기)
- Scenario!F71 | 값: 2029E (헤더 행 — 남색 채우기)
- Scenario!G71 | 값: 2030E (헤더 행 — 남색 채우기)
- Scenario!A72 | 값: 빅4 HBM 금액 ($B)
- Scenario!B72 | 값: Bear
- Scenario!C72 | 값: 23.74
- Scenario!D72 | 값: 40.53
- Scenario!E72 | 값: 40.39
- Scenario!F72 | 값: 44.34
- Scenario!G72 | 값: 45.32
- Scenario!B73 | 값: Base
- Scenario!C73 | 값: 31.42
- Scenario!D73 | 값: 73.82
- Scenario!E73 | 값: 86.4
- Scenario!F73 | 값: 102.24
- Scenario!G73 | 값: 109.78
- Scenario!B74 | 값: Bull
- Scenario!C74 | 값: 40.95
- Scenario!D74 | 값: 148.4
- Scenario!E74 | 값: 184.34
- Scenario!F74 | 값: 217.24
- Scenario!G74 | 값: 225.67
- Scenario!A75 | 값: 지표 (헤더 행 — 남색 채우기)
- Scenario!B75 | 값: 케이스 (헤더 행 — 남색 채우기)
- Scenario!C75 | 값: 2026E (헤더 행 — 남색 채우기)
- Scenario!D75 | 값: 2027E (헤더 행 — 남색 채우기)
- Scenario!E75 | 값: 2028E (헤더 행 — 남색 채우기)
- Scenario!F75 | 값: 2029E (헤더 행 — 남색 채우기)
- Scenario!G75 | 값: 2030E (헤더 행 — 남색 채우기)
- Scenario!A76 | 값: 글로벌 HBM 시장 ($B)
- Scenario!B76 | 값: Bear
- Scenario!C76 | 값: 39.57
- Scenario!D76 | 값: 67.54
- Scenario!E76 | 값: 67.32
- Scenario!F76 | 값: 73.89
- Scenario!G76 | 값: 75.53
- Scenario!B77 | 값: Base
- Scenario!C77 | 값: 57.13
- Scenario!D77 | 값: 134.21
- Scenario!E77 | 값: 157.09
- Scenario!F77 | 값: 185.9
- Scenario!G77 | 값: 199.61
- Scenario!B78 | 값: Bull
- Scenario!C78 | 값: 81.9
- Scenario!D78 | 값: 296.8
- Scenario!E78 | 값: 368.67
- Scenario!F78 | 값: 434.47
- Scenario!G78 | 값: 451.33
- Scenario!A79 | 값: 지표 (헤더 행 — 남색 채우기)
- Scenario!B79 | 값: 케이스 (헤더 행 — 남색 채우기)
- Scenario!C79 | 값: 2026E (헤더 행 — 남색 채우기)
- Scenario!D79 | 값: 2027E (헤더 행 — 남색 채우기)
- Scenario!E79 | 값: 2028E (헤더 행 — 남색 채우기)
- Scenario!F79 | 값: 2029E (헤더 행 — 남색 채우기)
- Scenario!G79 | 값: 2030E (헤더 행 — 남색 채우기)
- Scenario!A80 | 값: SK hynix HBM 매출 ($B)
- Scenario!B80 | 값: Bear
- Scenario!C80 | 값: 21.77
- Scenario!D80 | 값: 37.15
- Scenario!E80 | 값: 35.68
- Scenario!F80 | 값: 38.43
- Scenario!G80 | 값: 38.52
- Scenario!B81 | 값: Base
- Scenario!C81 | 값: 28.56
- Scenario!D81 | 값: 63.08
- Scenario!E81 | 값: 70.69
- Scenario!F81 | 값: 81.79
- Scenario!G81 | 값: 85.83
- Scenario!B82 | 값: Bull
- Scenario!C82 | 값: 36.85
- Scenario!D82 | 값: 115.75
- Scenario!E82 | 값: 129.04
- Scenario!F82 | 값: 147.72
- Scenario!G82 | 값: 148.94
- Scenario!A83 | 값: 지표 (헤더 행 — 남색 채우기)
- Scenario!B83 | 값: 케이스 (헤더 행 — 남색 채우기)
- Scenario!C83 | 값: 2026E (헤더 행 — 남색 채우기)
- Scenario!D83 | 값: 2027E (헤더 행 — 남색 채우기)
- Scenario!E83 | 값: 2028E (헤더 행 — 남색 채우기)
- Scenario!F83 | 값: 2029E (헤더 행 — 남색 채우기)
- Scenario!G83 | 값: 2030E (헤더 행 — 남색 채우기)
- Scenario!A84 | 값: Samsung HBM 매출 ($B)
- Scenario!B84 | 값: Bear
- Scenario!C84 | 값: 10.69
- Scenario!D84 | 값: 17.56
- Scenario!E84 | 값: 18.18
- Scenario!F84 | 값: 20.69
- Scenario!G84 | 값: 21.15
- Scenario!B85 | 값: Base
- Scenario!C85 | 값: 18.28
- Scenario!D85 | 값: 45.63
- Scenario!E85 | 값: 54.98
- Scenario!F85 | 값: 66.92
- Scenario!G85 | 값: 71.86
- Scenario!B86 | 값: Bull
- Scenario!C86 | 값: 30.3
- Scenario!D86 | 값: 124.66
- Scenario!E86 | 값: 165.9
- Scenario!F86 | 값: 199.86
- Scenario!G86 | 값: 207.61
- Scenario!A87 | 값: 지표 (헤더 행 — 남색 채우기)
- Scenario!B87 | 값: 케이스 (헤더 행 — 남색 채우기)
- Scenario!C87 | 값: 2026E (헤더 행 — 남색 채우기)
- Scenario!D87 | 값: 2027E (헤더 행 — 남색 채우기)
- Scenario!E87 | 값: 2028E (헤더 행 — 남색 채우기)
- Scenario!F87 | 값: 2029E (헤더 행 — 남색 채우기)
- Scenario!G87 | 값: 2030E (헤더 행 — 남색 채우기)
- Scenario!A88 | 값: Micron HBM 매출 ($B)
- Scenario!B88 | 값: Bear
- Scenario!C88 | 값: 7.12
- Scenario!D88 | 값: 12.83
- Scenario!E88 | 값: 13.46
- Scenario!F88 | 값: 14.78
- Scenario!G88 | 값: 15.86
- Scenario!B89 | 값: Base
- Scenario!C89 | 값: 10.28
- Scenario!D89 | 값: 25.5
- Scenario!E89 | 값: 31.42
- Scenario!F89 | 값: 37.18
- Scenario!G89 | 값: 41.92
- Scenario!B90 | 값: Bull
- Scenario!C90 | 값: 14.74
- Scenario!D90 | 값: 56.39
- Scenario!E90 | 값: 73.73
- Scenario!F90 | 값: 86.89
- Scenario!G90 | 값: 94.78


## 시트 `Consolidated` (42행 × 8열, 범위 A1:H42)

- 연도 헤더: 행 4: B4=2025A, C4=2026E, D4=2027E, E4=2028E, F4=2029E, G4=2030E
...
- Consolidated!A3 | 수식: `="현재 시나리오: "&Scenario!C4` | 계산값: 현재 시나리오: Base
...
## 시트 `MSFT` (44행 × 8열, 범위 A1:H44)

- 연도 헤더: 행 5: B5=2025A, C5=2026E, D5=2027E, E5=2028E, F5=2029E, G5=2030E

- 노란 입력 셀(총 52개): C7, D7, E7, F7, G7, B8, B9, C9, D9, E9, F9, G9, D13, E13, F13, G13, B14, C14, B18, C18, D18, E18, F18, G18, B21, C21, D21, E21, F21, G21, B26, C28, D28, E28, F28, G28, C31, D31, E31, F31, G31, C32, D32, E32, F32, G32, B34, B39, D44, E44, F44, G44
...
- MSFT!D13 | 수식: `=D44+Scenario!D11` | 계산값: 0.25 (입력 셀 — 노란 채우기)
- MSFT!E13 | 수식: `=E44+Scenario!E11` | 계산값: 0.15 (입력 셀 — 노란 채우기)
- MSFT!F13 | 수식: `=F44+Scenario!F11` | 계산값: 0.1 (입력 셀 — 노란 채우기)
- MSFT!G13 | 수식: `=G44+Scenario!G11` | 계산값: 0.05 (입력 셀 — 노란 채우기)
- MSFT!H13 | 값: 44행 기본값 + Scenario 조정(%p)
...
- MSFT!A44 | 값: Capex 성장률 기본값 (Base 입력)
- MSFT!D44 | 값: 0.25 (입력 셀 — 노란 채우기)
- MSFT!E44 | 값: 0.15 (입력 셀 — 노란 채우기)
- MSFT!F44 | 값: 0.1 (입력 셀 — 노란 채우기)
- MSFT!G44 | 값: 0.05 (입력 셀 — 노란 채우기)
- MSFT!H44 | 값: 2027E~ capex 성장률 기본 가정 (시나리오 조정은 Scenario 시트)


## 시트 `GOOGL` (44행 × 8열, 범위 A1:H44)
...
- GOOGL!D13 | 수식: `=D44+Scenario!D11` | 계산값: 0.3 (입력 셀 — 노란 채우기)
- GOOGL!E13 | 수식: `=E44+Scenario!E11` | 계산값: 0.15 (입력 셀 — 노란 채우기)
- GOOGL!F13 | 수식: `=F44+Scenario!F11` | 계산값: 0.1 (입력 셀 — 노란 채우기)
- GOOGL!G13 | 수식: `=G44+Scenario!G11` | 계산값: 0.05 (입력 셀 — 노란 채우기)
- GOOGL!H13 | 값: 44행 기본값 + Scenario 조정(%p)
...
- GOOGL!A44 | 값: Capex 성장률 기본값 (Base 입력)
- GOOGL!D44 | 값: 0.3 (입력 셀 — 노란 채우기)
- GOOGL!E44 | 값: 0.15 (입력 셀 — 노란 채우기)
- GOOGL!F44 | 값: 0.1 (입력 셀 — 노란 채우기)
- GOOGL!G44 | 값: 0.05 (입력 셀 — 노란 채우기)
- GOOGL!H44 | 값: 2027E~ capex 성장률 기본 가정 (시나리오 조정은 Scenario 시트)


## 시트 `AMZN` (44행 × 8열, 범위 A1:H44)
...
- AMZN!D13 | 수식: `=D44+Scenario!D11` | 계산값: 0.2 (입력 셀 — 노란 채우기)
- AMZN!E13 | 수식: `=E44+Scenario!E11` | 계산값: 0.12 (입력 셀 — 노란 채우기)
- AMZN!F13 | 수식: `=F44+Scenario!F11` | 계산값: 0.08 (입력 셀 — 노란 채우기)
- AMZN!G13 | 수식: `=G44+Scenario!G11` | 계산값: 0.05 (입력 셀 — 노란 채우기)
- AMZN!H13 | 값: 44행 기본값 + Scenario 조정(%p)
...
- AMZN!A44 | 값: Capex 성장률 기본값 (Base 입력)
- AMZN!D44 | 값: 0.2 (입력 셀 — 노란 채우기)
- AMZN!E44 | 값: 0.12 (입력 셀 — 노란 채우기)
- AMZN!F44 | 값: 0.08 (입력 셀 — 노란 채우기)
- AMZN!G44 | 값: 0.05 (입력 셀 — 노란 채우기)
- AMZN!H44 | 값: 2027E~ capex 성장률 기본 가정 (시나리오 조정은 Scenario 시트)


## 시트 `META` (44행 × 8열, 범위 A1:H44)
...
- META!D13 | 수식: `=D44+Scenario!D11` | 계산값: 0.2 (입력 셀 — 노란 채우기)
- META!E13 | 수식: `=E44+Scenario!E11` | 계산값: 0.12 (입력 셀 — 노란 채우기)
- META!F13 | 수식: `=F44+Scenario!F11` | 계산값: 0.08 (입력 셀 — 노란 채우기)
- META!G13 | 수식: `=G44+Scenario!G11` | 계산값: 0.05 (입력 셀 — 노란 채우기)
- META!H13 | 값: 44행 기본값 + Scenario 조정(%p)
...
- META!A44 | 값: Capex 성장률 기본값 (Base 입력)
- META!D44 | 값: 0.2 (입력 셀 — 노란 채우기)
- META!E44 | 값: 0.12 (입력 셀 — 노란 채우기)
- META!F44 | 값: 0.08 (입력 셀 — 노란 채우기)
- META!G44 | 값: 0.05 (입력 셀 — 노란 채우기)
- META!H44 | 값: 2027E~ capex 성장률 기본 가정 (시나리오 조정은 Scenario 시트)


## 시트 `HBM_Inputs` (77행 × 12열, 범위 A1:L77)

- 연도 헤더: 행 15: C15=2026E, D15=2027E, E15=2028E, F15=2029E, G15=2030E / 행 23: C23=2026E, D23=2027E, E23=2028E, F23=2029E, G23=2030E / 행 41: C41=2026E, D41=2027E, E41=2028E, F41=2029E, G41=2030E / 행 50: C50=2026E, D50=2027E, E50=2028E, F50=2029E, G50=2030E / 행 59: C59=2026E, D59=2027E, E59=2028E, F59=2029E, G59=2030E / 행 68: C68=2026E, D68=2027E, E68=2028E, F68=2029E, G68=2030E
...
- 연도 헤더: 행 4: C4=2026E, D4=2027E, E4=2028E, F4=2029E, G4=2030E / 행 40: C40=2026E, D40=2027E, E40=2028E, F40=2029E, G40=2030E / 행 72: C72=2026E, D72=2027E, E72=2028E, F72=2029E, G72=2030E / 행 104: C104=2026E, D104=2027E, E104=2028E, F104=2029E, G104=2030E / 행 136: C136=2026E, D136=2027E, E136=2028E, F136=2029E, G136=2030E
...
- HBM_Demand!A3 | 수식: `="현재 시나리오: "&Scenario!C4` | 계산값: 현재 시나리오: Base
...
- HBM_Demand!C25 | 수식: `=IF((HBM_Inputs!C31+Scenario!C$41)=0,0,C11/(HBM_Inputs!C31+Scenario!C$41))` | 계산값: 57.1285066666667
- HBM_Demand!D25 | 수식: `=IF((HBM_Inputs!D31+Scenario!D$41)=0,0,D11/(HBM_Inputs!D31+Scenario!D$41))` | 계산값: 134.210685000258
- HBM_Demand!E25 | 수식: `=IF((HBM_Inputs!E31+Scenario!E$41)=0,0,E11/(HBM_Inputs!E31+Scenario!E$41))` | 계산값: 157.088977962308
- HBM_Demand!F25 | 수식: `=IF((HBM_Inputs!F31+Scenario!F$41)=0,0,F11/(HBM_Inputs!F31+Scenario!F$41))` | 계산값: 185.897169002455
- HBM_Demand!G25 | 수식: `=IF((HBM_Inputs!G31+Scenario!G$41)=0,0,G11/(HBM_Inputs!G31+Scenario!G$41))` | 계산값: 199.607921268707
...
- HBM_Demand!C27 | 수식: `=IF((HBM_Inputs!C31+Scenario!C$41)=0,0,C8/(HBM_Inputs!C31+Scenario!C$41))` | 계산값: 5.41687175757576
- HBM_Demand!D27 | 수식: `=IF((HBM_Inputs!D31+Scenario!D$41)=0,0,D8/(HBM_Inputs!D31+Scenario!D$41))` | 계산값: 6.90582562849774
- HBM_Demand!E27 | 수식: `=IF((HBM_Inputs!E31+Scenario!E$41)=0,0,E8/(HBM_Inputs!E31+Scenario!E$41))` | 계산값: 8.52420491169569
- HBM_Demand!F27 | 수식: `=IF((HBM_Inputs!F31+Scenario!F$41)=0,0,F8/(HBM_Inputs!F31+Scenario!F$41))` | 계산값: 9.9011297305052
- HBM_Demand!G27 | 수식: `=IF((HBM_Inputs!G31+Scenario!G$41)=0,0,G8/(HBM_Inputs!G31+Scenario!G$41))` | 계산값: 11.2674412317305
- HBM_Demand!A28 | 값: SK hynix HBM 매출 ($B)
- HBM_Demand!C28 | 수식: `=C25*(HBM_Inputs!C24-Scenario!C$47)` | 계산값: 28.5642533333333
- HBM_Demand!D28 | 수식: `=D25*(HBM_Inputs!D24-Scenario!D$47)` | 계산값: 63.0790219501212
- HBM_Demand!E28 | 수식: `=E25*(HBM_Inputs!E24-Scenario!E$47)` | 계산값: 70.6900400830387
- HBM_Demand!F28 | 수식: `=F25*(HBM_Inputs!F24-Scenario!F$47)` | 계산값: 81.7947543610803
- HBM_Demand!G28 | 수식: `=G25*(HBM_Inputs!G24-Scenario!G$47)` | 계산값: 85.8314061455442
- HBM_Demand!A29 | 값: Samsung HBM 매출 ($B)
- HBM_Demand!C29 | 수식: `=C25*(HBM_Inputs!C25+Scenario!C$47)` | 계산값: 18.2811221333333
- HBM_Demand!D29 | 수식: `=D25*(HBM_Inputs!D25+Scenario!D$47)` | 계산값: 45.6316329000877
- HBM_Demand!E29 | 수식: `=E25*(HBM_Inputs!E25+Scenario!E$47)` | 계산값: 54.9811422868079
- HBM_Demand!F29 | 수식: `=F25*(HBM_Inputs!F25+Scenario!F$47)` | 계산값: 66.9229808408839
- HBM_Demand!G29 | 수식: `=G25*(HBM_Inputs!G25+Scenario!G$47)` | 계산값: 71.8588516567347
...
- HBM_Demand!C32 | 수식: `=C11*(HBM_Inputs!C24-Scenario!C$47)` | 계산값: 15.7103393333333
- HBM_Demand!D32 | 수식: `=D11*(HBM_Inputs!D24-Scenario!D$47)` | 계산값: 34.6934620725667
- HBM_Demand!E32 | 수식: `=E11*(HBM_Inputs!E24-Scenario!E$47)` | 계산값: 38.8795220456713
- HBM_Demand!F32 | 수식: `=F11*(HBM_Inputs!F24-Scenario!F$47)` | 계산값: 44.9871148985942
- HBM_Demand!G32 | 수식: `=G11*(HBM_Inputs!G24-Scenario!G$47)` | 계산값: 47.2072733800493
- HBM_Demand!A33 | 값:   Samsung — 빅4 향 ($B)
- HBM_Demand!C33 | 수식: `=C11*(HBM_Inputs!C25+Scenario!C$47)` | 계산값: 10.0546171733333
- HBM_Demand!D33 | 수식: `=D11*(HBM_Inputs!D25+Scenario!D$47)` | 계산값: 25.0973980950482
- HBM_Demand!E33 | 수식: `=E11*(HBM_Inputs!E25+Scenario!E$47)` | 계산값: 30.2396282577443
- HBM_Demand!F33 | 수식: `=F11*(HBM_Inputs!F25+Scenario!F$47)` | 계산값: 36.8076394624861
- HBM_Demand!G33 | 수식: `=G11*(HBM_Inputs!G25+Scenario!G$47)` | 계산값: 39.5223684112041
...
- HBM_Demand!A37 | 값: 시나리오: Scenario 시트 B4 선택값에 따라 가속기 비중·ASP·GB/칩·$/GB·빅4 비중·삼성 점유율이 조정됨 (HBM_Inputs 값은 Base 기준).
...
- HBM_Demand!C42 | 수식: `=C41*(HBM_Inputs!$B$35+Scenario!C$17)` | 계산값: 82.075
- HBM_Demand!D42 | 수식: `=D41*(HBM_Inputs!$B$35+Scenario!D$17)` | 계산값: 102.59375
- HBM_Demand!E42 | 수식: `=E41*(HBM_Inputs!$B$35+Scenario!E$17)` | 계산값: 114.4609375
- HBM_Demand!F42 | 수식: `=F41*(HBM_Inputs!$B$35+Scenario!F$17)` | 계산값: 125.90703125
- HBM_Demand!G42 | 수식: `=G41*(HBM_Inputs!$B$35+Scenario!G$17)` | 계산값: 132.2023828125
- HBM_Demand!A43 | 값: 가속기 대수 (백만 개)
- HBM_Demand!A44 | 값:   NVIDIA Blackwell (B200/B300)
- HBM_Demand!C44 | 수식: `=IF(HBM_Inputs!C6=0,0,C42*HBM_Inputs!C42/(HBM_Inputs!C6*Scenario!C$35))` | 계산값: 1.231125
- HBM_Demand!D44 | 수식: `=IF(HBM_Inputs!D6=0,0,D42*HBM_Inputs!D42/(HBM_Inputs!D6*Scenario!D$35))` | 계산값: 0.712456597222222
- HBM_Demand!E44 | 수식: `=IF(HBM_Inputs!E6=0,0,E42*HBM_Inputs!E42/(HBM_Inputs!E6*Scenario!E$35))` | 계산값: 0.17884521484375
- HBM_Demand!F44 | 수식: `=IF(HBM_Inputs!F6=0,0,F42*HBM_Inputs!F42/(HBM_Inputs!F6*Scenario!F$35))` | 계산값: 0
- HBM_Demand!G44 | 수식: `=IF(HBM_Inputs!G6=0,0,G42*HBM_Inputs!G42/(HBM_Inputs!G6*Scenario!G$35))` | 계산값: 0
- HBM_Demand!A45 | 값:   NVIDIA Rubin (VR200)
- HBM_Demand!C45 | 수식: `=IF(HBM_Inputs!C7=0,0,C42*HBM_Inputs!C43/(HBM_Inputs!C7*Scenario!C$35))` | 계산값: 0.410375
- HBM_Demand!D45 | 수식: `=IF(HBM_Inputs!D7=0,0,D42*HBM_Inputs!D43/(HBM_Inputs!D7*Scenario!D$35))` | 계산값: 0.982280585106383
- HBM_Demand!E45 | 수식: `=IF(HBM_Inputs!E7=0,0,E42*HBM_Inputs!E43/(HBM_Inputs!E7*Scenario!E$35))` | 계산값: 1.04055397727273
- HBM_Demand!F45 | 수식: `=IF(HBM_Inputs!F7=0,0,F42*HBM_Inputs!F43/(HBM_Inputs!F7*Scenario!F$35))` | 계산값: 0.767725800304878
- HBM_Demand!G45 | 수식: `=IF(HBM_Inputs!G7=0,0,G42*HBM_Inputs!G43/(HBM_Inputs!G7*Scenario!G$35))` | 계산값: 0.521851511101974
- HBM_Demand!A46 | 값:   NVIDIA Rubin Ultra / Feynman
- HBM_Demand!C46 | 수식: `=IF(HBM_Inputs!C8=0,0,C42*HBM_Inputs!C44/(HBM_Inputs!C8*Scenario!C$35))` | 계산값: 0
- HBM_Demand!D46 | 수식: `=IF(HBM_Inputs!D8=0,0,D42*HBM_Inputs!D44/(HBM_Inputs!D8*Scenario!D$35))` | 계산값: 0.256484375
- HBM_Demand!E46 | 수식: `=IF(HBM_Inputs!E8=0,0,E42*HBM_Inputs!E44/(HBM_Inputs!E8*Scenario!E$35))` | 계산값: 0.715380859375
- HBM_Demand!F46 | 수식: `=IF(HBM_Inputs!F8=0,0,F42*HBM_Inputs!F44/(HBM_Inputs!F8*Scenario!F$35))` | 계산값: 1.089580078125
- HBM_Demand!G46 | 수식: `=IF(HBM_Inputs!G8=0,0,G42*HBM_Inputs!G44/(HBM_Inputs!G8*Scenario!G$35))` | 계산값: 1.37710815429687
- HBM_Demand!A47 | 값:   Google TPU (v7 Ironwood~)
- HBM_Demand!C47 | 수식: `=IF(HBM_Inputs!C9=0,0,C42*HBM_Inputs!C45/(HBM_Inputs!C9*Scenario!C$35))` | 계산값: 0
- HBM_Demand!D47 | 수식: `=IF(HBM_Inputs!D9=0,0,D42*HBM_Inputs!D45/(HBM_Inputs!D9*Scenario!D$35))` | 계산값: 0
- HBM_Demand!E47 | 수식: `=IF(HBM_Inputs!E9=0,0,E42*HBM_Inputs!E45/(HBM_Inputs!E9*Scenario!E$35))` | 계산값: 0
- HBM_Demand!F47 | 수식: `=IF(HBM_Inputs!F9=0,0,F42*HBM_Inputs!F45/(HBM_Inputs!F9*Scenario!F$35))` | 계산값: 0
- HBM_Demand!G47 | 수식: `=IF(HBM_Inputs!G9=0,0,G42*HBM_Inputs!G45/(HBM_Inputs!G9*Scenario!G$35))` | 계산값: 0
- HBM_Demand!A48 | 값:   AWS Trainium (2/3)
- HBM_Demand!C48 | 수식: `=IF(HBM_Inputs!C10=0,0,C42*HBM_Inputs!C46/(HBM_Inputs!C10*Scenario!C$35))` | 계산값: 0
- HBM_Demand!D48 | 수식: `=IF(HBM_Inputs!D10=0,0,D42*HBM_Inputs!D46/(HBM_Inputs!D10*Scenario!D$35))` | 계산값: 0
- HBM_Demand!E48 | 수식: `=IF(HBM_Inputs!E10=0,0,E42*HBM_Inputs!E46/(HBM_Inputs!E10*Scenario!E$35))` | 계산값: 0
- HBM_Demand!F48 | 수식: `=IF(HBM_Inputs!F10=0,0,F42*HBM_Inputs!F46/(HBM_Inputs!F10*Scenario!F$35))` | 계산값: 0
- HBM_Demand!G48 | 수식: `=IF(HBM_Inputs!G10=0,0,G42*HBM_Inputs!G46/(HBM_Inputs!G10*Scenario!G$35))` | 계산값: 0
- HBM_Demand!A49 | 값:   기타 (AMD MI/ MTIA / Maia)
- HBM_Demand!C49 | 수식: `=IF(HBM_Inputs!C11=0,0,C42*HBM_Inputs!C47/(HBM_Inputs!C11*Scenario!C$35))` | 계산값: 0.410375
- HBM_Demand!D49 | 수식: `=IF(HBM_Inputs!D11=0,0,D42*HBM_Inputs!D47/(HBM_Inputs!D11*Scenario!D$35))` | 계산값: 0.51296875
- HBM_Demand!E49 | 수식: `=IF(HBM_Inputs!E11=0,0,E42*HBM_Inputs!E47/(HBM_Inputs!E11*Scenario!E$35))` | 계산값: 0.817578125
- HBM_Demand!F49 | 수식: `=IF(HBM_Inputs!F11=0,0,F42*HBM_Inputs!F47/(HBM_Inputs!F11*Scenario!F$35))` | 계산값: 1.4527734375
- HBM_Demand!G49 | 수식: `=IF(HBM_Inputs!G11=0,0,G42*HBM_Inputs!G47/(HBM_Inputs!G11*Scenario!G$35))` | 계산값: 1.92795141601562
- HBM_Demand!A50 | 값:   소계
...
- HBM_Demand!C52 | 수식: `=C44*HBM_Inputs!$B$6*Scenario!C$29/1000` | 계산값: 0.29547
- HBM_Demand!D52 | 수식: `=D44*HBM_Inputs!$B$6*Scenario!D$29/1000` | 계산값: 0.170989583333333
- HBM_Demand!E52 | 수식: `=E44*HBM_Inputs!$B$6*Scenario!E$29/1000` | 계산값: 0.0429228515625
- HBM_Demand!F52 | 수식: `=F44*HBM_Inputs!$B$6*Scenario!F$29/1000` | 계산값: 0
- HBM_Demand!G52 | 수식: `=G44*HBM_Inputs!$B$6*Scenario!G$29/1000` | 계산값: 0
- HBM_Demand!A53 | 값:   NVIDIA Rubin (VR200)
- HBM_Demand!C53 | 수식: `=C45*HBM_Inputs!$B$7*Scenario!C$29/1000` | 계산값: 0.118188
- HBM_Demand!D53 | 수식: `=D45*HBM_Inputs!$B$7*Scenario!D$29/1000` | 계산값: 0.282896808510638
- HBM_Demand!E53 | 수식: `=E45*HBM_Inputs!$B$7*Scenario!E$29/1000` | 계산값: 0.299679545454545
- HBM_Demand!F53 | 수식: `=F45*HBM_Inputs!$B$7*Scenario!F$29/1000` | 계산값: 0.221105030487805
- HBM_Demand!G53 | 수식: `=G45*HBM_Inputs!$B$7*Scenario!G$29/1000` | 계산값: 0.150293235197368
- HBM_Demand!A54 | 값:   NVIDIA Rubin Ultra / Feynman
- HBM_Demand!C54 | 수식: `=C46*HBM_Inputs!$B$8*Scenario!C$29/1000` | 계산값: 0
- HBM_Demand!D54 | 수식: `=D46*HBM_Inputs!$B$8*Scenario!D$29/1000` | 계산값: 0.09849
- HBM_Demand!E54 | 수식: `=E46*HBM_Inputs!$B$8*Scenario!E$29/1000` | 계산값: 0.27470625
- HBM_Demand!F54 | 수식: `=F46*HBM_Inputs!$B$8*Scenario!F$29/1000` | 계산값: 0.41839875
- HBM_Demand!G54 | 수식: `=G46*HBM_Inputs!$B$8*Scenario!G$29/1000` | 계산값: 0.52880953125
- HBM_Demand!A55 | 값:   Google TPU (v7 Ironwood~)
- HBM_Demand!C55 | 수식: `=C47*HBM_Inputs!$B$9*Scenario!C$29/1000` | 계산값: 0
- HBM_Demand!D55 | 수식: `=D47*HBM_Inputs!$B$9*Scenario!D$29/1000` | 계산값: 0
- HBM_Demand!E55 | 수식: `=E47*HBM_Inputs!$B$9*Scenario!E$29/1000` | 계산값: 0
- HBM_Demand!F55 | 수식: `=F47*HBM_Inputs!$B$9*Scenario!F$29/1000` | 계산값: 0
- HBM_Demand!G55 | 수식: `=G47*HBM_Inputs!$B$9*Scenario!G$29/1000` | 계산값: 0
- HBM_Demand!A56 | 값:   AWS Trainium (2/3)
- HBM_Demand!C56 | 수식: `=C48*HBM_Inputs!$B$10*Scenario!C$29/1000` | 계산값: 0
- HBM_Demand!D56 | 수식: `=D48*HBM_Inputs!$B$10*Scenario!D$29/1000` | 계산값: 0
- HBM_Demand!E56 | 수식: `=E48*HBM_Inputs!$B$10*Scenario!E$29/1000` | 계산값: 0
- HBM_Demand!F56 | 수식: `=F48*HBM_Inputs!$B$10*Scenario!F$29/1000` | 계산값: 0
- HBM_Demand!G56 | 수식: `=G48*HBM_Inputs!$B$10*Scenario!G$29/1000` | 계산값: 0
- HBM_Demand!A57 | 값:   기타 (AMD MI/ MTIA / Maia)
- HBM_Demand!C57 | 수식: `=C49*HBM_Inputs!$B$11*Scenario!C$29/1000` | 계산값: 0.105056
- HBM_Demand!D57 | 수식: `=D49*HBM_Inputs!$B$11*Scenario!D$29/1000` | 계산값: 0.13132
- HBM_Demand!E57 | 수식: `=E49*HBM_Inputs!$B$11*Scenario!E$29/1000` | 계산값: 0.2093
- HBM_Demand!F57 | 수식: `=F49*HBM_Inputs!$B$11*Scenario!F$29/1000` | 계산값: 0.37191
- HBM_Demand!G57 | 수식: `=G49*HBM_Inputs!$B$11*Scenario!G$29/1000` | 계산값: 0.4935555625
- HBM_Demand!A58 | 값:   소계
...
- HBM_Demand!C60 | 수식: `=IFERROR(C52*INDEX(HBM_Inputs!C$16:C$18,MATCH(HBM_Inputs!H6,HBM_Inputs!$A$16:$A$18,0))*Scenario!C$23,0)` | 계산값: 2.9547
- HBM_Demand!D60 | 수식: `=IFERROR(D52*INDEX(HBM_Inputs!D$16:D$18,MATCH(HBM_Inputs!I6,HBM_Inputs!$A$16:$A$18,0))*Scenario!D$23,0)` | 계산값: 2.22286458333333
- HBM_Demand!E60 | 수식: `=IFERROR(E52*INDEX(HBM_Inputs!E$16:E$18,MATCH(HBM_Inputs!J6,HBM_Inputs!$A$16:$A$18,0))*Scenario!E$23,0)` | 계산값: 0.51507421875
- HBM_Demand!F60 | 수식: `=IFERROR(F52*INDEX(HBM_Inputs!F$16:F$18,MATCH(HBM_Inputs!K6,HBM_Inputs!$A$16:$A$18,0))*Scenario!F$23,0)` | 계산값: 0
- HBM_Demand!G60 | 수식: `=IFERROR(G52*INDEX(HBM_Inputs!G$16:G$18,MATCH(HBM_Inputs!L6,HBM_Inputs!$A$16:$A$18,0))*Scenario!G$23,0)` | 계산값: 0
- HBM_Demand!A61 | 값:   NVIDIA Rubin (VR200)
- HBM_Demand!C61 | 수식: `=IFERROR(C53*INDEX(HBM_Inputs!C$16:C$18,MATCH(HBM_Inputs!H7,HBM_Inputs!$A$16:$A$18,0))*Scenario!C$23,0)` | 계산값: 1.77282
- HBM_Demand!D61 | 수식: `=IFERROR(D53*INDEX(HBM_Inputs!D$16:D$18,MATCH(HBM_Inputs!I7,HBM_Inputs!$A$16:$A$18,0))*Scenario!D$23,0)` | 계산값: 5.65793617021277
- HBM_Demand!E61 | 수식: `=IFERROR(E53*INDEX(HBM_Inputs!E$16:E$18,MATCH(HBM_Inputs!J7,HBM_Inputs!$A$16:$A$18,0))*Scenario!E$23,0)` | 계산값: 5.39423181818182
- HBM_Demand!F61 | 수식: `=IFERROR(F53*INDEX(HBM_Inputs!F$16:F$18,MATCH(HBM_Inputs!K7,HBM_Inputs!$A$16:$A$18,0))*Scenario!F$23,0)` | 계산값: 3.53768048780488
- HBM_Demand!G61 | 수식: `=IFERROR(G53*INDEX(HBM_Inputs!G$16:G$18,MATCH(HBM_Inputs!L7,HBM_Inputs!$A$16:$A$18,0))*Scenario!G$23,0)` | 계산값: 2.10410529276316
- HBM_Demand!A62 | 값:   NVIDIA Rubin Ultra / Feynman
- HBM_Demand!C62 | 수식: `=IFERROR(C54*INDEX(HBM_Inputs!C$16:C$18,MATCH(HBM_Inputs!H8,HBM_Inputs!$A$16:$A$18,0))*Scenario!C$23,0)` | 계산값: 0
- HBM_Demand!D62 | 수식: `=IFERROR(D54*INDEX(HBM_Inputs!D$16:D$18,MATCH(HBM_Inputs!I8,HBM_Inputs!$A$16:$A$18,0))*Scenario!D$23,0)` | 계산값: 2.36376
- HBM_Demand!E62 | 수식: `=IFERROR(E54*INDEX(HBM_Inputs!E$16:E$18,MATCH(HBM_Inputs!J8,HBM_Inputs!$A$16:$A$18,0))*Scenario!E$23,0)` | 계산값: 6.0435375
- HBM_Demand!F62 | 수식: `=IFERROR(F54*INDEX(HBM_Inputs!F$16:F$18,MATCH(HBM_Inputs!K8,HBM_Inputs!$A$16:$A$18,0))*Scenario!F$23,0)` | 계산값: 8.367975
- HBM_Demand!G62 | 수식: `=IFERROR(G54*INDEX(HBM_Inputs!G$16:G$18,MATCH(HBM_Inputs!L8,HBM_Inputs!$A$16:$A$18,0))*Scenario!G$23,0)` | 계산값: 9.5185715625
- HBM_Demand!A63 | 값:   Google TPU (v7 Ironwood~)
- HBM_Demand!C63 | 수식: `=IFERROR(C55*INDEX(HBM_Inputs!C$16:C$18,MATCH(HBM_Inputs!H9,HBM_Inputs!$A$16:$A$18,0))*Scenario!C$23,0)` | 계산값: 0
- HBM_Demand!D63 | 수식: `=IFERROR(D55*INDEX(HBM_Inputs!D$16:D$18,MATCH(HBM_Inputs!I9,HBM_Inputs!$A$16:$A$18,0))*Scenario!D$23,0)` | 계산값: 0
- HBM_Demand!E63 | 수식: `=IFERROR(E55*INDEX(HBM_Inputs!E$16:E$18,MATCH(HBM_Inputs!J9,HBM_Inputs!$A$16:$A$18,0))*Scenario!E$23,0)` | 계산값: 0
- HBM_Demand!F63 | 수식: `=IFERROR(F55*INDEX(HBM_Inputs!F$16:F$18,MATCH(HBM_Inputs!K9,HBM_Inputs!$A$16:$A$18,0))*Scenario!F$23,0)` | 계산값: 0
- HBM_Demand!G63 | 수식: `=IFERROR(G55*INDEX(HBM_Inputs!G$16:G$18,MATCH(HBM_Inputs!L9,HBM_Inputs!$A$16:$A$18,0))*Scenario!G$23,0)` | 계산값: 0
- HBM_Demand!A64 | 값:   AWS Trainium (2/3)
- HBM_Demand!C64 | 수식: `=IFERROR(C56*INDEX(HBM_Inputs!C$16:C$18,MATCH(HBM_Inputs!H10,HBM_Inputs!$A$16:$A$18,0))*Scenario!C$23,0)` | 계산값: 0
- HBM_Demand!D64 | 수식: `=IFERROR(D56*INDEX(HBM_Inputs!D$16:D$18,MATCH(HBM_Inputs!I10,HBM_Inputs!$A$16:$A$18,0))*Scenario!D$23,0)` | 계산값: 0
- HBM_Demand!E64 | 수식: `=IFERROR(E56*INDEX(HBM_Inputs!E$16:E$18,MATCH(HBM_Inputs!J10,HBM_Inputs!$A$16:$A$18,0))*Scenario!E$23,0)` | 계산값: 0
- HBM_Demand!F64 | 수식: `=IFERROR(F56*INDEX(HBM_Inputs!F$16:F$18,MATCH(HBM_Inputs!K10,HBM_Inputs!$A$16:$A$18,0))*Scenario!F$23,0)` | 계산값: 0
- HBM_Demand!G64 | 수식: `=IFERROR(G56*INDEX(HBM_Inputs!G$16:G$18,MATCH(HBM_Inputs!L10,HBM_Inputs!$A$16:$A$18,0))*Scenario!G$23,0)` | 계산값: 0
- HBM_Demand!A65 | 값:   기타 (AMD MI/ MTIA / Maia)
- HBM_Demand!C65 | 수식: `=IFERROR(C57*INDEX(HBM_Inputs!C$16:C$18,MATCH(HBM_Inputs!H11,HBM_Inputs!$A$16:$A$18,0))*Scenario!C$23,0)` | 계산값: 1.05056
- HBM_Demand!D65 | 수식: `=IFERROR(D57*INDEX(HBM_Inputs!D$16:D$18,MATCH(HBM_Inputs!I11,HBM_Inputs!$A$16:$A$18,0))*Scenario!D$23,0)` | 계산값: 2.6264
- HBM_Demand!E65 | 수식: `=IFERROR(E57*INDEX(HBM_Inputs!E$16:E$18,MATCH(HBM_Inputs!J11,HBM_Inputs!$A$16:$A$18,0))*Scenario!E$23,0)` | 계산값: 3.7674
- HBM_Demand!F65 | 수식: `=IFERROR(F57*INDEX(HBM_Inputs!F$16:F$18,MATCH(HBM_Inputs!K11,HBM_Inputs!$A$16:$A$18,0))*Scenario!F$23,0)` | 계산값: 7.4382
- HBM_Demand!G65 | 수식: `=IFERROR(G57*INDEX(HBM_Inputs!G$16:G$18,MATCH(HBM_Inputs!L11,HBM_Inputs!$A$16:$A$18,0))*Scenario!G$23,0)` | 계산값: 8.884000125
- HBM_Demand!A66 | 값:   소계
...
- HBM_Demand!C74 | 수식: `=C73*(HBM_Inputs!$B$36+Scenario!C$17)` | 계산값: 90
- HBM_Demand!D74 | 수식: `=D73*(HBM_Inputs!$B$36+Scenario!D$17)` | 계산값: 117
- HBM_Demand!E74 | 수식: `=E73*(HBM_Inputs!$B$36+Scenario!E$17)` | 계산값: 134.55
- HBM_Demand!F74 | 수식: `=F73*(HBM_Inputs!$B$36+Scenario!F$17)` | 계산값: 148.005
- HBM_Demand!G74 | 수식: `=G73*(HBM_Inputs!$B$36+Scenario!G$17)` | 계산값: 155.40525
- HBM_Demand!A75 | 값: 가속기 대수 (백만 개)
- HBM_Demand!A76 | 값:   NVIDIA Blackwell (B200/B300)
- HBM_Demand!C76 | 수식: `=IF(HBM_Inputs!C6=0,0,C74*HBM_Inputs!C51/(HBM_Inputs!C6*Scenario!C$35))` | 계산값: 0.675
- HBM_Demand!D76 | 수식: `=IF(HBM_Inputs!D6=0,0,D74*HBM_Inputs!D51/(HBM_Inputs!D6*Scenario!D$35))` | 계산값: 0.325
- HBM_Demand!E76 | 수식: `=IF(HBM_Inputs!E6=0,0,E74*HBM_Inputs!E51/(HBM_Inputs!E6*Scenario!E$35))` | 계산값: 0.126140625
- HBM_Demand!F76 | 수식: `=IF(HBM_Inputs!F6=0,0,F74*HBM_Inputs!F51/(HBM_Inputs!F6*Scenario!F$35))` | 계산값: 0
- HBM_Demand!G76 | 수식: `=IF(HBM_Inputs!G6=0,0,G74*HBM_Inputs!G51/(HBM_Inputs!G6*Scenario!G$35))` | 계산값: 0
- HBM_Demand!A77 | 값:   NVIDIA Rubin (VR200)
- HBM_Demand!C77 | 수식: `=IF(HBM_Inputs!C7=0,0,C74*HBM_Inputs!C52/(HBM_Inputs!C7*Scenario!C$35))` | 계산값: 0.18
- HBM_Demand!D77 | 수식: `=IF(HBM_Inputs!D7=0,0,D74*HBM_Inputs!D52/(HBM_Inputs!D7*Scenario!D$35))` | 계산값: 0.497872340425532
- HBM_Demand!E77 | 수식: `=IF(HBM_Inputs!E7=0,0,E74*HBM_Inputs!E52/(HBM_Inputs!E7*Scenario!E$35))` | 계산값: 0.458693181818182
- HBM_Demand!F77 | 수식: `=IF(HBM_Inputs!F7=0,0,F74*HBM_Inputs!F52/(HBM_Inputs!F7*Scenario!F$35))` | 계산값: 0.360987804878049
- HBM_Demand!G77 | 수식: `=IF(HBM_Inputs!G7=0,0,G74*HBM_Inputs!G52/(HBM_Inputs!G7*Scenario!G$35))` | 계산값: 0.327168947368421
- HBM_Demand!A78 | 값:   NVIDIA Rubin Ultra / Feynman
- HBM_Demand!C78 | 수식: `=IF(HBM_Inputs!C8=0,0,C74*HBM_Inputs!C53/(HBM_Inputs!C8*Scenario!C$35))` | 계산값: 0
- HBM_Demand!D78 | 수식: `=IF(HBM_Inputs!D8=0,0,D74*HBM_Inputs!D53/(HBM_Inputs!D8*Scenario!D$35))` | 계산값: 0.156
- HBM_Demand!E78 | 수식: `=IF(HBM_Inputs!E8=0,0,E74*HBM_Inputs!E53/(HBM_Inputs!E8*Scenario!E$35))` | 계산값: 0.360401785714286
- HBM_Demand!F78 | 수식: `=IF(HBM_Inputs!F8=0,0,F74*HBM_Inputs!F53/(HBM_Inputs!F8*Scenario!F$35))` | 계산값: 0.512325
- HBM_Demand!G78 | 수식: `=IF(HBM_Inputs!G8=0,0,G74*HBM_Inputs!G53/(HBM_Inputs!G8*Scenario!G$35))` | 계산값: 0.647521875
- HBM_Demand!A79 | 값:   Google TPU (v7 Ironwood~)
- HBM_Demand!C79 | 수식: `=IF(HBM_Inputs!C9=0,0,C74*HBM_Inputs!C54/(HBM_Inputs!C9*Scenario!C$35))` | 계산값: 4.5
- HBM_Demand!D79 | 수식: `=IF(HBM_Inputs!D9=0,0,D74*HBM_Inputs!D54/(HBM_Inputs!D9*Scenario!D$35))` | 계산값: 5.85
- HBM_Demand!E79 | 수식: `=IF(HBM_Inputs!E9=0,0,E74*HBM_Inputs!E54/(HBM_Inputs!E9*Scenario!E$35))` | 계산값: 7.58372727272727
- HBM_Demand!F79 | 수식: `=IF(HBM_Inputs!F9=0,0,F74*HBM_Inputs!F54/(HBM_Inputs!F9*Scenario!F$35))` | 계산값: 8.74575
- HBM_Demand!G79 | 수식: `=IF(HBM_Inputs!G9=0,0,G74*HBM_Inputs!G54/(HBM_Inputs!G9*Scenario!G$35))` | 계산값: 10.10134125
- HBM_Demand!A80 | 값:   AWS Trainium (2/3)
- HBM_Demand!C80 | 수식: `=IF(HBM_Inputs!C10=0,0,C74*HBM_Inputs!C55/(HBM_Inputs!C10*Scenario!C$35))` | 계산값: 0
- HBM_Demand!D80 | 수식: `=IF(HBM_Inputs!D10=0,0,D74*HBM_Inputs!D55/(HBM_Inputs!D10*Scenario!D$35))` | 계산값: 0
- HBM_Demand!E80 | 수식: `=IF(HBM_Inputs!E10=0,0,E74*HBM_Inputs!E55/(HBM_Inputs!E10*Scenario!E$35))` | 계산값: 0
- HBM_Demand!F80 | 수식: `=IF(HBM_Inputs!F10=0,0,F74*HBM_Inputs!F55/(HBM_Inputs!F10*Scenario!F$35))` | 계산값: 0
- HBM_Demand!G80 | 수식: `=IF(HBM_Inputs!G10=0,0,G74*HBM_Inputs!G55/(HBM_Inputs!G10*Scenario!G$35))` | 계산값: 0
- HBM_Demand!A81 | 값:   기타 (AMD MI/ MTIA / Maia)
- HBM_Demand!C81 | 수식: `=IF(HBM_Inputs!C11=0,0,C74*HBM_Inputs!C56/(HBM_Inputs!C11*Scenario!C$35))` | 계산값: 0
- HBM_Demand!D81 | 수식: `=IF(HBM_Inputs!D11=0,0,D74*HBM_Inputs!D56/(HBM_Inputs!D11*Scenario!D$35))` | 계산값: 0.078
- HBM_Demand!E81 | 수식: `=IF(HBM_Inputs!E11=0,0,E74*HBM_Inputs!E56/(HBM_Inputs!E11*Scenario!E$35))` | 계산값: 0.240267857142857
- HBM_Demand!F81 | 수식: `=IF(HBM_Inputs!F11=0,0,F74*HBM_Inputs!F56/(HBM_Inputs!F11*Scenario!F$35))` | 계산값: 0.398475
- HBM_Demand!G81 | 수식: `=IF(HBM_Inputs!G11=0,0,G74*HBM_Inputs!G56/(HBM_Inputs!G11*Scenario!G$35))` | 계산값: 0.4532653125
- HBM_Demand!A82 | 값:   소계
...
- HBM_Demand!C84 | 수식: `=C76*HBM_Inputs!$B$6*Scenario!C$29/1000` | 계산값: 0.162
- HBM_Demand!D84 | 수식: `=D76*HBM_Inputs!$B$6*Scenario!D$29/1000` | 계산값: 0.078
- HBM_Demand!E84 | 수식: `=E76*HBM_Inputs!$B$6*Scenario!E$29/1000` | 계산값: 0.03027375
- HBM_Demand!F84 | 수식: `=F76*HBM_Inputs!$B$6*Scenario!F$29/1000` | 계산값: 0
- HBM_Demand!G84 | 수식: `=G76*HBM_Inputs!$B$6*Scenario!G$29/1000` | 계산값: 0
- HBM_Demand!A85 | 값:   NVIDIA Rubin (VR200)
- HBM_Demand!C85 | 수식: `=C77*HBM_Inputs!$B$7*Scenario!C$29/1000` | 계산값: 0.05184
- HBM_Demand!D85 | 수식: `=D77*HBM_Inputs!$B$7*Scenario!D$29/1000` | 계산값: 0.143387234042553
- HBM_Demand!E85 | 수식: `=E77*HBM_Inputs!$B$7*Scenario!E$29/1000` | 계산값: 0.132103636363636
- HBM_Demand!F85 | 수식: `=F77*HBM_Inputs!$B$7*Scenario!F$29/1000` | 계산값: 0.103964487804878
- HBM_Demand!G85 | 수식: `=G77*HBM_Inputs!$B$7*Scenario!G$29/1000` | 계산값: 0.0942246568421053
- HBM_Demand!A86 | 값:   NVIDIA Rubin Ultra / Feynman
- HBM_Demand!C86 | 수식: `=C78*HBM_Inputs!$B$8*Scenario!C$29/1000` | 계산값: 0
- HBM_Demand!D86 | 수식: `=D78*HBM_Inputs!$B$8*Scenario!D$29/1000` | 계산값: 0.059904
- HBM_Demand!E86 | 수식: `=E78*HBM_Inputs!$B$8*Scenario!E$29/1000` | 계산값: 0.138394285714286
- HBM_Demand!F86 | 수식: `=F78*HBM_Inputs!$B$8*Scenario!F$29/1000` | 계산값: 0.1967328
- HBM_Demand!G86 | 수식: `=G78*HBM_Inputs!$B$8*Scenario!G$29/1000` | 계산값: 0.2486484
- HBM_Demand!A87 | 값:   Google TPU (v7 Ironwood~)
- HBM_Demand!C87 | 수식: `=C79*HBM_Inputs!$B$9*Scenario!C$29/1000` | 계산값: 0.972
- HBM_Demand!D87 | 수식: `=D79*HBM_Inputs!$B$9*Scenario!D$29/1000` | 계산값: 1.2636
- HBM_Demand!E87 | 수식: `=E79*HBM_Inputs!$B$9*Scenario!E$29/1000` | 계산값: 1.63808509090909
- HBM_Demand!F87 | 수식: `=F79*HBM_Inputs!$B$9*Scenario!F$29/1000` | 계산값: 1.889082
- HBM_Demand!G87 | 수식: `=G79*HBM_Inputs!$B$9*Scenario!G$29/1000` | 계산값: 2.18188971
- HBM_Demand!A88 | 값:   AWS Trainium (2/3)
- HBM_Demand!C88 | 수식: `=C80*HBM_Inputs!$B$10*Scenario!C$29/1000` | 계산값: 0
- HBM_Demand!D88 | 수식: `=D80*HBM_Inputs!$B$10*Scenario!D$29/1000` | 계산값: 0
- HBM_Demand!E88 | 수식: `=E80*HBM_Inputs!$B$10*Scenario!E$29/1000` | 계산값: 0
- HBM_Demand!F88 | 수식: `=F80*HBM_Inputs!$B$10*Scenario!F$29/1000` | 계산값: 0
- HBM_Demand!G88 | 수식: `=G80*HBM_Inputs!$B$10*Scenario!G$29/1000` | 계산값: 0
- HBM_Demand!A89 | 값:   기타 (AMD MI/ MTIA / Maia)
- HBM_Demand!C89 | 수식: `=C81*HBM_Inputs!$B$11*Scenario!C$29/1000` | 계산값: 0
- HBM_Demand!D89 | 수식: `=D81*HBM_Inputs!$B$11*Scenario!D$29/1000` | 계산값: 0.019968
- HBM_Demand!E89 | 수식: `=E81*HBM_Inputs!$B$11*Scenario!E$29/1000` | 계산값: 0.0615085714285714
- HBM_Demand!F89 | 수식: `=F81*HBM_Inputs!$B$11*Scenario!F$29/1000` | 계산값: 0.1020096
- HBM_Demand!G89 | 수식: `=G81*HBM_Inputs!$B$11*Scenario!G$29/1000` | 계산값: 0.11603592
- HBM_Demand!A90 | 값:   소계
...
- HBM_Demand!C92 | 수식: `=IFERROR(C84*INDEX(HBM_Inputs!C$16:C$18,MATCH(HBM_Inputs!H6,HBM_Inputs!$A$16:$A$18,0))*Scenario!C$23,0)` | 계산값: 1.62
- HBM_Demand!D92 | 수식: `=IFERROR(D84*INDEX(HBM_Inputs!D$16:D$18,MATCH(HBM_Inputs!I6,HBM_Inputs!$A$16:$A$18,0))*Scenario!D$23,0)` | 계산값: 1.014
- HBM_Demand!E92 | 수식: `=IFERROR(E84*INDEX(HBM_Inputs!E$16:E$18,MATCH(HBM_Inputs!J6,HBM_Inputs!$A$16:$A$18,0))*Scenario!E$23,0)` | 계산값: 0.363285
- HBM_Demand!F92 | 수식: `=IFERROR(F84*INDEX(HBM_Inputs!F$16:F$18,MATCH(HBM_Inputs!K6,HBM_Inputs!$A$16:$A$18,0))*Scenario!F$23,0)` | 계산값: 0
- HBM_Demand!G92 | 수식: `=IFERROR(G84*INDEX(HBM_Inputs!G$16:G$18,MATCH(HBM_Inputs!L6,HBM_Inputs!$A$16:$A$18,0))*Scenario!G$23,0)` | 계산값: 0
- HBM_Demand!A93 | 값:   NVIDIA Rubin (VR200)
- HBM_Demand!C93 | 수식: `=IFERROR(C85*INDEX(HBM_Inputs!C$16:C$18,MATCH(HBM_Inputs!H7,HBM_Inputs!$A$16:$A$18,0))*Scenario!C$23,0)` | 계산값: 0.7776
- HBM_Demand!D93 | 수식: `=IFERROR(D85*INDEX(HBM_Inputs!D$16:D$18,MATCH(HBM_Inputs!I7,HBM_Inputs!$A$16:$A$18,0))*Scenario!D$23,0)` | 계산값: 2.86774468085106
- HBM_Demand!E93 | 수식: `=IFERROR(E85*INDEX(HBM_Inputs!E$16:E$18,MATCH(HBM_Inputs!J7,HBM_Inputs!$A$16:$A$18,0))*Scenario!E$23,0)` | 계산값: 2.37786545454546
- HBM_Demand!F93 | 수식: `=IFERROR(F85*INDEX(HBM_Inputs!F$16:F$18,MATCH(HBM_Inputs!K7,HBM_Inputs!$A$16:$A$18,0))*Scenario!F$23,0)` | 계산값: 1.66343180487805
- HBM_Demand!G93 | 수식: `=IFERROR(G85*INDEX(HBM_Inputs!G$16:G$18,MATCH(HBM_Inputs!L7,HBM_Inputs!$A$16:$A$18,0))*Scenario!G$23,0)` | 계산값: 1.31914519578947
- HBM_Demand!A94 | 값:   NVIDIA Rubin Ultra / Feynman
- HBM_Demand!C94 | 수식: `=IFERROR(C86*INDEX(HBM_Inputs!C$16:C$18,MATCH(HBM_Inputs!H8,HBM_Inputs!$A$16:$A$18,0))*Scenario!C$23,0)` | 계산값: 0
- HBM_Demand!D94 | 수식: `=IFERROR(D86*INDEX(HBM_Inputs!D$16:D$18,MATCH(HBM_Inputs!I8,HBM_Inputs!$A$16:$A$18,0))*Scenario!D$23,0)` | 계산값: 1.437696
- HBM_Demand!E94 | 수식: `=IFERROR(E86*INDEX(HBM_Inputs!E$16:E$18,MATCH(HBM_Inputs!J8,HBM_Inputs!$A$16:$A$18,0))*Scenario!E$23,0)` | 계산값: 3.04467428571429
- HBM_Demand!F94 | 수식: `=IFERROR(F86*INDEX(HBM_Inputs!F$16:F$18,MATCH(HBM_Inputs!K8,HBM_Inputs!$A$16:$A$18,0))*Scenario!F$23,0)` | 계산값: 3.934656
- HBM_Demand!G94 | 수식: `=IFERROR(G86*INDEX(HBM_Inputs!G$16:G$18,MATCH(HBM_Inputs!L8,HBM_Inputs!$A$16:$A$18,0))*Scenario!G$23,0)` | 계산값: 4.4756712
- HBM_Demand!A95 | 값:   Google TPU (v7 Ironwood~)
- HBM_Demand!C95 | 수식: `=IFERROR(C87*INDEX(HBM_Inputs!C$16:C$18,MATCH(HBM_Inputs!H9,HBM_Inputs!$A$16:$A$18,0))*Scenario!C$23,0)` | 계산값: 9.72
- HBM_Demand!D95 | 수식: `=IFERROR(D87*INDEX(HBM_Inputs!D$16:D$18,MATCH(HBM_Inputs!I9,HBM_Inputs!$A$16:$A$18,0))*Scenario!D$23,0)` | 계산값: 25.272
- HBM_Demand!E95 | 수식: `=IFERROR(E87*INDEX(HBM_Inputs!E$16:E$18,MATCH(HBM_Inputs!J9,HBM_Inputs!$A$16:$A$18,0))*Scenario!E$23,0)` | 계산값: 29.4855316363636
- HBM_Demand!F95 | 수식: `=IFERROR(F87*INDEX(HBM_Inputs!F$16:F$18,MATCH(HBM_Inputs!K9,HBM_Inputs!$A$16:$A$18,0))*Scenario!F$23,0)` | 계산값: 37.78164
- HBM_Demand!G95 | 수식: `=IFERROR(G87*INDEX(HBM_Inputs!G$16:G$18,MATCH(HBM_Inputs!L9,HBM_Inputs!$A$16:$A$18,0))*Scenario!G$23,0)` | 계산값: 39.27401478
- HBM_Demand!A96 | 값:   AWS Trainium (2/3)
- HBM_Demand!C96 | 수식: `=IFERROR(C88*INDEX(HBM_Inputs!C$16:C$18,MATCH(HBM_Inputs!H10,HBM_Inputs!$A$16:$A$18,0))*Scenario!C$23,0)` | 계산값: 0
- HBM_Demand!D96 | 수식: `=IFERROR(D88*INDEX(HBM_Inputs!D$16:D$18,MATCH(HBM_Inputs!I10,HBM_Inputs!$A$16:$A$18,0))*Scenario!D$23,0)` | 계산값: 0
- HBM_Demand!E96 | 수식: `=IFERROR(E88*INDEX(HBM_Inputs!E$16:E$18,MATCH(HBM_Inputs!J10,HBM_Inputs!$A$16:$A$18,0))*Scenario!E$23,0)` | 계산값: 0
- HBM_Demand!F96 | 수식: `=IFERROR(F88*INDEX(HBM_Inputs!F$16:F$18,MATCH(HBM_Inputs!K10,HBM_Inputs!$A$16:$A$18,0))*Scenario!F$23,0)` | 계산값: 0
- HBM_Demand!G96 | 수식: `=IFERROR(G88*INDEX(HBM_Inputs!G$16:G$18,MATCH(HBM_Inputs!L10,HBM_Inputs!$A$16:$A$18,0))*Scenario!G$23,0)` | 계산값: 0
- HBM_Demand!A97 | 값:   기타 (AMD MI/ MTIA / Maia)
- HBM_Demand!C97 | 수식: `=IFERROR(C89*INDEX(HBM_Inputs!C$16:C$18,MATCH(HBM_Inputs!H11,HBM_Inputs!$A$16:$A$18,0))*Scenario!C$23,0)` | 계산값: 0
- HBM_Demand!D97 | 수식: `=IFERROR(D89*INDEX(HBM_Inputs!D$16:D$18,MATCH(HBM_Inputs!I11,HBM_Inputs!$A$16:$A$18,0))*Scenario!D$23,0)` | 계산값: 0.39936
- HBM_Demand!E97 | 수식: `=IFERROR(E89*INDEX(HBM_Inputs!E$16:E$18,MATCH(HBM_Inputs!J11,HBM_Inputs!$A$16:$A$18,0))*Scenario!E$23,0)` | 계산값: 1.10715428571429
- HBM_Demand!F97 | 수식: `=IFERROR(F89*INDEX(HBM_Inputs!F$16:F$18,MATCH(HBM_Inputs!K11,HBM_Inputs!$A$16:$A$18,0))*Scenario!F$23,0)` | 계산값: 2.040192
- HBM_Demand!G97 | 수식: `=IFERROR(G89*INDEX(HBM_Inputs!G$16:G$18,MATCH(HBM_Inputs!L11,HBM_Inputs!$A$16:$A$18,0))*Scenario!G$23,0)` | 계산값: 2.08864656
- HBM_Demand!A98 | 값:   소계
...
- HBM_Demand!C106 | 수식: `=C105*(HBM_Inputs!$B$37+Scenario!C$17)` | 계산값: 79.2
- HBM_Demand!D106 | 수식: `=D105*(HBM_Inputs!$B$37+Scenario!D$17)` | 계산값: 95.04
- HBM_Demand!E106 | 수식: `=E105*(HBM_Inputs!$B$37+Scenario!E$17)` | 계산값: 106.4448
- HBM_Demand!F106 | 수식: `=F105*(HBM_Inputs!$B$37+Scenario!F$17)` | 계산값: 114.960384
- HBM_Demand!G106 | 수식: `=G105*(HBM_Inputs!$B$37+Scenario!G$17)` | 계산값: 120.7084032
- HBM_Demand!A107 | 값: 가속기 대수 (백만 개)
- HBM_Demand!A108 | 값:   NVIDIA Blackwell (B200/B300)
- HBM_Demand!C108 | 수식: `=IF(HBM_Inputs!C6=0,0,C106*HBM_Inputs!C60/(HBM_Inputs!C6*Scenario!C$35))` | 계산값: 0.792
- HBM_Demand!D108 | 수식: `=IF(HBM_Inputs!D6=0,0,D106*HBM_Inputs!D60/(HBM_Inputs!D6*Scenario!D$35))` | 계산값: 0.396
- HBM_Demand!E108 | 수식: `=IF(HBM_Inputs!E6=0,0,E106*HBM_Inputs!E60/(HBM_Inputs!E6*Scenario!E$35))` | 계산값: 0.16632
- HBM_Demand!F108 | 수식: `=IF(HBM_Inputs!F6=0,0,F106*HBM_Inputs!F60/(HBM_Inputs!F6*Scenario!F$35))` | 계산값: 0
- HBM_Demand!G108 | 수식: `=IF(HBM_Inputs!G6=0,0,G106*HBM_Inputs!G60/(HBM_Inputs!G6*Scenario!G$35))` | 계산값: 0
- HBM_Demand!A109 | 값:   NVIDIA Rubin (VR200)
- HBM_Demand!C109 | 수식: `=IF(HBM_Inputs!C7=0,0,C106*HBM_Inputs!C61/(HBM_Inputs!C7*Scenario!C$35))` | 계산값: 0.2376
- HBM_Demand!D109 | 수식: `=IF(HBM_Inputs!D7=0,0,D106*HBM_Inputs!D61/(HBM_Inputs!D7*Scenario!D$35))` | 계산값: 0.505531914893617
- HBM_Demand!E109 | 수식: `=IF(HBM_Inputs!E7=0,0,E106*HBM_Inputs!E61/(HBM_Inputs!E7*Scenario!E$35))` | 계산값: 0.48384
- HBM_Demand!F109 | 수식: `=IF(HBM_Inputs!F7=0,0,F106*HBM_Inputs!F61/(HBM_Inputs!F7*Scenario!F$35))` | 계산값: 0.420586770731707
- HBM_Demand!G109 | 수식: `=IF(HBM_Inputs!G7=0,0,G106*HBM_Inputs!G61/(HBM_Inputs!G7*Scenario!G$35))` | 계산값: 0.317653692631579
- HBM_Demand!A110 | 값:   NVIDIA Rubin Ultra / Feynman
- HBM_Demand!C110 | 수식: `=IF(HBM_Inputs!C8=0,0,C106*HBM_Inputs!C62/(HBM_Inputs!C8*Scenario!C$35))` | 계산값: 0
- HBM_Demand!D110 | 수식: `=IF(HBM_Inputs!D8=0,0,D106*HBM_Inputs!D62/(HBM_Inputs!D8*Scenario!D$35))` | 계산값: 0.12672
- HBM_Demand!E110 | 수식: `=IF(HBM_Inputs!E8=0,0,E106*HBM_Inputs!E62/(HBM_Inputs!E8*Scenario!E$35))` | 계산값: 0.28512
- HBM_Demand!F110 | 수식: `=IF(HBM_Inputs!F8=0,0,F106*HBM_Inputs!F62/(HBM_Inputs!F8*Scenario!F$35))` | 계산값: 0.442155323076923
- HBM_Demand!G110 | 수식: `=IF(HBM_Inputs!G8=0,0,G106*HBM_Inputs!G62/(HBM_Inputs!G8*Scenario!G$35))` | 계산값: 0.553246848
- HBM_Demand!A111 | 값:   Google TPU (v7 Ironwood~)
- HBM_Demand!C111 | 수식: `=IF(HBM_Inputs!C9=0,0,C106*HBM_Inputs!C63/(HBM_Inputs!C9*Scenario!C$35))` | 계산값: 0
- HBM_Demand!D111 | 수식: `=IF(HBM_Inputs!D9=0,0,D106*HBM_Inputs!D63/(HBM_Inputs!D9*Scenario!D$35))` | 계산값: 0
- HBM_Demand!E111 | 수식: `=IF(HBM_Inputs!E9=0,0,E106*HBM_Inputs!E63/(HBM_Inputs!E9*Scenario!E$35))` | 계산값: 0
- HBM_Demand!F111 | 수식: `=IF(HBM_Inputs!F9=0,0,F106*HBM_Inputs!F63/(HBM_Inputs!F9*Scenario!F$35))` | 계산값: 0
- HBM_Demand!G111 | 수식: `=IF(HBM_Inputs!G9=0,0,G106*HBM_Inputs!G63/(HBM_Inputs!G9*Scenario!G$35))` | 계산값: 0
- HBM_Demand!A112 | 값:   AWS Trainium (2/3)
- HBM_Demand!C112 | 수식: `=IF(HBM_Inputs!C10=0,0,C106*HBM_Inputs!C64/(HBM_Inputs!C10*Scenario!C$35))` | 계산값: 4.455
- HBM_Demand!D112 | 수식: `=IF(HBM_Inputs!D10=0,0,D106*HBM_Inputs!D64/(HBM_Inputs!D10*Scenario!D$35))` | 계산값: 5.28
- HBM_Demand!E112 | 수식: `=IF(HBM_Inputs!E10=0,0,E106*HBM_Inputs!E64/(HBM_Inputs!E10*Scenario!E$35))` | 계산값: 6.50496
- HBM_Demand!F112 | 수식: `=IF(HBM_Inputs!F10=0,0,F106*HBM_Inputs!F64/(HBM_Inputs!F10*Scenario!F$35))` | 계산값: 7.40855808
- HBM_Demand!G112 | 수식: `=IF(HBM_Inputs!G10=0,0,G106*HBM_Inputs!G64/(HBM_Inputs!G10*Scenario!G$35))` | 계산값: 8.04722688
- HBM_Demand!A113 | 값:   기타 (AMD MI/ MTIA / Maia)
- HBM_Demand!C113 | 수식: `=IF(HBM_Inputs!C11=0,0,C106*HBM_Inputs!C65/(HBM_Inputs!C11*Scenario!C$35))` | 계산값: 0
- HBM_Demand!D113 | 수식: `=IF(HBM_Inputs!D11=0,0,D106*HBM_Inputs!D65/(HBM_Inputs!D11*Scenario!D$35))` | 계산값: 0.06336
- HBM_Demand!E113 | 수식: `=IF(HBM_Inputs!E11=0,0,E106*HBM_Inputs!E65/(HBM_Inputs!E11*Scenario!E$35))` | 계산값: 0.19008
- HBM_Demand!F113 | 수식: `=IF(HBM_Inputs!F11=0,0,F106*HBM_Inputs!F65/(HBM_Inputs!F11*Scenario!F$35))` | 계산값: 0.309508726153846
- HBM_Demand!G113 | 수식: `=IF(HBM_Inputs!G11=0,0,G106*HBM_Inputs!G65/(HBM_Inputs!G11*Scenario!G$35))` | 계산값: 0.402361344
...
- HBM_Demand!C116 | 수식: `=C108*HBM_Inputs!$B$6*Scenario!C$29/1000` | 계산값: 0.19008
- HBM_Demand!D116 | 수식: `=D108*HBM_Inputs!$B$6*Scenario!D$29/1000` | 계산값: 0.09504
- HBM_Demand!E116 | 수식: `=E108*HBM_Inputs!$B$6*Scenario!E$29/1000` | 계산값: 0.0399168
- HBM_Demand!F116 | 수식: `=F108*HBM_Inputs!$B$6*Scenario!F$29/1000` | 계산값: 0
- HBM_Demand!G116 | 수식: `=G108*HBM_Inputs!$B$6*Scenario!G$29/1000` | 계산값: 0
- HBM_Demand!A117 | 값:   NVIDIA Rubin (VR200)
- HBM_Demand!C117 | 수식: `=C109*HBM_Inputs!$B$7*Scenario!C$29/1000` | 계산값: 0.0684288
- HBM_Demand!D117 | 수식: `=D109*HBM_Inputs!$B$7*Scenario!D$29/1000` | 계산값: 0.145593191489362
- HBM_Demand!E117 | 수식: `=E109*HBM_Inputs!$B$7*Scenario!E$29/1000` | 계산값: 0.13934592
- HBM_Demand!F117 | 수식: `=F109*HBM_Inputs!$B$7*Scenario!F$29/1000` | 계산값: 0.121128989970732
- HBM_Demand!G117 | 수식: `=G109*HBM_Inputs!$B$7*Scenario!G$29/1000` | 계산값: 0.0914842634778948
- HBM_Demand!A118 | 값:   NVIDIA Rubin Ultra / Feynman
- HBM_Demand!C118 | 수식: `=C110*HBM_Inputs!$B$8*Scenario!C$29/1000` | 계산값: 0
- HBM_Demand!D118 | 수식: `=D110*HBM_Inputs!$B$8*Scenario!D$29/1000` | 계산값: 0.04866048
- HBM_Demand!E118 | 수식: `=E110*HBM_Inputs!$B$8*Scenario!E$29/1000` | 계산값: 0.10948608
- HBM_Demand!F118 | 수식: `=F110*HBM_Inputs!$B$8*Scenario!F$29/1000` | 계산값: 0.169787644061538
- HBM_Demand!G118 | 수식: `=G110*HBM_Inputs!$B$8*Scenario!G$29/1000` | 계산값: 0.212446789632
- HBM_Demand!A119 | 값:   Google TPU (v7 Ironwood~)
- HBM_Demand!C119 | 수식: `=C111*HBM_Inputs!$B$9*Scenario!C$29/1000` | 계산값: 0
- HBM_Demand!D119 | 수식: `=D111*HBM_Inputs!$B$9*Scenario!D$29/1000` | 계산값: 0
- HBM_Demand!E119 | 수식: `=E111*HBM_Inputs!$B$9*Scenario!E$29/1000` | 계산값: 0
- HBM_Demand!F119 | 수식: `=F111*HBM_Inputs!$B$9*Scenario!F$29/1000` | 계산값: 0
- HBM_Demand!G119 | 수식: `=G111*HBM_Inputs!$B$9*Scenario!G$29/1000` | 계산값: 0
- HBM_Demand!A120 | 값:   AWS Trainium (2/3)
- HBM_Demand!C120 | 수식: `=C112*HBM_Inputs!$B$10*Scenario!C$29/1000` | 계산값: 0.64152
- HBM_Demand!D120 | 수식: `=D112*HBM_Inputs!$B$10*Scenario!D$29/1000` | 계산값: 0.76032
- HBM_Demand!E120 | 수식: `=E112*HBM_Inputs!$B$10*Scenario!E$29/1000` | 계산값: 0.93671424
- HBM_Demand!F120 | 수식: `=F112*HBM_Inputs!$B$10*Scenario!F$29/1000` | 계산값: 1.06683236352
- HBM_Demand!G120 | 수식: `=G112*HBM_Inputs!$B$10*Scenario!G$29/1000` | 계산값: 1.15880067072
- HBM_Demand!A121 | 값:   기타 (AMD MI/ MTIA / Maia)
- HBM_Demand!C121 | 수식: `=C113*HBM_Inputs!$B$11*Scenario!C$29/1000` | 계산값: 0
- HBM_Demand!D121 | 수식: `=D113*HBM_Inputs!$B$11*Scenario!D$29/1000` | 계산값: 0.01622016
- HBM_Demand!E121 | 수식: `=E113*HBM_Inputs!$B$11*Scenario!E$29/1000` | 계산값: 0.04866048
- HBM_Demand!F121 | 수식: `=F113*HBM_Inputs!$B$11*Scenario!F$29/1000` | 계산값: 0.0792342338953846
- HBM_Demand!G121 | 수식: `=G113*HBM_Inputs!$B$11*Scenario!G$29/1000` | 계산값: 0.103004504064
...
- HBM_Demand!C124 | 수식: `=IFERROR(C116*INDEX(HBM_Inputs!C$16:C$18,MATCH(HBM_Inputs!H6,HBM_Inputs!$A$16:$A$18,0))*Scenario!C$23,0)` | 계산값: 1.9008
- HBM_Demand!D124 | 수식: `=IFERROR(D116*INDEX(HBM_Inputs!D$16:D$18,MATCH(HBM_Inputs!I6,HBM_Inputs!$A$16:$A$18,0))*Scenario!D$23,0)` | 계산값: 1.23552
- HBM_Demand!E124 | 수식: `=IFERROR(E116*INDEX(HBM_Inputs!E$16:E$18,MATCH(HBM_Inputs!J6,HBM_Inputs!$A$16:$A$18,0))*Scenario!E$23,0)` | 계산값: 0.4790016
- HBM_Demand!F124 | 수식: `=IFERROR(F116*INDEX(HBM_Inputs!F$16:F$18,MATCH(HBM_Inputs!K6,HBM_Inputs!$A$16:$A$18,0))*Scenario!F$23,0)` | 계산값: 0
- HBM_Demand!G124 | 수식: `=IFERROR(G116*INDEX(HBM_Inputs!G$16:G$18,MATCH(HBM_Inputs!L6,HBM_Inputs!$A$16:$A$18,0))*Scenario!G$23,0)` | 계산값: 0
- HBM_Demand!A125 | 값:   NVIDIA Rubin (VR200)
- HBM_Demand!C125 | 수식: `=IFERROR(C117*INDEX(HBM_Inputs!C$16:C$18,MATCH(HBM_Inputs!H7,HBM_Inputs!$A$16:$A$18,0))*Scenario!C$23,0)` | 계산값: 1.026432
- HBM_Demand!D125 | 수식: `=IFERROR(D117*INDEX(HBM_Inputs!D$16:D$18,MATCH(HBM_Inputs!I7,HBM_Inputs!$A$16:$A$18,0))*Scenario!D$23,0)` | 계산값: 2.91186382978724
- HBM_Demand!E125 | 수식: `=IFERROR(E117*INDEX(HBM_Inputs!E$16:E$18,MATCH(HBM_Inputs!J7,HBM_Inputs!$A$16:$A$18,0))*Scenario!E$23,0)` | 계산값: 2.50822656
- HBM_Demand!F125 | 수식: `=IFERROR(F117*INDEX(HBM_Inputs!F$16:F$18,MATCH(HBM_Inputs!K7,HBM_Inputs!$A$16:$A$18,0))*Scenario!F$23,0)` | 계산값: 1.93806383953171
- HBM_Demand!G125 | 수식: `=IFERROR(G117*INDEX(HBM_Inputs!G$16:G$18,MATCH(HBM_Inputs!L7,HBM_Inputs!$A$16:$A$18,0))*Scenario!G$23,0)` | 계산값: 1.28077968869053
- HBM_Demand!A126 | 값:   NVIDIA Rubin Ultra / Feynman
- HBM_Demand!C126 | 수식: `=IFERROR(C118*INDEX(HBM_Inputs!C$16:C$18,MATCH(HBM_Inputs!H8,HBM_Inputs!$A$16:$A$18,0))*Scenario!C$23,0)` | 계산값: 0
- HBM_Demand!D126 | 수식: `=IFERROR(D118*INDEX(HBM_Inputs!D$16:D$18,MATCH(HBM_Inputs!I8,HBM_Inputs!$A$16:$A$18,0))*Scenario!D$23,0)` | 계산값: 1.16785152
- HBM_Demand!E126 | 수식: `=IFERROR(E118*INDEX(HBM_Inputs!E$16:E$18,MATCH(HBM_Inputs!J8,HBM_Inputs!$A$16:$A$18,0))*Scenario!E$23,0)` | 계산값: 2.40869376
- HBM_Demand!F126 | 수식: `=IFERROR(F118*INDEX(HBM_Inputs!F$16:F$18,MATCH(HBM_Inputs!K8,HBM_Inputs!$A$16:$A$18,0))*Scenario!F$23,0)` | 계산값: 3.39575288123077
- HBM_Demand!G126 | 수식: `=IFERROR(G118*INDEX(HBM_Inputs!G$16:G$18,MATCH(HBM_Inputs!L8,HBM_Inputs!$A$16:$A$18,0))*Scenario!G$23,0)` | 계산값: 3.824042213376
- HBM_Demand!A127 | 값:   Google TPU (v7 Ironwood~)
- HBM_Demand!C127 | 수식: `=IFERROR(C119*INDEX(HBM_Inputs!C$16:C$18,MATCH(HBM_Inputs!H9,HBM_Inputs!$A$16:$A$18,0))*Scenario!C$23,0)` | 계산값: 0
- HBM_Demand!D127 | 수식: `=IFERROR(D119*INDEX(HBM_Inputs!D$16:D$18,MATCH(HBM_Inputs!I9,HBM_Inputs!$A$16:$A$18,0))*Scenario!D$23,0)` | 계산값: 0
- HBM_Demand!E127 | 수식: `=IFERROR(E119*INDEX(HBM_Inputs!E$16:E$18,MATCH(HBM_Inputs!J9,HBM_Inputs!$A$16:$A$18,0))*Scenario!E$23,0)` | 계산값: 0
- HBM_Demand!F127 | 수식: `=IFERROR(F119*INDEX(HBM_Inputs!F$16:F$18,MATCH(HBM_Inputs!K9,HBM_Inputs!$A$16:$A$18,0))*Scenario!F$23,0)` | 계산값: 0
- HBM_Demand!G127 | 수식: `=IFERROR(G119*INDEX(HBM_Inputs!G$16:G$18,MATCH(HBM_Inputs!L9,HBM_Inputs!$A$16:$A$18,0))*Scenario!G$23,0)` | 계산값: 0
- HBM_Demand!A128 | 값:   AWS Trainium (2/3)
- HBM_Demand!C128 | 수식: `=IFERROR(C120*INDEX(HBM_Inputs!C$16:C$18,MATCH(HBM_Inputs!H10,HBM_Inputs!$A$16:$A$18,0))*Scenario!C$23,0)` | 계산값: 6.4152
- HBM_Demand!D128 | 수식: `=IFERROR(D120*INDEX(HBM_Inputs!D$16:D$18,MATCH(HBM_Inputs!I10,HBM_Inputs!$A$16:$A$18,0))*Scenario!D$23,0)` | 계산값: 15.2064
- HBM_Demand!E128 | 수식: `=IFERROR(E120*INDEX(HBM_Inputs!E$16:E$18,MATCH(HBM_Inputs!J10,HBM_Inputs!$A$16:$A$18,0))*Scenario!E$23,0)` | 계산값: 16.86085632
- HBM_Demand!F128 | 수식: `=IFERROR(F120*INDEX(HBM_Inputs!F$16:F$18,MATCH(HBM_Inputs!K10,HBM_Inputs!$A$16:$A$18,0))*Scenario!F$23,0)` | 계산값: 17.06931781632
- HBM_Demand!G128 | 수식: `=IFERROR(G120*INDEX(HBM_Inputs!G$16:G$18,MATCH(HBM_Inputs!L10,HBM_Inputs!$A$16:$A$18,0))*Scenario!G$23,0)` | 계산값: 20.85841207296
- HBM_Demand!A129 | 값:   기타 (AMD MI/ MTIA / Maia)
- HBM_Demand!C129 | 수식: `=IFERROR(C121*INDEX(HBM_Inputs!C$16:C$18,MATCH(HBM_Inputs!H11,HBM_Inputs!$A$16:$A$18,0))*Scenario!C$23,0)` | 계산값: 0
- HBM_Demand!D129 | 수식: `=IFERROR(D121*INDEX(HBM_Inputs!D$16:D$18,MATCH(HBM_Inputs!I11,HBM_Inputs!$A$16:$A$18,0))*Scenario!D$23,0)` | 계산값: 0.3244032
- HBM_Demand!E129 | 수식: `=IFERROR(E121*INDEX(HBM_Inputs!E$16:E$18,MATCH(HBM_Inputs!J11,HBM_Inputs!$A$16:$A$18,0))*Scenario!E$23,0)` | 계산값: 0.87588864
- HBM_Demand!F129 | 수식: `=IFERROR(F121*INDEX(HBM_Inputs!F$16:F$18,MATCH(HBM_Inputs!K11,HBM_Inputs!$A$16:$A$18,0))*Scenario!F$23,0)` | 계산값: 1.58468467790769
- HBM_Demand!G129 | 수식: `=IFERROR(G121*INDEX(HBM_Inputs!G$16:G$18,MATCH(HBM_Inputs!L11,HBM_Inputs!$A$16:$A$18,0))*Scenario!G$23,0)` | 계산값: 1.854081073152
...
- HBM_Demand!C138 | 수식: `=C137*(HBM_Inputs!$B$38+Scenario!C$17)` | 계산값: 60.5
- HBM_Demand!D138 | 수식: `=D137*(HBM_Inputs!$B$38+Scenario!D$17)` | 계산값: 72.6
- HBM_Demand!E138 | 수식: `=E137*(HBM_Inputs!$B$38+Scenario!E$17)` | 계산값: 81.312
- HBM_Demand!F138 | 수식: `=F137*(HBM_Inputs!$B$38+Scenario!F$17)` | 계산값: 87.81696
- HBM_Demand!G138 | 수식: `=G137*(HBM_Inputs!$B$38+Scenario!G$17)` | 계산값: 92.207808
- HBM_Demand!A139 | 값: 가속기 대수 (백만 개)
- HBM_Demand!A140 | 값:   NVIDIA Blackwell (B200/B300)
- HBM_Demand!C140 | 수식: `=IF(HBM_Inputs!C6=0,0,C138*HBM_Inputs!C69/(HBM_Inputs!C6*Scenario!C$35))` | 계산값: 0.983125
- HBM_Demand!D140 | 수식: `=IF(HBM_Inputs!D6=0,0,D138*HBM_Inputs!D69/(HBM_Inputs!D6*Scenario!D$35))` | 계산값: 0.504166666666667
- HBM_Demand!E140 | 수식: `=IF(HBM_Inputs!E6=0,0,E138*HBM_Inputs!E69/(HBM_Inputs!E6*Scenario!E$35))` | 계산값: 0.12705
- HBM_Demand!F140 | 수식: `=IF(HBM_Inputs!F6=0,0,F138*HBM_Inputs!F69/(HBM_Inputs!F6*Scenario!F$35))` | 계산값: 0
- HBM_Demand!G140 | 수식: `=IF(HBM_Inputs!G6=0,0,G138*HBM_Inputs!G69/(HBM_Inputs!G6*Scenario!G$35))` | 계산값: 0
- HBM_Demand!A141 | 값:   NVIDIA Rubin (VR200)
- HBM_Demand!C141 | 수식: `=IF(HBM_Inputs!C7=0,0,C138*HBM_Inputs!C70/(HBM_Inputs!C7*Scenario!C$35))` | 계산값: 0.3025
- HBM_Demand!D141 | 수식: `=IF(HBM_Inputs!D7=0,0,D138*HBM_Inputs!D70/(HBM_Inputs!D7*Scenario!D$35))` | 계산값: 0.695106382978723
- HBM_Demand!E141 | 수식: `=IF(HBM_Inputs!E7=0,0,E138*HBM_Inputs!E70/(HBM_Inputs!E7*Scenario!E$35))` | 계산값: 0.7392
- HBM_Demand!F141 | 수식: `=IF(HBM_Inputs!F7=0,0,F138*HBM_Inputs!F70/(HBM_Inputs!F7*Scenario!F$35))` | 계산값: 0.535469268292683
- HBM_Demand!G141 | 수식: `=IF(HBM_Inputs!G7=0,0,G138*HBM_Inputs!G70/(HBM_Inputs!G7*Scenario!G$35))` | 계산값: 0.363978189473684
- HBM_Demand!A142 | 값:   NVIDIA Rubin Ultra / Feynman
- HBM_Demand!C142 | 수식: `=IF(HBM_Inputs!C8=0,0,C138*HBM_Inputs!C71/(HBM_Inputs!C8*Scenario!C$35))` | 계산값: 0
- HBM_Demand!D142 | 수식: `=IF(HBM_Inputs!D8=0,0,D138*HBM_Inputs!D71/(HBM_Inputs!D8*Scenario!D$35))` | 계산값: 0.1815
- HBM_Demand!E142 | 수식: `=IF(HBM_Inputs!E8=0,0,E138*HBM_Inputs!E71/(HBM_Inputs!E8*Scenario!E$35))` | 계산값: 0.5082
- HBM_Demand!F142 | 수식: `=IF(HBM_Inputs!F8=0,0,F138*HBM_Inputs!F71/(HBM_Inputs!F8*Scenario!F$35))` | 계산값: 0.759954461538462
- HBM_Demand!G142 | 수식: `=IF(HBM_Inputs!G8=0,0,G138*HBM_Inputs!G71/(HBM_Inputs!G8*Scenario!G$35))` | 계산값: 0.960498
- HBM_Demand!A143 | 값:   Google TPU (v7 Ironwood~)
- HBM_Demand!C143 | 수식: `=IF(HBM_Inputs!C9=0,0,C138*HBM_Inputs!C72/(HBM_Inputs!C9*Scenario!C$35))` | 계산값: 0
- HBM_Demand!D143 | 수식: `=IF(HBM_Inputs!D9=0,0,D138*HBM_Inputs!D72/(HBM_Inputs!D9*Scenario!D$35))` | 계산값: 0
- HBM_Demand!E143 | 수식: `=IF(HBM_Inputs!E9=0,0,E138*HBM_Inputs!E72/(HBM_Inputs!E9*Scenario!E$35))` | 계산값: 0
- HBM_Demand!F143 | 수식: `=IF(HBM_Inputs!F9=0,0,F138*HBM_Inputs!F72/(HBM_Inputs!F9*Scenario!F$35))` | 계산값: 0
- HBM_Demand!G143 | 수식: `=IF(HBM_Inputs!G9=0,0,G138*HBM_Inputs!G72/(HBM_Inputs!G9*Scenario!G$35))` | 계산값: 0
- HBM_Demand!A144 | 값:   AWS Trainium (2/3)
- HBM_Demand!C144 | 수식: `=IF(HBM_Inputs!C10=0,0,C138*HBM_Inputs!C73/(HBM_Inputs!C10*Scenario!C$35))` | 계산값: 0
- HBM_Demand!D144 | 수식: `=IF(HBM_Inputs!D10=0,0,D138*HBM_Inputs!D73/(HBM_Inputs!D10*Scenario!D$35))` | 계산값: 0
- HBM_Demand!E144 | 수식: `=IF(HBM_Inputs!E10=0,0,E138*HBM_Inputs!E73/(HBM_Inputs!E10*Scenario!E$35))` | 계산값: 0
- HBM_Demand!F144 | 수식: `=IF(HBM_Inputs!F10=0,0,F138*HBM_Inputs!F73/(HBM_Inputs!F10*Scenario!F$35))` | 계산값: 0
- HBM_Demand!G144 | 수식: `=IF(HBM_Inputs!G10=0,0,G138*HBM_Inputs!G73/(HBM_Inputs!G10*Scenario!G$35))` | 계산값: 0
- HBM_Demand!A145 | 값:   기타 (AMD MI/ MTIA / Maia)
- HBM_Demand!C145 | 수식: `=IF(HBM_Inputs!C11=0,0,C138*HBM_Inputs!C74/(HBM_Inputs!C11*Scenario!C$35))` | 계산값: 0.201666666666667
- HBM_Demand!D145 | 수식: `=IF(HBM_Inputs!D11=0,0,D138*HBM_Inputs!D74/(HBM_Inputs!D11*Scenario!D$35))` | 계산값: 0.363
- HBM_Demand!E145 | 수식: `=IF(HBM_Inputs!E11=0,0,E138*HBM_Inputs!E74/(HBM_Inputs!E11*Scenario!E$35))` | 계산값: 0.5808
- HBM_Demand!F145 | 수식: `=IF(HBM_Inputs!F11=0,0,F138*HBM_Inputs!F74/(HBM_Inputs!F11*Scenario!F$35))` | 계산값: 1.01327261538462
- HBM_Demand!G145 | 수식: `=IF(HBM_Inputs!G11=0,0,G138*HBM_Inputs!G74/(HBM_Inputs!G11*Scenario!G$35))` | 계산값: 1.3446972
...
- HBM_Demand!C148 | 수식: `=C140*HBM_Inputs!$B$6*Scenario!C$29/1000` | 계산값: 0.23595
- HBM_Demand!D148 | 수식: `=D140*HBM_Inputs!$B$6*Scenario!D$29/1000` | 계산값: 0.121
- HBM_Demand!E148 | 수식: `=E140*HBM_Inputs!$B$6*Scenario!E$29/1000` | 계산값: 0.030492
- HBM_Demand!F148 | 수식: `=F140*HBM_Inputs!$B$6*Scenario!F$29/1000` | 계산값: 0
- HBM_Demand!G148 | 수식: `=G140*HBM_Inputs!$B$6*Scenario!G$29/1000` | 계산값: 0
- HBM_Demand!A149 | 값:   NVIDIA Rubin (VR200)
- HBM_Demand!C149 | 수식: `=C141*HBM_Inputs!$B$7*Scenario!C$29/1000` | 계산값: 0.08712
- HBM_Demand!D149 | 수식: `=D141*HBM_Inputs!$B$7*Scenario!D$29/1000` | 계산값: 0.200190638297872
- HBM_Demand!E149 | 수식: `=E141*HBM_Inputs!$B$7*Scenario!E$29/1000` | 계산값: 0.2128896
- HBM_Demand!F149 | 수식: `=F141*HBM_Inputs!$B$7*Scenario!F$29/1000` | 계산값: 0.154215149268293
- HBM_Demand!G149 | 수식: `=G141*HBM_Inputs!$B$7*Scenario!G$29/1000` | 계산값: 0.104825718568421
- HBM_Demand!A150 | 값:   NVIDIA Rubin Ultra / Feynman
- HBM_Demand!C150 | 수식: `=C142*HBM_Inputs!$B$8*Scenario!C$29/1000` | 계산값: 0
- HBM_Demand!D150 | 수식: `=D142*HBM_Inputs!$B$8*Scenario!D$29/1000` | 계산값: 0.069696
- HBM_Demand!E150 | 수식: `=E142*HBM_Inputs!$B$8*Scenario!E$29/1000` | 계산값: 0.1951488
- HBM_Demand!F150 | 수식: `=F142*HBM_Inputs!$B$8*Scenario!F$29/1000` | 계산값: 0.291822513230769
- HBM_Demand!G150 | 수식: `=G142*HBM_Inputs!$B$8*Scenario!G$29/1000` | 계산값: 0.368831232
- HBM_Demand!A151 | 값:   Google TPU (v7 Ironwood~)
- HBM_Demand!C151 | 수식: `=C143*HBM_Inputs!$B$9*Scenario!C$29/1000` | 계산값: 0
- HBM_Demand!D151 | 수식: `=D143*HBM_Inputs!$B$9*Scenario!D$29/1000` | 계산값: 0
- HBM_Demand!E151 | 수식: `=E143*HBM_Inputs!$B$9*Scenario!E$29/1000` | 계산값: 0
- HBM_Demand!F151 | 수식: `=F143*HBM_Inputs!$B$9*Scenario!F$29/1000` | 계산값: 0
- HBM_Demand!G151 | 수식: `=G143*HBM_Inputs!$B$9*Scenario!G$29/1000` | 계산값: 0
- HBM_Demand!A152 | 값:   AWS Trainium (2/3)
- HBM_Demand!C152 | 수식: `=C144*HBM_Inputs!$B$10*Scenario!C$29/1000` | 계산값: 0
- HBM_Demand!D152 | 수식: `=D144*HBM_Inputs!$B$10*Scenario!D$29/1000` | 계산값: 0
- HBM_Demand!E152 | 수식: `=E144*HBM_Inputs!$B$10*Scenario!E$29/1000` | 계산값: 0
- HBM_Demand!F152 | 수식: `=F144*HBM_Inputs!$B$10*Scenario!F$29/1000` | 계산값: 0
- HBM_Demand!G152 | 수식: `=G144*HBM_Inputs!$B$10*Scenario!G$29/1000` | 계산값: 0
- HBM_Demand!A153 | 값:   기타 (AMD MI/ MTIA / Maia)
- HBM_Demand!C153 | 수식: `=C145*HBM_Inputs!$B$11*Scenario!C$29/1000` | 계산값: 0.0516266666666667
- HBM_Demand!D153 | 수식: `=D145*HBM_Inputs!$B$11*Scenario!D$29/1000` | 계산값: 0.092928
- HBM_Demand!E153 | 수식: `=E145*HBM_Inputs!$B$11*Scenario!E$29/1000` | 계산값: 0.1486848
- HBM_Demand!F153 | 수식: `=F145*HBM_Inputs!$B$11*Scenario!F$29/1000` | 계산값: 0.259397789538462
- HBM_Demand!G153 | 수식: `=G145*HBM_Inputs!$B$11*Scenario!G$29/1000` | 계산값: 0.3442424832
...
- HBM_Demand!C156 | 수식: `=IFERROR(C148*INDEX(HBM_Inputs!C$16:C$18,MATCH(HBM_Inputs!H6,HBM_Inputs!$A$16:$A$18,0))*Scenario!C$23,0)` | 계산값: 2.3595
- HBM_Demand!D156 | 수식: `=IFERROR(D148*INDEX(HBM_Inputs!D$16:D$18,MATCH(HBM_Inputs!I6,HBM_Inputs!$A$16:$A$18,0))*Scenario!D$23,0)` | 계산값: 1.573
- HBM_Demand!E156 | 수식: `=IFERROR(E148*INDEX(HBM_Inputs!E$16:E$18,MATCH(HBM_Inputs!J6,HBM_Inputs!$A$16:$A$18,0))*Scenario!E$23,0)` | 계산값: 0.365904
- HBM_Demand!F156 | 수식: `=IFERROR(F148*INDEX(HBM_Inputs!F$16:F$18,MATCH(HBM_Inputs!K6,HBM_Inputs!$A$16:$A$18,0))*Scenario!F$23,0)` | 계산값: 0
- HBM_Demand!G156 | 수식: `=IFERROR(G148*INDEX(HBM_Inputs!G$16:G$18,MATCH(HBM_Inputs!L6,HBM_Inputs!$A$16:$A$18,0))*Scenario!G$23,0)` | 계산값: 0
- HBM_Demand!A157 | 값:   NVIDIA Rubin (VR200)
- HBM_Demand!C157 | 수식: `=IFERROR(C149*INDEX(HBM_Inputs!C$16:C$18,MATCH(HBM_Inputs!H7,HBM_Inputs!$A$16:$A$18,0))*Scenario!C$23,0)` | 계산값: 1.3068
- HBM_Demand!D157 | 수식: `=IFERROR(D149*INDEX(HBM_Inputs!D$16:D$18,MATCH(HBM_Inputs!I7,HBM_Inputs!$A$16:$A$18,0))*Scenario!D$23,0)` | 계산값: 4.00381276595745
- HBM_Demand!E157 | 수식: `=IFERROR(E149*INDEX(HBM_Inputs!E$16:E$18,MATCH(HBM_Inputs!J7,HBM_Inputs!$A$16:$A$18,0))*Scenario!E$23,0)` | 계산값: 3.8320128
- HBM_Demand!F157 | 수식: `=IFERROR(F149*INDEX(HBM_Inputs!F$16:F$18,MATCH(HBM_Inputs!K7,HBM_Inputs!$A$16:$A$18,0))*Scenario!F$23,0)` | 계산값: 2.46744238829268
- HBM_Demand!G157 | 수식: `=IFERROR(G149*INDEX(HBM_Inputs!G$16:G$18,MATCH(HBM_Inputs!L7,HBM_Inputs!$A$16:$A$18,0))*Scenario!G$23,0)` | 계산값: 1.4675600599579
- HBM_Demand!A158 | 값:   NVIDIA Rubin Ultra / Feynman
- HBM_Demand!C158 | 수식: `=IFERROR(C150*INDEX(HBM_Inputs!C$16:C$18,MATCH(HBM_Inputs!H8,HBM_Inputs!$A$16:$A$18,0))*Scenario!C$23,0)` | 계산값: 0
- HBM_Demand!D158 | 수식: `=IFERROR(D150*INDEX(HBM_Inputs!D$16:D$18,MATCH(HBM_Inputs!I8,HBM_Inputs!$A$16:$A$18,0))*Scenario!D$23,0)` | 계산값: 1.672704
- HBM_Demand!E158 | 수식: `=IFERROR(E150*INDEX(HBM_Inputs!E$16:E$18,MATCH(HBM_Inputs!J8,HBM_Inputs!$A$16:$A$18,0))*Scenario!E$23,0)` | 계산값: 4.2932736
- HBM_Demand!F158 | 수식: `=IFERROR(F150*INDEX(HBM_Inputs!F$16:F$18,MATCH(HBM_Inputs!K8,HBM_Inputs!$A$16:$A$18,0))*Scenario!F$23,0)` | 계산값: 5.83645026461539
- HBM_Demand!G158 | 수식: `=IFERROR(G150*INDEX(HBM_Inputs!G$16:G$18,MATCH(HBM_Inputs!L8,HBM_Inputs!$A$16:$A$18,0))*Scenario!G$23,0)` | 계산값: 6.638962176
- HBM_Demand!A159 | 값:   Google TPU (v7 Ironwood~)
- HBM_Demand!C159 | 수식: `=IFERROR(C151*INDEX(HBM_Inputs!C$16:C$18,MATCH(HBM_Inputs!H9,HBM_Inputs!$A$16:$A$18,0))*Scenario!C$23,0)` | 계산값: 0
- HBM_Demand!D159 | 수식: `=IFERROR(D151*INDEX(HBM_Inputs!D$16:D$18,MATCH(HBM_Inputs!I9,HBM_Inputs!$A$16:$A$18,0))*Scenario!D$23,0)` | 계산값: 0
- HBM_Demand!E159 | 수식: `=IFERROR(E151*INDEX(HBM_Inputs!E$16:E$18,MATCH(HBM_Inputs!J9,HBM_Inputs!$A$16:$A$18,0))*Scenario!E$23,0)` | 계산값: 0
- HBM_Demand!F159 | 수식: `=IFERROR(F151*INDEX(HBM_Inputs!F$16:F$18,MATCH(HBM_Inputs!K9,HBM_Inputs!$A$16:$A$18,0))*Scenario!F$23,0)` | 계산값: 0
- HBM_Demand!G159 | 수식: `=IFERROR(G151*INDEX(HBM_Inputs!G$16:G$18,MATCH(HBM_Inputs!L9,HBM_Inputs!$A$16:$A$18,0))*Scenario!G$23,0)` | 계산값: 0
- HBM_Demand!A160 | 값:   AWS Trainium (2/3)
- HBM_Demand!C160 | 수식: `=IFERROR(C152*INDEX(HBM_Inputs!C$16:C$18,MATCH(HBM_Inputs!H10,HBM_Inputs!$A$16:$A$18,0))*Scenario!C$23,0)` | 계산값: 0
- HBM_Demand!D160 | 수식: `=IFERROR(D152*INDEX(HBM_Inputs!D$16:D$18,MATCH(HBM_Inputs!I10,HBM_Inputs!$A$16:$A$18,0))*Scenario!D$23,0)` | 계산값: 0
- HBM_Demand!E160 | 수식: `=IFERROR(E152*INDEX(HBM_Inputs!E$16:E$18,MATCH(HBM_Inputs!J10,HBM_Inputs!$A$16:$A$18,0))*Scenario!E$23,0)` | 계산값: 0
- HBM_Demand!F160 | 수식: `=IFERROR(F152*INDEX(HBM_Inputs!F$16:F$18,MATCH(HBM_Inputs!K10,HBM_Inputs!$A$16:$A$18,0))*Scenario!F$23,0)` | 계산값: 0
- HBM_Demand!G160 | 수식: `=IFERROR(G152*INDEX(HBM_Inputs!G$16:G$18,MATCH(HBM_Inputs!L10,HBM_Inputs!$A$16:$A$18,0))*Scenario!G$23,0)` | 계산값: 0
- HBM_Demand!A161 | 값:   기타 (AMD MI/ MTIA / Maia)
- HBM_Demand!C161 | 수식: `=IFERROR(C153*INDEX(HBM_Inputs!C$16:C$18,MATCH(HBM_Inputs!H11,HBM_Inputs!$A$16:$A$18,0))*Scenario!C$23,0)` | 계산값: 0.516266666666667
- HBM_Demand!D161 | 수식: `=IFERROR(D153*INDEX(HBM_Inputs!D$16:D$18,MATCH(HBM_Inputs!I11,HBM_Inputs!$A$16:$A$18,0))*Scenario!D$23,0)` | 계산값: 1.85856
- HBM_Demand!E161 | 수식: `=IFERROR(E153*INDEX(HBM_Inputs!E$16:E$18,MATCH(HBM_Inputs!J11,HBM_Inputs!$A$16:$A$18,0))*Scenario!E$23,0)` | 계산값: 2.6763264
- HBM_Demand!F161 | 수식: `=IFERROR(F153*INDEX(HBM_Inputs!F$16:F$18,MATCH(HBM_Inputs!K11,HBM_Inputs!$A$16:$A$18,0))*Scenario!F$23,0)` | 계산값: 5.18795579076923
- HBM_Demand!G161 | 수식: `=IFERROR(G153*INDEX(HBM_Inputs!G$16:G$18,MATCH(HBM_Inputs!L11,HBM_Inputs!$A$16:$A$18,0))*Scenario!G$23,0)` | 계산값: 6.1963646976
