...
LAYER = {
    ...
    'points': [
        ...
        '<b>넓히는 이유는 비선형이 일할 자리를 만들기 위해서다.</b> 곱셈만 이으면 아무리 층을 '
        '쌓아도 한 번의 곱셈으로 접힌다 — 선형에 선형을 이으면 선형이다. 「A 이면서 B 는 아닐 '
        '때만」 같은 조건을 익히려면 중간에 껐다 켜는 장치(게이트)가 있어야 하는데, 좁은 자리에 '
        '서는 끄고 켤 것이 몇 개 없다. 넓혀 놓아야 검출기를 많이 두고 맞는 것만 살릴 수 있다.',
