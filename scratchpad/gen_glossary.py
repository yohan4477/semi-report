...
LAYER = {
    ...
    'points': [
        ...
        '<b>곱셈만으로는 「이럴 땐 이렇게」를 못 만든다.</b> 곱셈과 덧셈은 들어온 수에 늘 '
        '같은 규칙을 먹인다 — 입력이 커지면 출력도 그만큼 커질 뿐이다. 그런데 말을 다루려면 '
        '경우를 갈라야 한다. 「앞에 부정어가 붙었으면 뜻을 뒤집는다」 같은 판단은 항상 같은 '
        '비율로 늘렸다 줄였다 하는 방식에서는 안 나온다. 게다가 곱셈만 이으면 층을 쌓아도 한 '
        '층으로 접힌다 — 2 를 곱하고 3 을 더한 뒤 5 를 곱하고 1 을 더하면 10 을 곱하고 16 을 '
        '더한 것과 같다.',
        '<b>문턱이 그 경우를 나눈다.</b> 들어온 수에서 5 를 뺀 뒤 음수면 0 으로 만들어 보자. '
        '5 보다 큰 입력만 살아남고 나머지는 0 이 된다. 이 「음수는 0 으로」는 곱셈이 아니라 '
        '접히지 않고, 이것 하나로 「어떤 선을 넘었을 때만」이 생긴다. FFN 에서 그 일을 하는 '
        '것이 게이트다.',
        '<b>넓히는 이유는 그 문턱을 많이 두려고다.</b> 문턱은 칸마다 하나씩 붙어서 칸이 '
        + str(_DM) + ' 개면 문턱도 ' + str(_DM) + ' 개, ' + _DF + ' 개로 늘리면 문턱도 ' + _DF
        + ' 개다. 칸 하나가 「이 패턴이면 켠다」는 스위치라 칸 수가 곧 세울 수 있는 조건의 '
        '수이고, 「A 이면서 B 는 아닐 때」를 가리려면 A 를 재는 칸과 B 를 재는 칸이 따로 있어야 '
        '한다. 그렇게 넓혀 두고 입력마다 맞는 칸만 켠다 — 그래서 ' + _DF + ' 칸 중 ' + _QUIET
        + '% 가 꺼진 채로 지나간다.',
