...
FIG_ATTN = (1, '어텐션 계산 한 번 — 14초짜리 한 바퀴',
            ga.embed(ga.load_trace(), 'attn-scene'),
            '실제로 Qwen2.5-0.5B 를 한 번 돌려 뽑은 수치다. 문장이 뜨고 → 「그것은」 자리에서 '
            'Q 가 내려오고 → 앞선 토큰들의 K 가 깔리고 → Q 가 K 위를 왼쪽부터 지나며 막대가 '
            '하나씩 서고 → 그 막대가 합 1 의 비율로 다시 서고 → 그 비율을 가중치로 V 를 가중평균해 '
            '남는다. 저절로 다시 돈다.')

ATTN = {
    'section': SEC_MODEL,
    'topic': ('compute', '모델 구조'),
    'title': '어텐션(attention) — 앞선 토큰을 얼마나 볼지 정하는 계산',
    'gain': '「그것은」이 문장 앞의 무엇을 가리키는지를 모델이 어떻게 정하는지. 계산 한 번이 '
            '도는 14초를 눈으로 따라간다.',
    'meta': ['용어 풀이', '정리 2026-09-20', '모델 구조'],
    'oneliner': '어텐션은 지금 토큰이 앞선 토큰들을 각각 얼마나 참고할지 정하는 계산이다. '
                '토큰마다 쿼리(Q — 내가 무엇을 찾는가), 키(K — 나는 무엇을 들고 있는가), '
                '밸류(V — 내가 줄 내용) 세 벡터를 뽑아 두고, 지금 토큰의 Q 를 앞선 토큰들의 K 와 '
                '하나씩 곱해 점수를 낸 뒤 그 점수를 합이 1 인 비율로 바꿔 그 비율을 가중치로 V 를 '
                '가중평균한다. 그 가중평균이 이 자리의 출력이다.',
    'points': [
        '<b>Q 는 한 자리 것 하나뿐이고 K·V 는 앞선 토큰 전부다.</b> Q 는 지금 토큰이 무엇을 '
        '찾는지라 그 스텝에서 쓰고 버린다. K·V 는 한 번 만들면 그 토큰이 대화에 남아 있는 한 '
        '계속 다시 쓰여서 캐시에 쌓인다 — 「KV 캐시」가 커진다는 말이 이 뜻이다.',
        '<b>점수는 닮은 정도다.</b> Q 와 K 를 곱하면(내적) 그 수가 클수록 그 토큰을 많이 본다. '
        '위 그림에서 「그것은」이 「장을」에 준 점수가 가장 컸다.',
        '<b>소프트맥스가 점수를 비율로 바꾼다.</b> 점수 자체는 크기가 제각각이라 그대로 못 쓴다. '
        '합이 1 이 되게 눌러 「어느 토큰을 몇 퍼센트 볼까」로 만든 뒤 그 비율을 가중치로 V 를 '
        '가중평균한다 — 비율 × V 를 토큰마다 구해 전부 더하는 것이다.',
        '<b>한 번으로 안 끝난다.</b> 이 계산이 모델의 층마다, 층 안에서도 헤드 여럿이 동시에 '
        '돈다. 헤드마다 보는 데가 달라서 어떤 헤드는 지시대명사가 가리키는 명사를, 어떤 헤드는 '
        '바로 앞 낱말을 본다. 위 그림은 그중 「그것은」이 앞 명사를 가장 세게 되짚는 헤드 하나를 '
        '골라 그린 것이다.',
        '<b>토큰이 늘수록 볼 것이 늘어난다.</b> 새 토큰마다 앞선 토큰 전부와 점수를 내야 해서 '
        '문맥이 길어질수록 계산과 캐시가 같이 커진다. 긴 문맥이 비싼 이유가 여기에 있다.',
    ],
    'figs': [FIG_ATTN],
    'note': 'Q·K·V 옆에 적힌 숫자는 벡터의 첫 성분 하나다 — 실제로는 64 개짜리 벡터이고 '
            '화면에 다 적을 수 없어 앞의 하나만 보였다. 그 수치를 굽는 자리는 '
            '<code>scratchpad/attn_trace.py</code>, 장면을 그리는 자리는 '
            '<code>scratchpad/gen_anim_page.py</code> 다.',
    'links': [],
}


FIG_FFN = (1, 'FFN 한 번 — 21초짜리 한 바퀴',
           gf.embed(gf.load_trace(), 'ffn-scene'),
           '어텐션과 같은 층, 같은 자리에서 잰 실제 수치다. 문장이 떴다가 「그것은」만 남고 → '
           '어텐션이 낸 결과가 입력으로 서고 → 넓은 층으로 펼쳐지고 → 게이트가 대부분을 눌러 '
           '일부만 밝게 남고 → 다시 원래 폭으로 접힌다. 칸 밝기가 그 자리에 남은 신호 크기다.')

FFN = {
    'section': SEC_MODEL,
    'topic': ('compute', '모델 구조'),
    'title': 'FFN(피드포워드) — 토큰마다 따로 넓혔다 접는 층',
    'gain': '어텐션 다음에 무엇이 오는지. 모델 무게의 큰 몫이 왜 이 층에 몰려 있는지.',
    'meta': ['용어 풀이', '정리 2026-09-20', '모델 구조'],
    'oneliner': 'FFN 은 트랜스포머 한 층에서 어텐션 다음에 오는 블록이다. 어텐션이 낸 결과를 '
                '받아 훨씬 넓은 층으로 펼쳤다가 다시 원래 폭으로 접는다. 어텐션은 토큰끼리 '
                '서로 보고 섞이는 계산이었는데 FFN 은 그 결과를 토큰 하나하나에 따로 먹인다 — '
                '옆 토큰을 안 본다.',
    'points': [
        '<b>넓혔다 접는다.</b> 이 모델(Qwen2.5-0.5B)에서는 896 칸이 4,864 칸으로 펼쳐졌다가 '
        '다시 896 칸으로 접힌다. 펼치는 가중치와 접는 가중치가 따로 있어서, 모델 무게의 큰 '
        '몫이 이 층에 들어간다.',
        '<b>대부분은 잠잠하다.</b> 넓힌 4,864 칸 중 97.7% 가 가장 센 자리의 10% 에도 못 미쳤다. '
        '게이트가 문맥에 안 맞는 자리를 눌러 끄기 때문이고, 그래서 한 토큰이 실제로 쓰는 자리는 '
        '몇 십 개다.',
        '<b>토큰끼리 안 섞인다.</b> 어텐션은 앞선 토큰 전부를 읽지만 FFN 은 지금 토큰의 벡터 '
        '하나만 본다. 그래서 토큰 수가 늘어도 이 층의 계산은 토큰마다 독립으로 늘어날 뿐 '
        '서로 얽히지 않는다.',
        '<b>MoE 는 이 층을 쪼갠 것이다.</b> FFN 한 벌 대신 여러 벌(전문가)을 두고 토큰마다 '
        '그중 일부만 골라 돌린다. 「전문가 병렬」이라는 말이 가리키는 것이 이 여러 벌을 GPU '
        '여러 장에 나눠 놓는 일이다.',
        '<b>엔그램은 또 다르다.</b> FFN 은 매번 곱셈을 하고 MoE 는 곱셈을 골라서 하는데, '
        '엔그램(N-gram 임베딩)은 곱셈 없이 표에서 행을 읽어 온다. 셋을 섞는 비율이 요즘 모델 '
        '설계의 다툼거리다.',
    ],
    'figs': [FIG_FFN],
    'note': '입력·출력 칸 옆의 숫자는 벡터의 첫 성분 하나다. 넓힌 칸 64 개는 4,864 칸에서 '
            '고르게 뽑아 보인 것이고 전부가 아니다. 그 수치를 굽는 자리는 '
            '<code>scratchpad/ffn_trace.py</code> 이고, 거기서 직접 계산한 결과가 모델이 낸 '
            'MLP 출력과 같은지를 굽는 단계에서 검사한다(차이 0.0).',
    'links': [],
}
