...
FIG_FFN = (5, '2막 — FFN (21초짜리 한 바퀴)',
           ...
           '「그것은」만 남는 것은 FFN 이 옆 토큰을 안 보기 때문이다. 칸 밝기가 그 자리에 남은 '
           '신호 크기다.')
...
LAYER = {
    ...
    'oneliner': '트랜스포머 한 층은 두 걸음이다. 먼저 어텐션이 지금 토큰과 앞선 토큰들을 '
                '섞고, 그다음 FFN 이 그 결과를 토큰 하나하나에 따로 먹여 다듬는다. '
                '어텐션은 토큰마다 쿼리(Q — 내가 무엇을 찾는가), 키(K — 나는 무엇을 들고 '
                '있는가), 밸류(V — 내가 줄 내용) 세 벡터를 뽑아 두고, 지금 토큰의 Q 를 앞선 '
                '토큰들의 K 와 하나씩 곱해 점수를 낸 뒤 그 점수를 합이 1 인 비율로 바꿔 그 '
                '비율을 가중치로 V 를 가중평균한다. 섞는 걸음은 토큰끼리 서로 보고 다듬는 '
                '걸음은 옆을 안 본다. 이 두 걸음이 층마다 되풀이된다.',
    'points': [
        '<b>어텐션 — Q 는 한 자리 것 하나, K·V 는 앞선 토큰 전부다.</b> Q 는 지금 토큰이 '
        '무엇을 찾는지라 그 스텝에서 쓰고 버린다. K·V 는 한 번 만들면 그 토큰이 대화에 '
        '남아 있는 한 계속 다시 쓰여서 캐시에 쌓인다 — 「KV 캐시」가 커진다는 말이 이 뜻이다.',
        '<b>Q·K·V 는 같은 토큰에서 뽑은 세 벡터다.</b> 같은 임베딩에 서로 다른 가중치를 '
        '곱해 만든다. Q 는 그 토큰이 무엇을 찾는지, K 는 그 토큰이 무엇을 들고 있는지, '
        'V 는 골라졌을 때 실제로 내줄 내용이다. 그래서 Q 와 K 가 만나 「얼마나 볼까」를 '
        '정하고, 그 비율로 섞이는 것은 V 다 — 어디를 많이 보느냐와 결과를 많이 정하느냐가 '
        '같지 않은 이유가 여기 있다.',
        ...
        '<b>FFN 의 「넓힌다」는 숫자 개수를 늘린다는 뜻이다.</b> 들어온 벡터에는 숫자가 '
        + str(_DM) + ' 개 있는데 ' + str(_DM) + ' × ' + _DF + ' 짜리 행렬을 곱하면 ' + _DF
        + ' 개가 나온다. 정보가 늘어난 것이 아니라 같은 내용을 더 넓은 자리에 펼쳐 놓은 것이다.',
        '<b>펼친 칸 하나가 질문 하나다.</b> 넓히는 행렬의 열 하나가 학습된 방향 하나이고, '
        '그 열과 입력을 곱한 수가 「이 입력이 그 방향과 얼마나 닮았나」다. 넓힌다는 것은 '
        + _DF + ' 개의 질문을 동시에 던지는 일이고, 무엇을 묻는지는 학습이 정했다.',
        '<b>곱셈만 겹치면 층을 쌓는 뜻이 없다.</b> 어떤 수에 2 를 곱하고 3 을 더한 다음 거기에 '
        '5 를 곱하고 1 을 더해 보자. 두 단계를 거친 것 같지만 결과는 「10 을 곱하고 16 을 '
        '더한 것」과 똑같다 — 한 단계로 접힌다. 행렬도 같아서, 곱셈만 이으면 층을 백 개 쌓아도 '
        '한 층짜리와 할 수 있는 일이 같다.',
        '<b>그래서 중간에 문턱을 끼운다.</b> 곱셈으로는 못 만드는 것이 하나 있다 — 「어떤 선을 '
        '넘었을 때만」이다. 들어온 수에서 5 를 뺀 뒤 음수면 0 으로 만들면, 그때부터 5 보다 큰 '
        '입력만 살아남고 나머지는 0 이 된다. 이 「음수는 0 으로」가 곱셈이 아니라 접히지 않고, '
        '덕분에 조건이 생긴다. FFN 에서 그 일을 하는 것이 게이트다.',
        '<b>넓히는 이유는 그 문턱을 많이 두려고다.</b> 문턱은 칸마다 하나씩 붙는다. 칸이 '
        + str(_DM) + ' 개면 문턱도 ' + str(_DM) + ' 개, ' + _DF + ' 개로 늘리면 문턱도 ' + _DF
        + ' 개다. 문턱 하나가 「이 패턴이면 켜고 아니면 끈다」는 스위치 하나이므로 칸 수가 곧 '
        '세울 수 있는 조건의 수다. 「A 이면서 B 는 아닐 때」를 판단하려면 A 를 재는 칸과 B 를 '
        '재는 칸이 따로 있어야 하는데, 좁은 자리에는 그만큼을 못 둔다.',
        '<b>그래서 대부분은 꺼진다.</b> 넓힌 ' + _DF + ' 칸 중 ' + _QUIET
        + '% 가 가장 센 자리의 10% 에도 못 미쳤다. 게이트가 문맥에 안 맞는 질문을 눌러 끄기 '
        '때문이고, 한 토큰이 실제로 쓰는 자리는 몇 십 개다.',
        '<b>다시 접는 이유도 분명하다.</b> 살아남은 질문들의 답을 조합해 원래 폭으로 되돌려야 '
        '다음 층이 받는다. 층들이 같은 폭을 주고받아야 쌓을 수 있고, 잔차 연결로 들어온 벡터에 '
