...
FIG_LAYER = (1, '트랜스포머 한 층 — 36초짜리 한 바퀴',
             gl.embed(*gl.load(), uid='layer-scene'),
             '실제로 Qwen2.5-0.5B 를 한 번 돌려 뽑은 수치다. 1막에서 「그것은」 자리의 Q 가 '
             '앞선 토큰들의 K 를 훑어 점수를 내고, 그 점수가 비율이 되어 V 를 가중평균한다. '
             '그렇게 맺힌 상자가 그대로 올라가 2막의 입력이 된다 — 넓은 층으로 펼쳐졌다가 '
             '게이트에 눌려 일부만 남고, 다시 원래 폭으로 접힌다. 저절로 다시 돈다.')
...
LAYER = {
    ...
    'points': [
        ...
        '<b>FFN — 넓혔다 접는다.</b> 이 모델에서는 896 칸이 4,864 칸으로 펼쳐졌다가 다시 '
        '896 칸으로 접힌다. 펼치는 가중치와 접는 가중치가 따로 있어서 모델 무게의 큰 몫이 '
        '이 층에 들어간다.',
        '<b>넓힌 자리 대부분은 잠잠하다.</b> 4,864 칸 중 97.7% 가 가장 센 자리의 10% 에도 '
        '못 미쳤다. 게이트가 문맥에 안 맞는 자리를 눌러 끄기 때문이고, 그래서 한 토큰이 실제로 '
        '쓰는 자리는 몇 십 개다.',
        '<b>두 걸음의 성격이 다르다.</b> 어텐션은 토큰이 늘수록 볼 것이 늘어 계산과 캐시가 같이 '
        '커지지만, FFN 은 토큰마다 독립이라 서로 얽히지 않는다. 긴 문맥이 비싸지는 쪽은 '
        '어텐션이다.',
        '<b>MoE 와 엔그램은 FFN 자리를 바꾼 것이다.</b> MoE 는 FFN 한 벌 대신 여러 벌(전문가)을 '
        '두고 토큰마다 일부만 골라 돌린다. 엔그램(N-gram 임베딩)은 아예 곱셈 없이 표에서 행을 '
        '읽어 온다. 셋을 섞는 비율이 요즘 모델 설계의 다툼거리다.',
    ],
    'figs': [FIG_LAYER],
    'note': 'Q·K·V 옆과 FFN 입력·출력 옆에 적힌 숫자는 벡터의 첫 성분 하나다 — 실제로는 '
            '어텐션 헤드가 64 개, FFN 입출력이 896 개짜리 벡터다. 넓힌 칸 64 개도 4,864 칸에서 '
            '고르게 뽑아 보인 것이고 전부가 아니다. 그 수치를 굽는 자리는 '
            '<code>scratchpad/attn_trace.py</code> 와 <code>scratchpad/ffn_trace.py</code> 이고, '
            '거기서 직접 계산한 결과가 모델이 낸 것과 같은지를 굽는 단계에서 검사한다.',
    'links': [],
}
