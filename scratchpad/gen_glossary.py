...
FIG_FFN = (5, '2막 — FFN (21초짜리 한 바퀴)',
           ...
           '열쇠-값 짝 하나이고, 칸 밝기가 지금 문맥에 그 열쇠가 얼마나 걸렸는지다.')
...
LAYER = {
    ...
    'points': [
        ...
        '곱해 만든다. Q 는 그 토큰이 무엇을 찾는지라 그 스텝에서 쓰고 버리고, K(무엇을 들고 '
        '있는지)와 V(골라졌을 때 내줄 내용)는 한 번 만들면 그 토큰이 대화에 남아 있는 한 계속 '
        ...
        '<b>이 층은 기억 장치로 밝혀졌다.</b> 게바 등이 2021 년에 보인 것이 지금 통용되는 '
        '설명이다 — 넓히는 행렬의 열 하나가 <b>열쇠</b>이고 접는 행렬의 행 하나가 <b>값</b>이라, '
        '둘이 짝을 이룬 열쇠-값 기억 장치로 움직인다. 열쇠는 학습 데이터에서 본 글의 패턴에 '
        '반응하고, 짝지어진 값은 그 패턴 다음에 올 법한 토큰 쪽으로 확률을 민다.',
        '<b>그래서 넓힐수록 외울 자리가 는다.</b> 열쇠-값 짝 하나가 기억 한 칸이므로 칸 수가 곧 '
        '그 층이 담을 수 있는 패턴의 수다. 아래쪽 층은 얕은 패턴(글자 꼴·어구)에, 위쪽 층은 '
        '뜻에 가까운 패턴에 반응한다고 같은 연구가 짚었다. 이 모델에서 ' + _DF + ' 칸 중 '
        + _QUIET + '% 가 꺼진 채로 지나가는 것도 그래서다 — 지금 문맥에 걸리는 열쇠만 켜진다.',
        ...
        '<b>곱셈만 이어서는 그 일이 안 된다.</b> 곱셈과 덧셈은 들어온 수에 늘 같은 규칙을 '
        '먹여서 「이 패턴일 때만 켠다」가 안 나온다. 들어온 수에서 5 를 뺀 뒤 음수면 0 으로 '
        '만드는 식의 문턱이 있어야 그 열쇠가 걸렸는지 아닌지가 갈린다. 문턱이 없으면 층을 '
        '아무리 쌓아도 한 층으로 접힌다 — 2 를 곱하고 3 을 더한 뒤 5 를 곱하고 1 을 더하면 '
        '10 을 곱하고 16 을 더한 것과 같다.',
        '<b>다시 접는 이유도 분명하다.</b> 걸린 열쇠들의 값을 모아 원래 폭으로 되돌려야 '
    ...
    'links': [
        ('FFN 은 열쇠-값 기억 장치다 (Geva et al., EMNLP 2021)',
