...
#
# **한 편에 둘까지다.** 처음엔 아홉 장을 붙였는데 일곱이 옆 문단을 19px로 다시
# 타이핑한 상자 목록이었다. 목록은 표로 내리고, 판에는 무엇을 주고 무엇을 받는지가
# 보이는 것만 남긴다 — 도해 스킬의 「상자만 나열했다」가 정확히 그 사고다.
...
_R_STATE = svg(452,
    ...
    + box(_SLX, 32, _SB, 150, ['하네스', '(아무것도 안 쥠)'], 'fig-box fig-outside')
    + box(_SRX, 32, _SB, 150, ['모델', '지금 몇 단계인가', '다음은 무엇인가',
                               '레슨이 끝났는가', '무슨 말을 할까'], 'fig-box fig-stage')
    + arrow(_SLX + _SB + 8, _SRX - 8, 107, '전부 다 해 줘')
    + head(0, 232, W, '하네스가 흐름을 쥘 때')
    + box(_SLX, 242, _SB, 150, ['하네스', '지금 몇 단계인가', '다음은 무엇인가',
                                '레슨이 끝났는가'], 'fig-box fig-agent')
    + box(_SRX, 242, _SB, 150, ['모델', '이번 단계의 말 하나'], 'fig-box fig-stage')
    + arrow(_SLX + _SB + 8, _SRX - 8, 292, '이 한 가지만 하고')
    + arrow(_SLX + _SB + 8, _SRX - 8, 352, '결과 (제안일 뿐)', True)
    + legend([('fig-agent', '하네스가 쥔 것'), ('fig-stage', '모델이 쥔 것')], 414),
    '위에서는 지금 몇 단계인지와 다음이 무엇인지와 끝났는지를 모델이 다 쥐고 있고, '
    '아래에서는 그 셋이 하네스로 넘어가 모델에게는 이번 단계의 말 하나만 남는다')
...
# ── 한 단계가 도는 순서 ─────────────────────────────────────────────
# 위 그림이 「무엇을 누가 쥐나」를 말하니, 이 장은 「그래서 한 번에 무엇이 오가나」만 맡는다.
_lw = w_of(['학생'], ['하네스'], ['모델'])
_L1X = _lw / 2
_L3X = W - _lw / 2
_L2X = (_L1X + _L3X) / 2
_R_SEQ = svg(430,
    ...
    '학생과 하네스와 모델 사이에 오가는 한 단계의 순서. 하네스가 입력을 만들어 모델에 넘기고 '
    '결과를 검증한 뒤 상태를 넘긴다')
...
_R_SEQ_CAP = ('앞 장이 무엇을 누가 쥐는지였다면 이 장은 한 번에 무엇이 오가는지다. '
              ...
              '상태를 넘긴다는 설명과 화이트보드 하네스가 따로 있다는 설명을 한 턴으로 엮은 그림이다.')
