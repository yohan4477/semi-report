...
FIG_IDMSEQ = _svg(640, 296, '같은 IDM 에 찍은 장면이 들어가느냐 그린 장면이 들어가느냐', ''.join([
    _box(14, 34, 196, 68, ['카메라가 찍은 장면', '사람·로봇이 실제로 한 일'], 'var(--ink-3)', 1.5),
    _box(14, 162, 196, 68, ['모델이 그려 낸 장면', '아직 일어나지 않은 일'], 'var(--ink-3)', 1.5),
    _a(212, 68, 240, 116), _a(212, 196, 240, 148),
    _box(242, 100, 148, 64, ['IDM', '장면 두 장의 차이를', '관절 움직임으로'], 'var(--accent)', 1.8),
    _a(392, 116, 420, 68), _a(392, 148, 420, 196),
    _box(422, 34, 204, 68, ['학습 재료가 된다', '정책이 나중에 이걸로 배운다'], 'var(--accent)', 1.8),
    _box(422, 162, 204, 68, ['로봇 팔로 그대로 간다', '지금 실행한다'], 'var(--accent)', 1.8),
    _t(320, 262, '두 줄의 차이는 하나뿐이다 — IDM 이 받은 장면이 찍힌 것이냐 그려 낸 것이냐', 't-sm'),
    _t(320, 282, '나오는 관절 값은 같은 종류이고, 그것을 나중에 쓰느냐 지금 쓰느냐만 갈린다', 't-sm'),
]))
...
# 같은 일을 순서로 — 후보가 들어가서 점수가 되어 나온다
FIG_WMSEQ = _svg(640, 178, '행동 후보가 상태가치로 바뀌어 걸러지는 순서', ''.join([
    _lt(14, 22, '한 후보가 걸러지는 순서'),
    _chain(32, [
        (14, 158, ['① 정책이 낸', '행동 후보'], False),
        (196, 176, ['② 1XWM 이 상상한다', '미래 1인칭 영상 4초'], True),
        (396, 118, ['③ 상태가치', '0 이하면 실패'], True),
        (538, 88, ['④ 남은 것만', '실물에서'], False)]),
    _t(320, 126, '후보 여럿을 같은 초기 상태에서 돌려 놓고 점수로 줄을 세운다', 't-sm'),
    _t(320, 148, '실물 로봇은 마지막 칸에서 한 번만 움직인다 — 앞 세 칸은 모델 안에서 끝난다', 't-sm'),
    _t(320, 170, '1X Technologies 가 에어프라이어 열기 같은 과제를 이렇게 걸렀다', 't-sm'),
]))
...
def summary_layer():
    figs = [('관절까지 가는 네 갈래', FIG_ROUTE4,
            ...
            ('같은 IDM, 들어가는 장면만 다르다', FIG_IDMSEQ,
             '위 그림이 <b>IDM 이 무엇을 채우나</b>라면 이 그림은 <b>그 장면이 어디서 오나</b>다. '
             '위로 들어가는 것은 카메라가 찍은 장면이다 — 1X Technologies 는 사람 1인칭 영상 '
             '<b>900시간</b>에 이렇게 관절 값을 붙여 학습 재료로 바꿨고, 엔비디아 DreamGen 도 같은 '
             '방식으로 뽑은 의사(疑似) 액션으로 VLA 를 학습시킨다. 아래로 들어가는 것은 아직 '
             '일어나지 않은 장면, 즉 모델이 그려 낸 것이다 — Rhoda AI 의 DVA 가 이쪽이고, 그 관절 '
             '값은 학습에 쌓이지 않고 곧장 로봇 팔로 간다.'),
            ...
            ('후보가 점수로 바뀌는 순서', FIG_WMSEQ,
             '위 그림이 <b>무엇을 대신하나</b>라면 이 그림은 <b>어떤 순서로 대신하나</b>다. '
             '들어가는 것은 정책이 낸 행동 후보이고 나오는 것은 숫자 하나다. 그 숫자로 줄을 세워 '
             '떨어뜨리고 나면, 실물 로봇은 <b>마지막에 한 번</b>만 움직인다. 월드모델이 관절을 '
             '직접 내지 않는데도 갈래 하나를 차지하는 이유가 이것이다 — 관절을 내는 대신 '
             '<b>어느 후보를 버릴지</b>를 정해 준다.'),
