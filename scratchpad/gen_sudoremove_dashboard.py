...
CARDS = [{
    ...
    'slim_points': [
        ...
        '다음 장면을 잇는다.',
        '<b>행동은 그 장면에서 거꾸로 계산해 나온다.</b> 예측한 프레임을 인버스 다이내믹스 모델(상태 '
        '변화로부터 그 변화를 만든 행동을 되짚는 모델)에 넣어 관절 명령으로 바꾸고, 비디오 모델과 '
        '번갈아 돌린다. 지금 실행하는 행동은 앞서 예측해 둔 것이다.',
        ...
        '<b>Sharpa는 손안 조작만 떼어 강화학습으로 풀었다.</b> PPO로 아이작 랩에서 학습한 정책 둘이 '
        '물체를 안정적으로 쥐는 것과 축을 기준으로 돌리는 것을 맡고, 조종자는 발 페달 세 개로 그 '
        '스킬을 켠다. 모방학습 단계에서 페달을 누른 시점까지 배워 VLA의 출력 하나가 스킬 켜기가 된다.',
