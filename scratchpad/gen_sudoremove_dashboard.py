...
FIG_WMSEQ = _svg(640, 452, '같은 자리에서 후보 둘을 상상시켜 상태가치로 가른다', ''.join([
    ...
    _t(320, 404, '상태가치 — 상상한 장면이 과제를 끝낸 쪽에 얼마나 가까운지를 숫자 하나로 매긴 값이다', 't-sm'),
    _t(320, 424, '후보는 사람이 짠 동작이 아니라 학습 도중 저장해 둔 정책 여러 개다', 't-sm'),
    _t(320, 444, '문이 열리고 안 열리는 두 장면은 과제를 보이려고 세운 예시다', 't-sm'),
]))
...
def summary_layer():
    figs = [('관절까지 가는 네 갈래', FIG_ROUTE4,
            ...
            ('에어프라이어 문 하나로 보는 한 판', FIG_WMSEQ,
             ...
             '정책 여러 개</b>다. 왼쪽 한 칸이 그 둘의 출발점이다 — 로봇이 조리기구 앞에 서 있고 문은 '
             '닫혀 있다. 여기서 후보 A 와 후보 B 가 각각 낼 행동을 1XWM 에 넣으면, 모델이 그 행동대로 '
             '갔을 때의 <b>미래 1인칭 영상 4초</b>를 그린다. 오른쪽 두 칸이 그렇게 그려 낸 장면이다. '
             '<b>상태가치</b>는 그 장면이 과제를 끝낸 쪽에 얼마나 가까운지를 숫자 하나로 매긴 값이다 — '
             '1XWM 은 영상과 이 숫자를 같은 표현에서 함께 내고, <b>0 이하면 실패로 친다</b>. '
             '문이 열린 쪽만 남기고 나머지는 그 자리에서 떨어지므로, 실물 로봇은 살아남은 후보에 대해 '
             '<b>마지막에 한 번</b>만 움직인다.'),
