...
FIG_IDMSEQ = _svg(640, 390, '카메라가 찍은 장면과 모델이 그려 낸 장면이 같은 IDM 으로 들어간다',
                  ''.join([
    _lt(14, 22, '카메라가 찍은 장면 — 실제로 일어난 일이다'),
    ...
    _a(378, 150, 400, 96),
    _a(378, 196, 400, 290),
    _box(402, 62, 224, 66, ['학습 재료가 된다', '정책이 나중에 이걸로 배운다'], 'var(--accent)', 1.8),
    _box(402, 256, 224, 66, ['로봇 팔로 그대로 간다', '지금 실행한다'], 'var(--accent)', 1.8),
    _t(320, 348, '나오는 관절 값은 두 줄이 같은 종류다 — 어깨를 이만큼, 팔꿈치를 이만큼', 't-sm'),
    _t(320, 370, '갈리는 것은 들어간 장면이 찍힌 것이냐 그려 낸 것이냐 하나뿐이다', 't-sm'),
    _t(320, 386, '점선 팔과 흐린 컵이 모델이 그려 낸 자리다', 't-sm'),
]))
...
def _wmpanel(ox, oy, hand, open_, ghost=False):
    ...
    col = 'var(--accent)' if ghost else 'var(--ink-2)'
    ty = oy + 118
    return ''.join([
...
FIG_WMSEQ = _svg(640, 268, '같은 자리에서 후보 셋을 상상시켜 점수로 줄을 세운다', ''.join([
    _lt(14, 22, '① 지금 — 문이 닫혀 있다'),
    _wmpanel(14, 30, hand=(90, 92), open_=False),
    _lt(222, 22, '② 후보 A 를 넣고 상상한 4초 뒤'),
    _wmpanel(222, 30, hand=(126, 74), open_=True, ghost=True),
    _lt(430, 22, '③ 후보 B 를 넣고 상상한 4초 뒤'),
    _wmpanel(430, 30, hand=(88, 96), open_=False, ghost=True),
    _t(111, 182, '실제로 찍힌 장면이다', 't-sm'),
    _t(319, 182, '점선 — 문이 열린다', 't-sm'),
    _t(527, 182, '점선 — 문이 그대로다', 't-sm'),
    _t(319, 206, '상태가치 0 위 · 실물에서 확인', 't-lab'),
    _t(527, 206, '상태가치 0 이하 · 버린다', 't-lab'),
    _t(320, 236, '후보는 사람이 짠 동작이 아니라 학습 도중 저장해 둔 정책 여러 개다', 't-sm'),
    _t(320, 258, '문이 열리고 안 열리는 두 장면은 과제를 보이려고 세운 예시다', 't-sm'),
]))
...
def summary_layer():
    figs = [('관절까지 가는 네 갈래', FIG_ROUTE4,
            ...
            ('에어프라이어 문 하나로 보는 한 판', FIG_WMSEQ,
             '「행동 후보」가 무엇인지부터 짚는다. 사람이 짠 동작 목록이 아니라 <b>학습 도중 저장해 둔 '
             '정책 여러 개</b>다. 같은 초기 상태 — 로봇이 조리기구 앞에 서 있고 문은 닫혀 있다 — 를 '
             '놓고, 후보마다 그 정책이 낼 행동을 1XWM 에 넣는다. 모델은 그 행동대로 갔을 때의 '
             '<b>미래 1인칭 영상 4초</b>를 그리고, 같은 자리에서 <b>상태가치</b>를 함께 낸다. '
             '문이 열리는 쪽은 값이 0 위로 나오고 그대로인 쪽은 0 이하로 나와 그 자리에서 떨어진다. '
             '1X Technologies 가 에어프라이어 열기 같은 과제를 이렇게 걸러 두고, 살아남은 것만 '
             '실물 로봇으로 확인한다 — 실물은 <b>마지막에 한 번</b>만 움직인다.'),
