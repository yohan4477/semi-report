...
# 위계형 — 층 사이에 오가는 것을 로봇 장면 둘 사이에 끼워 넣는다
FIG_HIERSEQ = _svg(640, 286, '판단층이 말로 시키고 반사층이 관절을 내고 팔이 움직인다',
                   ''.join([
    _lt(14, 30, '① 지금 — 컵 옆에 있다'),
    _scene(14, 40, 160, 120, hand=(104, 84), cup=(120, 74)),
    _a(178, 100, 182, 100),
    _box(186, 66, 128, 68, ['② 판단층', '「컵을 집어」', '말로 시킨다'], 'var(--accent)', 1.8),
    _a(318, 100, 322, 100),
    _box(326, 66, 128, 68, ['③ 반사층', '어깨를 이만큼,', '팔꿈치를 이만큼'], 'var(--accent)', 1.8),
    _a(458, 100, 462, 100),
    _lt(466, 30, '④ 컵을 들었다'),
    _scene(466, 40, 160, 120, hand=(104, 52), cup=(112, 42)),
    '<path class="flow" fill="none" d="M546 166 L546 196 L250 196 L250 140"/>',
    _t(398, 216, '⑤ 그 일이 끝나면 판단층이 다시 고른다 — 여기가 고리다', 't-sm'),
    _t(320, 246, '내려가는 것은 말이고 올라오는 것은 끝났다는 신호다. 관절 값은 반사층에서만 나온다', 't-sm'),
    _t(320, 268, '컵을 집는 장면은 층 사이에 무엇이 오가는지 보이려고 든 예시다', 't-sm'),
]))
...
def summary_layer():
    figs = [('관절까지 가는 네 갈래', FIG_ROUTE4,
            ...
            ('층 사이에 무엇이 오가나', FIG_HIERSEQ,
             '왼쪽 칸에서 오른쪽 칸으로 가는 사이에 층 둘을 지난다. <b>판단층</b>이 내리는 것은 '
             '관절 값이 아니라 <b>말</b>이고 — 「컵을 집어」 같은 자연어다 — 그 말을 관절 값으로 '
             '바꾸는 것이 <b>반사층</b>이다. 팔이 실제로 움직이는 것은 마지막 칸에서다. '
             '되돌아오는 줄이 이 구조의 요점이다. 위층은 매 순간을 정하지 않고 <b>그 일이 끝났다</b>는 '
             '신호를 받은 다음에만 다시 고른다 — 그래서 판단이 1초에 한 번이어도 손은 그보다 훨씬 '
             '빠르게 움직인다. 거꾸로 갑자기 빨간불이 켜지는 식으로 판단을 바꿔야 할 때는 '
             '반사층이 아니라 위층이 멈추라고 정한다.'),
