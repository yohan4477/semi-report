...
# 위계형 — 고리를 한 바퀴가 아니라 두 판으로 편다. 「다시 고른다」가 앞 판과
# 무엇이 다른지는 판을 두 번 그려야 보인다 — 층은 같고 보는 장면과 내는 말이 다르다.
FIG_HIERSEQ = _svg(640, 396, '같은 두 층이 장면이 바뀔 때마다 다른 말을 낸다', ''.join([
    _lt(14, 30, '① 컵이 책상에 있다'),
    _scene(14, 40, 160, 120, hand=(104, 72), cup=(120, 62)),
    _a(178, 100, 182, 100),
    _box(186, 66, 128, 68, ['② 판단층', '「컵을 집어」'], 'var(--accent)', 1.8),
    _a(318, 100, 322, 100),
    _box(326, 66, 128, 68, ['③ 반사층', '어깨를 이만큼,', '팔꿈치를 이만큼'], 'var(--accent)', 1.8),
    _a(458, 100, 462, 100),
    _lt(466, 30, '④ 컵을 들었다'),
    _scene(466, 40, 160, 120, hand=(104, 40), cup=(112, 30)),

    # 끝났다는 신호가 올라가고, 판단층은 바뀐 장면을 보고 다음 말을 낸다
    '<path class="flow" fill="none" d="M546 164 L546 182 L94 182 L94 202"/>',
    _t(320, 176, '④ 가 끝나면 그 사실이 판단층으로 올라간다', 't-sm'),

    _lt(14, 200, '⑤ 컵을 든 채로 서 있다'),
    _scene(14, 210, 160, 120, hand=(104, 40), cup=(112, 30)),
    _a(178, 270, 182, 270),
    _box(186, 236, 128, 68, ['⑥ 판단층', '「왼쪽에 내려놔」'], 'var(--accent)', 1.8),
    _a(318, 270, 322, 270),
    _box(326, 236, 128, 68, ['⑦ 반사층', '어깨를 저만큼,', '팔꿈치를 저만큼'], 'var(--accent)', 1.8),
    _a(458, 270, 462, 270),
    _lt(466, 200, '⑧ 내려놓았다'),
    _scene(466, 210, 160, 120, hand=(96, 44), cup=(64, 62)),

    _t(320, 356, '두 판에서 층은 같다 — 달라지는 것은 층이 보는 장면과 그래서 내는 말이다', 't-sm'),
    _t(320, 378, '컵을 집고 내려놓는 두 판은 층 사이에 무엇이 오가는지 보이려고 든 예시다', 't-sm'),
]))
...
def summary_layer():
    figs = [('관절까지 가는 네 갈래', FIG_ROUTE4,
            ...
            ('층 사이에 무엇이 오가나', FIG_HIERSEQ,
             '판단층이 내리는 것은 관절 값이 아니라 <b>말</b>이고 — 「컵을 집어」 같은 자연어다 — '
             '그 말을 관절 값으로 바꾸는 것이 <b>반사층</b>이다. 팔이 실제로 움직이는 것은 오른쪽 '
             '칸에서다. 아랫줄을 따로 그린 것은 <b>다시 고른다</b>가 앞 판과 무엇이 다른지 보이려는 '
             '것이다 — 층은 그대로이고, 달라지는 것은 층이 보는 장면(컵이 책상에 있다 → 컵을 든 채로 '
             '서 있다)과 그래서 나오는 말(「컵을 집어」 → 「왼쪽에 내려놔」)이다. 위층은 매 순간을 정하지 않고 '
             '<b>그 일이 끝났다</b>는 신호를 받은 다음에만 다음 말을 내므로, 판단이 1초에 한 번이어도 '
             '손은 그보다 훨씬 빠르게 움직인다. 거꾸로 갑자기 빨간불이 켜지는 식으로 판단을 바꿔야 할 '
             '때는 반사층이 아니라 위층이 멈추라고 정한다.'),
