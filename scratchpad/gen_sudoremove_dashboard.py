...
def summary_layer():
    figs = [('관절까지 가는 네 갈래', FIG_ROUTE4,
             ...
             '넷은 <b>다음 관절 값을 무엇에서 뽑느냐</b>로 갈린다.'),
            ('같은 IDM, 들어가는 장면만 다르다', FIG_IDMSEQ,
             'IDM 과 비디오 모델은 같은 것이 아니라 <b>방향이 반대</b>다 — 비디오 모델은 지금 프레임과 '
             '지시를 받아 다음 장면을 그리고, IDM 은 장면 두 장에서 관절 값을 되짚는다. '
             '위 칸 쪽으로 1X Technologies 는 사람 1인칭 영상 <b>900시간</b>을 학습 재료로 바꿨고, '
             '엔비디아 DreamGen 도 이렇게 뽑은 의사(疑似) 액션으로 VLA 를 학습시킨다. '
            ...
            ('월드모델 — 시켜 보기 전에 결과를 미리 본다', FIG_WMSEQ,
             ...
             '1XWM 은 미래 영상과 상태가치를 같은 표현에서 함께 내고 <b>0 이하면 실패로 친다</b>. '
             '그림에 적은 +0.6·−0.2 와 서랍이 열리고 안 열리는 장면은 예시다 — 원문이 밝힌 것은 '
             '0 이라는 경계뿐이다.'),
            ('위계형 — 판단과 실행을 층으로 나눈다', FIG_HIER,
    ...
    h = ['<p class="ins-lede">카드 서른넷을 다 열지 않고도 이 페이지가 무엇을 다루는지 잡히게 '
         '하는 층입니다. 여기 실린 회사들이 답하는 물음은 하나입니다 — '
         '<b>다음 순간 관절을 얼마씩 움직일지를 무엇에서 뽑을 것인가</b>.</p>']
    h.append(''.join(fig_html(f) for f in figs))
    h.append('<p class="ins-lede"><b>속도가 갈래를 가릅니다.</b> 제미나이 로보틱스 2 는 판단을 '
             '1초에 한 번, 관절을 1초에 쉰 번 돌립니다. 반대로 비디오 액션모델은 5초짜리 영상 하나를 '
             '그리는 데 11초가 걸리고, DreamZero 는 GB200 두 대를 쓰고도 초당 일곱 장입니다. '
             '장면을 그려서 행동을 뽑는 길은 아직 반응 속도를 내주고 얻는 길입니다.</p>')
