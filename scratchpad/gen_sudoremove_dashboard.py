...
FIG_IDMSEQ = _seq(
    ...
    ['학습할 때 — 라벨 없는 영상을 재료로 바꾼다',
     (_V, _I, '① 앞뒤 장면 두 장'),
     (_I, _I, '② 차이를 되짚는다'),
     (_I, _P, '③ 장면에 관절 값을 붙여 넘긴다 — 이것으로 배운다'),
...
def summary_layer():
    figs = [('관절까지 가는 네 갈래', FIG_ROUTE4,
            ...
            ('IDM 이 서는 두 자리', FIG_IDMSEQ,
             ...
             'IDM 이 뽑은 관절 값은 영상으로 되돌아가지 않고 <b>배우는 모델</b>로 간다 — 엔비디아 DreamGen 은 '
             '비디오모델로 장면을 만들고 IDM 으로 뽑은 의사(疑似) 액션을 붙여 그 데이터로 '
             '<b>VLA 를 학습</b>시킨다. 가운데 기둥을 「모델」이라 적은 것은 그 자리에 서는 것이 '
             '회사마다 달라서다 — Rhoda AI 는 언어가 들어가지 않아 VLA 가 아니다.'),
