...
FIG_IDMSEQ = _svg(640, 384, '카메라가 찍은 장면과 모델이 그려 낸 장면이 같은 IDM 으로 들어간다',
    ...
    _t(320, 374, '위 칸은 실제로 찍힌 장면, 아래 칸은 모델이 그려 낸 장면이다', 't-sm'),
]))
...
# 위계형 — 층마다 도는 주기가 다르다
FIG_HIER = _svg(640, 246, '판단·행동·균형이 서로 다른 주기로 돈다', ''.join([
...
# 목표가 없으면 두 판이 서로 무관한 미세 조정으로 읽힌다.
FIG_HIERSEQ = _svg(640, 462, '사람이 준 일 하나를 판단층이 단계로 쪼개 아래층에 넘긴다',
                   ''.join([
    ...
    _lt(14, 96, '① 컵이 책상에 있다'),
    _scene(14, 106, 160, 120, hand=(104, 72), cup=(120, 62)),
    _a(178, 166, 182, 166),
    _box(186, 132, 128, 68, ['② 판단층', '「컵을 집어」', '첫째 단계'], 'var(--accent)', 1.8),
    _a(318, 166, 322, 166),
    _box(326, 132, 128, 68, ['③ 행동층', '어깨를 이만큼,', '팔꿈치를 이만큼'], 'var(--accent)', 1.8),
    _a(458, 166, 462, 166),
    _lt(466, 96, '④ 컵을 들었다'),
    _scene(466, 106, 160, 120, hand=(104, 40), cup=(112, 30)),

    '<path class="flow" fill="none" d="M546 230 L546 248 L94 248 L94 268"/>',
    _t(320, 242, '④ 가 끝나면 그 사실이 판단층으로 올라가고, 판단층은 다음 단계를 꺼낸다', 't-sm'),

    _lt(14, 266, '⑤ 컵을 든 채로 서 있다'),
    _scene(14, 276, 160, 120, hand=(104, 40), cup=(112, 30)),
    _a(178, 336, 182, 336),
    _box(186, 302, 128, 68, ['⑥ 판단층', '「왼쪽에 내려놔」', '둘째 단계'], 'var(--accent)', 1.8),
    _a(318, 336, 322, 336),
    _box(326, 302, 128, 68, ['⑦ 행동층', '어깨를 저만큼,', '팔꿈치를 저만큼'], 'var(--accent)', 1.8),
    _a(458, 336, 462, 336),
    _lt(466, 266, '⑧ 내려놓았다 — 준 일이 끝났다'),
    _scene(466, 276, 160, 120, hand=(96, 44), cup=(64, 62)),

    _t(320, 422, '두 판은 서로 다른 일이 아니라 위에서 받은 일 하나를 쪼갠 첫째·둘째 단계다', 't-sm'),
    _t(320, 444, '층은 그대로다 — 달라지는 것은 층이 보는 장면과 그래서 내는 말이다', 't-sm'),
]))
...
def summary_layer():
    figs = [('관절까지 가는 네 갈래', FIG_ROUTE4,
            ...
            ('같은 IDM, 들어가는 장면만 다르다', FIG_IDMSEQ,
             ...
             'VLA 를 학습시킨다. 아래 칸은 아무도 움직이지 않은 장면이다. 모델이 <b>다음에 이렇게 '
            ...
            ('위계형 — 판단과 실행을 층으로 나눈다', FIG_HIER,
             ...
             '가운데를 <b>행동층</b>이라 적은 것은 그 자리에 서는 것이 대개 명령을 받아 관절 값을 '
             '만드는 VLA 라서다 — 몸에 익어 생각 없이 나가는 <b>반사</b>는 맨 아래 균형층 쪽이고, '
             'Sharpa 가 강화학습으로 미리 구워 둔 손안 조작이 그 성격이다. '
             '1초에 50번이라는 값은 초기 발표 슬라이드에만 있었고 이후 발표에서는 빠졌다.'),
            ('층 사이에 무엇이 오가나', FIG_HIERSEQ,
             '맨 위가 사람이 준 일이다. 판단층은 그 한 마디를 <b>단계로 쪼개</b> 아래층에 하나씩 '
             '넘긴다 — 내려가는 것은 관절 값이 아니라 <b>말</b>이고, 그 말을 관절 값으로 바꾸는 것이 '
             '<b>행동층</b>이다. 팔이 실제로 움직이는 것은 오른쪽 칸에서다. 아랫줄을 따로 그린 것은 '
             '<b>다시 고른다</b>가 앞 판과 무엇이 다른지 보이려는 것이다 — 두 판은 서로 다른 일이 '
             '아니라 같은 일의 첫째·둘째 단계이고, 층은 그대로인 채 층이 보는 장면(컵이 책상에 있다 '
             '→ 컵을 든 채로 서 있다)과 그래서 나오는 말(「컵을 집어」 → 「왼쪽에 내려놔」)만 바뀐다. '
             '쪼갠다는 것만으로는 이 갈래가 아니다 — Physical Intelligence 의 π0.5 도 「방 정리해줘」 '
             '같은 모호한 지시를 스스로 하위 작업으로 쪼개지만, 그 일을 <b>한 모델 안에서</b> 하므로 '
             'VLA 갈래다. 가르는 것은 쪼개느냐가 아니라 <b>층이 따로 도느냐</b>다. '
             '위층은 매 순간을 정하지 않고 <b>그 단계가 끝났다</b>는 신호를 받은 다음에만 다음 말을 '
             '내므로, 판단이 1초에 한 번이어도 손은 그보다 훨씬 빠르게 움직인다. 거꾸로 갑자기 '
             '빨간불이 켜지는 식으로 계획을 바꿔야 할 때는 행동층이 아니라 위층이 멈추라고 정한다.'),
    ...
    h.append('<p class="ins-lede"><b>네 갈래를 한 줄로 줄이면 이렇습니다.</b> '
             ...
             '합니다. 위계형은 무엇을 할지 정하는 층과 그 말을 관절 값으로 만드는 층을 갈라 '
             '주기가 다른 모델에 나눠 맡깁니다 — 몸에 익어 바로 나가는 동작은 그 아래 층에 미리 '
             '구워 둡니다.</p>')
