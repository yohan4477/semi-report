...
FIG_HELIX = _svg(640, 248, 'Helix 01과 02의 차이', ''.join([
    _box(20, 40, 280, 150, ['Helix 01', '상체 32자유도', '하체는 고정',
                            'System 2 + System 1']),
    _box(340, 40, 280, 150, ['Helix 02', '골반·다리까지 온몸', 'System 2(7B) 판단',
                             'System 1(80M) 행동', 'System 0(10M) 균형 1000Hz'],
         'var(--accent)', 1.8),
    _t(320, 214, 'System 0을 강화학습이 아니라 이미테이션 러닝으로 배웠다', 't-sm'),
    _t(320, 232, '균형까지 사람 시연만으로 푼 셈이다', 't-sm'),
]))

# 열린 것과 안 열린 것 — 「RLDX-1」 카드
FIG_OPEN = _svg(640, 236, '오픈웨이트가 덮는 범위', ''.join([
    _t(320, 24, '「SOTA 오픈웨이트」가 덮는 범위', 't-sm'),
    _box(20, 36, 280, 160, ['열었다', '모델 가중치 세 종', '인퍼런스·학습 코드',
                            'LoRA 파인튜닝 코드', '데이터 구성 비율'], 'var(--accent)', 1.8),
    _box(340, 36, 280, 160, ['안 열었다', '어블레이션 실험', '벤치마크 설계',
                             '학습 데이터 원본']),
    _t(320, 218, '어블레이션이 없으면 어느 선택이 성능을 냈는지 밖에서 못 가른다', 't-sm'),
]))
...
FIG_MIRA = _svg(640, 232, '월드모델을 무엇의 목표로 두느냐', ''.join([
    _t(320, 24, '같은 기술, 다른 목표', 't-sm'),
    _box(20, 36, 280, 116, ['영상을 목표로 두면', '사람 눈에 그럴듯한가',
                            '해상도 · 길이 · 일관성', '평가자는 사람']),
    _box(340, 36, 280, 116, ['훈련장으로 두면', '에이전트가 배우는가',
                             '반응 속도 · 조작 가능성', '평가자는 에이전트'],
         'var(--accent)', 1.8),
    _t(320, 176, 'MIRA — 5B · B200 한 장 · 20Hz · 컨텍스트 4초', 't-lab'),
    _t(320, 200, '20Hz 로 돌아야 에이전트가 그 안에서 움직여 볼 수 있다', 't-sm'),
    _t(320, 218, '해상도를 올리는 대신 속도를 택한 이유가 그것이다', 't-sm'),
]))
...
FIG_LOGI = _svg(640, 226, '2025년 6월 시점 Helix 가 보인 것과 안 보인 것', ''.join([
    _box(20, 30, 280, 116, ['보인 것', '물류 한 과제를 1시간', '거의 실패 없이 이어감',
                            '상체만 쓴다'], 'var(--accent)', 1.8),
    _box(340, 30, 280, 116, ['안 보인 것', '여러 과제 사이 전환', '온몸 균형',
                             '낯선 환경']),
    _t(320, 174, '40분 지점에서 택배를 한 번 떨어뜨린다', 't-sm'),
    _t(320, 192, '긴 데모는 오래 버틴다는 증거이지 넓게 된다는 증거가 아니다', 't-sm'),
    _t(320, 210, '온몸 균형은 여덟 달 뒤 Helix 02 에서야 나온다', 't-sm'),
]))
...
FIG_CES = _svg(640, 242, 'CES 부스에서 본 아틀라스와 Sharpa', ''.join([
    _box(20, 26, 280, 130, ['아틀라스', '손가락 3자유도에서 4로',
                            '판단은 제미나이 로보틱스에', '맡기기로 했다']),
    _box(340, 26, 280, 130, ['Sharpa', '촉각으로 카드를 나눈다',
                             'CraftNet — 위계형 VTLA', 'System 2 는 약 1Hz',
                             '손 하나 5만 달러'], 'var(--accent)', 1.8),
    _t(320, 186, '데모 영상이 아니라 부스에서 직접 봤다', 't-sm'),
    _t(320, 204, '카드 딜링은 배속 없이 1배속이었다', 't-sm'),
    _t(320, 222, '빨리 감은 영상과 실물이 갈리는 자리가 여기다', 't-sm'),
]))
...
FIG_FINGERS = _svg(640, 244, '손가락 수를 두고 갈리는 판단', ''.join([
    _t(320, 24, '같은 물음에 답이 갈린다 — 손가락 몇 개면 되는가', 't-sm'),
    _box(20, 36, 280, 116, ['적게 두자는 쪽', '보스턴 다이내믹스는 3지를 유지',
                            '신뢰도 · 값 · 개발 속도', '망가질 자리를 줄인다']),
    _box(340, 36, 280, 116, ['많이 두자는 쪽', '사람 손에 가깝게 5지로',
                             '사람이 쓰는 도구를 그대로', '잡는 방식이 넓어진다'],
         'var(--accent)', 1.8),
    _t(320, 176, '이 편에서 결론이 나지 않는다', 't-sm'),
    _box(140, 188, 360, 44, ['병뚜껑 하나가 아직 안 열린다'], 'var(--line)', 1.3),
]))
...
FIG_PRED = _svg(640, 236, '신년 예측에서 갈린 것과 합의한 것', ''.join([
    _box(20, 30, 280, 116, ['둘 다 아니라고 본 것', '모라벡의 역설이 1년 안에 풀린다',
                            'Mamba 가 어텐션을 밀어낸다', '합의는 쉬웠다']),
    _box(340, 30, 280, 116, ['정면으로 갈린 것', 'Transformer 대체 구조',
                             'RAG 소멸 · 추론 칩 시장', 'VLA 산업표준화'],
         'var(--accent)', 1.8),
    _box(140, 160, 360, 44, ['VLA 엔드투엔드 산업표준은 2028년이라고 봤다'], 'var(--line)', 1.3),
    _t(320, 224, '판정은 우리가 하지 않는다 — 시한이 지나야 갈린다', 't-sm'),
]))

# 사람 느낌은 어디서 났나 — 「GPT 5.4와 Figure 데모」 카드
FIG_FEEL = _svg(640, 226, '데모에서 사람 느낌이 난 자리', ''.join([
    _t(320, 24, '「사람 같다」는 인상이 어디서 오는가', 't-sm'),
    _box(20, 36, 280, 104, ['손가락이 아니었다', '집는 동작 자체는',
                            '앞 편들과 크게 다르지 않다']),
    _box(340, 36, 280, 104, ['몸통이었다', '무게를 옮기고 기울이는',
                             '큰 움직임에서 났다'], 'var(--accent)', 1.8),
    _t(320, 168, '데모 인상을 성능으로 읽으면 어긋난다', 't-sm'),
    _t(320, 186, '무엇이 눈을 속였는지 밝혀야 그 인상이 근거가 된다', 't-sm'),
    _t(320, 204, 'CES 편이 잡은 배속 문제와 같은 함정이다', 't-sm'),
]))
