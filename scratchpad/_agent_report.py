...
def report_html(fig_seq, stamp):
    ...
    # ── 9 ──────────────────────────────────────────────────────────────────
    sec('9. 플러그인은 다섯 중 무엇을 담나')
    p('플러그인은 새로운 능력이 아닙니다. 앞의 다섯을 한 상자에 담아 남에게 넘기는 '
      '포장입니다. 마켓플레이스라 부르는 것은 깃 저장소 하나이고, 설치는 그 저장소에서 '
      '폴더를 받아 오는 일입니다.')
    fig(FIG_PLUG)
    p('두 상자를 나란히 두면 「플러그인을 깔았다」가 한 가지 뜻이 아님이 보입니다. '
      '<code>superpowers</code>는 스킬 %d개로 절차만 줍니다. 안 부르면 아무 일도 '
      '안 일어납니다. <code>caveman</code>은 훅을 %s 두 시점에 걸어 둡니다. 세션이 열리는 '
      '순간 이미 응답 문체가 바뀌어 있고, 사용자가 무엇을 부른 적은 없습니다.'
      % ((F.SUPERPOWERS or {}).get('skills', 0),
         ' · '.join('<code>%s</code>' % e for e in (F.CAVEMAN or {}).get('hook_events', []))))
    p('그래서 플러그인을 받을 때 볼 것은 이름이나 소개가 아닙니다. 상자 안에 '
      '<code>hooks</code>가 들었는지입니다. 스킬과 명령만 든 상자는 안 부르면 조용하지만, '
      '훅이 든 상자는 깔린 순간부터 판을 바꿉니다.')

    # ── 10 ─────────────────────────────────────────────────────────────────
    sec('10. 이 저장소는 이 다섯을 어떻게 나눠 쓰나')
    p('실제 배치는 이렇습니다. 스킬 %d개 · 서브에이전트 %d개 · 슬래시 명령 %d개 · '
      '훅 %d개 · MCP 서버 %d개 · 플러그인 %d개.'
      % (F.N_SKILL, F.N_AGENT_REPO, F.N_CMD_REPO, len(F.HOOK_EVENTS), len(F.MCP),
         len(F.ENABLED)))
    p('스킬로 몰린 이유가 있습니다. 이 저장소의 규칙은 대부분 「글을 쓰기 전에 반드시 이 '
      '절차를 연다」는 꼴입니다. 시점을 사람이 정하지 않고 모델이 상황을 보고 집어야 '
      '합니다. 명령으로 두면 사람이 매번 쳐야 합니다.')
    p('그런데 정작 「반드시」를 지키게 하는 것은 스킬 문장이 아닙니다. 검사기 여섯 개'
      '(<code>check_notes</code>·<code>check_prose</code>·<code>check_read</code>·'
      '<code>check_cite</code>·<code>check_fresh</code>·<code>check_report</code>)가 '
      '파이썬으로 돌면서 막습니다. 은유를 주어 자리에 두면 <code>check_prose</code>의 P14가 '
      'FAIL을 냅니다. 스킬에 「은유를 주어 자리에 두지 않는다」고 적어만 두었을 때는 두 번 '
      '그대로 나갔습니다.')
    p('이것이 앞 절들의 요점을 되풀이합니다. 읽히는 글로 두면 안 따르는 날이 생기고, '
      '코드로 옮기면 안 따를 방법이 없습니다. 훅이 하는 일과 검사기가 하는 일이 같은 '
      '성격입니다.')

    # ── 11 ─────────────────────────────────────────────────────────────────
    sec('11. 이 글이 검증하지 않은 것')
    p('<b>부품을 다섯으로 나눈 것은 우리 분류입니다.</b> 공식 문서가 이 다섯을 한 줄에 '
      '세워 놓고 견주지 않습니다. 「어디에 꽂히나」로 나눈 것은 이 글의 물음에 맞춘 '
      '것이고, 다른 물음으로 보면 다르게 갈립니다.')
    p('<b>숫자는 %s 이 컴퓨터를 센 값입니다.</b> 스킬 %d개도 플러그인 %d개도 이 설정에서만 '
      '참입니다. 다른 사람 컴퓨터에서는 다릅니다. 세는 코드는 '
      '<code>scratchpad/agent_facts.py</code>에 있고, 생성기를 다시 돌리면 값이 그때 '
      '기준으로 바뀝니다.' % (stamp, F.N_SKILL, len(F.ENABLED)))
    p('<b>판의 번호는 설명을 위한 것입니다.</b> ②·③·④·⑥은 용어 카드의 그림에서 온 '
      '번호이고, 하네스 제품마다 실제 단계 수와 순서가 다릅니다. 우리가 하네스 소스를 열어 '
      '확인한 것은 아닙니다.')
    p('<b>두 단계 로딩의 내부 동작도 확인하지 않았습니다.</b> 스킬 설명 한 줄만 평소에 '
      '실린다는 것은 문서에 적힌 설계이고, 우리가 실제 요청 본문을 떠서 센 것이 '
      '아닙니다. %s자와 %s자는 파일을 센 값이지 전송된 글자를 잰 값이 아닙니다.'
      % ('{:,}'.format(F.CH_DESC), '{:,}'.format(F.CH_BODY)))
    return ''.join(h)
