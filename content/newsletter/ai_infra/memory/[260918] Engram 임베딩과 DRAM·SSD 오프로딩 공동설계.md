...
추천 시스템(recsys) 업계는 이미 자주·최근 접근한 임베딩 행을 빠른 메모리에 캐시하고, 차가운(잘 안 쓰는) 행은 SSD로 백업해 두는 이 방식을 써왔다.
이번 보고서 뒷부분에서는 Engram 오프로딩 실험 결과와 함께, InferenceX의 공식 에이전틱 추론 서빙 결과(DeepSeek-V4.1-Flash 등 Engram 모델을 엔비디아 GPU 6종 전체와 AMD MI355X에서 실측)를 보여준다 — CUDA 해자(생태계 우위)는 이 인기 모델에서도 여전히 MI355X를 압도한다.
나아가 HBM 용량이 넉넉한 고급 SKU에서도, Engram을 HBM에 그대로 두는 것보다 DRAM으로 오프로드하는 편이 대부분의 성능-비용 구간(파레토)에서 더 나은 결과를 낸다는 점도 보인다.

이 벤치마크(InferenceX)는 구글 클라우드·마이크로소프트 애저·오라클·메타를 비롯한 컴퓨트 구매자 대부분이 재현·검증했고, vLLM·LMCache·SGLang·PyTorch·HuggingFace 같은 ML 커뮤니티와 OpenAI·MiniMax·ZAI·Qwen·Moonshot Kimi 등 주요 랩의 지지를 받는다.
InferenceX는 TPUv7·Jalapeño·엔비디아 루빈 NVL72·AMD를 모두 갖춘(곧 SambaNova·Trainium도 추가) 세계 유일의 추론 벤치마크이며, AgentX 시나리오가 실제 에이전틱 추론 워크로드와 매우 흡사해 AMD도 MI455X UALoE72로 협업을 약속했다.
...
같은 모델 품질을 유지하는 조건이면, Engram을 DRAM으로 내보낸 만큼 GPU당 HBM은 덜 필요해진다.
그래서 추론 워크로드에서는 HBM 용량보다 대역폭이 우선순위가 높아지고, 4-hi HBM이 달러당 대역폭 기준으로 가장 유리하다. 중국이 이런 모델 아키텍처 혁신을 계속 내놓으면 언젠가 HBM을 아예 쌓지 않는 "0-hi" 구성까지 갈 수 있다는 관측도 나온다.

### SSD 오프로딩

B200에서는 비최적화 vLLM 포크를 만들어 Engram 표를 로컬 SSD의 메모리맵(mmap) 파일에 두고 측정했다.
파일 기반 방식은 다른 프로그램이 메모리를 필요로 할 때 운영체제가 Engram 표의 페이지를 회수할 수 있게 해주고, 이미 캐시된 페이지는 SSD를 다시 읽지 않고 서빙할 수 있다는 장점이 있다 — 다만 이번 실험에서는 GDS(GPUDirect Storage, GPU가 스토리지를 직접 읽는 기능)를 켜지 못했다.
...
## 7. Engram의 구현 메커니즘 - DeepSeek·LongCat·Qwen 비교

**📌 핵심:**
- Engram(또는 N-gram 임베딩)의 기본 동작은 토큰 ID의 접미사 n-gram(연속된 토큰 몇 개)을 해시해 여러 임베딩 표에서 벡터를 모아 이어붙이고, 이를 키·값으로 바꿔 은닉 상태와의 유사도로 시그모이드 게이트를 계산해 문맥과 안 맞는 조회를 억제하는 구조다 — 그 뒤 짧은 합성곱으로 위치 간 정보를 섞어 모델의 잔차 스트림(residual stream)에 합류시킨다.
- DeepSeek 논문은 비활성 파라미터 예산(전문가+Engram에 배분되는 몫) 중 전문가(MoE)에 얼마를 배정할지를 나타내는 비율 ρ(로)를 정의했는데, 전부를 전문가나 Engram 한쪽에만 배정하는 것보다 둘을 섞는 편이 손실이 낮은 U자형 곡선이 나타난다 — 앞서 2절의 U자형 스케일링과 같은 원리를 파라미터 배분 관점에서 다시 보여준다.
- DeepSeek·LongCat·Qwen 세 모델은 같은 아이디어를 서로 다르게 구현한다. DeepSeek-V4.1-Flash는 1·14번째 층에 2\~4토큰 접미사 해시를 배치해 표 약 196.6B 파라미터(오프로드 시 188.8GiB)를 쓰고, LongCat은 N-gram 벡터를 표준 토큰 임베딩과 평균해 합치며 전체 파라미터의 최대 50%까지만 N-gram에 배정하라고 권고한다(DeepSeek의 ρ 기준과 다름). Qwen3.8-Flash-Next는 2번째 층에서 바이그램 해시 8개·트라이그램 해시 8개를 써서 51.2B 파라미터 표 중 토큰당 16개 행(BF16 기준 5KiB)만 조회한다.
- 결론: 세 회사 모두 "층을 깊게 배치할수록 검색·전송과 겹칠 연산이 많아 유리하다"는 원칙에 동의한다 — 다만 LongCat은 모델 폭이 커질수록 이득이 크고 깊이가 커질수록 상대적 이득이 작아진다고 보는데, 이는 Engram이 표현을 더 일찍 성숙시킨다는 2절의 관찰과 같은 방향이다.

---

```mermaid
flowchart TD
    Hash["토큰 ID의 접미사 n-gram을 해시<br/>곱셈 해싱+XOR 믹싱으로 충돌 감소"] --> Gather["여러 임베딩 표에서 벡터를 모아 이어붙임"]
    Gather --> KV["키·값으로 투영"]
    KV --> Gate["은닉 상태와 키의 내적 → 시그모이드 게이트<br/>문맥과 안 맞는 조회를 억제"]
    Gate --> Conv["짧은 인과적 합성곱으로 위치 간 정보를 섞음"]
    Conv --> Residual["잔차 스트림에 합류"]

    style Gate fill:#fff7ed,stroke:#ea580c
```

합성곱은 수용 영역(receptive field)을 넓히고 비선형성을 더하지만, 논문의 제거 실험에서는 이를 빼도 성능 저하가 작았다. 또한 이 구조는 얕은 층에 놓일수록 유리하지만, 깊은 층에 놓으면 그만큼 앞선 연산이 많아 조회·전송 작업과 겹칠(overlap) 시간이 늘어나는 트레이드오프가 있다.

### 세 모델의 구현 비교

```mermaid
flowchart TD
    Compare["세 모델의 Engram/N-gram 배치"] --> DS["DeepSeek-V4.1-Flash<br/>1·14번째 층, 2~4토큰 접미사 해시<br/>표 약 196.6B 파라미터(188.8GiB)"]
    Compare --> LC["LongCat<br/>N-gram 벡터를 표준 토큰 임베딩과 평균<br/>파라미터 배분 상한 50%"]
    Compare --> QW["Qwen3.8-Flash-Next<br/>2번째 층, 바이그램 8개+트라이그램 8개 해시<br/>표 51.2B 파라미터 중 토큰당 16행(5KiB)만 조회"]

    style DS fill:#eff6ff,stroke:#3b82f6
```

### LongCat

LongCat은 DeepSeek와 별도로 N-gram 임베딩을 개발해 LongCat-Flash-Lite에 이어 LongCat-2.0에 반영했다. 저자들은 다섯 가지를 권고한다.

1. N-gram 임베딩은 전문가 수가 그 모델의 "적정 지점"을 넘어선 뒤에만 도입한다.
2. N-gram 임베딩에는 전체 파라미터의 50%를 넘지 않게 배정한다(DeepSeek의 ρ 기준과 다름).
3. N-gram 어휘 크기는 기본 어휘 크기의 정수배를 피해 해시 충돌을 줄인다.
4. 모델 폭(width)이 커질수록 이득이 커진다.
5. 모델 깊이(depth)가 커질수록 상대적 이득은 작아진다.

깊이가 커질수록 이득이 줄어든다는 다섯 번째 결과는, Engram이 표현을 더 일찍 성숙시킨다는 2절의 관찰과 일치한다. LongCat은 또한 N-gram을 길게 잡을수록 문맥 정보는 풍부해지지만 등장 빈도가 낮아져 임베딩을 학습하기 어려워진다는 트레이드오프도 짚는다.

### Qwen 3.8 Flash Next

Qwen3.8-Flash-Next는 두 번째 디코더 층에 n-gram 메모리를 더했다 — 원본 토큰 ID에 대해 바이그램 해시 8개와 트라이그램 해시 8개를 쓴다.
표는 51.2B 파라미터를 담고 있지만, 토큰 하나가 실제로 조회하는 행은 160차원 16개뿐이라 BF16 정밀도 기준 총 5KiB에 그친다. 두 번째 층에 배치한 이유는 임베딩 조회와 겹칠 만큼 충분한 연산이 그 앞에 있도록 하기 위해서다.

### DeepSeek V4.1 Flash

DeepSeek-V4.1-Flash는 1번째·14번째 층에 Engram을 배치해, 해시된 2\~4토큰 접미사와 학습된 게이팅을 잔차 스트림에 결합한다.
표는 약 196.6B 파라미터를 담고, 이 글의 오프로드 구성 기준 약 188.8GiB를 차지한다 — 그래서 이 메모리를 HBM 밖에서 서빙하려면 희소 행 조회(sparse row retrieval)가 핵심이 된다.

---

*작성 진행률: 100% 완료 — 전체 7절 변환 완료*
