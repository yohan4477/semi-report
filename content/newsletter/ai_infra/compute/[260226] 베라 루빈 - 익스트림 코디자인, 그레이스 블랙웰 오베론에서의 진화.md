...
## 10. 네트워킹: NVLink 6 스케일업과 스케일아웃

**📌 핵심:**
- NVLink 6는 신호 변조·전송속도를 올리는 대신 **양방향(Bi-directional) SerDes**로 같은 구리선에서 데이터를 양쪽으로 동시에 보내 대역폭을 2배(224G→448G/레인)로 늘림 → 배선(케이블) 개수는 그대로 유지
- VR NVL72는 NVLink 6 스위치 칩 수를 GB200의 2배인 36개(트레이당 4개 × 9트레이)로 늘려 스위칭 용량을 2배로 확보 — 칩 1개당 대역폭(28.8T)은 NVLink5와 동일하기 때문
- 스케일아웃(InfiniBand·이더넷)에서는 GPU 1개당 로직 포트를 2개(800G씩, 총 1.6T) 갖춰 멀티플레인 배치가 가능 → 3계층 네트워크로도 최대 74만 6,496 GPU급 초대형 클러스터 구성 가능
- 결론: CPO(공동 패키징 광학) 스위치는 전송장치 전력·비용을 70\~75% 줄이지만, 클러스터 전체 전력에서 차지하는 비중은 약 1%뿐이라 "게임체인저"보다는 "점진적 개선"에 가까움

---

### 스케일업: 양방향 SerDes와 NVLink 6

```mermaid
flowchart TD
    OldWay["기존 방식(NVLink5)<br/>전송속도를 올려 대역폭 확장<br/>224G/레인"] --> New["NVLink 6<br/>양방향 SerDes로<br/>같은 선에서 양쪽 동시 전송<br/>448G/레인"]
    New --> Benefit["케이블 개수는 그대로,<br/>대역폭만 2배<br/>(구리선을 1만개로 늘릴 필요 없음)"]

    style New fill:#fff7ed,stroke:#ea580c,stroke-width:2px
    style Benefit fill:#f0fdf4,stroke:#16a34a
```

**📌 용어 풀이: 양방향 SerDes가 어려운 이유**
> - 같은 구리선에 양방향 신호를 동시에 보내면, 받는 쪽에서 "내가 보낸 신호"와 "상대가 보낸 신호"가 뒤섞여 도착함(전기적으로 두 신호가 합쳐짐)
> - 광통신은 순환기(광신호를 방향별로 분리하는 부품)로 쉽게 해결하지만, 구리선은 물리적으로 신호가 합쳐지므로 "하이브리드" 회로로 자신이 보낸 신호의 반대 파형을 만들어 상쇄(에코 제거)하는 방식을 씀
> - 기존에는 5mm 이내 칩 간(die-to-die) 초단거리에만 쓰이던 기술을 Nvidia가 1m 이상 백플레인 장거리까지 확장 적용

케이블을 그대로 유지하면서 대역폭을 늘려야 했던 이유가 있습니다. 이미 5천 개에 달하는 백플레인 구리 케이블이 Blackwell 세대에서도 상당한 고장 요인이었기 때문입니다.

- 대안 1(기각): 케이블을 1만 개로 2배 늘리기 — 신뢰성 문제 악화
- 채택: 같은 케이블 수로 SerDes 속도를 2배로 올림
- 대안 2(기각): 랙 폭을 넓히는 방식(AMD Helios) — 신호가 이동할 PCB 거리가 길어져 신호 품질 저하 우려

```mermaid
flowchart TD
    GB200sw["GB200<br/>NVLink5 스위치 18개/랙"] --> VRsw["VR NVL72<br/>NVLink6 스위치 36개/랙<br/>(트레이당 4개 x 9트레이)"]
    VRsw --> Why["이유: 칩 1개당 대역폭(28.8T)은<br/>NVLink5와 동일<br/>→ 스위치 수를 2배로 늘려<br/>전체 용량 2배 확보"]

    style VRsw fill:#fff7ed,stroke:#ea580c,stroke-width:2px
```

NVLink 6 스위치 트레이는 GB200의 첫 오베론 스위치 트레이보다 설계가 단순합니다 — 플라이오버 케이블을 아예 쓰지 않기 때문입니다(예외는 스위치 트레이-시스템관리모듈 연결 하나뿐).

- NVLink 6 스위치 보드는 신호 손실에 특히 민감해 PCB를 32층·M8 이상 등급 CCL로 업그레이드
- 대역폭이 2배로 늘었어도 양방향 SerDes 덕분에 백플레인 케이블 개수·커넥터 수는 GB300과 동일

### 스케일아웃: HGX와 NVL72의 대역폭 차이

```mermaid
flowchart TD
    HGX["HGX Rubin NVL8<br/>서버당 GPU 8개+스위치 4개<br/>GPU당 스케일아웃 800G"] --> NVL72["VR NVL72<br/>GPU당 스케일아웃 1.6T<br/>(800G NIC 2개를 연결)"]

    style NVL72 fill:#fff7ed,stroke:#ea580c,stroke-width:2px
```

VR NVL72 컴퓨트 트레이 1개는 800G CX-9 NIC 8개를 갖추며, OSFP 케이지 구성은 GPU당 1.6T 1개(트레이당 4개) 또는 GPU당 800G 2개(트레이당 8개) 방식이 가능한데, 후자가 더 널리 쓰일 전망입니다.

### InfiniBand 스케일아웃과 멀티플레인 클러스터

VR NVL72의 스케일아웃 배치는 크게 세 갈래입니다 — Nvidia Quantum 시리즈(InfiniBand), Nvidia Spectrum 시리즈(이더넷), Broadcom Tomahawk·Cisco Silicon One 등 비Nvidia 이더넷. Rubin 세대에서 처음으로 스케일아웃 백엔드에 CPO(공동 패키징 광학) 스위치가 등장합니다.

- **Quantum X800-Q3400**: 일반 트랜시버형, Quantum-3 ASIC 4개를 묶어 논리적으로 144포트 단일 스위치처럼 동작
- **Quantum X800-Q3450**: 위와 같은 144포트지만 CPO(광학 엔진 내장) 버전

```mermaid
flowchart TD
    Formula["클러스터 최대 크기 공식<br/>2×(k/2)^L<br/>(k=스위치 포트 수, L=계층 수)"] --> F1["1플레인, GPU당 1.6T 로직포트 1개<br/>3계층 기준 최대 93,312 GPU"]
    Formula --> F2["2플레인, GPU당 800G 로직포트 2개로 분할<br/>3계층 기준 최대 746,496 GPU"]

    style F2 fill:#f0fdf4,stroke:#16a34a,stroke-width:2px
```

GPU 1개당 800G 로직 포트를 2개(총 1.6T) 갖추면, 이 두 포트를 서로 다른 리프 스위치·네트워크 플레인에 연결하는 "멀티플레인" 구성이 가능해집니다. SemiAnalysis는 실제로는 2플레인 구성이 VR NVL72 InfiniBand 배치의 주된 참조 아키텍처가 될 것으로 봅니다.

클러스터 규모가 10만 GPU를 넘는 대형 배치는 대부분 멀티플레인 구조를 씁니다 — 단일 플레인으로는 현재 세대 스위치 포트 수로 3계층 이상 없이 이 정도 규모를 감당할 논리 포트가 부족하기 때문입니다.

### 이더넷 기반 클러스터: Spectrum-6 스위치 3종

```mermaid
flowchart TD
    Spectrum["Spectrum-6 이더넷 스위치 3종"] --> SN6600["SN6600<br/>102.4T, 일반 트랜시버형<br/>(8플레인 네트워크에 사용)"]
    Spectrum --> SN6800["SN6800<br/>409.6T, CPO 4칩 512x800G<br/>(Neocloud 선호 예상)"]
    Spectrum --> SN6810["SN6810<br/>102.4T, CPO 1칩<br/>(다양한 포트 구성 지원)"]

    style SN6800 fill:#fff7ed,stroke:#ea580c,stroke-width:2px
```

SN6600 스위치는 GPU 1개가 8개 플레인으로 나뉘어 연결되는 8플레인 네트워크가 기본 참조 구조입니다. 512포트 스위치 기준으로는 2계층에서 최대 13만 1,072 GPU, 3계층에서는 최대 3,355만 4,432 GPU까지 이론상 확장 가능합니다.

```mermaid
flowchart TD
    CPO["CPO(공동 패키징 광학) 스위치의<br/>3가지 이점"] --> B1["전력: 트랜시버 전력 약 70%↓<br/>→ 클러스터 전체 전력의 약 1%↓"]
    CPO --> B2["비용: 트랜시버 비용 약 75%↓"]
    CPO --> B3["신뢰성: Meta 실측 1,500만<br/>포트-시간 무결함 운영"]

    style CPO fill:#eff6ff,stroke:#3b82f6,stroke-width:2px
```

전력·비용 절감 효과는 있지만, 서버 자체의 전력 예산이 워낙 커서 클러스터 총전력에서 차지하는 비중은 약 1%에 불과합니다. 그럼에도 SN6800처럼 광섬유 셔플(배선 경로 분배)을 스위치 박스 안에 내장한 제품은 멀티플레인 배치의 설치·유지보수를 크게 단순화한다는 이점이 있습니다.

**📌 용어 풀이: 비Nvidia 이더넷 스위치 진영과 하이퍼스케일러별 전략**
> - Broadcom(Tomahawk 6·Tomahawk 6 CPO), Cisco(G300, 102.4T)가 Nvidia 외 주요 스위치 ASIC 공급사
> - 하이퍼스케일러 백엔드 배치는 크게 "8플레인 평면형"(512 라딕스 전체 사용)과 "1.6T OSFP 단일 플레인형" 두 갈래로 나뉨

```mermaid
flowchart TD
    Strategy["하이퍼스케일러별<br/>스케일아웃 배치 전략"] --> Meta["Meta<br/>Tomahawk6 Minipack-4(102.4T)<br/>+1.6T AEC(NIC-TOR)<br/>2026년 하반기 램프 예정"]
    Strategy --> XAI["xAI<br/>단일 플레인, 1.6T AEC를<br/>리프·스파인·코어 전 구간 적용<br/>(Credo에 유리한 가격 결정력)"]

    style Meta fill:#eff6ff,stroke:#3b82f6
    style XAI fill:#eff6ff,stroke:#3b82f6
```

Meta는 스위치 간 연결에는 광케이블을 쓰되, NIC-TOR(서버-랙상단 스위치) 구간은 1.6T AEC(액티브 전기 케이블)를 씁니다.

다만 102.4T Minipack-4 스위치가 VR NVL72 랙 배치 시점까지 출하되지 않을 경우, 일부 클러스터는 51.2T Minipack-3 스위치 + 기어박스(200G↔100G 레인 변환)로 대체될 가능성이 있습니다.

---

## 11. 하이퍼스케일러 커스터마이징과 조립 물류

**📌 핵심:**
- GB300은 네트워킹·전력·저장·관리 모듈 대부분을 하이퍼스케일러가 자유롭게 바꿀 수 있었지만, VR NVL72는 케이블 없는 모듈형 설계 특성상 **커스터마이징 범위가 전력공급·BlueField-4·관리 모듈 3곳으로 축소**
- 대부분 하이퍼스케일러는 비싼 BlueField-4 대신 자체 DPU를 쓰되, Nvidia가 정한 폼팩터·치수에 맞춰 보드 레이아웃을 재설계해야 하는 제약을 감수
- L10(컴퓨트 트레이 조립) 단계에서 자동화 역량을 갖춘 업체가 **Foxconn·Quanta·Wistron 단 3곳**으로 좁혀짐 — Blackwell 시절엔 Bianca 보드를 사다 쓰는 다양한 제조사가 있었던 것과 대비
- 결론: 케이블 제거로 조립 시간은 2시간→5분으로 줄었지만, 그 대가로 조립 가능 공급업체 수는 오히려 줄어드는 집중화가 진행 중

---

```mermaid
flowchart TD
    GB300cust["GB300<br/>커스터마이징 폭 넓음<br/>(네트워킹·전력·저장·관리 대부분)"] --> VRcust["VR NVL72<br/>커스터마이징 범위 축소<br/>(전력공급·BlueField-4·관리 모듈만)"]
    VRcust --> Why["이유: 케이블 없는 모듈형 설계상<br/>폼팩터·치수 규격을<br/>Nvidia 기준에 맞춰야 함"]

    style VRcust fill:#fff7ed,stroke:#ea580c,stroke-width:2px
```

GB300 참조 설계는 백엔드용 ConnectX-8 4개와 프런트엔드용 BlueField-3 1개를 기본으로 하지만, 실제로는 하이퍼스케일러 대부분이 자체 설계를 씁니다.

- Amazon은 GB300 상당수 랙에 ConnectX-8을 직접 배치
- 대부분 하이퍼스케일러는 BlueField-3 대신 자체 DPU로 프런트엔드 네트워킹을 대체
- 전력공급·관리 모듈도 고객마다 선호가 달라 커스터마이징 폭이 컸음

VR NVL72에서는 전력공급·BlueField-4·관리 모듈만 커스터마이징이 허용됩니다. 하이퍼스케일러는 BlueField-4 대신 자체 DPU를 쓰되, 폼팩터·치수를 Nvidia 기준에 맞춰 보드를 재설계해야 합니다. Amazon은 VR NVL72용으로 JBOK/Nitro Box NIC 버전을 준비 중입니다.

### 조립 자동화: 3개 계층, 3개 업체로 집중

```mermaid
flowchart TD
    L6["L6: 보드 단위 조립(PCBA)<br/>Wistron·Foxconn 주도<br/>(Blackwell·Rubin 공통)"] --> L10["L10: 컴퓨트 트레이 조립<br/>자동화 역량 보유는 단 3곳<br/>(Foxconn·Quanta·Wistron)"]
    L10 --> L11["L11: 트레이를 랙에 최종 조립<br/>다양한 ODM·OEM이 수행"]

    style L10 fill:#fff7ed,stroke:#ea580c,stroke-width:2px
```

Jensen이 CES 2026에서 밝힌 조립 시간 단축(2시간→5분)은 자동화가 가능한 이 3개 업체를 전제로 한 수치입니다. 규모가 작은 ODM·OEM은 이 3개 업체에 위탁하거나, 자동화 없이 직접 조립합니다 — 자동화가 없어 효율은 떨어지지만, 모듈형 설계 덕분에 Blackwell보다는 훨씬 조립이 수월합니다.

---

## 12. VR NVL72 TCO 분석: BoM과 전력 예산

**📌 핵심:**
- VR NVL72는 GPU당 초기 투자비(Capex)가 GB300 대비 약 45%, AMD MI4XX 대비 약 14\~15% 높음(예: Arista 네트워킹 기준 시간당 GPU당 $3.28 vs MI4XX $2.86)
- Nvidia는 SOCAMM 메모리를 직접 조달해 장기계약·물량할인으로 가격 급등을 방어(LPDDR $6.77/GB)하지만, AMD는 LPDDR5만 직접 조달하고 DDR5는 랙 조립사가 별도 구매해야 해 D램 총량(2배)만큼 가격 급등 노출도 더 큼
- 스펙(마케팅 FLOPS)만 보면 AMD MI355가 유리해 보여도, 실측(InferenceX) 결과 DeepSeek R1 추론에서 B300이 약 6.3배 높은 처리량을 냄 — TCO는 1.75배만 높아 성능당비용은 오히려 B300이 우월
- 결론: 스펙시트상 숫자(FLOPS·대역폭)만으로는 실제 성능을 예측할 수 없고, 소프트웨어·네트워크 완성도가 실제 토큰 처리 성능을 좌우함

---

```mermaid
flowchart TD
    Cap["VR NVL72 GPU당 초기비용(Capex)"] --> C1["GB300 대비 약 45% 높음"]
    Cap --> C2["AMD MI4XX 대비 약 14\~15% 높음"]
    Cap --> C3["예시(Arista 네트워킹 기준)<br/>VR $3.28/hr vs MI4XX $2.86/hr<br/>(GPU당·4년 상각 기준)"]

    style Cap fill:#fff7ed,stroke:#ea580c,stroke-width:2px
```

**📌 용어 풀이: 왜 4년 상각인데 5\~6년을 언급하나**
> - SemiAnalysis TCO 모델은 보수적 기준으로 4년 사용연한(내용연수)을 적용해 시간당 자본비용을 계산
> - 하지만 실제 대다수 Neocloud·하이퍼스케일러는 5\~6년 감가상각을 적용 — 상각 기간에 영향받지 않는 프로젝트 IRR(내부수익률)을 더 선호하는 지표로 제시

### 메모리 가격 리스크: Nvidia의 헤지 vs AMD의 노출

```mermaid
flowchart TD
    Mem["메모리 가격 급등 시<br/>구매자 노출도 비교"] --> NV["Nvidia VR<br/>SOCAMM 메모리 직접 조달<br/>장기계약·물량할인으로<br/>가격 급등 방어(LPDDR $6.77/GB)"]
    Mem --> AMD["AMD MI4XX<br/>LPDDR5만 조달, DDR5는<br/>랙 조립사가 별도 구매<br/>→ D램 총량도 2배라 노출 2배"]

    style NV fill:#f0fdf4,stroke:#16a34a,stroke-width:2px
    style AMD fill:#fef2f2,stroke:#dc2626
```

AMD MI400 랙은 GPU당 LPDDR5 55TB + DDR5 55TB(Nvidia의 약 2배)를 탑재합니다.

- AMD는 LPDDR5만 직접 조달할 뿐 DDR5는 랙 조립사·ODM이 각자 구매 → 시장 계약가($10.63/GB) 대비 AMD의 물량할인 LPDDR가($8.70/GB)는 Nvidia($6.77/GB)보다 비쌈
- DDR5 부분은 헤지(가격 방어) 자체가 안 됨
- SemiAnalysis AI 메모리 모델: 2026년 2분기 이후 LPDDR5·DDR5 계약가가 크게 오를 것으로 예상

### 운영비(Opex): Max-P·Max-Q 전력 옵션의 차이

```mermaid
flowchart TD
    Opex["운영비(Opex) 비교"] --> O1["VR Max-P(2,300W)<br/>MI4XX와 TDP 비슷해<br/>운영비도 유사"]
    Opex --> O2["VR Max-Q(1,800W)<br/>MI4XX 대비 운영비 약 20%↓<br/>(예: $0.75/hr/GPU, 동일 Arista 네트워킹)"]

    style O2 fill:#f0fdf4,stroke:#16a34a,stroke-width:2px
```

Nvidia의 2,300W(Max-P)·1,800W(Max-Q) 두 설정 모두 같은 최고 클록에 도달해 마케팅 수치인 50 PFLOPS FP4에 도달할 수 있지만, 전력 소비 차이가 그대로 운영비 차이로 이어집니다 — 운영비 대부분이 IT 전력 소요량에 비례하기 때문입니다.

### 성능당비용(TCO/PFLOP)의 역설: 스펙과 실측의 괴리

```mermaid
flowchart TD
    Spec["스펙상 비교<br/>B300 4,500 vs MI355 5,000<br/>FP8 TFLOPS(밀집, 마케팅 기준)<br/>→ B300이 10% 불리해 보임"] --> Real["실측(InferenceX) 결과<br/>DeepSeek R1 추론에서<br/>B300이 약 6.3배 높은 처리량"]
    Real --> Concl["TCO는 1.75배만 높아<br/>성능당비용은 B300이 우월<br/>→ 스펙만으론 실제 성능 예측 불가"]

    style Concl fill:#fff7ed,stroke:#ea580c,stroke-width:2px
```

두 칩은 메모리 대역폭(8TB/s)이 완전히 같은데도 추론 성능이 크게 갈립니다 — 소프트웨어·네트워크 완성도가 실제 토큰 처리 성능을 좌우한다는 방증입니다. FP4 희소성(Sparsity) 활용 여부도 성능당비용을 크게 바꿉니다.

```mermaid
flowchart TD
    Sparse["FP4 희소성 활용 여부"] --> S1["35 PFLOPS(밀집) 기준<br/>보수적 성능당비용"]
    Sparse --> S2["50 PFLOPS(추론, 압축 활용)<br/>성능당비용(시간당 마케팅PFLOP당 비용) 35%↓<br/>단, 실제 워크로드 희소성 정도에 좌우"]

    style S2 fill:#eff6ff,stroke:#3b82f6
```

AMD는 CDNA4부터 추론용 데이터 타입의 희소성 지원을 없앤 반면, Nvidia는 적응형 압축으로 이를 자동 활용합니다. 다만 실제 워크로드에서 50 PFLOPS에 도달할 가능성은 낮고, 1,800W(Max-Q) 설정에서는 2,300W(Max-P)보다 희소 FP4 성능이 더 낮을 것으로 예상됩니다.
...
**📌 핵심:**
- Nvidia는 2025년 12월 Groq의 기술 라이선스 계약을 맺고 핵심 인력을 영입 → SemiAnalysis는 GTC 2026에서 신형 "LPU 랙"이 공개될 것으로 전망
- LPU(Groq의 언어처리장치)는 고대역폭 SRAM에 강점이 있어 **디코드(생성) 단계**에 특화 — CPX가 프리필(입력 처리) 전용인 것과 정반대 역할 분담
- 총처리량은 희생하는 대신, 사용자 1명당 초당 토큰 생성 속도가 매우 높아야 하는 요청(그만큼 $/토큰 단가도 높음)에 특화된 포지셔닝
- 결론: Nvidia는 GPU를 일반 디코드용으로 두고, CPX(프리필)·LPU(초고속 디코드)로 추론 워크로드를 3갈래로 세분화하는 전략을 완성해가는 중

---

```mermaid
flowchart TD
    Infer["Nvidia의 추론 특화 3분할 전략"] --> CPX["CPX 랙<br/>프리필 전용<br/>(연산량 집약적)"]
    Infer --> GPU2["VR NVL72 GPU<br/>범용 디코드<br/>(일반 인터랙티비티 요청)"]
    Infer --> LPU["Groq LPU 랙(예정)<br/>초고속 디코드 전용<br/>(고대역폭 SRAM,<br/>사용자당 초고속 토큰 요구 대응)"]

    style LPU fill:#fff7ed,stroke:#ea580c,stroke-width:2px
```

LPU 랙은 Nvidia GPU와 나란히 배치돼, GPU가 상대적으로 낮은 인터랙티비티(초당 토큰 요구가 덜 급한) 요청의 디코드를 처리하는 동안 LPU가 초고속 응답이 필요한 요청을 전담하는 구조로 설계될 전망입니다.

---

*작성 진행률: 100% 완료*
*업데이트: 전체 13개 섹션 작성 완료(서론부터 Groq LPU 디코드 랙까지). 검증 스크립트 통과 확인 완료*
