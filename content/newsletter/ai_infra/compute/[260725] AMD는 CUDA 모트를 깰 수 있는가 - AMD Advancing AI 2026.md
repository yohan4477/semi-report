...
## 13. MI455X TCO 분석 - GB300·VR NVL72와 비교

**📌 핵심:**
- 2027년 HBM 가격이 크게 오를 전망(메모리 공급사들이 범용 D램 수준의 마진을 HBM에도 요구하기 시작) — AMD는 대형 구매자에게 협상력이 크게 밀리는 구조라 이 인상분을 온전히 전가하지 못하고, 마크업률도 자사 평균 마진보다 낮게 적용해 **MI455X 패키지의 매출총이익률이 눌리는 구조**
- **BOM(부품 명세) 대폭 조정**: 2025년 원래 계획은 LPDDR5X·DDR5 MRDIMM 합산 138TB에 달하는 메모리를 욱여넣는 것이었으나, 현재는 LPDDR5X를 완전히 빼고 DDR5도 대폭 줄인 구성으로 후퇴 — 가격 상승분 상당 부분이 이런 디스펙으로 상쇄되고, 남은 상승분은 HBM 원가 상승과 비HBM 부품비 증가가 원인
- **전력 소비는 최고 수준**: 서버당 소비전력이 **약 240kW(순수 IT 전력)·약 257kW(전체 시설 전력 포함)**로 VR NVL72(Vera Rubin)보다도 높고, GB300 NVL72(약 142kW·약 159kW)를 크게 웃돎 — 자본비 측면에서는 MI455X가 GB300 NVL72와 VR NVL72 사이(가운데)에 위치
- 결론: 마케팅상 FP8 연산량(PFLOPs) 기준 TCO로 보면 MI455X가 VR NVL72와도 경쟁력 있는 수준(TCO/PFLOPs, TFLOPs/W 모두 양호) — 다만 저자들은 **AMD가 과거 실제 달성 가능한 수준보다 TFLOPs 스펙을 부풀려 온 전례가 있어 실효 연산 활용률(MFU)이 낮을 가능성**을 경고, 마케팅 수치를 그대로 믿으면 실제 학습 처리량은 이보다 낮을 수 있음

---

```mermaid
flowchart TD
    HBM2["2027년 HBM 가격<br/>인상 전망"] --> Margin["메모리 공급사,<br/>범용 D램급 마진 요구"]
    Margin --> AMDPass["AMD: 대형 구매자<br/>협상력에 밀려<br/>전가율 낮음"]
    AMDPass --> GM["MI455X 매출총이익률<br/>압박"]

    style GM fill:#fff7ed,stroke:#ea580c,stroke-width:2px
```

```mermaid
flowchart TD
    BOM["MI455X BOM 변화"] --> Plan2["2025년 원계획:<br/>메모리 합산 138TB"]
    Plan2 --> Now["현재: LPDDR5X 삭제,<br/>DDR5 대폭 축소"]
    Now --> Offset["가격 상승분 상당수<br/>디스펙으로 상쇄"]
...
```mermaid
flowchart TD
    Power3["서버당 전력 소비<br/>(순수IT/전체시설)"] --> MI455["MI455X: 약 240kW<br/>/ 약 257kW"]
    Power3 --> VR["VR NVL72: MI455X보다<br/>낮음"]
    Power3 --> GB300["GB300 NVL72: 약 142kW<br/>/ 약 159kW"]

    style MI455 fill:#fef2f2,stroke:#dc2626,stroke-width:2px
    style GB300 fill:#f0fdf4,stroke:#16a34a
```

**📌 용어 풀이: 원차트 프레임워크와 MFU**
> - **원차트 프레임워크(One Chart to Rule Them All)**: GPU 임대료의 적정 범위를 "이 밑이면 임대업체 손해(원가 기반 하한)"와 "이 위면 고객이 구형 카드로 갈아탐(가치 기반 상한)" 두 선 사이로 표시하는 SemiAnalysis의 가격 분석 틀 — GB300→VR NVL72 구간에서 FP8 연산량이 5,000TFLOPs→17,500TFLOPs(3.5배)로 뛰며 VR NVL72의 이론상 임대료 상한을 크게 높인 것처럼, MI455X에도 같은 틀을 적용해 GB300 NVL72 대비 여유 있는 임대료 상한을 확인
> - **MFU(Model FLOPs Utilization, 실효 연산 활용률)**: 칩이 이론상 낼 수 있는 최대 연산량 중 실제 학습에 쓰이는 비율 — 스펙상 TFLOPs가 아무리 높아도 MFU가 낮으면 실제 학습 속도는 그만큼 느려짐

---

## 14. OpenAI·Meta 지분 리베이트 구조 - 최대 105% 할인

**📌 핵심:**
- AMD는 2025년 10월(OpenAI)·2026년 2월(Meta)에 각각 최대 1억 6천만 주(발행주식의 약 10%)를 주당 1센트에 살 수 있는 성과 연동 워런트를 발급 — **AMD GPU 6GW 구매·배치와 주가 단계별 목표치(최종 $600) 달성에 연동해 단계적으로 베스팅**되는 구조, Meta 계약은 앞서 다룬 절반 사양 Recsys 커스텀 MI455X를 포함하고 OpenAI는 표준 사양·후속 SKU를 구매
- **주가 $600 도달 시 효과적 리베이트**: GW(기가와트)당 약 $16.0B 상당의 주식 가치를 얻는데, OpenAI·Meta가 실제 지불하는 금액은 각각 GW당 약 $15.2B·$18.7B — 이는 **파트너 할인율로 환산하면 각각 약 105%·85%**에 해당, 즉 조건이 모두 충족되면 OpenAI는 이론상 MI455X 서버를 사고 워런트를 행사하는 것만으로 오히려 이득을 볼 수 있는 구조(단, AMD 주가가 실제로 $600에 도달하고 OpenAI가 전 발행 구간에 걸쳐 주식을 보유하고 있어야 함)
- **왜 이런 구조까지 동원하나**: Nvidia의 CUDA 모트와 폭넓은 오픈소스 라이브러리 생태계에 맞서려면 프론티어 랩·하이퍼스케일러의 참여가 필수 — 게다가 AMD는 Nvidia뿐 아니라 Google TPU·AWS Trainium이라는 자체 개발 ASIC과도 동시에 싸워야 하는 "양면 전쟁" 상황이라, 이런 파격적 딜이 거의 불가피한 선택
- 결론: 이런 백스톱(보증)·금융 구조는 벤더 간에 서로 닮아가는 추세 — AMD는 Crusoe에 3억 달러 대출 보증을 제공(2026년 2월, 고객이 안 나서면 AMD가 직접 칩을 임대하겠다는 약속으로 Crusoe의 대출 금리를 6%까지 낮춤)했고, 7월 22일에는 Anthropic과 MI455X 최대 2GW 배치 파트너십(1기가와트는 2027년 상반기부터 Helios 랙으로 가동)을 공식화하며 AMD가 Anthropic에 최대 50억 달러를 직접 투자하는 조건도 포함 — Nvidia도 2025년 10월 8.6억 달러였던 리스 보증을 2026년 1월 35억 달러로 늘리는 등 비슷한 패턴을 보이며, 이런 구조는 벤더 록인을 강화하는 동시에 네오클라우드의 대출 신용도(뱅커빌리티)를 개선하는 효과

---

```mermaid
flowchart TD
    Warrant["지분 리베이트 구조"] --> Grant["OpenAI(2025-10)·Meta(2026-02):<br/>주당 1센트, 최대 10%씩<br/>워런트 발급"]
    Grant --> Vest["6GW 구매·배치 +<br/>주가 $600 단계별<br/>베스팅"]

    style Vest fill:#eff6ff,stroke:#3b82f6,stroke-width:2px
```

```mermaid
flowchart TD
    Discount["주가 $600 도달 시<br/>효과적 할인율"] --> OpenAIV["OpenAI: 지불 $15.2B/GW<br/>vs 워런트 가치 $16.0B/GW<br/>= 약 105% 할인"]
    Discount --> MetaV["Meta: 지불 $18.7B/GW<br/>vs 워런트 가치 $16.0B/GW<br/>= 약 85% 할인"]

    style OpenAIV fill:#f0fdf4,stroke:#16a34a,stroke-width:2px
```

```mermaid
flowchart TD
    Backstop["백스톱·금융 구조<br/>확산 추세"] --> Crusoe["AMD→Crusoe:<br/>3억달러 대출보증<br/>(금리 6%로 인하)"]
    Backstop --> Anthropic2["AMD→Anthropic:<br/>MI455X 2GW 배치 +<br/>AMD가 50억달러 투자"]
    Backstop --> Nvidia2["Nvidia: 리스보증<br/>8.6억→35억달러<br/>(2025-10→2026-01)"]
...
## 15. MI500 - 차세대 광학 인터커넥트(CPC·NPO) 로드맵

**📌 핵심:**
- 차세대 MI500 시리즈는 **공동 패키지 구리(CPC)·근접 패키지 광학(NPO)**로 전환할 전망 — SemiAnalysis 추정으로는 6.4Tbit/s급 광학 엔진을 패키지당 최소 6개(스케일업 대역폭 최소 38.4T) 탑재할 것으로 보이나, 랙 규모 확장에 필요한 충분한 라딕스(포트 밀도)를 확보하려면 57.6T가 최적이라는 것이 저자들의 판단
- 설계는 **OCI MSA(광학 상호접속 표준) 사양을 밀접히 따를 전망** — 6.4T 광학 엔진 1개가 200Gbit/s 레인 32개를 지원하고, 각 레인은 50G NRZ 변조 방식의 파장(송신 4개+수신 4개) 8개로 구성, 광원은 6.4Tbit/s급 ELSFP(외장형 레이저 소스 광모듈) 사용
- 결론: **스케일업 토폴로지는 여전히 단순한 1계층 평면 구조 유지** — GPU 576개(8개 랙)를 하나의 홉(hop)으로 연결해 꼬리 지연시간(tail latency)을 억제하는 방향, 스위치는 기존 Tomahawk6의 다른 SKU(115.2T 지원 버전, 표준판은 102.4T)를 사용할 것으로 예상되며 8랙 스케일업 규모에서는 스위치 288개가 GPU 576개 전체와 연결돼 경로가 여러 개라 장애에도 강한 구조를 만들 전망

---

```mermaid
flowchart TD
    MI500["MI500 광학 로드맵"] --> CPC["공동패키지 구리(CPC)"]
    MI500 --> NPO["근접패키지 광학(NPO)"]
    NPO --> Engine["광학엔진 최소 6개<br/>(38.4T), 최적 57.6T"]
...
```mermaid
flowchart TD
    Topology2["MI500 스케일업<br/>토폴로지"] --> Flat["1계층 평면 구조<br/>유지(576GPU·8랙)"]
    Flat --> Switch2["Tomahawk6 신규SKU:<br/>115.2T(vs 표준 102.4T)"]
    Switch2 --> Resilient["8랙 규모: 스위치 288개,<br/>다중 경로로<br/>장애 내성 확보"]

    style Flat fill:#f0fdf4,stroke:#16a34a,stroke-width:2px
```

---

*작성 진행률: 100% 완료*
*업데이트: 전체 15개 섹션(개요·리더십 권고, MI455X 실리콘·Meta 커스텀, Helios 랙 네트워킹·아키텍처·토폴로지, CDNA5, 소프트웨어 현황·InferenceX·WideEP·분산스택·NIXL·오버랩, TCO·지분 리베이트, MI500 광학 로드맵) 작성 완료*
