...
- 가격 범위 밖의 것들. Wayne은 스위치·네트워킹에는 가격을 안 붙이고 GPU·HPC 하드웨어 본체에만 붙인다고 밝혔고, 4년 뒤 이 칩들이 지리적으로 어디로 향할지(남미·동유럽·아프리카 등 신흥시장) 자신도 관심 있게 지켜보는 영역이라고 덧붙였다.
- 전력은 값에 안 넣는다. Vik가 전력 가격은 어떻게 정하냐고 묻자, Wayne은 전력이 입지마다 달라(site-specific) 하드웨어 가격을 정할 때 전력은 넣지 않는다고 답했다 — 다만 추론 중심 시장에서 토큰당 비용이 전력가격에 크게 좌우된다는 점은 인정했다.
