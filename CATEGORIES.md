...
<!-- 2026-07-12: [260205] EDA 시장 개관 추가 — ai-infra +1 (일반 상위 태그로 분류, AWS 마진 문서와 동일 폴더). 참고: 이 문서는 EDA(전자설계자동화) 소프트웨어 산업 — 반도체 설계 툴링·라이선싱 비즈니스로, 기존 ai-infra 하위 카테고리(power/cooling/compute/memory)는 물리 인프라 대상이라 잘 맞지 않음. 향후 EDA·반도체 설계 소프트웨어 문서가 더 늘면 `semiconductors` 또는 `eda` 같은 신규 대분류 카테고리 신설을 검토할 가치가 있음(이번 문서 단독으로는 신설하지 않음, CATEGORIES.md 변경은 별도 검토 필요) -->
