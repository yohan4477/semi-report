...
- **`ai-infra/security`** — AI 인프라 보안 (멀티테넌트 격리 실패, 컨테이너·VM 탈옥, 네트워크 격리(InfiniBand 보안 키·쿠버네티스 NetworkPolicy), DPU·BMC 같은 관리 평면 보안, 취약점 공개·임베고 프로그램, 보안 감사·컴플라이언스 체계) — 다른 `ai-infra` 하위 카테고리가 설비·조달·건설 자체를 다루는 것과 달리, 이 카테고리는 그 인프라를 여러 고객이 함께 쓸 때 벌어지는 침해·격리 실패라는 보안 운영 축을 다룸
