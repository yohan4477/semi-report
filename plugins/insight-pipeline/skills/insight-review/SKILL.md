...
**④ 링크드인을 인용할 때.** 판단 재료가 링크드인 글이면 `insights/notes/`에 노트를 새로 만들지 않는다 — `content/linkedin/[YYMM] 링크드인 게시물.md`를 `sources:`에 바로 넣고 줄 번호로 인용한다. 쓰기 전에 `PYTHONIOENCODING=utf-8 python scripts/gen_li_source.py`를 돌려 그날 게시물까지 원문에 반영한다. 이 파일은 매번 통째로 다시 쓰이지만 게시 시각 오름차순이라 새 글은 끝에만 붙고 기존 줄 번호는 안 밀린다. 소셜 글은 한 줄에 여러 사실이 뭉쳐 있어 `check_cite.py`의 정밀도가 뉴스레터보다 떨어진다 — 인용한 줄을 사람이 한 번 열어 확인한다.
