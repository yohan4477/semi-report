...
def check_density(text, where, actor=True, claims=False):
    ...
    if claims:
        ...
        # P19 도 같은 자리에만 댄다. 노트는 제3자 글을 옮긴 중간물이라
        # 필자가 쓴 「가른다」까지 우리 문체 규칙으로 잡으면 안 된다
        check_verb(text, where)
...
# 「이자를 문다」는 정상 한국말이라 넣지 않는다 — 뜻이 하나로 굳은 관용구까지
# 막으면 규칙이 죽는다. 「값을 치른다」는 2026-09-09에 사용자가 번역투로 지목해
# P19 로 옮겼다 — pay the price 를 그대로 옮긴 자리다. 잔금·시험·홍역처럼
# 목적어가 구체적인 관용구는 거기서도 안 잡는다.
DEAD_FIGURE = [
    (r'(?<![가-힣])값(을|도)\s*받',
     '받는 것은 돈이다 — 「돈을 번다」·「가격이 정해진다」 중 뜻에 맞는 쪽으로'),
    (r'(?<![가-힣])값에 (없다|있다)',
...
# P19 — 번역투 동사 둘. 2026-08-28에 사용자가 「가른다」·「값을 매긴다」를 지목했다.
# 「가르다」는 영어 divide/separate 를 그대로 옮긴 자리에 붙고, 한국말에서는 나누거나
# 쪼개거나 구분한다. 「값을 매기다」는 price 를 옮긴 것이고, 값은 정하는 것이다.
# 피동형 「갈린다」는 막지 않는다 — 「무엇을 세느냐로 갈린다」는 정상 한국말이고
# P16 의 권고문도 그 꼴을 쓴다. 능동형만 잡는다.
BANNED_VERB = [
    # 「가를」은 뺀다 — 원가를·단가를·주가를·평가를에 그대로 걸린다.
    # 명사+를 과 겹치는 조각을 넣으면 규칙이 오탐 기계가 된다.
    # 「판가름」은 순우리말이다 — 「가름」만 보면 그 안에 걸린다.
    (r'가른|가르는|가르고|가르며|가르지|(?<!판)가름|갈랐',
     '나눈다·쪼갠다·구분한다·정한다 중 뜻에 맞는 쪽으로. 피동 「갈린다」는 정상이다'),
    (r'값[을이] 매[기겨]',
     '값은 정하는 것이다 — 「가격을 정한다」·「얼마로 보는지 정한다」로'),
    # 「값·대가·대금을 치른다」는 pay the price 를 그대로 옮긴 자리다. 낸다·문다·
    # 떠안는다·감당한다 중 누가 무엇을 내주는지가 보이는 쪽으로 쓴다. 목적어가
    # 잔금·시험·홍역처럼 구체적인 관용구는 여기 안 걸린다 — 그 셋은 정상 한국말이다.
    (r'(값|대가|대금|비용)[을를] ?(?:(?![가-힣]{1,3}[을를] )[가-힣]{1,3} ){0,2}치[르러른렀룰]',
     '낸다·문다·떠안는다·감당한다 중 누가 무엇을 내주는지 보이는 쪽으로'),
]


# 제3자가 쓴 제목·인용은 우리 문체 규칙의 대상이 아니다. 원제를 우리 말로 바꾸면
# 그 글이 무엇이었는지가 기록에서 사라진다. 여기 적은 문자열은 검사 전에 떼어 낸다 —
# 늘리기 전에 「이것이 정말 남의 말인가」를 묻는다.
QUOTED = (
    '멀티플을 가르는 것은 성장률이 아니라 이익 지속기간',
    '시장은 이 계산값의 세 배 넘는 값을 매기고 있다',
    '같은 불황에도 운전자본이 갈랐다',
    '그래도 산 가격이 수익을 가른다',
    '레드불은 그 비용을 치렀고',
)
...
def check_verb(text, where):
    """P19 — 번역투 동사가 남은 자리. 제3자 제목·인용은 떼고 본다."""
    for qs in QUOTED:
        text = text.replace(qs, '')
    for pat, fix in BANNED_VERB:
        for m in re.finditer(pat, text):
            at = max(0, m.start() - 20)
            snip = ' '.join(text[at:m.end() + 30].split())
            add('FAIL', where, 'P19',
...
# 관리자 대시보드의 오류 대장은 걸린 문장을 그대로 인용한다. 「돈이 도는 자리가」를
# 예시로 적으면 그 문서가 P14 로 걸린다 — 오답을 못 적는 검사기가 된다. 인용 블록은
# class="mk-case" 로 표시하고 여기서 뺀다(2026-08-29).
_ECHO_CLASSES = ('uc-kin', 'mk-case')
