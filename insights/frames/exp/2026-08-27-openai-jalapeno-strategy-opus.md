...
```mermaid
flowchart TD
    A["칩 파는 회사"] --> B["고객은 구매 부서"]
    B --> C["총소유비용 최적화"]
    C --> D["장부상 유리한 설계"]
    E["모델 파는 회사"] --> F["고객은 최종 사용자"]
    F --> G["끝 토큰 지연·요청당 에너지"]
    G --> H["체감 속도 우선 설계"]
    D --> I["같은 반도체, 다른 결론"]
    H --> I
...
```mermaid
flowchart TD
    A["범용 추론 칩 완성"] --> B{"외부에 파나"}
    B -->|판다| C["테이프아웃 비용 분산"]
    C --> D["기업·뉴클라우드 대상"]
    D --> E["직접 경쟁사는 제외"]
    E --> F["추론 서비스 형태 판매"]
    B -->|안 판다| G["자기 모델에 극한 공동설계"]
    G --> H["작업부하 데이터 독점 활용"]
    H --> I["원가·속도·마진 우위"]
    F --> J["매출은 늘고 우위는 희석"]
    I --> K["우위는 지키고 비용은 홀로"]
