...
flowchart TD
    subgraph R["유휴 처리 — 역할 분리"]
        R1["작업 비율 변동"] --> R2["한쪽 랙 유휴"]
        R2 --> R3["자본이 논다"]
    end
    subgraph B["유휴 처리 — 균형 단일 칩"]
        B1["작업 비율 변동"] --> B2["안 쓰는 구역 차단"]
        B2 --> B3["꺼진 실리콘만"]
    end
```
...
이 회사는 컴퓨트 최대 소비자 둘 중 하나다. 자기 칩이 자기 요구에 맞아떨어져서 기가와트 예산이 점점 자기 칩으로 옮겨 간다면, 그 돈은 아래로 흐른다. 회차에서 이름이 나온 곳만 적는다.

| 자리 | 이름 | 성격 |
|---|---|---|
| 시스템 설계 파트너 | 브로드컴, 셀레스티카 | 발표 슬라이드에서 직접 감사 표시 |
| 스케일업 네트워크 | 브로드컴 토마호크 6 스위치 | 128칩 도메인, 칩당 600Gb/s |
| 네트워크 규격 | ESUN | 브로드컴이 주도, UALink와는 다른 진영 |
| 파운드리 | TSMC N3 | N3P·N3E 변형으로 추정 |
| CPU | x86, 튜린급으로 추정 | AMD 베니스나 인텔 대안도 가능 |
| HBM | 삼성 HBM4 | **추측**이라고 명시됨. 핀당 속도가 더 나은 것으로 보인다는 정도 |
| 시스템 조립 | 셀레스티카 | SemiAnalysis 추정, OEM 위치 |

**성격 표시가 중요하다.** 브로드컴·셀레스티카는 발표에서 직접 호명됐고, 삼성 HBM4와 셀레스티카의 시스템 역할은 추정이다. 표에 나란히 있다고 무게가 같지 않다.

```mermaid
flowchart TD
    P["컴퓨트 예산"] -->|자체 칩 쪽| S["자체 칩 지출"]
    P -->|GPU 구매 쪽| G["GPU 지출"]
    S --> S1["브로드컴"]
    S --> S2["셀레스티카"]
    G --> G1["엔비디아·AMD"]
...
```mermaid
flowchart TD
    subgraph P["문제 — HBM 가동률"]
        P1["피연산자 지연"] --> P3["연산 유닛 대기"]
        P2["이웃 칩 경합"] --> P3
        P3 --> P4["이론치 미달"]
    end
    subgraph S["처방 — 어디를 고치나"]
        S1["핀당 속도 인상"]
        S2["전용 HBM 슬라이스"]
    end
    P4 -->|업계 관행| S1
