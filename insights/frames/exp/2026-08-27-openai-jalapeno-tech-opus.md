...
```mermaid
flowchart LR
    subgraph 이전["기존 통합 메모리"]
        X1["가속기 여럿"] --> X2["공유 HBM 풀"]
        X2 --> X3["경합 발생"]
    end
    subgraph 이후["NUMA 슬라이스"]
        Y1["가속기 A"] --> Y2["로컬 슬라이스 A"]
        Y3["가속기 B"] --> Y4["로컬 슬라이스 B"]
        Y2 --- Y5["컬렉티브 망"]
        Y4 --- Y5
        Y5 --- Y6["일반 NoC"]
    end
...
```mermaid
flowchart LR
    A["작은 드래프트 모델"] --> B["토큰 8개 생성"]
    B --> C["검증 부하 큼"]
    D["똑똑한 드래프트 모델"] --> E["토큰 2개 생성"]
    E --> F["검증 부하 작음"]
    C --> G["연산 배분 변동"]
    F --> G
    G --> H["균형 칩이 흡수"]
