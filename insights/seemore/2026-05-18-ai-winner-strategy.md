---
slug: 2026-05-18-ai-winner
lane: strategy
persona: 전략 컨설턴트 출신 애널리스트
model: claude-opus
source: https://youtu.be/0f1rhR-aqJA
sent: content/understanding/채널 씨모어/raw/2026-05-18-ai-winner.md (전사 전문)
date: 2026-09-07
title: 철도가 깔릴 때 누가 돈을 벌었는지를 세어 이번 차례를 묻는다
---
...
## 1. (시간 흐름) 응용에서 시작해 건설까지 번진 순서를 편다
...
## 3. (행위자) 결론은 플랫폼과 고객 접점으로 간다

빅테크들의 전략을 하나씩 짚는다. 엔비디아·아마존·구글·마이크로소프트·애플·메타가 각자 플랫폼을 넓히고 고객과 닿는 자리를 늘리고 있다는 것이다.

근거는 이력이다. PC·인터넷·클라우드·모바일 시대마다 최종 승자였던 빅테크의 주가가 300~400배씩 올랐으니, 이번에도 생태계와 표준, 플랫폼과 고객 접점을 쥔 쪽을 봐야 한다고 본다(L136~L137).

개별 판단도 둘 나온다. 마이크로소프트가 코파일럿 요금을 정액제에서 쓴 만큼 내는 방식으로 바꾸면 수익이 크게 늘 것으로 보고(L110~L112), 애플은 최고경영자가 다시 하드웨어 엔지니어 출신으로 바뀐 것을 근거로 조만간 AI 관련 혁신 제품을 내놓을 것으로 본다(L117~L118).

마지막으로 아직 주목받지 않은 자리를 하나 꼽는다. 피지컬 AI의 센서 부문이다. 독창적인 데이터를 센서로 모아 모델을 학습시키는 업체에서 성공 사례가 나올 수 있다고 본다(L165~L166).
