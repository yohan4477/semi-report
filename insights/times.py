...
OFFSET 의 뒤쪽 조건이 기간과 시점을 가른다. 「20년 전력공급 계약」은 20년짜리
계약이지 20년 전이 아니고, 「10~20년 뒤처짐」은 뒤가 아니라 뒤처지다다. 시점을
가리키는 「N년 전/뒤/후」는 조사나 공백이 뒤따르므로, 뒤에 다른 한글 음절이
붙으면 버린다.
...
OFFSET = re.compile(
    r'(?<![0-9])(\d{1,2})\s*년\s*(전|뒤|후)(?=[에엔은는이가만도부까와과의보면쯤]|[^가-힣]|$)')
