...
# 지시어는 홑말일 때만 지시어다. 「전자」는 삼성전자·LG전자·안전자산·전자상거래 안에도
# 들어 있고 「이 둘」은 「높이 둘러싸다」 안에, 「두 가지」는 「모두 가지고」 안에 들어 있다.
# 2026-08-20에 ⑥ 되돌아옴 편이 삼성전자 여섯 번과 안전자산 한 번으로 R2 FAIL 을 받아,
...
JOSA = frozenset([
    ...
    '에', '에서', '에게', '보다', '부터', '까지', '라', '란', '이라', '이란',
    '다', '이다', '이며', '이고', '만이', '만은', '뿐', '조차', '마저', '처럼',
])
HANGUL = re.compile(r'[가-힣]')


def standalone(text, m):
    """붙어 있는 낱말의 일부면 지시어가 아니다.

    앞 — 바로 앞 글자가 한글이나 로마자면 합성어다(삼성전자·LG전자·안전자산).
    뒤 — 이어지는 한글 덩어리가 조사여야 지시어다(전자가 ○ / 전자상거래 ×).
    """
    i, j = m.start(), m.end()
    if i and text[i - 1].isalpha():
        return False
    k = j
    while k < len(text) and HANGUL.match(text[k]):
        k += 1
    return text[j:k] in JOSA
...
# 눈길을 끄는 문장일수록 근거 없이 써진다. 세 갈래를 잡는다.
#   ① 통념 부정  ② "A가 아니라 B" 대구  ③ 원인 단정
# 메모리 편은 ①, 모델학습·모델랩 편은 ②, Helios 건은 ③에서 났다.
...
def check_file(path):
    ...
    # R10 — 첫 문단은 그 글에서 제일 눈에 띄는 자리라 없는 말을 지어내기 가장 쉽다.
