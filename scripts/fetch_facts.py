...
def ttm(facts, name, tags, annual, as_of=None):
    ...
    # 시점 값(재무상태표). 더하지 않고 잔액 하나를 쓰되 **손익과 같은 날짜**를 쓴다.
    #
    # 날짜를 안 맞추면 순현금 같은 합계가 서로 다른 날의 값으로 만들어진다.
    # 2026-08-27에 엔비디아가 그랬다 — 다른 잔액은 전부 2026-07-26인데 시장성증권만
    # 2025-10-26 이었다. MarketableSecuritiesCurrent 태그가 그 날짜로 끊겼는데 우리가
    # 마지막 값을 계속 들고 있었던 것이다. 아홉 달 낡은 491억을 순현금에 더하고 있었다.
    cands = []
    for pri, tag in enumerate(tags):
        ...
        for x in node.get('units', {}).get('USD', []):
            if 'start' in x or x.get('form') not in ('10-Q', '10-K'):
                continue
            cands.append(dict(end=x['end'], val=x['val'], filed=x.get('filed', ''),
                              tag=tag, pri=pri))
    if not cands:
        return None
    if as_of:
        same = [x for x in cands if x['end'] == as_of]
        if same:
            # 같은 날짜가 여럿이면 태그 순서를 따른다. 최신 제출본을 고른다.
            pick = min(same, key=lambda x: (x['pri'], x['filed']))
            return dict(val=pick['val'], window=pick['end'], end=pick['end'], tag=pick['tag'],
                        method='시점 값이라 잔액 하나를 쓴다. 손익 기준일과 같은 날이다')
    pick = max(cands, key=lambda x: (x['end'], x['filed']))
    out = dict(val=pick['val'], window=pick['end'], end=pick['end'], tag=pick['tag'],
               method='시점 값이라 잔액 하나를 쓴다')
    if as_of and pick['end'] != as_of:
        gap = (datetime.strptime(as_of, '%Y-%m-%d')
               - datetime.strptime(pick['end'], '%Y-%m-%d')).days
        out['stale_days'] = gap
        out['method'] += '. 손익 기준일 %s 에 이 항목이 없어 %d일 낡은 잔액을 쓴다' % (as_of, gap)
    return out
...
def sec_facts(ticker):
    ...
    # 손익부터 돌려 기준일을 잡는다. 시점 값은 그 날짜에 맞춰 고른다.
    for name, tags in CONCEPTS.items():
        if name not in FLOW:
            continue
        ...
        if got:
            out['ttm'][name] = got
    as_of = (out['ttm'].get('revenue') or {}).get('end')
    for name, tags in CONCEPTS.items():
        if name in FLOW:
            continue
        got = ttm(facts, name, tags, out['concepts'], as_of=as_of)
        if got:
            out['ttm'][name] = got
