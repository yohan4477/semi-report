...
VIEWSTEPS = [
 ('1', '무엇을 보낼지 고른다',
  '유료 원문은 밖으로 안 보낸다. 링크나 우리 요약본만 보낸다.',
  ['그 매체가 전문을 무료로 푸는지 먼저 본다 — Semi Doped 는 무료라 링크를 보낸다',
   '유료 구독물이면 링크 대신 우리 요약본을 보내거나 아예 안 묻는다'],
  ['CLAUDE.md 「유료 원문은 밖으로 안 보낸다」'],
  ['보내는 자리']),
 ('2', '모델을 고른다',
 ...
 ('3', '묻는다',
  ...
  ['전략 뷰는 「너는 전략 컨설턴트다」, 기술 뷰는 「너는 업계 기술 전문가다」',
   '도식을 받으려면 「도식은 [ 이름 ] 꼴 상자로」를 붙인다 — 안 붙이면 안 그린다',
 ...
 ('6', '싣는다',
  ...
  ['마크다운을 카드 마크업으로 옮긴다 — 제목·표·목록·문단',
   '아스키 도식은 상자로 굽는다. 못 읽는 꼴이면 아스키 그대로 둔다',
...
MISTAKES = [
 ...
 ('보내는 자리', '무엇을 밖으로 보내는지가 되돌릴 수 없는 결정이다.', [
  ('유료 원문을 통째로 보낸다', '사람만',
   ...
   '링크나 우리 요약본만 보낸다. 그 매체가 전문을 무료로 푸는지 먼저 본다.'),
 ]),
 ...
 ('묻는 자리', '무엇을 시키느냐가 무엇이 오는지를 정한다.', [
  ...
  ('요청을 걷으면서 도식까지 걷는다', '사람만',
   '2026-08-31 에 규칙을 다 걷었더니 도식이 하나도 안 왔다. 여섯 편에 코드펜스 0개.',
   '도식을 원하면 그 한 줄은 남긴다 — 「도식은 [ 이름 ] 꼴 상자로」.'),
 ]),
