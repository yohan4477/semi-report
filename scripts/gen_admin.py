...
FIGRULES = [
 ...
 ('좌표를 손으로 안 찍는다', '눈으로 맞춘 좌표는 상자를 옮기는 순간 어긋난다.', [
  ('선은 상자의 변에서 뽑는다', 'check_fig F5',
   '2026-08-29 까지 Semi Doped 도해가 M485 80 L485 92 처럼 끝점을 눈으로 맞췄다. '
   '상자를 옮길 때마다 선이 떨어졌고 검사기는 겹침만 재느라 FAIL 0 을 냈다.',
   'scripts/fig_layout.py 의 connect(a, b) 를 쓴다. 끝점이 두 상자의 변에서 계산된다.'),
  ('선은 x·y 축에 평행하게만 간다', 'check_fig F6',
   '비스듬한 선은 어디서 나와 어디로 들어가는지가 눈에 안 잡히고, 가로·세로만 보는 '
   '「선에 깔림」 검사도 빠져나간다.',
   '곧장 못 가면 꺾어서 간다. 가운데에서 한 번 꺾어 가로·세로·가로로 잇는다.'),
