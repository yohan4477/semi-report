...
VIEWSTEPS = [
 ('1', '무엇을 보낼지 고른다',
  '무료로 푸는 매체면 전문을 보낸다. 유료 원문은 밖으로 안 보낸다.',
  ['그 매체가 전문을 무료로 푸는지 먼저 본다 — Semi Doped 는 무료다',
   '무료면 회차 전문을 긁어 싣는다 — 링크만 주면 못 읽는 일이 있다',
   '유료 구독물이면 원문 대신 우리 요약본을 보낸다. 원문 문장은 안 나간다',
   '요약본은 사실을 골라 담은 중간물이라 그 선택까지 넘어간다 — 무료면 안 쓴다'],
  ['scripts/semidoped_clip.py', 'CLAUDE.md 「유료 원문은 밖으로 안 보낸다」'],
  ['보내는 자리']),
 ('2', '모델을 고른다',
...
MISTAKES = [
 ...
 ('보내는 자리', '무엇을 밖으로 보내는지가 되돌릴 수 없는 결정이다.', [
  ('유료 원문을 통째로 보낸다', '사람만',
   '아직 안 났다. 나면 되돌릴 수 없다 — 보낸 뒤에 지울 수 없다.',
   '그 매체가 전문을 무료로 푸는지 먼저 본다. 유료면 링크만 준다.'),
  ...
  ('무료 매체에 요약본을 보낸다', '사람만',
   ...
   '넘어가 뷰가 우리 선택을 되풀이한다.',
   '긁을 수 있으면 전문을 보낸다. 무엇을 고를지는 받는 쪽이 해야 뷰가 뷰다. '
   '요약본은 원문을 못 보내는 유료 자리에서만 쓴다.'),
 ]),
