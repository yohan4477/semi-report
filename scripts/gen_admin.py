...
# 노드 왼쪽 띠 — 들어오는 것 · 가공물 · 막는 것 · 스크립트 · 화면을 색으로 나눈다
  ...
  [
   ...
   ('⑤ 생성', 'gen', '섹션 번호는 손대지 않는다 — SEC_ORDER가 다시 매긴다',
    ...
    ['CSS 원본은 <code>scripts/dash_base_css.html</code>다. 예전에는 언더스탠딩 대시보드 HTML의 '
     ...
     '<b>손유지 페이지는 이제 없다.</b> 2026-08-17에 언더스탠딩 27장을 생성기 둘로 나눴다 — '
...
STAGES = [
 ...
 ('3', '무엇을 쓸지 고른다',
  ...
  ['뜯은 것마다 「이 편의 물음에 답하나」를 묻는다 — 원문에 있다는 것은 넣을 이유가 아니다',
...
# 뷰 받아 오는 법 — 다른 모델에게 같은 원문을 여러 눈으로 설명하게 하고 그 답을 싣는다.
# 2026-08-31 에 이 길을 처음 냈고, 아래는 그날 실제로 걸린 자리들이다.
VIEWSTEPS = [
 ('1', '무엇을 보낼지 고른다',
  '원문 전문을 보낸다. 유료든 무료든 같다.',
  ['회차 전문을 긁어 프롬프트에 싣는다 — 링크만 주면 못 읽는 일이 있다',
   '우리 요약본은 안 보낸다. 사실을 이미 골라 담은 중간물이라 그 선택까지 넘어간다',
   '유료 구독물도 전문을 보낸다 (2026-08-31 결정). 구독 계약과 저작권이 걸리는 자리라 '
   '무엇을 보냈는지 frontmatter 의 source 로 남긴다'],
  ['scripts/semidoped_clip.py', 'CLAUDE.md 「원문 전문을 보낸다」'],
  ['보내는 자리']),
 ('2', '모델을 고른다',
  '고르고 끝내지 않고 걸렸는지 확인한다.',
  ['단추를 눌러 3.1 Pro 를 고른다',
   '고른 뒤 단추 글자가 바뀌었는지 본다 — 안 바뀌면 안 걸린 것이다',
   '메뉴에서 회색이면 한도가 찬 것이다. 그때는 묻지 않고 풀릴 때까지 기다린다',
   '못 걸면 멈춘다 — 기본값(Flash-Lite)으로 받아 오면 어느 모델 글인지 모른다'],
  ['scripts/gem_ask.py 의 pick_model()'],
  ['모델 고르는 자리']),
 ('3', '묻는다',
  '페르소나와 링크만 준다. 규칙을 길게 달아도 회차 밖 값은 그대로 온다. 물을 때마다 임시 채팅을 켠다 — 새 대화만으로는 저장된 정보가 따라온다.',
  ['전략 뷰는 「너는 시니어 전략 컨설턴트다」, 기술 뷰는 「너는 시니어 업계 기술 전문가다」',
   '그 아래 링크와 회차 전문을 붙인다. 규칙은 안 단다',
   '프롬프트 꼬리에 도해를 청한다 — 그림으로 보이면 쉬운 대목에 넣어 달라는 한 줄만',
   '어떻게 그리라고는 안 적는다. 꼴을 길게 달아도 그대로 안 오고, 꼴은 frame_view 가 맞춘다',
   '통합 뷰는 앞의 두 답을 프롬프트에 넣어 합쳐 달라고 한다'],
  ['scripts/prompts/{strategy,tech,merged}.txt', 'scripts/gem_views.py'],
  ['묻는 자리']),
 ('4', '받는다',
  '답 밑의 「복사」 버튼을 눌러 마크다운 그대로 가져온다.',
  ['화면 글자를 긁지 않는다 — 표가 한 줄로 뭉개진다',
   '답이 멈췄는지는 길이가 여러 번 그대로인지로 본다'],
  ['scripts/gem_ask.py 의 copy_via_button()'],
  ['받는 자리']),
 ('5', '대조한다',
  '답이 스스로 붙인 「회차 안팎」 표시를 믿지 않는다.',
  ['insights/frames/ 에 원본 그대로 두고 frontmatter 에 source 를 적는다',
   'check_frame 을 돌려 원문 밖 이름과 수를 목록으로 받는다',
   '카드로 옮긴 것은 used, 이름만 댄 것은 named 에 적는다'],
  ['insights/check_frame.py'],
  ['대조하는 자리']),
 ('6', '싣는다',
  '문장을 고치지 않는다. 받은 글은 인용이지 우리 주장이 아니다.',
  ['마크다운을 카드 마크업으로 옮긴다 — 제목·표·목록·문단',
   '섹션이 회차, 카드가 뷰다 — 첫 화면 타일에 회차 제목이 서고 그 아래 뷰 카드가 선다',
   '앞머리(통합 뷰의 첫 제목 앞 문단)는 섹션 머리 아래에 한 번만 세운다',
   '통합 뷰는 카드로 안 세운다 — 대조해 보니 혼자 가진 대목이 없었다(앞머리 자리만)',
   '카드마다 어느 모델이 쓴 글인지 적는다 — 한도가 차면 낮은 모델로 답이 온다',
   '꼬리에 붙은 요약·제언은 그 뷰 카드 앞면으로 올린다 — 첫 문단은 앞면, 나머지는 본문 맨 위',
   '올린 문단은 몸에서 걷는다. 문장은 안 고치고 자리만 옮긴다',
...
MISTAKES = [
 ...
 ('보내는 자리', '무엇을 밖으로 보내는지가 되돌릴 수 없는 결정이다.', [
  ('무엇을 보냈는지 안 남긴다', 'check_frame 이 source 를 읽는다',
   '보낸 것은 되돌릴 수 없다. 어느 판을 보냈는지가 안 남으면 답을 원문과 대조할 수도 없다.',
   '프레임 파일 frontmatter 의 source 에 보낸 파일 경로를 적는다.'),
  ('링크만 주고 읽었다고 여긴다', 'gem_views 의 assert',
   '2026-08-31 에 기술 뷰가 「유료 구독자 전용이라 원문을 긁을 수 없지만 제목과 업계 '
   '동향을 바탕으로」로 시작했다. 회차가 아니라 배경지식으로 쓴 답이었다.',
   '무료 매체면 전문을 긁어 프롬프트에 싣는다. 전문 파일이 없으면 물음 자체를 멈춘다.'),
  ('요약본을 보낸다', '사람만',
   '요약본은 우리가 사실을 골라 담은 중간물이다. 그것을 주면 고르는 일이 이미 끝난 채로 '
   '넘어가 뷰가 우리 선택을 되풀이한다.',
   '전문을 보낸다. 무엇을 고를지는 받는 쪽이 해야 뷰가 뷰다.'),
 ]),
 ('모델 고르는 자리', '골랐다고 걸린 것이 아니다.', [
  ('고르고 확인을 안 한다', 'pick_model() 이 라벨로 확인',
   '2026-08-31 에 「확장된 사고 모델」을 골랐더니 클릭은 되는데 단추 라벨이 안 바뀌었고, '
   '답도 자기가 Flash 라고 말했다.',
   '고른 뒤 단추 글자가 바뀌는 것만 쓴다. 안 바뀌면 안 걸린 것으로 친다.'),
 ]),
 ('묻는 자리', '무엇을 시키느냐가 무엇이 오는지를 정한다.', [
  ('페르소나를 길게 적는다', '사람만',
   'Flash 에서 「업계 기술 전문가처럼 딥다이브」로 물었더니 거절이 왔다 — '
   '「저는 언어 모델일 뿐이라서 그것을 도와드릴 수가 없습니다」.',
   '페르소나는 한 줄로 짧게 준다. 거절이 오면 갈래를 나눠 다시 묻는다.'),
  ('규칙을 길게 달면 값이 걸러진다고 믿는다', '사람만',
   '회차 밖 값을 쓰지 말라고 적어도 13.4 PFLOPS 같은 수가 그대로 왔다. '
   '규칙은 표시를 만들지 값을 막지 못한다.',
   '값을 막는 것은 프롬프트가 아니라 대조다. 프롬프트는 짧게 두고 check_frame 을 돌린다.'),
  ('도식이 오기를 프롬프트로 기다린다', 'frame_view 의 _dia_span',
   '규칙을 다 걷은 뒤 여섯 편에 코드펜스가 0개였다. 청해서 받는 것은 청한 만큼만 온다.',
   '청하지 않는다. 오면 굽고 안 오면 글로 간다 — 싣는 자리가 울타리 밖 도식도 문다.'),
 ]),
 ('받는 자리', '어떻게 받느냐가 대조의 정확도를 정한다.', [
  ('화면 글자를 긁는다', '사람만',
   'inner_text 로 받으면 표가 한 줄로 붙어 어디까지가 열 이름인지 사라진다. '
   '전략 답이 표줄 0 · 제목 0 으로 왔다.',
   '답 밑의 「복사」 버튼을 눌러 마크다운으로 받는다 — 같은 답이 표줄 21 · 제목 7 로 온다.'),
  ('크롬 확장으로 읽으려 한다', '사람만',
   'gemini.google.com 은 스트리밍 연결을 계속 열어 두어 확장이 기다리는 document_idle 이 '
   '안 온다. 대기를 3·6·8·10초로 늘리고 탭을 새로 만들어도 같았다.',
   'playwright 로 CDP(9222)에 붙는다. 이 저장소가 이미 쓰는 길이다.'),
 ]),
 ('대조하는 자리', '답이 스스로 붙인 표시는 근거가 아니다.', [
  ('회차 안팎 표시를 믿는다', 'check_frame F1',
   '통합 답이 「회차에 실제로 나온 내용」이라고 적은 목록에 원문에 없는 값이 여섯이었다 — '
   '13.4 PFLOPS · MXFP4 · 216GiB · 15.4TB/s · TBT · GB300.',
   '표시를 읽지 말고 대조한다. 이름과 수를 뽑아 원문에서 찾는다.'),
  ('영문 표기라 없다고 잡힌 것을 결함으로 센다', '사람만',
   'F1 목록의 상당수가 Genius·Spillover·Host CPU 처럼 우리말로는 원문에 있는 말이다.',
   '목록은 목록으로 읽는다. 진짜 밖은 규격·제품 이름과 수치다.'),
 ]),
 ('싣는 자리', '남의 글에 우리 규칙을 대면 갚을 수 없는 빚만 쌓인다.', [
  ('인용에 우리 검사기를 댄다', 'check_frame · check_prose · check_struct 의 인용 예외',
   '받은 글을 카드에 싣자 check_frame 이 FAIL 126 을 냈고, 성격 열이 없다고 S6 가, '
   '번역투라고 P4 가 걸렸다. 전부 남의 문장이다.',
   '인용 자리를 검사기에서 뺀다. 우리가 옮겨 적은 것만 우리 규칙으로 잰다.'),
  ('아스키를 그대로 싣는다', '사람만',
   '폭이 넘치고 글꼴이 바뀌면 선이 어긋난다.',
   '[ 이름 ] 꼴을 상자로 굽는다. 못 읽는 꼴이면 아스키 그대로 두고 넘어간다.'),
 ]),
 ...
 ('넣을 것과 뺄 것을 고른다', '뜯은 것을 다 넣으면 카드가 원문의 두 번째 사본이 된다.', [
  ('원문에 있다는 이유로 넣는다', '사람만',
   '2026-08-29 원문과 대조해 빠진 것 열을 세운 뒤, 그중 여섯을 그대로 카드에 넣었다. '
   '셋은 이 카드의 물음에 답하지 않는 것이었다 — 앞 문단이 이미 한 말을 되풀이하는 인용, '
   '이 칩과 무관한 업계 로드맵, 진행자도 뜻을 모른다고 한 이름.',
   '빠진 것을 세는 일과 넣을 것을 고르는 일은 다른 판단이다. 대조는 목록을 주고, '
   '넣을지는 「이 편의 물음에 답하나」로 다시 고른다. 안 넣은 것은 한계 절에 이름만 올린다.'),
  ('버린 것을 안 적는다', '사람만',
   '안 세운 마디를 감추면 카드가 원문 전부인 것처럼 읽힌다. 독자는 그 편에 그 이야기가 '
   '없었다고 믿게 된다.',
   '한계 절에 이름만 올린다 — 무엇을 뺐고 왜 사슬 밖인지 한 줄이면 된다.'),
 ]),
 ...
 ('검사를 안 받고 나간다', '규칙이 있어도 그 글이 검사 대상 목록에 없으면 아무도 안 본다.', [
  ...
  ('다 쓰고 원문과 대조하지 않는다', '사람만 · check_cite 는 숫자만',
   '2026-08-29 한 카드에서 셋이 한꺼번에 나왔다 — 원문의 「데이터」를 「값」으로 좁혔고, '
   '「GPU 처럼」을 「다른 칩에 맡기는 길이 있다」로 넓혔고, 9개월의 양 끝을 뺐다. '
...
# 도해 작성법 — 그림에서만 나는 오류는 글 검사기가 안 잡는다.
# (묶음, 한 줄, [(규칙, 잡는 것, 왜 생겼나, 어떻게 한다)])
FIGRULES = [
 ('무엇을 그릴지', '한 그림은 하나만 말한다. 축이 둘이면 그림을 둘로 나눈다.', [
  ('말로 가는 절에만 붙인다', '사람만',
   '표가 이미 있는 절에 같은 값을 그림으로 또 그리면 읽는 사람이 두 번 읽고 같은 것을 얻는다.',
   '견주는 값이 둘 이상이면 표다. 흐름·순서·구조처럼 표로 안 되는 자리에 도해를 쓴다.'),
  ('주체가 셋 미만이면 흐름도를 안 그린다', '사람만',
   '둘 사이를 잇는 그림은 문장 하나가 더 빠르다. 여덟을 넘으면 반대로 한 장에 안 들어간다.',
   '주체 셋에서 일곱, 오가는 것 두 종류 이상일 때만 흐름도다. 여덟을 넘으면 둘로 쪼갠다.'),
  ('축에 무엇을 재는지와 어느 쪽이 큰지를 적는다', '사람만',
   '눈금 없는 그래프에 축 이름을 「지연」 한 낱말로 달면, 오른쪽이 좋은 쪽인지 나쁜 '
   '쪽인지를 읽는 사람이 찍어야 한다. 곡선의 뜻이 통째로 뒤집힌다.',
   '축에는 재는 것만 짧게 단다(「엔드투엔드 지연」). 어느 쪽이 큰지와 곡선을 어떻게 '
   '읽는지는 판 안 빈 자리에 상자로 앉힌다 — 축 이름에 다 적으면 가로축이 판보다 '
   '넓어지고 세로축은 돌아누운 채 잘린다.'),
  ('형태는 보이려는 것이 정한다', '사람만',
   '어디냐면 지도, 순서면 줄마다 손에 든 것과 받아 낸 것, 언제냐면 시간축 하나에 상태 둘, '
   '어긋남이면 좌우 대비다.',
   'docs/도해 유형 — 무엇을 언제 그리나.md 의 표에서 고른다. 유형을 먼저 정하고 그린다.'),
 ]),
 ('없는 값을 안 그린다', '가장 자주 깨지고 가장 티가 안 난다. 길이도 개수도 전부 수치로 읽힌다.', [
  ('막대 높이·아이콘 개수·상자 개수가 다 값이다', 'check_figval 은 글자만 · 도형은 사람만',
   '2026-08-29 스케일업 도해가 「여럿」을 상자 넷으로 그렸다. 원문에 넷이 없다. '
   '앞서 못 나오는 배를 열 척, 남은 배를 세 척 그린 적도 있다 — 원문에는 35척·100척뿐이었다.',
   '셀 수 없으면 세지 않는다. 높이를 없애고 상태 둘로 바꾸거나, 개수를 글자로 적고 도형은 하나만 둔다.'),
  ('그린 뒤 값을 원문과 전수 대조한다', 'check_figval',
   '라벨에 숫자를 안 썼다고 빠져나가지 못한다. 반대로 글자에 적은 값은 기계가 전부 세어 준다.',
   'insight-figure 스킬의 대조 명령을 돌린다. 도형 개수는 그 뒤에 사람이 센다.'),
 ]),
 ('좌표를 손으로 안 찍는다', '눈으로 맞춘 좌표는 상자를 옮기는 순간 어긋난다.', [
  ('칸 폭은 글자에서 낸다', 'fig_layout 이 계산 · check_fig 가 삐짐을 잡는다',
   '폭을 손으로 정하면 글자 양옆에 빈자리가 남고 그만큼 글자를 못 키운다. 반대로 좁게 잡으면 삐진다.',
   '가장 긴 줄에 여백만 더해 폭을 낸다. 검사기의 어림보다 좁게 재지 않는다.'),
  ('지도·시간축 좌표는 내보낸 값을 쓴다', '사람만',
   '주 경계와 한가운데를 어림해 찍으면 지시선 끝이 엉뚱한 데를 가리킨다.',
   'data/us_lower48_albers.json · data/world_robinson.json 을 쓴다. 시간축은 개월 수를 받는 함수 하나만 부른다.'),
 ]),
 ('선 그리는 법', '선 하나가 어긋나면 그림 전체가 손으로 그린 것으로 보인다. 오늘 난 것만 적는다.', [
  ('축에 평행하게만 간다', 'check_fig slanted()',
   '비스듬한 선은 어느 상자에서 나와 어디로 드는지가 눈에 안 잡히고, 가로·세로만 재는 '
   '「선에 깔림」 검사도 통째로 빠져나간다. 2026-08-29 할라페뇨 도해에 아홉 줄이 있었다.',
   '곧장 못 가면 꺾어서 간다. 가로 · 세로 · 가로 세 토막으로 잇는다.'),
  ('끝점은 상자의 변에서 낸다', 'check_fig F5',
   'M485 80 L485 92 처럼 끝점을 눈으로 맞추면 상자를 옮길 때마다 선이 떨어진다. '
   '검사기는 겹침만 재느라 떠 있는 선에 FAIL 0 을 냈다.',
   'scripts/fig_layout.py 의 connect(a, b) 를 쓴다. 손으로 그릴 때는 상자 좌표에서 계산한다.'),
  ('잇는 두 도형의 중심을 같은 식으로 낸다', '사람만',
   '2026-08-29 가운데 칩은 x0+118 로 내려가고 스위치로 드는 줄은 x0+120 이었다. '
   '2픽셀인데 화면에서 꺾인 자리가 어긋나 보였다.',
   '두 좌표를 따로 적지 않는다. 한 식에서 내거나 한 변수를 둘이 같이 쓴다.'),
  ('여럿이 한 곳으로 들면 공통 줄에 모은다', '사람만',
   '상자 셋에서 스위치 하나로 각각 그으면 부챗살이 되고 그 셋은 전부 비스듬하다.',
   '각자 수직으로 내려 공통 가로 줄에 붙이고, 그 줄에서 한 번만 목적지로 내려간다.'),
  ('공통 줄은 첫 갈래에서 끝 갈래까지만', '사람만',
   '버스를 마지막 세로줄보다 4픽셀 더 그어 오른쪽에 꼬리가 남았다.',
   '양 끝을 세로줄의 x 로 그대로 쓴다. 여유를 두지 않는다.'),
  ('굵기는 어느 도해에서나 같다', '사람만',
   '2026-08-29 묶음 그림만 굵기 3 대 1 로 속도 차이(600 ÷ 200)를 나타냈다. 그 한 장만 '
   '선이 다른 도해보다 굵어서, 장을 넘길 때 굵기가 뜻을 갖는 것처럼 읽혔다.',
   '선 굵기를 값으로 쓰지 않는다. 굵기는 1.2 로 고정하고 갈래는 색과 대시로 나눈다. '
   '수치는 판 아래 범례에 글자로 남긴다.'),
  ('굵은 선이 꺾이면 끝을 둥글게', '사람만',
   '3.5픽셀 선이 각지게 끝나면 꺾이는 모서리에 홈이 보인다.',
   'stroke-linecap=round 로 물린다.'),
 ]),
 ('판 위에 글자를 안 얹는다', '판은 그림만 진다. 판단과 범례는 그 아래로 내린다.', [
  ('캡션이 본문을 되풀이한다', '사람만',
   '2026-08-29 파레토 도해 캡션이 바로 아래 문단과 같은 말을 했다 — 지연과 전기가 서로 '
   '밀어낸다는 문장이 두 번 붙어 있었다. 다른 도해 셋도 같았다.',
   '캡션에는 그림에서만 읽히는 것을 적는다 — 왜 눈금이 없나, 무엇을 일부러 안 그렸나, '
   '개수가 값이 아니라는 표시. 그림이 무슨 뜻인지는 본문이 말한다.'),
  ('캡션이 그림보다 낡는다', '사람만',
   '선 굵기로 속도를 보이던 그림을 색으로 바꾼 뒤에도 캡션은 「선 굵기가 속도다」로 '
   '남아 있었다. 화면에 거짓이 하나 서 있었다.',
   '그림을 고치면 그 자리에서 캡션을 같이 읽는다. 캡션이 그림의 무엇을 가리키는지 '
   '짧을수록 낡을 자리도 적다.'),
  ('설명은 판 아래 한 줄로', 'check_fig',
   '지도에 얹은 설명은 배경과 겹쳐 안 읽힌다. 상자 안에 든 글자는 판 위가 아니라서 괜찮다.',
   '색 딱지 하나와 한 줄로 내린다. 판단은 그 아래 상자가 말한다.'),
  ('글자끼리·선과 글자가 겹치는지는 기계가 센다', 'check_fig',
   '눈으로 보고 안 겹친다고 보고했다가 두 번 틀렸다. 처음에는 글자끼리만 재서 범례를 놓쳤고, '
   '다음에는 선을 안 봐서 갈고리가 글줄을 가로지르는 것을 놓쳤다.',
   'check_fig.py 를 돌린다. 카드에 아직 안 붙인 도해는 hits() 를 직접 부른다.'),
 ]),
 ('흐름도의 약속', '한 장만 볼 사람이 대부분이라 매번 같은 문법으로 그린다.', [
  ('선은 세 종류뿐', '사람만',
   '조건부 의무를 실선으로 그리면 보증이 있다로 읽힌다. 실제로는 부도 뒤에만 작동한다.',
   '돈은 강조색 실선, 물건·용역은 회색 실선, 조건부는 회색 점선. 색이 아니라 대시로 나눈다.'),
  ('역할 라벨은 상자 밖 도랑에', '사람만',
   '역할은 행의 성질이지 그 회사의 이름이 아니다. 상자 안에 넣으면 이름과 역할이 섞인다.',
   '가운데 도랑이나 상자 위에 작은 회색으로 세운다.'),
  ('강조색 상자는 그림당 한 종류', '사람만',
   '강조가 둘 이상이면 어느 쪽이 주인공인지 사라진다.',
   '이 그림이 뜯어보는 대상만 칠한다. 묶음은 더 옅은 같은 색 컨테이너로 감싼다.'),
  ('숫자는 그것을 가진 상자 안에', '사람만',
   '떠다니는 숫자 딱지는 어느 주체의 값인지가 없다.',
   '상자 둘째 줄에 적는다. 상한이 있으면 최대 노출까지 같이 적는다.'),
  ('제목은 라벨이 아니라 문장', '사람만',
   '「앤트로픽 컴퓨트 파이낸싱 구조」는 무엇을 알게 되는지 말하지 않는다.',
   '제목이 결론을 말하고 그림이 그 결론을 어떻게 아는지 보인다.'),
 ]),
]
...
def main():
    ...
    out.append('<details class="fold lead" open><summary>뷰 받아 오는 법'
               '<span>단계 %d</span></summary>' % len(VIEWSTEPS))
    out.append('<p class="lede2">같은 원문을 다른 눈으로 설명하게 하고 그 답을 그대로 싣는다. '
               '받은 글은 재료가 아니라 <b>인용</b>이다 — 문장을 고치지 않는 대신, 원문이 '
               '받쳐 주는지는 따로 센다.</p>')
    for st in VIEWSTEPS:
        out.append(stage_block(*st))
    out.append('</details>')
