...
EXPERIMENTS = [
 ('exp-card-fit', '2026-08-31', '📏', '오푸스 답을 그대로 카드에 실으면 읽히나',
  '앞 실험이 남긴 물음 하나다 — 분량이 세 배인 답을 지금 카드 꼴(접었다 펴는 한 덩어리)에 '
  '그대로 실었을 때 읽히나.',
  '운영 대시보드는 안 건드리고 미리보기 HTML 을 임시 자리에 구웠다. 카드 넷(오푸스 둘·제미나이 둘)을 '
  '다 펴고 브라우저(폭 1280)에서 카드 높이를 쟀다.',
  [('오푸스 경영전략', '6,287 px', '판 2 · 아스키 9.'),
   ('오푸스 기술', '9,246 px', '판 23 · 아스키 8. 노트북 화면(800px)으로 11.5 화면이다.'),
   ('제미나이', '2,418 · 2,570 px', '같은 자로 잰 값. 2.6~3.6배 차이다.'),
   ('덤으로 나온 결함', '2건', '아스키 9px · 도식 들여쓰기 소실. 둘 다 운영에도 있었다.')],
  [
   ('결과 ① 한 카드에 안 담긴다', 'gate',
    '접었다 펴는 꼴로는 길다',
    [('오푸스 기술', '9,246 px'), ('화면 하나', '약 800 px'), ('나눗셈', '11.5 화면')],
    ['카드를 나누거나(절 단위) 카드 안에 목차를 세우지 않으면 못 읽는다.',
     '분량은 모델이 아니라 <b>카드 꼴</b>이 정하는 문제다 — 이 실험이 그것을 갈라 보였다.']),

   ('결과 ② 실어 보니 우리 결함이 드러났다', 'gate',
    '남의 답으로 우리 파서를 시험한 셈이다',
    [('아스키 글자', '9px  →  10.5px'),
     ('들여쓰기', '문단으로 쪼갰다 이으며 사라짐'),
     ('그 여파', '표·좌우 대조가 아스키로 떨어짐')],
    ['아스키를 폭에 맞춰 9px 까지 줄이고 있었다. 화면에서 못 읽는다 — 글자를 줄이는 일과 지우는 일이 '
     '그 크기에서 갈리지 않는다. 바닥을 10.5px 로 올렸다.',
     '카드가 앞면 문단을 걷을 때 본문을 문단으로 쪼갰다 다시 이었는데, 조각마다 strip 이 걸려 '
     '<b>도식 줄의 들여쓰기가 통째로 사라졌다</b>. 칸 정렬이 무너지니 표가 표로 안 읽혔다. '
     '걷은 문단 자리만 도려내도록 고쳤다.']),
  ],
  '오푸스 답을 그대로 싣는 길은 지금 카드 꼴에서 막힌다. 실으려면 카드를 절 단위로 쪼개거나 카드 안에 '
  '목차를 세워야 한다 — 그것은 모델을 고르는 일과 별개의 결정이다.'),

 ('exp-fig-width', '2026-08-31', '📐', '도해 폭을 프롬프트로 제한하면 판이 늘어나나',
  '우리 파서는 줄이 길면 판(SVG)을 못 만들고 아스키로 둔다. 프롬프트에 「한 줄 40자」를 넣으면 '
  '판으로 굽는 비율이 오르나.',
  '운영 프롬프트는 안 건드리고 사본에 한 줄을 더해 같은 회차를 3.1 Pro 로 다시 받았다. 40자는 판 폭 520px 과 '
  '본문 글자 13.4px 에서 낸 수다. 잰 것은 <code>frame_view._block_html</code> 이 실제로 무엇을 냈는가다.',
  [('최장 줄', '97~144칸  →  36~40칸', '지시는 들었다. 줄이 실제로 짧아졌다.'),
   ('판 비율', '50%  →  0%', '판 3개에서 0개. 목표와 반대로 갔다.'),
   ('도해 수', '6개  →  3개', '개수 자체가 절반으로 줄었다.'),
   ('낱말 손실', '0건', '상자 이름이 잘리거나 문장이 사라지지는 않았다.')],
  [
   ('결과 ① 폭이 아니라 그림 꼴이 바뀌었다', 'gate',
    '짧게 그리라니 다르게 그렸다',
    [('상자 틀', '┌ │ └ 대신 ├ └ 로 그림'),
     ('우리 판정', '_is_list 가 목록으로 읽음'),
     ('목적지 줄', '선 문자가 없어 도식 범위가 끊김')],
    ['전략 뷰의 유일한 도해는 <code>boxes()</code> 를 직접 부르면 판이 나온다. 그런데 '
     '<code>_is_list</code> 가 줄머리 <code>├ └</code> 를 목록 표시로 보고 글로 풀었다.',
     '기술 뷰는 소스 상자와 목적지 상자를 빈 줄 없이 이었는데 목적지 줄에 선 문자가 없어 '
     '<code>_is_dia</code> 가 그 줄을 도식으로 안 봤다 — 이음 0개짜리 그래프가 되어 아스키로 떨어졌다.']),

   ('결과 ② 넣지 않았다', 'work',
    '순서가 틀렸다',
    [('표본', '1편 · 2뷰 · 6도해'),
     ('먼저 할 것', '_is_list · _is_dia 판정'),
     ('그다음', '표본을 늘려 다시 잰다')],
    ['한 줄을 넣으면 이 회차처럼 역효과가 난다. 파서의 목록·도식 판정을 먼저 손보고 다시 재는 것이 순서다.',
     '실험 산출물은 2026-09-01 Semi Doped 장을 걷을 때 함께 지웠다 — 잰 값만 여기 남는다.']),
  ],
  '「폭을 줄이면 판이 는다」는 틀렸다. 판을 막는 것은 줄 길이가 아니라 우리 판정 규칙이었다. '
  '다음에 잴 것: 목록·도식 판정을 고친 뒤 같은 프롬프트로 다시 받으면 판 비율이 오르나.'),

 ('exp-opus-gemini', '2026-08-31', '⚖️', '같은 프롬프트에 오푸스는 다르게 답하나',
  '뷰를 받아 오는 자리에 <b>제미나이 3.1 Pro</b> 대신 <b>클로드 오푸스 5</b>를 세우면 무엇이 달라지나 — 길이·구조·도해·원문 밖 주장.',
  '할라페뇨 회차 하나(2026-08-27)로 뷰 둘을 각각 받았다. 프롬프트는 <code>scripts/prompts/{strategy,tech}.txt</code> 그대로, 회차 전문 50KB 를 같이 보냈다. 오푸스 쪽에는 저장소 규칙(CLAUDE.md·문체 규칙)을 <b>따르지 말라</b>고 못 박았다 — 안 그러면 조건이 달라 비교가 안 된다. 답은 <code>insights/frames/exp/</code> 에 두었다(검사기가 보는 자리 밖).',
  [('길이', '2.6~3.9배', '경영전략 3,776자 대 9,785자 · 기술 3,511자 대 13,560자.'),
   ('절', '3배 이상', '경영전략 4 대 13 · 기술 6 대 20. 오푸스는 절을 잘게 쪼갠다.'),
   ('도해', '3개 대 12~15개', '오푸스가 훨씬 많이 그린다. 표(마크다운)도 오푸스만 쓴다 — 26줄·59줄.'),
   ('판으로 굽힘', '50% 대 30%', '제미나이 도해 6개 중 3개, 오푸스 27개 중 8개가 SVG 판이 된다.')],
  [
   ('측정 ① 오푸스가 길고 잘게 쓴다', 'work',
    '같은 프롬프트인데 분량이 세 배다',
    [('경영전략', '3,776자 · 절 4  →  9,785자 · 절 13'),
     ('기술', '3,511자 · 절 6  →  13,560자 · 절 20'),
     ('표', '0줄  →  26줄 · 59줄')],
    ['카드 하나에 담기는 양이 달라진다 — 지금 카드 꼴(펼치면 한 덩어리)로는 오푸스 답이 길다.',
     '오푸스는 마크다운 표를 쓴다. 제미나이는 안 썼다. 표는 <code>frame_view</code> 가 이미 굽는다.']),

   ('측정 ② 도해는 많은데 판으로 덜 구워진다', 'gate',
    '아스키가 넓어 우리 파서가 못 읽는다',
    [('제미나이', '도해 6개 · 판 3 · 아스키 3'),
     ('오푸스', '도해 27개 · 판 8 · 아스키 19'),
     ('최장 줄', '53~78칸 (판 폭 520 을 넘긴다)')],
    ['오푸스 도해는 좌우 대조·막대·중첩 테두리를 즐겨 쓴다. 줄이 길어 판을 못 만들고 아스키로 남는다.',
     '아스키로 남아도 글자는 안 잃는다(<code>_kept</code>). 다만 그림이 문단과 안 붙는다 — 그 자리가 이 실험의 값이다.']),

   ('측정 ③ 원문 밖 수치는 둘 다 나온다', 'gate',
    '길이에 비례해 늘어난다',
    [('제미나이', '경영전략 1 · 기술 3'),
     ('오푸스', '경영전략 2 · 기술 7'),
     ('보기', '0.5TB · 1,000W · 100만')],
    ['세는 방법이 거칠다 — 목록 번호(「10.」)나 반올림한 값이 섞인다. 절대값이 아니라 <b>둘을 같은 자로 잰 비교</b>로만 읽어야 한다.',
     '판정은 <code>check_frame</code> 이 한다. 이 실험 파일은 그 검사기가 보는 자리 밖에 두었다.']),

   ('내용 ① 화자를 나누나', 'work',
    '원문은 두 사람이 주고받은 대담이다',
    [('제미나이', '화자 없음 — 「분석해 드리겠습니다」'),
     ('오푸스', 'Austin · Vik · Richard Ho 로 나눠 적는다'),
     ('원문', '세 이름 다 나온다')],
    ['제미나이는 대담을 한 사람의 결론처럼 쓴다. 누가 무엇을 말했는지가 사라진다.',
     '오푸스는 「Austin이 여기서 짚은 것」·「Vik은 여기에 젠슨 황의 말을 겹쳤다」로 적는다. '
     '우리 쟁점 카드(<code>check_debate</code>)가 요구하는 꼴에 가깝다.']),

   ('내용 ② 이견을 다루나', 'gate',
    '이 회차에는 두 진행자가 갈린 자리가 있다',
    [('제미나이', '없다 — 합의된 결론으로 쓴다'),
     ('오푸스', '절 하나를 통째로 세운다'),
     ('그 절', '「팔 것인가 — 갈린 유일한 지점」')],
    ['오푸스는 Austin(팔 길이 있다 · 파운드리 대 IDM 비유)과 Vik(오히려 더 좁혀야 한다 · '
     '자기 워크로드 데이터가 우위)을 나란히 놓고, <b>Vik이 스스로 뒤집은 대목</b>까지 적는다.',
     '제미나이 답에는 이 갈림이 아예 없다. 회차의 한 축이 통째로 빠진다.']),

   ('내용 ③ 원문 밖 이름은 둘 다 만든다', 'gate',
    '방향이 다르다',
    [('제미나이', 'GPT-5 · Q* — 회차에 없는 모델 이름'),
     ('오푸스', 'AWS Trainium — 회차에 없는 선례'),
     ('공통', '둘 다 원문 밖이다')],
    ['제미나이는 <b>앞날을 추정</b>하며 이름을 만든다(「Gen 2 이후는 GPT-5, Q*에 맞춰」). '
     '「사형 선고」 같은 표현도 회차에 없다.',
     '오푸스는 <b>비교 선례</b>로 밖 이름을 끌어온다(AWS Trainium 30% 인하). 둘 다 카드로 옮기면 '
     '<code>check_frame</code> 에 걸릴 자리다 — 이 실험은 그 자리가 모델에 따라 성격이 다르다는 것만 남긴다.']),

   ('내용 ④ 안 다룬 것을 적나', 'work',
    '한계를 적는 자리',
    [('제미나이', '없다'),
     ('오푸스', '「이 회차가 답하지 않은 것」 절 + 부록'),
     ('우리 규칙', '마지막 절은 한계')],
    ['오푸스 기술 답은 문제 → 원인 → 처방으로 마디를 쪼개고(6-1~6-4), 끝에 답 안 한 것을 세우고, '
     '「설계 결정 한눈에」 부록을 붙인다.',
     '제미나이는 제원 나열 뒤 이유를 붙이는 꼴이다. 우리 문서 규칙(<code>doc-structure</code>)이 '
     '요구하는 한계 절은 둘 중 오푸스에만 있다.']),

   ('측정 ④ 이모지는 조건이 달랐다', 'src',
    '제미나이 답이 먼저, 금지 줄이 나중이다',
    [('제미나이 경영전략', '이모지 13개'),
     ('제미나이 기술', '0개'),
     ('오푸스', '둘 다 0개')],
    ['제미나이 경영전략 답은 프롬프트에 「이모지는 쓰지 않는다」가 들어가기 <b>전</b>에 받은 것이다. 같은 조건이 아니므로 이 칸은 비교가 아니라 기록이다.']),
  ],
  '측정만 남긴다. 모델을 바꿀지는 이 값으로 못 정한다 — 회차 하나·뷰 둘이고, 카드 꼴(분량)과 파서(도해 폭)가 결과를 함께 정하기 때문이다. 다음에 잴 것은 둘이다: ① 오푸스 답을 그대로 카드에 실었을 때 읽히는가 ② 도해 폭을 프롬프트로 제한하면 판으로 굽는 비율이 올라가는가.'),
...
STAGES = [
 ...
 ('4', '보고서를 세운다',
  ...
  ['앞머리에 물음·바탕·축을 적는다',
   ...
   '말로만 가는 절에 도해를 붙인다',
   '마지막 절은 한계다'],
  ['card_lib.toc_html / report_html', 'yohan-figure 스킬', 'fig_layout · chart.py'],
  ['말이 빠진다', '숫자가 맥락 없이 선다']),
 ('5', '다 쓰고 검토한다',
...
FIGRULES = [
 ...
 ('없는 값을 안 그린다', '가장 자주 깨지고 가장 티가 안 난다. 길이도 개수도 전부 수치로 읽힌다.', [
  ...
  ('그린 뒤 값을 원문과 전수 대조한다', 'check_figval',
   ...
   'yohan-figure 스킬의 대조 명령을 돌린다. 도형 개수는 그 뒤에 사람이 센다.'),
 ]),
