...
PAGES = [
    ...
    # 탐색기라 밸류체인 장이 이 안에 든다(2026-09-13). 공시가 원문이라 잠그지 않는다. 용어사전 다음 첫 장 — 회사가 입구다(2026-09-13)
    ...
    # 2026-09-08 「링크드인 흐름」 장을 걷고 SemiAnalysis 장의 첫 절(①)로 넣었다 —
    # 재료가 같은 537건이라 두 주소에 서면 어느 쪽이 최신인지 못 고른다
...
('미국주식 사관학교 대시보드.html', 'usa-academy', '미국주식 사관학교', '🎓',
    ...
    # 2026-09-08 「메르 흐름」 장을 걷고 메르 대시보드의 첫 고정 층으로 넣었다 — 같은
    # 627편이 두 주소에 서 있으면 읽는 사람이 어느 쪽이 최신인지 못 고른다
    ...
    ('씨모어 대시보드.html', 'seemore', '채널 씨모어', '📈',
     ...
     False),
    # 씨모어 전사에서 뽑은 밸류체인만 따로 세운 장. 회차가 아니라 사슬이 단위라 씨모어
    # 장과 겹치지 않는다 — 저기서는 한 편을 읽고, 여기서는 여섯 편에 걸쳐 깐 사슬을 본다
    ('체인 대시보드.html', 'chain', '체인 인사이트', '⛓️',
     '산업마다 물건과 돈이 지나가는 순서 — 마디마다 원문이 이름을 댄 회사와 병목이라 '
     '지목한 자리', False),
    # SEC 공시(10-K·20-F)에서 뽑은 회사 사이 관계 그래프. 원문이 공개라 잠그지 않는다
    ('밸류체인 탐색기.html', 'valuechain', '밸류체인 탐색기', '🔗',
     '회사를 눌러 공급사와 고객을 계속 펼친다 — 선마다 무엇을 근거로 이었는지와 '
     '시점별 비중이 붙는다', False),
    # 공정위 대규모기업집단현황공시·DART 정기보고서에서만 뽑은 그룹 지분 사슬. 공시가 원문이라 잠그지 않는다(2026-09-17)
    ('지분 대시보드.html', 'ownership', '지분 대시보드', '🕸️',
     '일가에서 계열사까지 누가 누구의 최대주주인지 — 선마다 보통주 지분율과 공시 출처가 붙는다', False),
    ...
    # 유튜브 공개 영상 — 모의 케이스 면접에서 오간 말을 대본으로 옮긴다. 잠그지 않는다
    ('RocketBlocks 대시보드.html', 'rocketblocks', 'RocketBlocks', '🎯',
     '전략컨설팅 케이스 면접 모의 — 면접관과 지원자가 오간 말을 막 단위 대본으로 옮기고, '
     '판정은 글 맨 끝 평가표에 몰았다', False),
    # 2026-09-04 CPO 층을 더하며 다시 세운다. 재료가 유료 뉴스레터라 잠근다
    ('통합 보고서.html', 'report', '통합 보고서', '📑',
     '카드 여러 장을 한 물음으로 꿴 글 — CPO·선단 패키징·금리·메모리·트럼프·전력·하네스', True),
    ('모델링 대시보드.html', 'model', '모델링', '🧮',
     '발표된 계산기를 코드로 다시 세우고 발표치와 칸 단위로 대조한다', True),
    ('이선엽 시황 대시보드.html', 'leesunyeop', '이선엽 시황', '🎖️',
     '삼프로TV 유료 클럽의 실시간 텍스트 시황 — 원문은 싣지 않고 요약만', True),
    # 링크드인 공개 글 요약 — 잠그지 않는다. 글 한 편이 카드 한 장, 2026년 글만
    ('M&A 대시보드.html', 'manda', 'M&A 인사이트', '🤝',
     '회계법인 딜 자문가가 링크드인에 쓴 딜 해설 — 국내·해외 딜, 카브아웃·실사 실무, PE 구조', False),
    # 다른 장에 자리가 없는 원문 — 지금은 JAX 스케일링 북(깃허브 공개 책). 잠그지 않는다
    ('기타 대시보드.html', 'misc', '기타', '📦',
     '다른 장에 안 드는 긴 원문 — How To Scale Your Model 열세 장을 보고서로', False),
    ...
    # 원문 코퍼스에서 이름을 세어 세운 공정 지도. 2026-09-09 에 공정 다섯 장을 한 장으로
    # 몰았다 — 다섯 장은 「무엇이 몇 번 나왔나」만 다섯 번 되풀이하는 색인이었다.
    # 패널이 유료 뉴스레터 변환본의 파일 이름과 문장을 드러내므로 잠근다
    ('AI 인프라 지도.html', 'map', 'AI 인프라 지도', '🗺️',
     '공정 다섯(리소그래피·증착식각·패키징·메모리·네트워킹)을 기능·부품·공급·지표로 갈라 '
     '세우고, 이름마다 원문이 말한 것을 판단 한 줄로 붙인다', True),
...
REDIRECTS = {'finance': ('understanding', '산업/시장 인사이트'),
             # 공정마다 한 장이던 지도 다섯을 2026-09-09 에 한 장으로 몰았다
             'map-litho': ('map', 'AI 인프라 지도'),
             'map-etch': ('map', 'AI 인프라 지도'),
             'map-packaging': ('map', 'AI 인프라 지도'),
             'map-memory': ('map', 'AI 인프라 지도'),
             'map-network': ('map', 'AI 인프라 지도')}
