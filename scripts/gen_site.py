...
PAGES = [
    ...
    # 같은 계정을 재료로 삼지만 잠그지 않는다 — 링크드인 공개 글이고 요약은 우리가 쓴 것이라
    # M&A 장과 같은 갈래다. 유료 뉴스레터 본문은 한 줄도 안 들어간다
    ('링크드인 흐름.html', 'li-flow', '링크드인 흐름', '🔗',
     'SemiAnalysis 링크드인 537건을 줄기 하나로 — 늘어난 것은 물량이 아니라 값이고 '
     '값이 오르는 자리가 칩 바깥으로 밀려났다', False),
