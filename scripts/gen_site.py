...
대시보드/ 에서 공개 대상 4장만 골라 site/ 에 ASCII 슬러그로 복사하고,
...
def build_index() -> str:
    cards = '\n'.join(
      ...
      <span class="tx"><strong>{title}</strong><em>{desc}</em></span>
    </a>'''
        for _, slug, title, emoji, desc in PAGES
    )
