...
('미국주식 사관학교 대시보드.html', 'usa-academy', '미국주식 사관학교', '🎓',
    ...
    ('AI 인프라 지도 — 메모리.html', 'map-memory', 'AI 인프라 지도 — 메모리', '💾',
     '대역폭을 파는 HBM 과 용량을 파는 DRAM — 누가 무엇을 팔고 무엇으로 재는지', True),
    ...
    # 원문 코퍼스에서 이름을 세어 세운 공정 지도. 공정마다 페이지 하나다.
    # 패널이 유료 뉴스레터 변환본의 파일 이름과 문장을 드러내므로 잠근다
    ('AI 인프라 지도 — 리소그래피.html', 'map-litho', 'AI 인프라 지도 — 리소그래피', '🗺️',
     '빛으로 회로를 새기는 공정을 기능·부품·공급·지표 넷으로 갈라 세운다', True),
    ('AI 인프라 지도 — 증착·식각.html', 'map-etch', 'AI 인프라 지도 — 증착·식각', '⚗️',
     '막을 쌓고 다시 깎는 전공정 — 무엇을 만들고 누가 그 기계를 파는지', True),
    ('AI 인프라 지도 — 패키징.html', 'map-packaging', 'AI 인프라 지도 — 패키징', '🧩',
     '다이 여럿을 한 덩이로 묶는 공정 — CoWoS·HBM·인터포저가 어디에 붙는지', True),
