...
PAGES = [
    ...
    # 2026-09-08 잠갔다. 뉴스레터 변환본 11편을 근거로 들이면서 「링크드인 공개 글만
    # 재료」라는 전제가 깨졌다 — SemiAnalysis 장과 같은 취급으로 「비공개 자료」에 든다
    ('링크드인 흐름.html', 'li-flow', '링크드인 흐름', '🔗',
     '여섯 달을 줄기 하나로 — 막힌 자리는 웨이퍼·소재·전기였고 값은 그 병목의 온도계였다',
     True),
...
('미국주식 사관학교 대시보드.html', 'usa-academy', '미국주식 사관학교', '🎓',
    ...
    # 같은 재료를 사슬이 아니라 줄기로 읽는다. 사슬은 연재 주제에만 서서 단발 글이 갈 데가
    # 없었는데, 줄기는 단발도 조각으로 받는다. 위 장과 같이 잠그지 않는다
    ('메르 흐름.html', 'mer-flow', '메르 흐름', '📏',
     '열두 달치 627편을 줄기 하나로 — 한도가 값을 붙잡는 게 아니라 값이 한도를 끌고 간다', False),
    ...
    ('통합 보고서.html', 'report', '통합 보고서', '📑',
     '카드 여러 장을 한 물음으로 꿴 글 — 로봇·AI 밸류체인·빅테크 밸류에이션·AI 인프라 자금조달·CPO', True),
    ...
    # 원문 코퍼스에서 이름을 세어 세운 공정 지도 — 지금은 리소그래피 가지 하나.
    # 패널이 유료 뉴스레터 변환본의 파일 이름을 드러내므로 잠근다
    ('AI 인프라 지도.html', 'infra-map', 'AI 인프라 지도', '🗺️',
     '공정을 가지로 펴고 잎마다 이름을 단다 — 잎을 누르면 그 이름이 나온 원문이 선다', True),
