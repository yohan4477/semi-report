...
- **번호는 층마다 다르다.** 단원은 「1. 2. 3.」(카드는 `card_lib.toc_html`, 보고서 층은 `scratchpad/_rep_toc.py`가 붙인다 — 손으로 안 적는다), 그 아래 절과 나열은 ①②③. 「첫째·둘째」는 안 쓴다. 차례 꼴(한 줄에 절 하나·들여쓰기·색)은 `docs/글과 도해 — 확정 규칙.md` §4 「차례」가 정본이고, 어긴 층은 `_rep_toc.check_toc`가 물어 생성기를 멈춘다.
...
- **대시보드 첫 화면은 어느 장이든 최신순 이름 목록.** 줄마다 태그(그 카드의 섹션), 목록 위 태그 줄이 그 태그로 거른다 — 순서는 안 바뀐다. 관문 버튼도 섹션 타일도 없다(타일은 2026-09-08에 걷었다 — 주제를 먼저 고르게 해서 새로 들어온 글이 안 보였다). 안내·롤업은 목록 아래. 조립은 `scratchpad/dash_common.py`의 `render()`만, 규약은 같은 파일 `check_ui()`와 `scratchpad/check_flatlist.py`(브라우저로 스무 장을 훑는다).
