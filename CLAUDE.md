...
PYTHONIOENCODING=utf-8 python scripts/check_vcfigs.py  # 밸류체인 그림 값 — report_figs.json 의 숫자가 조사 보고서 본문에 있나, 워터폴 산수
```
