...
SK하이닉스의 독자 개발(NAMICS와 공동 개발) 몰딩 언더필 재료는 마이크론·삼성이 쓰는 비전도성 필름보다 열을 더 잘 빼냅니다. SK하이닉스는 다른 방식으로 휘어짐(뒤틀림)을 관리할 방법을 찾아 열압착 본딩(TCB)을 건너뛸 수 있었습니다. TCB의 장점은 힘을 가해 접합 물질을 안정화하는 것이지만, 그 힘이 오히려 범프를 손상시킬 위험을 키웁니다. SK하이닉스는 범프에 가해지는 스트레스를 역이용해 더미 범프를 더 넣을 수 있고, 이는 발열 방출에도 도움이 됩니다.
...
HBM은 고난도 3DIC 적층 제품이라 패키징 수율이 기존 DRAM 제품군과 비교가 안 됩니다. 다만 더 큰 문제는 전공정 수율 쪽이라는 게 SemiAnalysis의 판단입니다. 원인은 TSV로 전력을 스택 위층까지 실어날라야 하는 **전력배선망(PDN)** 설계에 있습니다 — TSV 배치·설계는 제조사별 핵심 차별화 지점이자 각사의 독자 기술입니다.
...
본딩(다이 접합) 단계는 수율에 결정적인 영향을 미쳐 정교한 장비가 필요합니다. TSV 간격이 약 40마이크로미터라 본더는 한 자릿수 마이크로미터 또는 서브마이크론급 정렬 정밀도를 갖춰야 하고, 여러 층에 걸쳐 뒤틀림이 누적되지 않도록 균일한 압력 분배도 중요하며, 원가를 좌우하는 처리량(throughput)도 핵심입니다.
...
한미반도체 입장에서는 HBM 공급망에서 가장 크고 중요한 고객(Nvidia)에 HBM을 공급하는 SK하이닉스 공정에 한화가 아직 인증조차 받지 않았는데도 더 비싼 가격에 물량을 따내자, 이해할 만한 수준의 반발이 뒤따랐습니다. 서비스 없이는 SK하이닉스가 대표 제품을 출하하지 못할 위기에 몰렸고, 마이크론·삼성도 그 생산 공백을 단기간에 메울 수 없어 전체 가속기 공급망이 흔들릴 수 있는 상황이었습니다. 결국 큰 압박 속에 SK하이닉스가 최근 몇 주 사이 한미반도체에 소규모 주문을 넣어 현장 서비스를 복구시켰지만, 이는 대량 물량 발주라기보다 달래기 성격이 강했던 것으로 보입니다. ASMPT·Besi 등 경쟁사가 HBM 전용 TC 본더 개선에 속도를 내고 있어, 한미반도체가 독점적 지위에서 더 짜낼 수 있는 여지는 많지 않을 수 있습니다.

---

*작성 진행률: 약 33% 완료*
*업데이트: 4\~6장(패키징 경쟁, 수율, 본딩 장비 갈등) 작성 완료*
