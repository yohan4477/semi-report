...
### 전체 섹션
1. [서론: AI 시대의 Datacenter 변화](#서론-ai-시대의-datacenter-변화)
2. [Datacenter 기초](#datacenter-기초)
3. [Datacenter 종류별 비교](#datacenter-종류별-비교)
4. [Datacenter 전기 시스템](#datacenter-전기-시스템)
5. [High Voltage Transformers](#high-voltage-transformers)
6. [Data Halls and Pods](#data-halls-and-pods)
7. [Generators, MV Transformers, Power Distribution](#generators-mv-transformers-power-distribution)
8. [UPS Systems](#ups-systems)
9. [OCP Racks and BBUs](#ocp-racks-and-bbus)
10. [AI Impact: Power Density 증가](#ai-impact-power-density-증가)
11. [Winners & Losers, CapEx Forecast](#winners--losers-capex-forecast)
...
자동 전환 스위치 (ATS) 바로 다음에는 무정전 전원 공급 장치 (UPS)가 있어 전력 공급이 절대 중단되지 않도록 보장합니다. 이 장치는 전력 전자 장치와 배터리를 통합하여 끊임없는 전력 흐름을 보장합니다.
...
- 배터리 백업 유닛 (BBU)을 랙 내부에 배치 → 중앙 UPS 불필요
...
### OCP (Open Compute Project) 랙 구조

위에서 설명한 것은 일반적인 datacenter의 전력 흐름이지만, Hyperscaler는 효율성을 추구하며 일반적인 배치에서 벗어나는 경우가 많습니다. 좋은 예가 Meta가 10년 전에 도입한 OCP (Open Compute Project) 랙입니다.
...
- Busbar를 통해 서버에 DC 전원 공급
- 맞춤 서버 설계 필요: DC busbar 연결용 bar clip 포함, 내부 정류기 없음
- Power Shelf는 모듈화: 일반적으로 유닛당 6개의 3kW 모듈

### 배터리 백업 유닛 (BBU)

Power Shelf는 배터리 백업 유닛 (BBU)을 통합할 수도 있으며, 리튬이온 배터리로 몇 분간 부하를 지원하여 "랙 내 UPS" 역할을 하므로 중앙 UPS가 필요 없습니다.
...
Generative AI는 datacenter 설계와 계획을 크게 변화시키는 새로운 컴퓨팅 요구 사항을 매우 큰 규모로 가져옵니다.
...
### Meta가 datacenter를 철거한 이유

Meta는 건설 중이던 datacenter 건물 전체를 철거했습니다. 이유는 단 하나: 전력 밀도가 너무 낮아서 GB200을 수용할 수 없었기 때문입니다.
...
## Winners & Losers, CapEx Forecast

**📌 핵심:**
- 전력 장비는 datacenter CapEx의 40-45% 차지
...
전력 장비는 일반적으로 총 CapEx의 40-45%를 차지합니다.
...
**구성 요소 공급업체:**
- Transformers: ABB, Siemens, Schneider
- Generators: Caterpillar, Cummins, Kohler
