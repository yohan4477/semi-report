...
### 전체 섹션
 1. [서론: AI 시대의 데이터센터 변화](#서론-ai-시대의-데이터센터-변화)
 2. [데이터센터 기초](#데이터센터-기초)
 3. [데이터센터 종류별 비교](#데이터센터-종류별-비교)
 4. [데이터센터 전기 시스템](#데이터센터-전기-시스템)
 5. [고압 변압기](#고압-변압기)
 6. [데이터 홀과 Pod](#데이터-홀과-pod)
 7. [발전기, 중압 변압기, 전력 배전](#발전기-중압-변압기-전력-배전)
 8. [UPS (무정전 전원 공급 장치) 시스템](#ups-무정전-전원-공급-장치-시스템)
 9. [OCP 랙과 BBU (배터리 예비 전원)](#ocp-랙과-bbu-배터리-예비-전원)
10. [AI 영향: 전력 밀도 증가](#ai-영향-전력-밀도-증가)
11. [승자와 패자, 투자 전망](#승자와-패자-투자-전망)

---

## 🔑 용어 정리

본문을 순서대로 읽기 전에 알아두면 좋은 용어들입니다. 각 용어는 본문에서 처음 등장하는 위치에 동일한 설명이 다시 나옵니다.
...
**📌 UPS (무정전 전원 공급 장치)**
> - 정전이 나도 발전기가 완전히 켜지기까지(약 60초) 배터리로 전력을 이어주는 장치
> - 응답 시간 10ms 미만 → 정전과 발전기 가동 사이의 공백을 메워 전력이 끊기지 않게 함
> - 위치: 변압기·발전기 다음, IT 장비로 가기 직전 단계

**📌 BBU (배터리 예비 전원)**
> - UPS 역할을 중앙 장비 대신 랙 안으로 옮긴 방식: 랙마다 배터리를 내장해 몇 분간 자체적으로 버팀
> - 장점: 중앙 UPS가 필요 없어져 효율↑, 전력 손실↓
> - 단점: 랙 안에 리튬 배터리를 두는 만큼 화재 규정을 훨씬 엄격히 지켜야 함

**📌 OCP 랙 (Open Compute Project)**
> - Meta가 만든 서버·랙 설계 방식. 서버마다 있던 AC→DC 변환 장치를 없애고, 랙 중앙 한 곳에서 변환한 뒤 여러 서버에 나눠주는 구조
> - 부품 수를 줄이고 효율을 높이기 위한 설계

**📌 PDU (전력 배전 장치)**
> - UPS에서 나온 전력을 랙 단위, 서버 단위로 나눠주는 배전함
> - 비유: 가정집 멀티탭을 대규모 데이터센터 단위로 확장한 장치

**📌 ATS / STS (자동 전환 스위치)**
> - **ATS (Auto Transfer Switch)**: 주 전원이 끊기면 기계식으로 발전기 등 예비 전원으로 자동 전환. 전환에 다소 시간이 걸림
> - **STS (Static Transfer Switch)**: 반도체(전력 전자) 기반이라 ATS보다 훨씬 빠르게 전환. 이중화된 UPS 사이를 순간적으로 오갈 때 사용
...
Transformer는 100년 이상 된 단순한 기술이지만, 데이터센터의 핵심 장비입니다. AC (교류, Alternating Current) 전류가 만드는 변화하는 자기장을 이용해 전압을 변환합니다.
