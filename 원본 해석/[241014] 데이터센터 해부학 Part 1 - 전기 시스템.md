...
데이터센터 (Datacenter) 산업이 AI 훈련과 추론의 대규모 수요로 인해 전례 없는 가속화를 경험하고 있습니다. 특히 Nvidia의 Blackwell GB200 출시는 데이터센터 설계의 근본적 변화를 요구합니다. 아래는 AI 수요 증가가 어떤 요구사항을 만들어내고, 기존 데이터센터가 왜 대응하지 못하는지 보여줍니다.
...
Meta는 건설 중이던 데이터센터 건물을 철거했습니다. 이유는 단 하나: 전력밀도가 너무 낮아서 GB200을 수용할 수 없었기 때문입니다. 아래는 Meta와 Google의 전력밀도 차이가 어떻게 다른 결과로 이어졌는지 보여줍니다.
...
데이터센터의 안정성과 가용성을 평가하는 업계 표준 분류 체계입니다. 아래는 Tier 1\~4의 가용성·다운타임 차이와, Tier 3가 왜 대형 시설의 표준으로 자리잡았는지 보여줍니다.
...
flowchart TD
    ...
    Tier1 --> Tier2["Tier 2<br/>99.741% 가용성<br/>다운타임 22시간/년<br/>단일 경로 + 일부 중복"]
    Tier2 --> Tier3["Tier 3<br/>99.982% 가용성<br/>다운타임 1.6시간/년<br/>동시 유지보수 가능<br/>(비행 중 엔진 교체 가능)<br/>N+1 redundancy"]
    Tier3 --> MostCommon["대형 시설에서 가장 일반적<br/>(비용 vs 안정성 균형)"]
...
아래는 N, N+1, 2N 백업 전략별 비용·안정성 차이와 Tier 3\~4가 각각 어떤 조합을 쓰는지 보여줍니다.
...
데이터센터는 규모(전력 용량)에 따라 Retail, Wholesale, Hyperscale로 분류됩니다. 아래는 세 종류의 전력 규모, 위치, 고객, 가치 제안이 어떻게 다른지 보여줍니다.
...
임대(Colocation)와 자체 구축(Self-Build) 중 어떤 방식을 택하느냐에 따라 비용 구조와 배치 속도가 달라집니다. 아래는 두 모델의 트레이드오프와, Hyperscaler가 실제로 쓰는 하이브리드 전략을 보여줍니다.
...
아래는 전력이 전력회사에서 최종 CPU/GPU까지 전달되는 전체 경로와, 각 단계의 전압 변환·백업 시스템을 보여줍니다.
...
아래는 150 MW 캠퍼스에 80 MVA 변압기를 2개(N) 둘지 3개(N+1) 둘지에 따라 비용과 안정성이 어떻게 달라지는지 보여줍니다.
...
현대 Hyperscale (초대형) 데이터센터는 모듈화되어 있습니다. Campus → Building → Data Hall → Pod 계층 구조로, 빠른 확장과 유연성을 제공합니다. 아래는 48 MW 건물이 Data Hall과 Pod 단위로 어떻게 나뉘는지 보여줍니다.

```mermaid
flowchart TD
    Campus["Campus<br/>(여러 건물)<br/>수백 MW"] --> Building["Building<br/>~250k sq ft<br/>48 MW<br/>(5개 Data Hall)"]

    Building --> Hall1["Data Hall 1<br/>9.6 MW<br/>(4개 Pod로 분할, 아래 참고)"]
...
Data Hall 1 내부는 4개의 동일한 Pod 모듈로 나뉘고, 각 Pod는 독립된 전기 장비 세트를 갖습니다.

```mermaid
flowchart TD
    Hall1Zoom["Data Hall 1<br/>9.6 MW"] --> Pods["Pod 1~4<br/>각 2.4 MW<br/>(4개 모두 동일 구성,<br/>독립 전기 시스템)"]
    Pods --> Equipment["Pod당 전용 장비:<br/>발전기 1개(3 MW) · 변압기 1개(3 MVA)<br/>UPS 2개(2N) · Switchgear 1개<br/>(모두 표준 규격)"]

    style Hall1Zoom fill:#fae8ff,stroke:#a855f7
    style Pods fill:#fef3c7,stroke:#f59e0b,stroke-width:2px
```

**참고**: Campus 규모의 예로 Google은 300MW급 캠퍼스를 운영 중이며, Building 하나는 일반적으로 6-12개월 만에 건설됩니다.

### Pod 시스템의 장점

아래는 Pod 모듈화가 주는 두 가지 핵심 장점(모듈성·표준화)과, 표준 크기 Pod가 왜 저렴하고 빠르게 조달되는지 보여줍니다.
