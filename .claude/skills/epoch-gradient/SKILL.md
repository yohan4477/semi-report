...
| **한 열에 나란한 상자들** | **`even([(x, y, w, 이름, [설명줄], 강조), …])`** — 높이를 맞춰 돌려준다 |
...
### 한 편은 한 카드다 — 갈래로 쪼개지 않는다

**원문 한 편이 카드 하나다.** 파이낸싱 편을 갈래 셋(구조·트랜치·데이터센터)으로 쪼갰다가
2026-08-25에 되돌렸다. 쪼개면 한 편을 읽으려고 카드 셋을 열어야 하고, 카드마다 도해가 한두
장으로 흩어지고, 같은 반론이 세 번 되풀이된다.

그래서 `insight-upload` 의 「slim_points 6~8개」는 이 장에서 안 지킨다. **원문 길이에 맞춰
늘린다** — 파이낸싱 편은 열둘이다. 도해도 카드 하나에 열 장이 실린다.

그 대신 카드 안에서 순서가 흐름이 되게 짠다. 도해의 `anchor` 로 슬림 포인트 사이사이에 끼워,
글을 읽어 내려가면 그 대목의 그림이 바로 아래 오게 한다.

### 도해는 원문에 있는 것 전부다

원문 그림 수보다 적게 실지 않는다. 그리고 **원문에 값이 있는데 그림이 없는 자리도 채운다** —
조달 구성·집행 일정·금리 비교·사업장 목록·인도 일정이 그렇게 나왔다.
...
5. **한 열에 나란히 서는 상자는 `even()` 으로 그린다.** `box()` 를 하나씩 부르면 줄 수만큼
   높이가 달라져 아래끝이 들쭉날쭉해지고, 열로 안 읽힌다. 열 열한 개가 그렇게 어긋나 있었다
   (2026-08-25). 줄바꿈은 `reflow()` 가 폭을 재서 다시 잡으므로 손으로 끊은 자리는 참고일 뿐,
   높이는 미리 알 수 없다 — 그래서 재는 일을 `even()` 에 맡긴다.
6. **상자 안쪽 여백은 `PAD`(12) 하나다.** 왼쪽 자리·오른쪽 자리·줄바꿈 한도가 모두 이 값을
   쓴다. 셋 중 하나만 손대면 좌우가 어긋난다.
7. **캡션 끝에 출처를 단다.** 「값은 Epoch AI 원문 도해(CC-BY)를 따랐다」. 형태를 바꿨으면
   그 앞에 이유를 한 문장 적는다.
